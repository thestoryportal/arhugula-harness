...
2. **Run the suite.** `just codex-check` (lint + typecheck + `uv run pytest` + the
   `codex-parity-check` shell lane, mechanism-α only — β/γ skip without credentials, by
   design). For a targeted area, `just test-one <path>`. Prefer `codex-check` over `check`
   here: `check` runs pytest ONLY, so it silently skips BOTH shell globs
   (`tools/hooks/test_*.sh` *and* `tools/statusline/test_*.sh`) that `codex-parity-check`
   runs — and a self-heal pass that never executed them cannot call the suite green.
