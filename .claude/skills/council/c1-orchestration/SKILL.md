...
\*\*Hand-off mechanics.\*\* The protocol of control transfer between agents — what is passed, when control is yielded, what the receiving agent's input contract requires. Includes typed return contracts (the OpenAI \`output\_type\` pattern, the Mastra \`inputSchema\`/\`outputSchema\` per step pattern) at the \*contract-shape\* level. C5 owns the validation of the payload; C1 owns the contract that the payload exists.  
...
\- \*\*T-perm-3: C1 ↔ C9\*\* (control flow vs. reliability) — \*\*permanent\*\* Layer-3 tension. Tunable parameter \`topology\_fault\_handling\`. Surface when retry-as-topology-change is at stake (e.g., breaker-trip routes to fallback path). When this tension fires, the topic is co-primary territory; route to the orchestrator.  
