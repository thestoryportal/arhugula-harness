...
*Synthesis of the lived workflow, the HIL guidance that keeps it on track, and the two reusable YAML specs. Distilled from the context-memory grounding council arc (roadmap `R-IF-council-context-memory`; PR #292; ledger at `.harness/council/context-memory-grounding/`). This doc is the prose companion to the machine-readable specs.*
...
E3 decorrelated ─ Codex (out-of-family, COLD primer) + the transcript-brief reviewer (in-family, transcript-aware — a fresh-context agent handed a written session brief); weight DIVERGENCE
...
**Reorder the operator may take:** collapse E2b + E3b into ONE consolidated reconcile *after* all reviewer input (adversarial + Codex + the brief-reviewer) is gathered — the council answers the merged finding-set in one pass. Preserves reconcile-to-zero, fewer full-council convenings. (This arc did so.)
...
4. **Decorrelated-reviewer wiring** — Codex gets a **descriptive primer only** (never the council's conclusions, never the adversarial findings, never the brief-reviewer's eval); the brief-reviewer is transcript-aware through the session brief; **weight where Codex disagrees** with the Claude-family reviewers. 3-way convergence → ship; divergence → dig.
...
`COUNCIL-WORKFLOW.md` (this doc) → the relevant YAML spec → the council orchestrator skill (`.claude/skills/council/council-orchestrator/`) → the lived precedent (`.harness/council/context-memory-grounding/`).
