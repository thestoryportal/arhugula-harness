...
Procedure:
1. **Pre-convene** — apply the nameable-tension gate (if no cross-domain tension can be named in advance, STOP and route to a single voice + a transcript-brief review instead). Identify which harness layer/axis the question touches, and convene the voices from the spec's `layer_voice_map` (primaries = the layer's domain center; consultants = cross-cutting). Name the convening spine-tension. Open a charter + ledger tree.
2. **Run the stages** E1 (A1 primaries-independent → A2 consultants-react-to-primaries → B cross-read DEBATE) → E2 adversarial #1 → E2b reconcile → E3 Codex(out-of-family, descriptive primer ONLY) + a transcript-brief Agent reviewer (transcript-aware) → E3b reconcile → E4 gate → close. Every voice/reviewer is a **genuine agent invocation** that adopts its skill (never core-agent reference-reads); fan out gently in small waves.
