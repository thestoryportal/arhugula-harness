"""B-HANDOFF-PAUSE (R-FS-1) — full-runtime `api.resume` of a paused DECENTRALIZED_HANDOFF.

The gold-standard close for the resumable `cascade_policy=pause` single-owner
sequential handoff: a `HandoffResumeState`-bearing `PauseSnapshot`, captured via a real
...
entry-point resume detection (`attempt_resume`, STRICT, MVP constant anchor → admitted)
→ the DECENTRALIZED_HANDOFF strategy re-entry with the snapshot → SUCCESS. Proves the
handoff stage-cursor snapshot survives the durable round-trip + the PAUSED→'paused'
...
HANDOFF-shaped: single-owner sequential — every declared step is a STAGE-expert (NO
orchestrator `steps[0]`, NO peer fan-out), so the resume state is `HandoffResumeState`
(the contiguous completed prefix + stage_count), the sequential sibling of the
ORCHESTRATOR_WORKERS `FanOutResumeState` / PARALLELIZATION `PeerFanOutResumeState`.

Substrate (provider/tracer/OD fakes + the get_tracer-capable tracer) lifted by value
from `test_b_fanout_pause_parallelization_resume_e2e.py` per FM-2 cross-test-file
independence.
"""
...
_WORKFLOW_ID = "wf-b-handoff-pause-resume-e2e"
...
def _config_opt_in(tmp_path: Path) -> RuntimeConfig:
    return RuntimeConfig(
        ...
        default_topology=TopologyPattern.DECENTRALIZED_HANDOFF,
        mcp_clients=[],
...
class _HandoffWorkflow:
    """A DECENTRALIZED_HANDOFF workflow: 3 sequential STAGE-experts (NO orchestrator,
    NO fan-out), under the TEAM_BINDING persona (→ cascade_policy = pause)."""
    ...
    def steps(self) -> Sequence[WorkflowStep]:
        return (
            WorkflowStep(
                ...
                step_payload={"stage": 0},
            ),
            WorkflowStep(
                step_id=StepID("stage-1"),
                step_kind=StepKind.DECLARATIVE_STEP,
                step_payload={"stage": 1},
            ),
            WorkflowStep(
                step_id=StepID("stage-2"),
                step_kind=StepKind.DECLARATIVE_STEP,
                step_payload={"stage": 2},
            ),
        )
...
@pytest.mark.asyncio
async def test_api_resume_handoff_pause_restart_proof_round_trip(
    tmp_path: Path,
    _patched_runtime: None,
) -> None:
    """A DECENTRALIZED_HANDOFF `PauseSnapshot` (stage-0 + stage-1 completed+recovered;
    stage-2 the failed tail), captured via the real bootstrapped protocol + JSON
    ...
    assert rehydrated.handoff_resume is not None
    ...
    result = await resume(_HandoffWorkflow(), pause_snapshot=rehydrated, config=config)
    ...
    # Only the failed-then-recovered tail (stage-2) ran on resume; the completed prefix
    # (stage-0, stage-1) was recovered, NOT re-dispatched (the at-most-once witness).
    assert _RESUME_DISPATCHED == ["stage-2"], (
