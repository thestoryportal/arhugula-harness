...
| ADR-F5 | v1.1 | Skills |
| ADR-D2 | v1.3 (B-25 row-7 self-contradiction resolved 2026-07-15 — `where:`-block row 7 reconciled to `(read-only, *)`; `is_deterministic_inhouse` documented non-gating/reserved; clearance at `.harness/clearance/adr-d2-v1-3-cleared-2026-07-15.md`) | Sandbox + blast radius |
...
Per U-AS-33 substrate seam exports manifest (C-AS-16). Cardinality at CXA v2.17: **19 edges CP→AS canonical (6 genuine) + 10 edges OD→AS = 29 inbound consumer edges** (v2.1 baseline was 24+10=34; refreshed at PR #91 per `[[class_3_tension_per_axis_claude_md_v2_1_to_v2_4_count_drift]]` PARTIAL-closure 2026-05-31; preserved verbatim at v2.17). Detailed per-bucket enumeration at `Cross_Axis_Composition_Document_v2_19.md` §2.3.3 (CP→AS; v2.17 preserves v2.15 row 6 U-CP-68 → U-AS-03 ToolContract Pattern-P1 seam) + §2.3.5 (OD→AS; preserved verbatim from v2.6 → v2.17; original §2.3.6 cite at v2.1 baseline was a typo — §2.3.6 is the OD→CP bucket per CXA §2.3 canonical numbering).

### 2.3 Cross-axis edge inventory (CXA v2.19 canonical reading; numbers last refreshed at the v2.16 → v2.17 arc, preserved verbatim through v2.19)

*The `§2.3.x` cites below name the **v2.19 canonical reading**; the per-bucket row tables are preserved verbatim from the last full re-table at **v2.3** (§2.3.3 CP→AS last amended at v2.15), not byte-tabled in the v2.19 delta — per the delta-only chain (root `CLAUDE.md` §2 delta-baseline §-cite convention). v2.19 itself only restated §2.1 (aggregate matrix) + §2.4 (per-axis attribution).*

| Edge direction | Edges | Source artifact |
|---|---|---|
| AS → IS (outbound; canonical) | **11** (CXA v2.6+ canonical; AS plan v1.2 §3.4 internally enumerates 13 — see §2.4 divergence note) | `Cross_Axis_Composition_Document_v2_19.md` §2.3.1 + §2.4 per-axis attribution |
| CP → AS (inbound) | **19 canonical / 6 genuine** (was 18/5 at v2.14; v2.15 row 6 U-CP-68 → U-AS-03 ToolContract Pattern-P1 seam landed) | `Cross_Axis_Composition_Document_v2_19.md` §2.3.3 |
| OD → AS (inbound) | 10 (unchanged v2.6 → v2.17) | `Cross_Axis_Composition_Document_v2_19.md` §2.3.5 (original v2.1 cite at `§2.3.6` was a typo — §2.3.6 is OD→CP) |
...
**Retirement status (live ledger, derived).** Current AS-axis substitution accounting is not hand-maintained here. `.harness/substitutions.yaml` is the source of truth and `tools/substitution_ledger.py` derives the counts; at current ledger head the AS axis is **11 / 11 RETIRED**. The table below keeps row-level lineage for operator context. Batch-52 back-flowed the former AS-8e/AS-8f accepted-indefinite-defer rows to `SUBSTANTIVE_RETIRED` after the R-810/R-820 live managed-cloud proofs.
...
| H_T-AS-8e (files.* 8-attribute observability namespace) | **SUBSTANTIVE_RETIRED** (batch-52 back-flow, 2026-06-08) | R-810 opened the former Files indefinite-defer arc and closed it with real Anthropic Files upload/reference/delete plus managed-cloud `files.operation` Cloud Trace proof. Authoritative row: `.harness/substitutions.yaml` `H_T-AS-8e`; carrier lookup: `just overlay-query --substitution H_T-AS-8e` |
| H_T-AS-8f (managed_agents.* 3-attribute observability namespace) | **SUBSTANTIVE_RETIRED** (batch-52 back-flow, 2026-06-08) | R-820 closed the former managed_agents production-only exclusion with the real Anthropic Managed Agents SDK/session integration plus managed-cloud `managed_agents.*` Cloud Trace proof. Authoritative row: `.harness/substitutions.yaml` `H_T-AS-8f`; carrier lookup: `just overlay-query --substitution H_T-AS-8f` |
| H_T-AS-9 (substrate seam exports manifest) | RETIRED (authoring close, v1 §1) | Authoring-only |

**AS-axis live substitution tally.** Derived via `tools/substitution_ledger.py` from `.harness/substitutions.yaml`: **11 / 11 RETIRED (100.0%)** and **11 / 11 pipeline-advanced (100.0%)** at current ledger head. Do not hand-maintain per-bucket AS counts here; a future transit must update the ledger row, retirement batch, snapshot pin, and substitution-ledger tests in the same commit. Historical batch-31/batch-25 posture remains in the old batch files, not in this live tally.
