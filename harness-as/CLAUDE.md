...
AS posture per `Cross_Axis_Composition_Document_v2_1.md` §2.1: producer of 13 outbound edges (all to IS); consumer of 0 cross-axis edges from CP / OD (CP and OD pull from AS via U-AS-33 substrate seam exports manifest — 24 CP→AS + 10 OD→AS = 34 inbound edges).
...
Per U-AS-33 substrate seam exports manifest (C-AS-16). Cardinality: 24 edges CP→AS + 10 edges OD→AS = 34 inbound consumer edges. Detailed per-bucket enumeration at `Cross_Axis_Composition_Document_v2_1.md` §2.3.4 (CP→AS) + §2.3.6 (OD→AS).
...
**Retirement status (post 7d batch 11 doc-hygiene-pass refresh, 2026-05-23).** Per cumulative batch records `.harness/phase-7d-retirement-events-batch-{1..11}.md` + v2 ledger `.harness/phase-7d-retirement-ledger-v2.md` §4 under operator-ratified runtime-only substitution-site reading + line-33 strict-reading discipline. **L9-sexies + L9-septies cluster closes 2026-05-22 materialize the tool-invocation runtime composer** (U-RT-64..U-RT-70 + U-RT-71..U-RT-75) that ledger-v2 §4 flagged as load-bearing for AS-2/4/5/8 retirement. Production tool-dispatch path operational at `harness-runtime/.../lifecycle/runtime_tool_dispatcher.py` with `sandbox.enter` + `mcp.tool.call` + `sandbox.exit` span emission; `mcp.*` 7-attr namespace fully covered via `MCPClientNamespaceEmitter`. Forward-only ledger discipline preserved.
...
| H_T-AS-2 (Tool contract schema) | **RETIRE-READY** (batch 11 doc-hygiene transition) | `runtime_tool_dispatcher.py:41` imports `from harness_as.tool_contract import ToolContract`; production tool-dispatch operational at L9-sexies close (commit `83d3b54` U-RT-67 `RuntimeToolDispatcher.dispatch()` body); L9-septies stage-5 callsite wire-up at `stage_5_loop_init.py:309` via `materialize_runtime_tool_dispatcher_stage` factory (U-RT-75). Criterion-A MET + structural criterion-B MET; operational e2e gated on operator-supplied `mcp_servers` config non-empty + external MCP server (same operator-opt-in pattern as H_T-CP-18 batch-10 + H_T-CP-21 batch-11) |
| H_T-AS-4 (sandbox.* 7-attribute namespace) | **PARTIAL** (batch 11 doc-hygiene transition) | 6 of 7 `sandbox.*` attrs declared at `runtime_tool_dispatcher.py:179-184` (`sandbox.tier`, `sandbox.tech`, `sandbox.provider`, `sandbox.policy.assigned_tier_reason`, `sandbox.cost.tier_overhead_ms`, `sandbox.fail.class`) emit at production `sandbox.enter` + `sandbox.exit` spans (commit `83d3b54` U-RT-67); 7th `sandbox.violation` deferred per `runtime_tool_dispatcher.py:414` comment ("sandbox.violation deferred"). Compound-irrelevance lifted post-L9-sexies — production tool-dispatch now actively emits sandbox.* spans |
| H_T-AS-5 (sandbox-event idempotency-key composition) | STILL-BOUNDED | `harness_as.sandbox_event_idempotency` import-able at `harness-as/__init__.py:112` but zero invocation at production tool-dispatch site. AS-2 RETIRE-READY transition does NOT auto-lift AS-5 — idempotency-key composition specifically gated on production tool-dispatch invoking the composition function (per X-AL-2 strict reading) |
| H_T-AS-8 (Anthropic + MCP 7-namespace observability) | **PARTIAL** (status preserved; coverage materially expanded post-L9-sexies/U-AS-28..33 closure) | `mcp.*` 7-attribute namespace fully landed at `harness-cp/.../mcp_client_namespace_emitter.py:73-79` (`mcp.server.name` + `mcp.server.trust_tier` + `mcp.protocol_version` + `mcp.transport` + `mcp.auth_present` + `mcp.primitive.kind` + `mcp.primitive.signature.sha256`); emission at `runtime_tool_dispatcher.py:375` (`mcp.tool.call` span). `anthropic.*` cache subset (4 of 10 attrs) still emitting at LLM dispatch (unchanged from batch 2; `llm_dispatch.py:348-363`). Remaining anthropic attrs (`thinking_*` / `batch_id` / `inference_geo`) NOT yet emitted; `anthropic.tokenizer_version` referenced at `cost_attribution_llm_dispatch.py:128` (read-side default, not span emission). U-AS-28..U-AS-33 cluster L2 close (Memory/Files/Skills/Anthropic-primitive carriers + sampling discipline + graceful degradation + AS-axis terminal exporter) landed but consumer-side emission paths for `skill.*` / `files.*` / `memory.*` / `managed_agents.*` not yet routed at runtime composers. §6.3.1 CP-1 → AS-8 cascade fully discharged for cache subset; full AS-8 RETIRED gates on remaining attrs at LLM-dispatch emission + cross-axis consumer namespace routing (operator-discretion future arc) |
| H_T-AS-9 (substrate seam exports manifest) | RETIRED (authoring close, v1 §1) | Authoring-only |

**AS-axis cumulative post-batch-11 (2026-05-23):** **2 / 6 RETIRED (33%)** + **1 / 6 RETIRE-READY (17%, AS-2)** + **2 / 6 PARTIAL (33%, AS-4 + AS-8)** + **1 / 6 STILL-BOUNDED (17%, AS-5)**. Pipeline advanced (R+RR+P): **5/6 = 83.3%** (up from 4/6 = 66.7% post-batch-2). Last STILL-BOUNDED row (AS-5) carries as bounded-residual gated on production tool-dispatch invoking `sandbox_event_idempotency` composition function (composes with AS-4 sandbox.* surface).

**Operator-opt-in RETIRE-READY pattern (now applies to 3 substitutions across CP + AS axes: H_T-CP-18 + H_T-CP-21 + H_T-AS-2).** All three advance to RETIRE-READY when production execution path branches on operator-supplied config (`ctx.mcp_servers=[]` default for CP-18; `ctx.validator_framework=None` default for CP-21; `ctx.mcp_servers=[]` default for AS-2 via shared MCP-client substrate). Full RETIRED gates on operator-config-non-default + e2e exercise per ledger-v2 + batch-10/11 reconciliation discipline.
...
*End of `harness-as/CLAUDE.md`. Parent guidance at workspace root `CLAUDE.md`. AS spec + plan + CXA v2.1 §2.3.1 / §2.3.4 / §2.3.6 at design-phase workspace.*
