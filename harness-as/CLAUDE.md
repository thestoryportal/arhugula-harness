...
|---|---|---|
| `Spec_Action_Surface_v1.md` | v1.6 (Class 1 fork resolution Reading B apply pass 2026-05-25 — NEW §15.8 + §15.9 + §15.10 MCPInvocationFailClass + dual-attribute emission + projection table; F4 enum + §15.1..§15.7 PRESERVED VERBATIM) | Contract authority — 16 contracts C-AS-01 through C-AS-16 |
...
| H_T-AS-8 (6-namespace observability per C-AS-14 §14.1: `anthropic.*` + `mcp.*` + `skill.*` + `managed_agents.*` + `files.*` + `memory.*`) | **PARTIAL-ADVANCE** (post AS-8 discriminator audit + anthropic.* 6-attr close, 2026-05-26) | **Per-namespace breakdown** (see `.harness/class_3_drift_as_8_partial_row_per_namespace_breakdown.md` for full audit). `anthropic.*` **10/10 LANDED** at `llm_dispatch.py:386-432` gen_ai span — 4/10 cache subset pre-existing + 6/10 closed this arc (`thinking_mode` / `thinking_budget_tokens` / `thinking_effort` / `batch_id` / `tokenizer_version` / `inference_geo` per `_AnthropicRequestAttrs` carrier + `_extract_anthropic_request_attrs` extractor; SDK shape resolved via context7 at discriminator audit). `mcp.*` **7/7 LANDED** at `mcp_client_namespace_emitter.py:73-79` + `runtime_tool_dispatcher.py:375` (`mcp.tool.call` span). `memory.*` **6/6 LANDED** at `memory_tool_dispatch.py:286-338` (`memory.operation` span; producer site at L9-octies arc `42c9a30`, 2026-05-23 — drift surfaced + patched at this arc per Class 3 drift §1). `skill.*` **0/6** — no producer site (Skills loading runtime composer not yet authored). `files.*` **0/8** — Files arc deferred indefinitely per runtime spec v1.17 §14.C. `managed_agents.*` **0/3** — Anthropic managed_agents beta SDK not yet integrated. Post-arc: **3/6 namespaces complete (50.0%)** + 0/6 partial + 3/6 absent; full RETIRED gates on 3 separate H_T primitive landings; decomposition to AS-8a/b/c/d/e/f at retirement-ledger v2 §6 surfaced as operator-discretion design-phase back-flow candidate per Class 3 drift §5(b) |
| H_T-AS-9 (substrate seam exports manifest) | RETIRED (authoring close, v1 §1) | Authoring-only |

**AS-axis cumulative post-batch-19 (2026-05-26):** **4 / 6 RETIRED (66.7%, AS-1 + AS-2 + AS-4 + AS-9)** + **0 / 6 RETIRE-READY (0%)** + **1 / 6 PARTIAL (16.7%, AS-8)** + **1 / 6 STILL-BOUNDED (16.7%, AS-5)**. Pipeline advanced (R+RR+P): **5/6 = 83.3%** (unchanged from post-batch-16; within-tier promotion AS-4 PARTIAL → RETIRED at batch-19). **AS-axis crosses 66.7% RETIRED threshold at this batch** (exits the at-most-3-RETIRED plateau held since batch-2 + batch-16 + AS-9 authoring close). Last STILL-BOUNDED row (AS-5) carries as bounded-residual gated on production tool-dispatch invoking `sandbox_event_idempotency` composition function. **AS-axis post-batch-16 prior:** 3 / 6 RETIRED (50.0%, AS-1 + AS-2 + AS-9) + 2 / 6 PARTIAL (AS-4 + AS-8) + 1 / 6 STILL-BOUNDED (AS-5).
