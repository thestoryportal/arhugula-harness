...
- Run the completion gate's probe as `uv run python tools/mutation_probe.py --file F --lines A-B --test "<cmd>"`. `just mutation-probe` is the same runner, but a `just` recipe's variadic `*ARGS` loses quoting on arguments containing `#` or spaces, which is the shape a `--test` command takes.
