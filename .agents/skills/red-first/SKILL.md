...
- The canonical "commit the arc's source work before running the gate" instruction translates to this flow's mandated lifecycle: `just codex-closeout` precedes the shipping commit (`AGENTS.md`), so run the completion gate's probes AFTER the shipping commit exists — the probe refuses a dirty target, and committing unverified source early solely to enable a probe is forbidden. A probe failure after the commit does not close the unit; it re-opens the arc as a new iteration with its own fix commit and re-run gates.
