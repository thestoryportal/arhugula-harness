---
name: fan-out
description: Use when the operator asks for parallel variants, N approaches, a tournament, or genuinely divergent solution exploration.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/fan-out/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
