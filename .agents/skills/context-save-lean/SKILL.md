...
description: Use to save working context to the workspace checkpoint directory (~/.gstack/projects/arhugula-v2/checkpoints/) at arc close, near compaction, or before a risky operation — the trimmed workspace copy of the gstack context-save save flow (U-SR-08, charter WR-15); the checkpoint it writes is loadable by the gstack context-restore skill.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/context-save-lean/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
