...
description: Use when the operator names C11 or asks about operator loops, HITL, or local deployment.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/council/c11-operator-loop-local-deployment/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
