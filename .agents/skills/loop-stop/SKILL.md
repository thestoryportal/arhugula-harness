---
name: loop-stop
description: Use when the operator asks to stop, pause, or disable autonomous loop mode.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/loop-stop/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
