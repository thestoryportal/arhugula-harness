...
| **Mode-agnostic** (workspace ops; edits are root `AGENTS.md` / `.github/` / `.Codex/` / etc.) | This skill is not the right tool. Suggest mode-agnostic work proceed without this skill's framing; defer to operator. |
| **Design-phase** (explicit declaration OR edit scope is `design-substrate/**` / `.harness/**` back-flow docs) | **HALT.** Atomic unit consumption at design-phase posture is X-AL-3 silent H_T design extension (`AGENTS.md` §4.4 + I-2). Surface the conflict; suggest the operator either (a) re-declare posture as Phase 7 if the intent IS impl execution, OR (b) invoke a design-phase skill (`implementation-planner`, `spec-writer`, `systems-architect`, `harness-adversarial-reviewer`) if the intent is plan/spec authoring. Do NOT proceed with atomic unit work. |
| **Unclear** (no explicit declaration; ambiguous edit scope) | Halt + ask per `AGENTS.md` §11.6. Present the 3 posture options; honor the response. |

A bundled-absorption arc (legitimate mixed-posture per `AGENTS.md` §11.4) — e.g., a spec amendment that cascades into impl in the same PR — DOES carry a `.harness/` back-flow doc (fork doc / architect recommendation / retirement event / clearance marker). If the operator declares "bundled-absorption arc" + names the back-flow doc, this skill activates for the impl portion while the design-phase skills handle the substrate portion in the same session. If no back-flow doc is named, treat as unclear → halt + ask.
...
Per workspace root `AGENTS.md` §1.3:
...
Canonical artifacts at design-phase workspace (see workspace root `AGENTS.md` §2):
...
Source: `Target_Stack_Commitment_v1.md` (design-phase workspace); workspace root `AGENTS.md` §3.1.

**Framework-pull discipline binds** per `Plan_Executability_Audit_v1.md` and workspace root `AGENTS.md` §3.2:
...
Per workspace root `AGENTS.md` §8 execution invariants and per-axis `AGENTS.md` §4.2 axis-specific rules:
...
Consult the relevant per-axis `AGENTS.md` §4.2 before any per-axis implementation:

| Axis | Anti-leakage location |
|---|---|
| IS | `harness-is/AGENTS.md` §4.2 (IS-AL-1 through IS-AL-4) |
| AS | `harness-as/AGENTS.md` §4.2 (AS-AL-1 through AS-AL-4) |
| CP | `harness-cp/AGENTS.md` §4.2 (CP-AL-1 through CP-AL-5); CP-AL-1 also at `Sub_Agent_Boundary_Specification_v1.md` §5.1 |
| OD | `harness-od/AGENTS.md` §4.2 (OD-AL-1 through OD-AL-3) |
...
|---|---|---|
| Workspace root `AGENTS.md` | This workspace | Workspace-level guidance + canonical artifact pointers |
| Per-axis `AGENTS.md` | `harness-{is,as,cp,od}/` | Per-axis substrate authority |
...
7b axis-stream execution begins from these entry-points per per-axis `AGENTS.md` §3.
