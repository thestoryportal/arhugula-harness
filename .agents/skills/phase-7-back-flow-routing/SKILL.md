...
description: Use when a Phase 7 fork, halt, specification mismatch, X-AL-3 risk, or design-phase back-flow route surfaces.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/phase-7-back-flow-routing/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
