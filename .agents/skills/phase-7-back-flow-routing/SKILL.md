...
This skill's existence is itself a posture-bridge primitive — it converts a Phase 7 defect-detection event into a design-phase authoring assignment. The X-AL-3 rule (`AGENTS.md` §4.4) is enforced HERE: silent absorption of a design-phase defect at Phase 7 = the worst possible failure mode. Halting on posture mismatch is part of the X-AL-3 enforcement triad (CI guard at §4.4 + clearance markers at §4.5 + posture declaration at §11 + this skill's halt discipline).
...
**Silent absorption of design-phase defects is the worst Phase 7 failure mode** per workspace root `AGENTS.md` §4.3. Defect absorption at execution-time:
...
Per workspace root `AGENTS.md` §8 invariant I-1: canonical artifact citations resolve byte-exact per Workflow v1.8 §7.4.2. Verify:
...
| Workspace root `AGENTS.md` §4.3 | This workspace | Back-flow routing summary + critical-failure-mode classification |
| Per-axis `AGENTS.md` §5 | This workspace | Per-axis Class 1 routing table |
