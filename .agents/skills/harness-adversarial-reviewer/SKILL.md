...
Concrete trigger: a PR touching `design-substrate/**` or a Phase 7 impl arc PR opening against a previously-cleared spec/plan should be reviewed before merge. The X-AL-3 guard (AGENTS.md §4.4) is a file-presence check; adversarial review is the substantive complement.
...
See `.Codex/skills/council/council-orchestrator/references/research-citations.md` for the voice → cluster mapping; the same mapping applies to the adversarial reviewer's external-canon mode (review CP-spec amendments against Cluster 1; OD against Cluster 4; etc.).
...
The workspace `AGENTS.md` framing (§1 project framing + canonical authority chain) enumerates what the project commits to and what it explicitly does not commit. Both directions are violations:
...
- Local development environment as design-time deployment target: any artifact that *excludes* cloud or hybrid deployment surfaces from downstream consideration is a Class 3 finding (the `AGENTS.md` framing notes this is a deployment-stage characteristic, not a local-first principles commitment).
...
Seven anti-fabrication / scope-discipline failure modes are reviewable in completed artifacts. (These were the V3-system-prompt failure modes in the design-phase skill; in the Phase-7 CLI workspace the same discipline is owned by the workspace `AGENTS.md` framing — anti-fabrication, citation grounding, scope discipline. The attacks are unchanged; only the framing authority's name changed.) Each mode below is encoded as a named attack the skill applies during review.
...
Does the artifact embed persona, stack, or deployment assumptions that the `AGENTS.md` framing explicitly does not commit? Apply via discriminator (c) of the severity tree — both directions:
...
- *decided* — the skill has classified a finding with a discriminator and a severity class, and the artifact's text supports a single reading of the defect. Use definite language: "This is a Class 3 finding because discriminator (c) fires: the ADR commits to X where the `AGENTS.md` framing explicitly does not commit X."
...
- workspace `AGENTS.md` (root + per-axis `harness-{is,as,cp,od}/AGENTS.md`) — the trace target for project-commitment violations; the source of truth for what is committed and what is not committed at project level (the §1 framing + canonical authority chain).
...
*Adapted for the Phase 7 Codex CLI workspace (2026-05-15): re-anchored from the V3 system prompt to the workspace `AGENTS.md` framing; project-KB reads → `design-substrate/` filesystem; council voice-FM substrate → axis-domain attack vocabulary; added Phase-7 pre-implementation review mode + the §4.1 / §2.7.6 Class-taxonomy disambiguation.*
