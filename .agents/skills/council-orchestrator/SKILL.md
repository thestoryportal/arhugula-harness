...
description: Use when an agent-harness design question spans multiple council voices or the operator asks to convene the council.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/council/council-orchestrator/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
