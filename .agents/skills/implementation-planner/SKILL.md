...
description: Use when an axis implementation plan is the deliverable or must absorb a settled specification revision.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/implementation-planner/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
