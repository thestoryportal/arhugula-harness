...
| Status | **Proposed** (v1.3 pending Phase 6 entry per `Project_Workflow_v1_2.md` §3.1); final-revision-pass post-P5-CK iter-2 close per `Project_Workflow_v1_2.md` §4.1.2 modified path; coherence pass preserved verbatim as v1 + v1.1 historical record per Change-note §"§[coherence pass] preservation discipline" |
| Date | 2026-05-13 |
...
| Revision date | 2026-05-13 |
| Revision | v1.2 → v1.3 (Phase-7 adversarial-review finding — count inconsistency at §3 C-IS-03: current-tense prose stated "four sub-roles" while the §3 sub-role table enumerates five rows. ADR-F2 v1.2 §Decision / ADD §2.2 Synthesis — verbatim-quoted in §3 — enumerate five sub-roles; the table matches the ADR; the prose count was the drift. §3 C-IS-03 current-tense prose count corrected `four` → `five` at five sites; all other contracts preserved verbatim; §[coherence pass] and prior change-notes preserved verbatim as historical record) |
...
**Scope of revision.** Single-finding revision pass clearing a Phase-7 adversarial-review finding (Class 1 mechanical — count inconsistency internal to §3 C-IS-03). The contract surface line and the §3 prose stated the combined git tier serves "four sub-roles", while the §3 sub-role table enumerates **five** rows (Versioning; State-ledger via commit stream; JSONL event ledger; Shadow-Git checkpointing; Worktree-isolation for concurrent sub-agent reads). The verbatim ADR/ADD authority quoted in the §3 "ADR commitment(s) honored" sub-section — ADR-F2 v1.2 §Decision / ADD §2.2 Synthesis — enumerates five distinct sub-roles (versioning + commit-stream state-ledger + JSONL event ledger + shadow-Git checkpointing + worktree-isolation). Per the canonical authority chain (`CLAUDE.md` §1.3: ADR → ADD → spec), the table and ADR quote are canonical; the prose count "four" is the drift. Fix direction: prose count `four` → `five`.

**Authority for the fix direction.** The fix is not a resolution chosen by the spec-writer — it is disambiguated in-file. §3 C-IS-03 "ADR commitment(s) honored" quotes ADR-F2 v1.2 §Decision / ADD §2.2 Synthesis verbatim: *"Git serves a combined role: code/spec/prompt/manifest/Skill versioning (foundational) + append-only state-ledger via commit stream + JSONL event ledger (foundational) + on-demand shadow-Git checkpointing (workload-class-opt-in) + worktree-isolation for concurrent sub-agent reads (workload-class-opt-in)"* — five distinct sub-roles. The §3 sub-role table's five rows match this verbatim enumeration. The prose count is corrected to agree.

**Changes inline.** §3 C-IS-03 current-tense prose count `four` → `five` at five sites:
1. §3 C-IS-03 "Contract surface." line — `Four-sub-role` → `Five-sub-role`.
2. §3 C-IS-03 "ADR commitment(s) honored." sub-section — the spec-author characterization prefix `combined git tier serving four-sub-role composition` → `combined git tier serving five-sub-role composition` (the parenthetical ADR/ADD verbatim quote that follows is **not** touched — it already enumerates five sub-roles and is preserved byte-exact).
3. §3 C-IS-03 "Specification content." opening prose — `The combined git tier serves four sub-roles within a single git repository` → `The combined git tier serves five sub-roles within a single git repository`.
4. §3 C-IS-03 "Sub-role co-residence contract." — `All four sub-roles share the same git repository identity` → `All five sub-roles share the same git repository identity`.
5. §3 C-IS-03 "Cross-sub-role consistency invariant." — `the four sub-roles share that ledger` → `the five sub-roles share that ledger`.

Status block (Status row revised; third pair of Revision row + Revision date row appended for v1.2 → v1.3). This Change-note section (new; appended between Status block and previous Change-note v1.1 → v1.2). No other content modified.

**Sections preserved verbatim at v1.3.** §Front-matter (Axis declaration; Axis-grounding note; PRD requirement scope; ADR scope; Persona-linkage substrate; Scope and out-of-scope); §1 C-IS-01 (canonical filesystem path contract); §2 C-IS-02 (five-tier layering schema; tier composition contract); §3 C-IS-03 sub-role table (the five-row enumeration — the canonical reading; the prose count aligns to the table, not vice-versa), "Persona linkage", the ADR/ADD verbatim quote, the co-residence bullet list, "Deferred to implementation discretion"; §4 C-IS-04 (atomic deploy contract); §5 C-IS-05 (state-ledger entry-shape signature); §6 C-IS-06 (hash-chain integrity); §7 C-IS-07 (read/write contract pair, all sub-sections, Deferred-to-implementation-discretion subsection); §8 C-IS-08 (shadow-Git checkpoint contract); §9 C-IS-09 (worktree-isolation contract, all sub-sections); §10 C-IS-10 (all substrate seam exports); §[carry-forwards]; §[traceability] matrix; §[coherence pass] (preserved verbatim as v1 + v1.1 point-in-time audit historical record); Change-note (v1.1 → v1.2) and Change-note (v1 → v1.1) (preserved verbatim as historical change-note record).

**Surfaced finding (not patched — preservation discipline).** §[coherence pass] Audit 6.1 "Contract grade" row historically attests *"C-IS-03 4-sub-role schema (enum)"*. This line is part of the §[coherence pass] section, which the v1.1 → v1.2 and v1 → v1.1 change-notes both explicitly declare preserved verbatim as the v1 + v1.1 point-in-time audit historical record. The v1 coherence pass did not catch the prose/table count drift; that line is an accurate historical record of what the v1 auditor recorded. Rewriting it would retroactively edit a historical audit attestation and break the §[coherence pass] preservation discipline. It is therefore **preserved verbatim** and surfaced here as a finding, not silently patched. A future fresh coherence pass (not in this revision's scope per the §[coherence pass] preservation discipline — `v1.1 → v1.2` change-note: "mechanical revision does not re-run the audit") would record "5-sub-role schema". The same preservation logic applies to the historical change-notes at the v1.1 → v1.2 and v1 → v1.1 entries, which describe what was preserved at prior versions and use the phrase "combined git tier role decomposition" without a count — no count drift exists there.

**Downstream absorption flagged for `implementation-planner` revision-pass.** Any IS-axis plan unit or cross-axis citation that characterizes C-IS-03 as a "four-sub-role" composition must absorb the corrected "five-sub-role" count. This is downstream plan-unit work, not a spec-writer edit; it is flagged here for `implementation-planner` revision-pass per the spec-writer back-reference reconciliation discipline. Intra-file: §4 C-IS-04's "Composition with C-IS-03 commit-stream sub-role" row and §8 C-IS-08 §8.4 "Composition with C-IS-03 sub-roles" cite C-IS-03 sub-roles without asserting a count — no intra-file stale count reference exists; no intra-file edit owed.

**§[coherence pass] preservation discipline.** §[coherence pass] section at v1.3 is preserved verbatim as the v1 + v1.1 point-in-time audit historical record, consistent with the v1.1 → v1.2 and v1 → v1.1 change-notes. The v1.2 → v1.3 mechanical count-correction does not re-run the audit.
