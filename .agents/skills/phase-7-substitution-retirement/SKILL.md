...
| **Unclear** | Halt + ask per `AGENTS.md` §11.6. |

Retirement-event filing legitimately edits per-axis `AGENTS.md` §4.1 rows + workspace `AGENTS.md` cumulative-counts — these are mode-agnostic operational files that DO appear under `harness-*/` (per-axis `AGENTS.md`) but the substantive change is bookkeeping, not impl. Treat as Phase 7 posture (the retirement-event filing closes a Phase 7 transit, not a design-substrate amendment).

Bundled-absorption arc discipline per `AGENTS.md` §11.4 applies — e.g., a retirement-event filing PR that also touches `design-substrate/**` to refresh a cross-cite carries a `.harness/` back-flow doc.
...
Per `Phase_7_Meta_Architecture_v1.md` §5 + workspace root `AGENTS.md` §4.1:
...
| Convention | 9 | `AGENTS.md` prose + sub-agent compliance; **most fragile retirement** (compliance verification at retirement) |
...
| (B) H_E surface no longer invoked at substitution site | Code review at the substitution site confirms the H_E surface (e.g., `Bash(git *)`, `AGENTS.md` convention, H_E Checkpointing) is no longer called for the substituted primitive |
...
| Workspace root `AGENTS.md` §4 | This workspace | Substitution + back-flow discipline summary |
| Per-axis `AGENTS.md` §4.1 | This workspace | Per-axis substitution surface summary |
