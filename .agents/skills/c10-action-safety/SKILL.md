...
description: Use when the operator names C10 or asks about action safety, trust boundaries, or blast radius.
---

# Canonical Claude Workflow Bridge

Read `.claude/skills/council/c10-action-safety-blast-radius/SKILL.md` completely from the workspace root before acting, including every reference it routes for the task. That canonical skill body is the full workflow; this Codex entrypoint does not summarize, trim, or replace it. Resolve its relative references from the canonical skill directory.
