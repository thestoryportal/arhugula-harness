...
| H_T-OD-3 (Composite Sampler) | **PARTIAL** 2026-05-28 (batch-34 STILL-BOUNDED → PARTIAL transit via PR #19 merge `b39dc50` per `.harness/phase-7d-retirement-events-batch-34.md`) | `HarnessCompositeSampler(Sampler)` ABC subclass at `harness-od/src/harness_od/composite_sampler.py` honoring §9.2 18-entry always-sampled set at SDK boundary via NEW `is_always_sampled(event_name)` helper at `sampling_mode.py` (decomposes `ALWAYS_SAMPLED_EVENT_CLASSES` frozenset into literals + dot-anchored prefixes; closes latent substrate bug — wildcards `"audit.*"` / `"validator.fail.*"` were stored as literal frozenset members; set-membership lookup would under-sample audit + permanent-fail spans; ZERO non-self callers pre-existed so bug never observable until SDK-boundary lift). `_DEFAULT_SAMPLER` at `harness-runtime/lifecycle/tracer_provider.py` swapped from stock `ParentBased(root=ALWAYS_ON)` → `build_default_sampler()` returning `ParentBased(root=HarnessCompositeSampler(base_rate=1.0))` per OTel canonical wrapping pattern. PARTIAL → RETIRE-READY gates on (a) tail-keep-on-classification at the OTLP collector boundary per §9.1 + (b) persona-tier-aware base_rate envelope per §10.3 — both deferred per §9.3 implementer-discretion clause. §9.2 4 conditional-by-attribute rows MVP-over-sampled conservatively (refining via `attributes` lookup is a follow-on arc). |
| H_T-OD-4 (Pre-Collector redaction SpanProcessor) | **PARTIAL** 2026-05-28 (batch-35 STILL-BOUNDED → PARTIAL transit via PR #22 RedactionSpanProcessor authoring + runtime wiring at `materialize_span_processor_stage` per `.harness/phase-7d-retirement-events-batch-35.md`) | `RedactionSpanProcessor(SpanProcessor)` ABC subclass at `harness-od/src/harness_od/redaction_span_processor.py` honoring OD spec C-OD-12 §12.1 13-attribute `DEFAULT_OFF_CONTENT_ATTRIBUTES` frozenset; strips at `on_end(span)` via `del span._attributes[key]` per OTel-Python redaction idiom (BoundedAttributes mutable while span non-immutable). Wired at `harness-runtime/src/harness_runtime/lifecycle/span_processor.py:materialize_span_processor_stage` BEFORE the BatchSpanProcessor on the TracerProvider per C-OD-13 §13.2 "pre-collector redaction at SDK / wrapper boundary BEFORE the BatchSpanProcessor buffer" mandate; `SpanProcessorStage` carries the redaction handle alongside processor + exporter for the shutdown chain. 14 unit tests at harness-od (default strip-set cardinality + OTel GenAI Opt-In subset + cross-namespace content surfaces + strip discipline + structure-attribute preservation + zero-attribute span tolerance + lifecycle no-ops + operator-injected custom strip-set) + 2 NEW runtime wiring tests verifying full §12.1 set stripped at export time. PARTIAL → RETIRE-READY gates on (a) per-persona-tier override toggle at solo-developer per C-OD-13 §13.1 acceptance #3 (persona_tier + per-session override flag deferred at HEAD) + (b) opaque-token tokenization mode per C-OD-13 §13.2 multi-tenant-compliance eval-grade pipeline (strip-not-tokenize MVP scope-lock per advisor 29th application). Default-off discipline IS hard-enforced at all 3 tiers at MVP; per-tier toggle deferred. Empirical posture: ZERO production `span.set_attribute(...)` calls against the 13 keys at HEAD (grep harness-{runtime,cp,as,od}/src) — PR is defense-in-depth; future producers gated. |
...
**OD-axis cumulative post-batch-35 (2026-05-28):** **3 / 8 RETIRED (37.5%, OD-2 GenAI binding + OD-5 cost-attribution 5-step chain + OD-8 authoring-close)** + **1 / 8 RETIRE-READY (12.5%, OD-6 sqlite-write-path 4-OD-B cluster wired; deployment-time-opt-in-gate terminal in-CLI state)** + **2 / 8 PARTIAL (25.0%, OD-3 composite sampler SDK-boundary wired + OD-4 NEW RedactionSpanProcessor wired at materialize_span_processor_stage)** + **2 / 8 STILL-BOUNDED (25.0%, OD-1 + OD-7)**. Pipeline advanced (R+RR+P): **6/8 = 75.0%** (+1 net advancement from batch-34 via OD-4 STILL-BOUNDED → PARTIAL at batch-35).
...
**PARTIAL → RETIRE-READY gates (OD-3 + OD-4 post-batch-35):**

- **OD-3 (Composite Sampler):** Substrate landed at batch-34 (`HarnessCompositeSampler` + SDK-boundary wiring at `_DEFAULT_SAMPLER`); PARTIAL → RETIRE-READY gates on (a) tail-keep-on-classification at the OTLP collector boundary per §9.1 (deferred per §9.3) + (b) persona-tier-aware base_rate envelope per §10.3 (deferred until `persona_tier` plumbed at materializer).
- **OD-4 (Pre-Collector redaction SpanProcessor):** Substrate landed at batch-35 (`RedactionSpanProcessor` + runtime wiring at `materialize_span_processor_stage`); PARTIAL → RETIRE-READY gates on (a) per-persona-tier override toggle at solo-developer per C-OD-13 §13.1 acceptance #3 (persona_tier + per-session override flag deferred at HEAD) + (b) opaque-token tokenization mode per C-OD-13 §13.2 multi-tenant-compliance eval-grade pipeline (strip-not-tokenize MVP scope-lock).
