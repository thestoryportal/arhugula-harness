...
**OD-axis cumulative post-batch-37 (2026-05-28):** **4 / 8 RETIRED (50.0%, OD-1 deferral-envelope authoring-only NEW + OD-2 GenAI binding + OD-5 cost-attribution 5-step chain + OD-8 authoring-close)** + **2 / 8 RETIRE-READY (25.0%, OD-3 composite sampler + OD-6 sqlite-write-path; both at deployment-time-opt-in-gate terminal in-CLI state)** + **1 / 8 PARTIAL (12.5%, OD-4 RedactionSpanProcessor — gate (a) §13.1 per-persona toggle PARTIALLY-CLOSED at PR #25 deployment-level; per-session toggle + gate (b) §13.2 opaque-token tokenization deferred)** + **1 / 8 STILL-BOUNDED (12.5%, OD-7 only)**. Pipeline advanced (R+RR+P): **7/8 = 87.5%** (+12.5 percentage points from batch-36 via OD-1 STILL-BOUNDED → RETIRED out-of-pipeline tier promotion per X-AL-2 — OD-1 categorical-mismatch H_E surface + substrate-IS-contract pattern closes at authoring-only RETIRED).
...
**STILL-BOUNDED → PARTIAL gates (OD-7 only post-batch-37; OD-1 closed at batch-37 via sub-species 10 audit):**
...
*OD-1 (deferral envelope) closed at batch-37 STILL-BOUNDED → RETIRED-AS-AUTHORING-ONLY via doc-hygiene reclassification per workflow v1.12 §7.4.7.2 sub-species 10 audit — see OD-1 row above + `.harness/phase-7d-retirement-events-batch-37.md`.*
