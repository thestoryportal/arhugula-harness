...
> **🎓 PHASE-8 GRADUATION (2026-06-02).** Phase 8 substitution accounting is CLOSED per `.harness/phase-8-graduation.md` (canonical 46/54 RETIRED + 49/54 pipeline-advanced). OD-axis dispositions: OD-2/3/5 RETIRED-substantive; OD-1/7/8 RETIRED-AS-AUTHORING-ONLY; OD-6 RETIRED-AS-BOUNDED-RESIDUAL; **OD-4 = `RETIRED-AS-CROSS-AXIS-DEFERRED`** (gate (a) §13.1 closed #244; gate (b) §13.2 cross-axis — pipeline-advanced PARTIAL, NOT counted in the 46; R-008). **Canonical source (R-600): these dispositions + counts are DERIVED from `.harness/substitutions.yaml` via `tools/substitution_ledger.py`; cite the derivation, don't hand-maintain.** Prior batch-cumulative figures below stand verbatim per forward-only discipline.
...
|---|---|---|
| H_T-OD-1 (deferral envelope) | **RETIRED** 2026-05-28 (batch-37 STILL-BOUNDED → RETIRED-AS-AUTHORING-ONLY via doc-hygiene reclassification per workflow v1.12 §7.4.7.2 sub-species 10 `gate-text-stale-vs-production-landings` audit) | Substrate `deferral_envelope.py` at `harness-od/src/harness_od/deferral_envelope.py` IS the C-OD-03 contract realization at U-OD-03 landing — 6-entry `COMMITTED_AT_D6_SURFACES` per plan acc #2 + 23-entry `DEFERRED_SURFACES` per plan acc #5 + §3.3 boundary-invariant `assert` at module load (lines 312–319). Pre-batch-37 gate text framed retirement as gated on "runtime composer importing `deferral_envelope` + scope-deferral typed primitive replacing `CLAUDE.md`-prose convention at runtime"; empirical audit this batch discriminates: (1) OD spec C-OD-03 §3 is a **static deferral-signature contract** — §3.1 12 committed surfaces; §3.2 5 deferred-surface categories; §3.3 boundary invariants enforced at static layer; selection-validation mechanism explicitly **deferred to implementer discretion** at line 263; NO mandate for runtime enforcement loop; (2) **Zero hits** at production grep for "CLAUDE.md-prose scope-deferral convention" across `harness-{runtime,cp,as,od,core}/` — phantom convention; (3) H_E substitution surface = `ToolSearch` per Meta-Architecture §5.5 row 1 = **categorical mismatch**; never engaged as substitution invocation site. X-AL-2 second conjunct vacuously satisfied. Mirror precedent: H_T-OD-8 (aggregate manifest + Stage 3b inversion) RETIRED at v1 §1 authoring-only. Authoring a runtime enforcement loop would be **X-AL-3 silent extension** under cover of stale gate-text framing. FIRST sub-species 10 closure in retirement ledger. Operator-ratified routing (α) at AskUserQuestion 2026-05-28 over (β) STILL-BOUNDED-INDEFINITELY + (γ) Class 1 fork + (δ) X-AL-3 build. ZERO production code change; ZERO test addition; ZERO spec / plan amendment; ZERO cross-axis cascade. |
...
| H_T-OD-3 (Composite Sampler) | **RETIRED** 2026-06-01 (batch-51 RETIRE-READY → RETIRED substantive transit per `.harness/phase-7d-retirement-events-batch-51.md` §1; closes roadmap R-007; gate-text-stale-vs-production-landings audit (sub-species 10, THIRD closure) — `HarnessCompositeSampler` live at MVP via `materialize_tracer_provider_stage`, H_E 7a-scaffold sampler no longer invoked, R-100-mvp-real-workflow-execution e2e exercised the production tracer provider with real spans; "tail-keep at OTLP collector observing §10.2 preservation" reframed as R-430 production-FEATURE-validation (infra-gated), NOT an X-AL-2 retirement gate. Operator-ratified AskUserQuestion 2026-06-01. Prior: RETIRE-READY 2026-05-28 batch-36 PARTIAL → RETIRE-READY transit per `.harness/phase-7d-retirement-events-batch-36.md`; gate (a) §9.1 tail-keep-on-classification CLOSED at OD spec v1.27 + `TailKeepSpanProcessor` substrate + production-surface materializer wrap; gate (b) §10.3 base_rate envelope CLOSED at PR #25 v1.26; both OD-3 PARTIAL → RETIRE-READY gates closed) | `HarnessCompositeSampler(Sampler)` ABC subclass at `harness-od/src/harness_od/composite_sampler.py` honoring §9.2 18-entry always-sampled set at SDK boundary via NEW `is_always_sampled(event_name)` helper at `sampling_mode.py` (decomposes `ALWAYS_SAMPLED_EVENT_CLASSES` frozenset into literals + dot-anchored prefixes; closes latent substrate bug — wildcards `"audit.*"` / `"validator.fail.*"` were stored as literal frozenset members; set-membership lookup would under-sample audit + permanent-fail spans; ZERO non-self callers pre-existed so bug never observable until SDK-boundary lift). **Sampler base_rate envelope materialized at PR #25 apply (gate (b) closed)**: `materialize_tracer_provider_stage` reads `config.persona_tier` + `config.deployment_surface` and resolves `base_rate` per OD spec §10.3 8-row table via `PER_CELL_BASE_RATE_ENVELOPE[CellID(...)].default_rate`; pre-PR-#25 module-level `_DEFAULT_SAMPLER` constant (forcing `base_rate=1.0` at every cell) RETIRED. **Gate (a) §9.1 tail-keep-on-classification CLOSED at batch-36 via OD spec v1.27 canonical-reading amendment + NEW `TailKeepSpanProcessor` substrate at `harness-od/src/harness_od/tail_keep_span_processor.py` + NEW `is_classification_trigger(span)` predicate at `harness-od/src/harness_od/tail_keep_classification.py` + production-surface wrap at `materialize_span_processor_stage` (wraps BSP iff `config.deployment_surface != LOCAL_DEVELOPMENT`; bypassed at local-development per §9.1 head-based mandate). Per-trace-completion-replay algorithm: buffer non-always-sampled spans by `trace_id`; root-close detected by `span.parent is None`; flush-or-drop materialization at root close based on §10.2 trigger-set (validator.fail.permanence=permanent + sandbox.violation span name + breaker.tripped span name). Wrap-BSP pattern: TailKeepSpanProcessor wraps BSP; registered on TracerProvider instead of BSP directly. Always-sampled spans (§9.2) bypass buffer (forward immediately); their tree-siblings benefit from per-trace keep flag.** Gate (b) persona-tier-aware base_rate envelope per §10.3 — **CLOSED at PR #25 merge**. Both PARTIAL → RETIRE-READY gates closed → tier transit per X-AL-2 at batch-36. RETIRE-READY → RETIRED gates on operator deploys harness at production deployment surface + real OTel span emission against OTLP collector observing §10.2 preservation semantic (classified traces preserved; unclassified dropped at root close). FOURTH member of sub-species 7.deployment-time-opt-in-gate (sibling to AS-8d batch-31 + OD-5 batch-32 + OD-6 batch-33). §9.2 4 conditional-by-attribute rows MVP-over-sampled conservatively (refining via `attributes` lookup is a follow-on arc). |
| H_T-OD-4 (Pre-Collector redaction SpanProcessor) | **PARTIAL (refined)** 2026-05-28 (batch-35 STILL-BOUNDED → PARTIAL transit via PR #22; gate (a) partially closed at PR #25 merge per `.harness/class_1_fork_od_3_od_4_retire_ready_persona_tier_plumbing.md` apply arc) | `RedactionSpanProcessor(SpanProcessor)` ABC subclass at `harness-od/src/harness_od/redaction_span_processor.py` honoring OD spec C-OD-12 §12.1 13-attribute `DEFAULT_OFF_CONTENT_ATTRIBUTES` frozenset; strips at `on_end(span)` via `del span._attributes[key]` per OTel-Python redaction idiom (BoundedAttributes mutable while span non-immutable). Wired at `harness-runtime/src/harness_runtime/lifecycle/span_processor.py:materialize_span_processor_stage` BEFORE the BatchSpanProcessor on the TracerProvider per C-OD-13 §13.2 "pre-collector redaction at SDK / wrapper boundary BEFORE the BatchSpanProcessor buffer" mandate; `SpanProcessorStage` carries the redaction handle alongside processor + exporter for the shutdown chain. 14 unit tests at harness-od + 2 NEW runtime wiring tests verifying full §12.1 set stripped at export time. **Per-persona override toggle deployment-level plumbed at PR #25 apply (gate (a) partially closed)**: `materialize_span_processor_stage` threads `config.persona_tier` (NEW `RuntimeConfig.persona_tier` field at runtime spec v1.37 §3 C-RT-03) to `RedactionSpanProcessor(persona_tier=config.persona_tier, ...)`; NEW `MultiTenantOverrideRefusedError` typed exception class refuses empty `redacted_attributes` frozenset at multi-tenant-compliance per §13.1 row 3 non-toggleability. Solo-developer's §13.1 "OPERATOR_SELF_REDACT" per-session toggle mechanism **CLOSED 2026-06-02 (R-008 gate (a) arc)**: `session_content_capture()` ContextVar + `on_end` honoring at `harness-od/src/harness_od/redaction_span_processor.py`, applied ONLY at solo-developer (`PER_PERSONA_TIER_REDACTION[...].toggleable=True`); team-binding + multi-tenant IGNORE it (§13.3 monotonic-tightening / downgrade-rejection). Grounding-first finding: the fork-doc-framed "session-control-substrate arc" was already satisfied by the existing per-session ContextVar idiom (`_CURRENT_TOOL_CTX`) — no heavy substrate owed; X-AL-3-clean (spec'd mechanism; §13.3 defers only the toggle UX). 10 new tests incl. async-boundary propagation (create_task + to_thread). **Gate (a) per §13.1 acceptance #3 now FULLY CLOSED** (deployment-level persona_tier + multi-tenant non-toggleability at PR #25; solo-developer per-session toggle this arc). PARTIAL → RETIRE-READY — SOLE REMAINING gate: (b) opaque-token tokenization mode per §13.2 multi-tenant-compliance eval-grade pipeline — **CROSS-AXIS-BLOCKED** (`c10-action-safety` SKILL.md eval-grade pipeline producing semantic `[REDACTED:PII]` tokens per OD spec v1.2 line 749 + §13.3 deferral; Phase-6+; NOT an OD-axis stdlib slice — semantic classification required). Default-off discipline IS hard-enforced at all 3 tiers at MVP (only an explicit solo-developer opt-in changes anything). Row stays at PARTIAL (refined) per workflow v1.12 §7.4.7.3.C + X-AL-2 (gate (a) FULLY closed; gate (b) cross-axis-blocked → no transit to RETIRE-READY; RETIRE-READY is NOT OD-axis-Claude-closeable). Empirical posture: ZERO production `span.set_attribute(...)` calls against the 13 keys at HEAD — PR is defense-in-depth; future producers gated. |
...
| H_T-OD-6 (Local-first OTLP ingestion) | **RETIRED-AS-BOUNDED-RESIDUAL** 2026-06-01 (batch-51 RETIRE-READY → RETIRED-AS-BOUNDED-RESIDUAL per `.harness/phase-7d-retirement-events-batch-51.md` §2; closes roadmap R-009; **FIRST bounded-residual close in the ledger** per X-AL-2 §5.3 — 4-OD-B SqliteWritePath substrate LANDED but `RuntimeRingBuffer.flush_to_sqlite` is dormant at MVP (zero production callers; collector→sqlite loop not wired into the run path), boundary unmoved; substantive RETIRED gated on operator deployment wiring the collector daemon (future milestone roadmap R-420/R-421, infra-gated). Operator-ratified AskUserQuestion 2026-06-01. Prior: RETIRE-READY 2026-05-28 batch-33 PARTIAL → RETIRE-READY transit via PR #18 4-OD-B SqliteWritePath cluster merge `406fbf5` per `.harness/phase-7d-retirement-events-batch-33.md`; sibling same-arc transit to OD-3 batch-34 — first within-OD-axis joint same-arc tier advancement) | 4-OD-B SqliteWritePath cluster LANDED: U-OD-42 (14-col `spans` schema per §C-OD-27.1 + WAL + foreign_keys=OFF pragmas per §27.2 row 2 + 4 indexes including composite `idx_workflow` + composite `idx_time_range` + idempotent `CREATE TABLE IF NOT EXISTS` re-init at `harness-od/src/harness_od/sqlite_span_store.py`) + U-OD-43 (`SpanInsertRow` Pydantic v2 frozen 14-field carrier + `insert_spans(conn, rows)` executemany INSERT OR IGNORE helper at OD axis + NEW `async RuntimeRingBuffer.flush_to_sqlite(conn, *, now_ns)` at runtime axis with `_project_span_row` schema-gap projection from placeholder 6-field `SpanRow` to 14-field `SpanInsertRow` via OTel-canonical defaults; asyncio.to_thread dispatch for blocking sqlite calls) + U-OD-44 (`retention_cleanup_lazy(conn, retention_days, now_ns)` helper per §27.5 row 2 lazy-on-write default; operator-configurable `CollectorConfig.sqlite_retention_days: int = 7` at runtime) + U-OD-45 (typed read interface at `harness-od/src/harness_od/sqlite_span_store_reader.py` — `read_spans_by_workflow` / `read_spans_by_trace` / `read_span_by_id` parameterized SQL only per §C-OD-27.3). Co-published OD plan v2.24 + OD spec v1.25 doc closures absorbing 4 of 5 drift carries (column count 12→14, foreign_keys=OFF pragma, phantom carrier-path → axis-split, retention config field; phantom `U-RT-30` cite refresh at §C-OD-27 spec body). Cross-axis convention: Option (B) axis-split sets axis-ownership precedent — schema-pure SQL helpers at OD axis; lifecycle composition at runtime axis. RETIRE-READY → RETIRED gates on (a) operator deploys harness against real workload generating spans; (b) collector daemon ingests spans into in-memory buffer; (c) orchestrator invokes `RuntimeRingBuffer.flush_to_sqlite(conn, ...)` against the runtime sqlite span store; (d) sqlite spans table observed populated at the deployment's `.harness/observability/spans.db` path. THIRD member of sub-species 7.deployment-time-opt-in-gate (sibling to AS-8d batch-31 + OD-5 batch-32). TUI substrate authoring still deferred per §C-OD-27.3 implementer-discretion. |
| H_T-OD-7 (Preservation invariants 5-dimension) | **RETIRED** 2026-05-28 (batch-38 STILL-BOUNDED → RETIRED-AS-AUTHORING-ONLY via doc-hygiene reclassification per workflow v1.12 §7.4.7.2 sub-species 10 `gate-text-stale-vs-production-landings` audit; SECOND sub-species 10 closure same-session-sequel to OD-1 batch-37) | Substrate `per_dimension_preservation_invariants.py` at `harness-od/src/harness_od/per_dimension_preservation_invariants.py` IS the C-OD-22 §22.2 + §22.4 contract realization at U-OD-33 landing — 5-entry `PreservationDimension` StrEnum verbatim per §22.2 (SAMPLING_DISCIPLINE + CARDINALITY_BUDGET + REDACTION_CLASS + GATE_POLICY + SANDBOX_TIER) + `PRESERVATION_INVARIANTS` declares per-dimension `invariant_form` + `enforcement_layer` + cross-axis composition targets per U-OD-33 AC #2. Per-dimension enforcement breakdown: 2 dims (SAMPLING_DISCIPLINE + REDACTION_CLASS) = `DESIGN_TIME_VERIFICATION` at U-OD-32 `verify_transition`; 1 dim (CARDINALITY_BUDGET) = `RUNTIME_ENFORCEMENT_AT_COLLECTOR_BOUNDARY` composes with U-OD-31 multi-tenant `cross_tenant_aggregation_forbidden=True` at `multi_tenant_trace_separation_and_audit_ledger.py:158+164` + audit-writer reference at `harness-runtime/.../lifecycle/audit_writer.py:32` (multi-tenant cells 7+8 only; operator deployment-time opt-in); 2 dims (GATE_POLICY + SANDBOX_TIER) = `CROSS_AXIS_COMPOSITION_VERIFICATION` satisfied at CXA v2.x landings (C-CP-19 + C-AS-12 §12.1). Pre-batch-38 gate text framed retirement as "runtime enforcement loop invoking `per_dimension_preservation_invariants` against runtime ledger entries"; empirical audit this batch discriminates: (1) C-OD-22 is **bridging-arc transition contract** at deployment-binding-time operator surface per §22.3 NOT continuous runtime ledger loop; (2) "Specific bridging-arc-binding state machine" explicitly **deferred to implementer discretion** at OD spec v1.2 line 1271; (3) H_E substitution surface = manual operator verification per Meta-Arch §5.5 row 7 = no automated H_E surface to retire (vacuous X-AL-2 second conjunct); (4) Meta-Arch §5.5 row 7 descriptive vocab `SCHEMA/CARDINALITY/ORDERING/IDEMPOTENCY/TRACEABILITY` drifts from canonical C-OD-22 §22.2 set — refreshed at this batch as Class 3 informational doc-hygiene per authority chain (C-OD-22 wins; Meta-Arch is Phase-7 governance). Mirror precedent: H_T-OD-1 batch-37 + H_T-OD-8 v1 §1 — H_T primitive whose contract is "the typed declaration itself" rather than "a runtime behavior." Authoring a runtime enforcement loop on the 5 dimensions would be **X-AL-3 silent extension** under cover of stale gate-text framing. SECOND sub-species 10 closure in retirement ledger (same-session-sequel to OD-1 batch-37). Operator-ratified routing (α) at AskUserQuestion 2026-05-28 over (β) defer-Meta-Arch + (γ) Class 1 fork + (δ) X-AL-3 build. ZERO production code change; ZERO test addition; ZERO OD spec / plan amendment; ZERO cross-axis cascade (intra-OD-axis doc-hygiene only + Meta-Arch descriptive vocab refresh). |
| H_T-OD-8 (aggregate manifest + Stage 3b inversion) | RETIRED (authoring close, v1 §1) | Authoring-only |

**OD-axis cumulative post-batch-51 (2026-06-01):** **7 / 8 RETIRED (87.5%, OD-1 deferral-envelope authoring-only + OD-2 GenAI binding + OD-3 composite sampler NEW substantive (batch-51) + OD-5 cost-attribution 5-step chain + OD-6 sqlite-write-path NEW RETIRED-AS-BOUNDED-RESIDUAL (batch-51) + OD-7 preservation-invariants authoring-only + OD-8 authoring-close)** + **0 / 8 RETIRE-READY (0.0%; OD-axis RETIRE-READY bucket EMPTY for the first time)** + **1 / 8 PARTIAL (12.5%, OD-4 RedactionSpanProcessor — gate (a) §13.1 per-session toggle CLOSED 2026-06-02 (R-008); gate (b) §13.2 opaque-token tokenization CROSS-AXIS-BLOCKED at c10-action-safety/Phase-6+ — within-PARTIAL refinement, no tier transit)** + **0 / 8 STILL-BOUNDED (0.0%)**. Pipeline advanced (R+RR+P): **8/8 = 100.0%** (UNCHANGED; OD-3 + OD-6 are within-pipeline-advanced RETIRE-READY → RETIRED transits at batch-51 per X-AL-2). OD-6 carries the **RETIRED-AS-BOUNDED-RESIDUAL** sub-disposition (FIRST in ledger) — substantive RETIRED gated on operator deployment wiring `flush_to_sqlite` (roadmap R-420/R-421, infra-gated). Closes roadmap R-007 + R-009 (2 R-700 Phase-8 blockers drained). *Prior: post-batch-38 = 5/8 RETIRED + 2/8 RETIRE-READY (OD-3 + OD-6) + 1/8 PARTIAL (OD-4); refreshed at this tier-transit per workflow v1.12 §7.4.7.3.C audit.*
...
- **OD-6 (local-first OTLP ingestion):** **RETIRED-AS-BOUNDED-RESIDUAL 2026-06-01 (batch-51).** Refreshed at this tier-transit per workflow v1.12 §7.4.7.3.C. The X-AL-2 condition-(B) audit found `RuntimeRingBuffer.flush_to_sqlite` dormant at MVP (zero production callers; the collector→sqlite loop is not wired into the run path), so the boundary has not moved — substantive RETIRED is not honestly available at MVP. Closed RETIRED-AS-BOUNDED-RESIDUAL per X-AL-2 §5.3 (FIRST bounded-residual close in the ledger; closes roadmap R-009). Future milestone unblocking substantive RETIRED: operator deploys with the collector daemon wired (`flush_to_sqlite` invoked; `.harness/observability/spans.db` observed populated) — roadmap R-420/R-421, infra-gated. *Prior: RETIRE-READY 2026-05-28 batch-33.*

**OD-3 + OD-6 PARTIAL → RETIRE-READY gates — CLOSED (both RETIRED at batch-51):**

- **OD-3 (Composite Sampler):** **RETIRED 2026-06-01 (batch-51, substantive).** Both PARTIAL → RETIRE-READY gates closed at batch-36 (gate (a) §9.1 tail-keep substrate + gate (b) §10.3 base_rate envelope); RETIRE-READY → RETIRED closed at batch-51 via the gate-text-stale-vs-production-landings audit — the sampler is live at MVP via `materialize_tracer_provider_stage`, the H_E 7a-scaffold sampler is no longer invoked, and "tail-keep at OTLP collector observing §10.2 preservation" is R-430 production-feature-validation (infra-gated), NOT an X-AL-2 retirement gate. Closes roadmap R-007. *Prior gate-text (pre-batch-36) framed OD-3 as PARTIAL with §9.1 tail-keep + §10.3 base_rate gates open; superseded by batch-36 (gates closed) then batch-51 (RETIRED).*

**STILL-BOUNDED → PARTIAL gates (NONE post-batch-38; OD-axis STILL-BOUNDED bucket EMPTY):**

*Both prior STILL-BOUNDED rows closed via sub-species 10 doc-hygiene audits same calendar day 2026-05-28:*
- *OD-1 (deferral envelope) closed at batch-37 STILL-BOUNDED → RETIRED-AS-AUTHORING-ONLY — see `.harness/phase-7d-retirement-events-batch-37.md`.*
- *OD-7 (preservation invariants) closed at batch-38 STILL-BOUNDED → RETIRED-AS-AUTHORING-ONLY — see `.harness/phase-7d-retirement-events-batch-38.md`.*
...
- **OD-4 (Pre-Collector redaction SpanProcessor):** Substrate landed at batch-35; **gate (a) §13.1 per-persona-tier override toggle PARTIALLY CLOSED at PR #25 merge** (deployment-level persona_tier plumbed at `materialize_span_processor_stage` → `RedactionSpanProcessor(persona_tier=config.persona_tier)`; multi-tenant-compliance non-toggleability NEW-enforced via `MultiTenantOverrideRefusedError` typed exception at construction per §13.1 row 3). **Gate (a)-residual per-session toggle at solo-developer CLOSED 2026-06-02 (R-008)** — `session_content_capture()` ContextVar at `redaction_span_processor.py`; grounding found the existing per-session ContextVar idiom already serves as the "session-control substrate", so no heavy arc was owed (`[[grounding-reveals-claude-closeable-slice-close-honestly]]`). SOLE REMAINING gate to RETIRE-READY: (b) opaque-token tokenization mode per §13.2 multi-tenant-compliance eval-grade pipeline — **CROSS-AXIS-BLOCKED** at `c10-action-safety` (Phase-6+ semantic-classification pipeline, NOT an OD stdlib slice). Row stays at PARTIAL (refined) per X-AL-2 (gate (a) FULLY closed; gate (b) cross-axis-blocked → no transit to RETIRE-READY; not OD-axis-Claude-closeable).
