...
| H_T-OD-5 (Cost-attribution 5-step chain) | **PARTIAL** (within-PARTIAL advance at batch-27 — 2 of 4 dispatch surfaces wired, 2026-05-28) | Cost-attribution wired at **LLM dispatch** site via U-OD-38 landing (`7104fd7`, cluster 4-OD-D) + **tool dispatch** site via U-OD-39 landing at `7e513c8`. U-OD-39 module + 12 unit tests at `0e84c94` + production binding at `7e513c8`: RuntimeToolDispatcher.__init__ extended with cost_chain/audit_writer/rate_table; `_attribute_tool_cost_best_effort` private method wires 5 invocation sites (1 success + 4 exception paths: timeout / protocol / transport / schema_violation) per AC #1 success+failure invocation discipline; mcp.tool.call piggybacks parent tool.dispatch per AC #3 (§C-OD-26.2 table). 5-step chain operational at 2 of 4 dispatch surfaces (LLM + tool); **validator + webhook dispatch sites remain at U-OD-40 next-arc scope** (bundled atomic unit per OD plan v2.14 — CPU-meter at validator + `WebhookRate.flat_per_attempt` + egress at webhook). Cross-axis blockers cited at the 2026-05-23 framing CLEARED: U-RT-67/U-RT-69 tool-invocation composer LANDED at L9-sexies/septies 2026-05-22; U-CP-60 validator framework LANDED at cluster 10-CP-A; U-CP-72 audit-write seam ALL-RESIDUALS-CLOSED at batch-18 2026-05-24 per `[[fork-u-cp-72-cost-and-pause-resume-prefix-gap]]`. RATE_TABLE_V1 substrate empirically exercised at production binding: `tool_rates` operative at 3 cost_kind formula branches; `cpu_rate_per_ms` + `webhook_rate` + `egress_rate_per_byte` substrate ready for U-OD-40 consumption. Audit-ledger wiring residual per `[[fork-cost-record-audit-ledger-wiring-residual]]` CLOSED at LLM landing; tool-dispatch reuses same audit-write seam (CXA v2.9 row 8 + OD spec v1.10 §C-OD-26.6 + cp_audit_conversion.py `cost:` branch all live) |
