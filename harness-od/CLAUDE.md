...
| H_T-OD-6 (Local-first OTLP ingestion) | PARTIAL | Collector daemon (U-RT-29) + in-memory ring buffer wired; sqlite write path NOT landed — canonical closure path is **`4-OD-B SqliteWritePath` cluster (U-OD-42 schema + U-OD-43 flush + U-OD-44 retention + U-OD-45 typed read; authored at OD plan v2.14 `Phase D iteration-1 F1-04 absorption`)**; sibling cluster `4-OD-C rate-table` LANDED but `sqlite_span_store.py` absent at HEAD; TUI also absent per OD spec v1.8 §C-OD-27.3 deferral. Pre-impl-arc bookkeeping refresh 2026-05-28 per `.harness/class_3_drift_od_6_phantom_u_rt_30_and_4_od_b_decomposition.md` — prior cite `U-RT-30 PARTIAL-LAND, AC #2 STRUCK` was a phantom unit-ID carry (`U-RT-30` was forward-looking informal cite at `collector_daemon.py:69 + :160 + :233` authored at U-RT-29 landing; never materialized as a plan unit; was decomposed into U-OD-42..U-OD-45 at OD plan v2.14). Advisor pre-substantive verification 25th application caught the phantom + missed-decomposition before any production work. NOTE: compound-irrelevance lifted post-batch-2 (spans actively emit) — collector now has actual ingest pressure; sqlite write site + TUI gaps materially observable |
...
**OD-axis cumulative post-batch-32 (2026-05-28):** **3 / 8 RETIRED (37.5%, OD-2 GenAI binding + OD-5 cost-attribution 5-step chain + OD-8 authoring-close)** + **0 / 8 RETIRE-READY (bucket EMPTY)** + **1 / 8 PARTIAL (12.5%, OD-6 collector daemon — sqlite write site + TUI gaps)** + **4 / 8 STILL-BOUNDED (50%, OD-1 + OD-3 + OD-4 + OD-7)**. Pipeline advanced (R+RR+P): **4/8 = 50.0%** (preserved across batches 12 → 28 within-tier PARTIAL → RETIRE-READY transit at OD-5).
...
- **OD-6 (local-first OTLP ingestion):** gate on `4-OD-B SqliteWritePath` cluster landing (U-OD-42 + U-OD-43 + U-OD-44 + U-OD-45 per OD plan v2.14 — `sqlite_span_store.py` NEW + `ring_buffer.py` EXTEND + `sqlite_span_store_reader.py` NEW) + TUI substrate authoring (deferred per OD spec v1.8 §C-OD-27.3). Operator-discretion timing. AC audit at impl arc owed: U-OD-42 AC #1 cites "12 columns" but OD spec v1.8 §C-OD-27.1 declares 14 columns — canonical-reading amendment owed at OD plan v2.24 (sibling delta); §C-OD-27 spec-body `U-RT-30` cites also phantom — refresh owed at OD spec v1.25 (informational; production follows spec semantics regardless of cite shape). Both deferred per FM-2.
