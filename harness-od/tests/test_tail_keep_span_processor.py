...
**Ordinary-root fixture: `ordinary.root`, and it must stay a NON-member.** Every
buffering test here needs a root that carries no §9.2 floor, so that reaching
`downstream` proves the tail's flush-or-drop decision rather than the name arm's
immediate forward. This module used `workflow.envelope` for that role until OD
spec v1.42 admitted the envelope to §9.2 as row 20 (`B-137` step (3) candidate
C1, operator-ratified 2026-08-16) — at which point seven tests went red and,
worse, several others would have kept passing for the opposite reason (forwarded
by the name arm, never buffered at all). Do not reintroduce a §9.2 member as this
fixture; a member name silently converts a buffering test into a passthrough test.
"""
...
def test_arbitrary_root_span_drops_when_no_trigger_present() -> None:
    ...
    tracer = provider.get_tracer(__name__)
    with tracer.start_as_current_span("ordinary.root") as _root:
        # ordinary.root is NOT in §9.2 always-sampled set; trace has no
...
def test_root_span_with_sandbox_violation_child_keeps_full_tree() -> None:
    ...
    # ordinary.root + sibling.work buffered, then flushed on root close
    ...
    assert "ordinary.root" in forwarded_names
...
def test_files_operation_mutation_force_forwards_at_tail() -> None:
    ...
    assert "ordinary.root" not in {s.name for s in recorder.on_end_calls}
...
def test_subagent_span_nonroot_failed_force_forwards_immediately() -> None:
    ...
    assert "ordinary.root" in {s.name for s in recorder.on_end_calls}
...
def test_force_flush_keeps_all_buffered_traces_to_avoid_silent_loss() -> None:
    ...
    span = tracer.start_span("ordinary.root.partial")
...
def test_classification_trigger_in_one_trace_does_not_affect_other_trace() -> None:
    ...
    with tracer.start_as_current_span("ordinary.root.A"):
        with tracer.start_as_current_span(SANDBOX_VIOLATION_SPAN_NAME):
            pass
    # Trace B: no trigger → drop the root.
    with tracer.start_as_current_span("ordinary.root.B"):
        with tracer.start_as_current_span("plain.work"):
            pass

    forwarded_names = [s.name for s in recorder.on_end_calls]
    assert "ordinary.root.A" in forwarded_names
    assert SANDBOX_VIOLATION_SPAN_NAME in forwarded_names
    assert "ordinary.root.B" not in forwarded_names
...
def test_in_memory_export_via_tail_keep_chain_preserves_classified_trace() -> None:
    ...
    assert "ordinary.root" in exported_names
...
def test_max_spans_per_trace_drops_overflow_non_root_spans() -> None:
    ...
    provider.add_span_processor(proc)
    tracer = provider.get_tracer(__name__)
    root = tracer.start_span("ordinary.root")  # un-ended (no root-close yet)
...
def test_bounds_do_not_affect_legitimate_classified_trace() -> None:
    ...
    assert "ordinary.root" in forwarded
...
def test_root_close_only_trace_does_not_evict_at_saturation() -> None:
    ...
    with tracer.start_as_current_span("ordinary.root.short"):
        pass
    assert proc.buffered_trace_count == 2
    assert proc.dropped_trace_count == 0
    assert len(roots) == 2
