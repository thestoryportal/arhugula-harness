...
**Status:** VETTED DESIGN 2026-06-22 — operator AUTHORIZED the build (AskUserQuestion, this session) over the other 2 gated arcs + R-CL-Q1. The two committed-behavior gates (a NEW §22.1 `PauseReason` value + the suppress-and-continue recovery semantic that relaxes the fence's deliberate fail-closed) are **operator-ratified by that choice**. This doc is the design proposal for the **fresh-context BUILD** to execute — authored with the grounding live + advisor-vetted, so fresh context executes a ratified design rather than re-deriving the crux. **NOT yet built.**
