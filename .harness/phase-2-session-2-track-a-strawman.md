...
| F-P2-3 | **Tracer provider initialization site** — U-OD-23's `get_tracer_provider()` requires a configured provider; no spec section names who configures it, when in bootstrap, or against what exporter endpoint. | §3 stage 4 | ✅ **RESOLVED 2026-05-19** — `harness-runtime/` owns lifecycle at bootstrap stage 4; OD spec stays authoritative for the schema. See `phase_2_forks_F-P2-3_4_5_runtime_lifecycle_ownership.md`. |
| F-P2-4 | **Provider SDK lifecycle ownership** — anthropic / openai / ollama clients have construction + close lifetimes. The CP routing core consumes them but no spec section names the owner. | §3 stage 3 | ✅ **RESOLVED 2026-05-19** — `harness-runtime/` owns lifecycle at bootstrap stage 3; CP spec stays authoritative for the capability-aware abstraction. See `phase_2_forks_F-P2-3_4_5_runtime_lifecycle_ownership.md`. |
| F-P2-5 | **In-process OTLP collector daemon start** — U-OD-27 is library-only ("no running collector, no live TUI, no sqlite connection, no daemon"). The runtime starts the daemon — the spec doesn't say when or how. | §2 OD bootstrap; §3 stage 4 | ✅ **RESOLVED 2026-05-19** — `harness-runtime/` owns daemon lifecycle at bootstrap stage 4; OD spec stays authoritative for the placement matrix + emission discipline. TUI lifecycle likely Track B. See `phase_2_forks_F-P2-3_4_5_runtime_lifecycle_ownership.md`. |
