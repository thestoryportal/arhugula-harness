...
|---|---|---|---|
| F-P2-1 | **Composition-root package placement** — new `harness-runtime/` vs co-locate in `harness-cxa/` vs another option. Runtime-gap record names new package as candidate. | §2 + repo layout | Operator design decision. Trade-off: new package = clean separation from CXA seam instantiation, mirrors axis-package shape, requires uv workspace member add. Co-locate in `harness-cxa/` = no new package, but conflates "seam instantiation" (currently empty) with "composition root" (different concerns). |
| F-P2-2 | **Workflow ingress** — what does the runtime actually *run*? The corpus specifies workflow lifecycle contracts but not the ingress source. CLI argument pointing at a manifest file? Stdin? MCP-server-triggered? Operator-typed prompt? | §3 stage 5; §4 | Genuinely missing spec surface. Routes to design back-flow per `Class_1_Tension` discipline before Track A's atomic decomposition can specify the ingress unit. |
