...
2. **Two separable mechanisms** (the load-bearing decomposition driver — advisor-confirmed):
   ...
   - **(B) The engine-layer recovery-loop firing** — `RuntimeEngineRecoveryLoop.capture_pause` / `.attempt_resume` (`engine_recovery_loop.py:52/:74`) → the C-CP-49/50 composers (`pause_resume_protocol.py:856/:951`) → the **R-CXA-2 CP→IS engine-layer seam goes LIVE**. The loop is bound at `ctx.engine_recovery_loop` (`r_cxa_2_producer_loop_factory.py:216`) but its methods have **ZERO production callers** — genuinely dormant. (Cite note: **R-CXA-2** is the roadmap / forward-register identifier for this CP→IS engine-layer producer seam — NOT a CXA-doc label; it is *realized* at the CXA document's **§2.3.2 CP→IS bucket** (the U-CP-49/50 engine-layer composers, frozen-baseline). Where this plan writes "R-CXA-2 (CXA §2.3.2)" it means "the seam realized at that bucket.")
...
| **U-CP-94** | WAL_SEGMENT engine materialization (segment replay + per-segment dedup; CP/IS-only — the Path-(i) `test_u_rt_95` durable e2e is U-RT-122) | E-2 | (A) resumption-semantics | (none) | C-CP-07 §7.1 row 5 + §7.4 ("specific WAL implementation" deferred) + C-CP-08 §8.1 `segment_replay` + §8.2 row 5 + §8.3 |
...
| Path-(i) `test_u_rt_95` durable e2e (WAL_SEGMENT = canonical DURABLE_ASYNC class per CP §18.1; `test_u_rt_95:129-131`) | **U-RT-122** (runtime v2.45) — **materialize the vacuous test body** (`:368-375`, currently a no-op `_ = patched_runtime, tmp_path`) into a real `execute_workflow` paused→resume e2e **against the durable U-RT-121 substrate** (un-skip ALONE = false green, Codex pre-merge catch), then remove skip + correct skip-reason + flip Path-(i) fork CLOSED-DEFERRED → CLOSED-BUILT. **Relocated from U-CP-94** (Codex 2nd-pass catch): it needs the durable substrate + the U-CP-95 firing, which the foundational `(none)` U-CP-94 cannot depend on. U-CP-94 materializes the class (CP/IS-only) so this e2e can run. |
...
| EVENT_SOURCED_REPLAY → a §18.1 DURABLE_ASYNC cell? (design OQ-2) | **RESOLVED — NO.** `test_u_rt_95:129-131` maps only RECONCILER_LOOP / WAL_SEGMENT to DURABLE_ASYNC cells → **the `test_u_rt_95` durable e2e lands at E-2 (U-RT-122)**, gated on WAL_SEGMENT materialization (U-CP-94); U-CP-93 (E-1) is unrelated to `test_u_rt_95`. |
...
**Both the design doc (`r-fs-1-e-engine-classes-design-v1.md` §0/§2/§3.3) AND the architect recommendation §5 (`:89` — "the R-CXA-2 … seam goes live at the first real producer (E-1). No fork") attributed R-CXA-2 activation to E-1.** **Empirical re-grounding at HEAD relocates it to E-2 — this OVERRIDES the architect rec §5, not merely the design doc** (the architect rec has been annotated at `:89`; its core verdict — firing C-CP-49/50 from a real driver is impl-not-fork, E-1/E-2 are impl-against-cleared-spec — STANDS; only the which-class attribution moves). This is a plan-level re-sequencing **within impl-against-cleared-spec** — no spec touch, no committed-decision sacrifice (the architect rec's load-bearing fork-vs-impl verdict is untouched) — so it is driven autonomously and surfaced here (the B3-plan §5 G2c pattern), per `[[feedback-gate-only-on-meaningful-architecture-change]]`. The relocation is empirically better-grounded than the architect rec §5's assertion (which did not probe the snapshot-vs-event-replay composition).
...
| **E-impl-2** | U-CP-94 + U-RT-121 + U-CP-95 + U-RT-122 | WAL_SEGMENT: (A) segment-replay resumption, CP/IS-only (U-CP-94); (B) hand-rolled segment-log substrate extending #475 (U-RT-121) → firing branch (U-CP-95) → durable factory bind + R-CXA-2 go-live e2e **= the `test_u_rt_95` materialization + un-skip + Path-(i) fork flip** (U-RT-122 — needs the durable substrate + firing). **This is the slice that activates R-CXA-2 + gives `RuntimeEngineRecoveryLoop` its first production driver.** |
...
**Decorrelated review (complete):** out-of-family **Codex** (`just codex-review`, TWO passes, 3 [P2] all applied — convergence asymptotic): **(1)** the `test_u_rt_95` un-skip was a false-green (vacuous body `:368-375`) → strengthened the AC to require a real `execute_workflow` paused→resume e2e; **(2a)** that AC then required U-RT-121's durable substrate while U-CP-94 is foundational `(none)` (DAG/AC conflict) → **relocated the whole `test_u_rt_95` e2e + un-skip + Path-(i) flip from U-CP-94 to U-RT-122**, narrowed U-CP-94 to CP/IS-only; **(2b)** roadmap `next_action` stale vs the head bump → handled by the post-merge §12.2 terminating-refresh. **harness-adversarial-reviewer** (dedicated agent, 37 tool-uses; report `.harness/adversarial-review-r-fs-1-e-plan.md`) — APPROVE-WITH-CHANGES (0 blocking / 1 substantive / 2 doc-hygiene): F2-01 (O-CP-4 overrode the architect rec §5, not just the design doc) + F3-01 (R-CXA-2 vs CXA §2.3.2 disambiguation) + F3-02 (`resumption.is_replay` §8.3 AC clause) all APPLIED. **advisor()** pre-done. **Decorrelation payoff:** Codex caught the false-green test + the DAG/AC dependency conflict; the adversarial agent caught the architect-rec-§5 self-divergence — each found what the others (and the main agent) missed.
