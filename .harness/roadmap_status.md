...
**Current next action (post-#1328).** *B-160 GROUNDED (a class of six, gated on `B-137`). B-161 CLOSED. The B-71 spec leg remains SCOPED but not landed at **draft PR #1325**.* **Two forward items, both Claude-closeable:** **(A) `B-137` + `B-160` together** — B-160's repair is a conformance sweep (four names into the §9.2 set, 19 → 23, PLUS two conditional-resolver entries) that CANNOT deliver the floor alone, because `ParentBased(root=…)` never consults a non-root child's name; B-137 already owns that starvation, so the two close as one arc. Read B-160's close_out — the grounding step is DONE, do not repeat it. **(B) the B-71 spec leg** — six artifacts (CP spec delta with FOUR carrier amendments; a **Runtime spec delta**; CP + Runtime plan deltas; clearance markers; then impl), starting FROM `b71-spec-leg-v1-119` and carrying its four settled sub-decisions. **Caution:** that draft took 4 review rounds and one caught regression, so a fresh session is likely the better venue for canonical-contract authoring — (A) is the lower-risk pick. Queue per §12.4.1: 30 `registered_finding`. **OPERATOR OWED (unchanged): (1) E7 move — `.harness/r-ctx-1-e7-machine-local-orchestrator-step.md` (re-probed 2026-08-12, re-denied), then `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust; (3) B-124 ratification (rec D at #1283).** B-148 Wave-5 PENDING-REMEASURE. `held=2` honored.
