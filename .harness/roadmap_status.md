...
| Field | Value |
|---|---|
| `workspace_state_hash` | `caef0d97247d` |
| `last_refreshed` | 2026-07-15T00:00:00Z |
| `git_head` | `706f9c4b` — B-32 pause_reason half closed + B-39 filed + CP spec v1.100 (stale-carry-text correction to v1.97's Change-note). |
...
| PR #1011 | 2026-07-15 | Closed B-31: paused-child resume guard now validates child_workflow_id identity (added optional field to PausedChildBranchResumeState, threaded through 6 capture sites, resume-guard comparison in both PARALLELIZATION and ORCHESTRATOR_WORKERS/HIERARCHICAL_DELEGATION closures via the existing _opaque_field convention -- no new cross-axis Protocol needed). CP spec v1.98->v1.99 + clearance marker; forward-register.yaml B-31 closed. Dogfooded the new merge-gate skill (3 parallel Agent-tool reviewers) on its first real code PR: round 1 found a genuine gap (missing byte-compat test for the child_workflow_id drop-when-None strip), fixed + mutation-probed, round 2 all-approved. just check 1581/1/1 (harness-cp); full workspace 5790+ passed. No standing Claude-derivable next action -- next up per the register: B-32 (nested paused-child HITL-response routing), or the R-600 recurring lane. |
| PR #1009 | 2026-07-15 | Added merge-gate skill: 3 parallel Agent-tool subagents (concurrency/race-conditions, spec-conformance-against-ledgers, test-witness-adequacy) gate code-touching PR merges with structured APPROVE/BLOCK verdicts, fail-closed on unparseable output, capped at 2 fix-and-re-gate rounds before escalating to the operator. Wired into ship-pr's pre-merge step alongside CI-green + codex-review; skips doc-only/refresh PRs by design (this PR itself was skipped by its own scope rule). No standing Claude-derivable roadmap next action changes -- resume B-31 grounding next. |
| PR #1007 | 2026-07-15 | Built the deterministic editing harness the two-pass session-friction audit (2026-07-15) motivated: `tools/roadmap_status_refresh.py` + `tools/memory_compact.py`, replacing raw `Edit` calls on roadmap_status.md/MEMORY.md with idempotent scripts per [[roadmap-ledger-edits-via-idempotent-script]]. Found + fixed a live 92-row cap violation in the Drift detection log (docs said cap 10) and a self-caught dry-run side-effect bug during dogfooding. Wired into `ship-pr`; added CI gate + just recipes. This refresh entry itself was produced by `just roadmap-status --refresh`, not hand-edited. |
---
