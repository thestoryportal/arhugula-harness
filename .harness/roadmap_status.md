...
**Frontier:** `B-70` CLOSED 2026-07-25 — the IMPL LEG landed at PR #1101: `compute_effect_fence_uniform_fallback_eligible_key`/`_collect_effect_fence_idempotency_keys` (a tree-walk across all three effect-fence-pause carriers — LINEAR `effect_fence_resume`, the ORCHESTRATOR's own `orchestrator_effect_fence_resume`, and fan-out `effect_fence_paused_branches`) + `_resolve_effect_fence_gated` (a genuine caller of the pre-existing `effect_fence_resolution_for` — an earlier draft reimplemented its logic inline, caught + fixed by a merge-gate spec-conformance BLOCK pre-merge) wired at the LINEAR site (round-3-corrected to become map-addressable via its own existing `idempotency_key`), the ORCHESTRATOR's own dispatch, and both fan-out consume sites; `effect_fence_uniform_fallback_eligible_key` threaded end-to-end `mcp_server.py` → `execute_workflow` → `StepExecutionContext` → `sub_agent_dispatch.py` → `child_workflow_runner.py`, mirroring the shipped `hitl_uniform_fallback_eligible_run_id` wiring. 2 merge-gate 3-lens rounds (round 1: BLOCK on spec-conformance [the reimplementation above] + BLOCK on test-witness [the new field's forwarding in `sub_agent_dispatch.py` had no assertion, and the PARALLELIZATION peer path lacked the ORCHESTRATOR_WORKERS-sibling 2-unaddressed-locations safety witness] — both fixed + mutation-probed; round 2: all-approve). `just codex-review-uncommitted` P1 (direct `execute_workflow` callers must compute the eligibility key themselves) verified by design via `advisor()` — matches the already-shipped HITL contract, only 2 production entry points exist, both wired. `advisor()`-flagged coverage gap (uniform-ABORT + 2-unaddressed path) closed with a dedicated witness. 1767 harness-cp + 3046 harness-runtime tests green; pyright + ruff clean. Register row flipped `open` → `closed` (0 open / 0 design_substrate_gated / 11 registered_finding / 1 operator_gated / 1 held / 60 closed). **Next action** per the §4 derivation rule: no `open` register row remains — per §12.4.1, ground the next forward-register `registered_finding` candidate (file order): B-16, B-30, B-44, B-45, B-56, B-57, B-66, B-68, B-69, B-71, B-72 — none of these labels is a park, ground each candidate's premise at HEAD before picking (`[[full-register-ground-every-arc-before-blocking]]`). `R-600-pattern-bake-in-sweep` cadence-9 CLOSED 2026-07-24 at PR #1100 — next run only after the cadence trips again (~10 PRs) or a concrete new candidate reaches instance-cardinality ≥2. B-39 CLOSED 2026-07-24 (prior frontier) — spec leg PR #1092 + impl leg Slice A PR #1094 + impl leg Slice B PR #1096/#1097 all landed; full detail at `.harness/post-phase-8-forward-register.md` B-39. B-33 CLOSED 2026-07-23 — all three ratified-fork (Option A) legs landed (PRs #1083/#1084/#1087).
