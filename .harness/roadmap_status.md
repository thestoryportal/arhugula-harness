...
| Field | Value |
|---|---|
| `workspace_state_hash` | `8cd04b7563ec` |
...
**Current next action.** **PR #1032 CLOSED (this refresh) — B-34 RESOLVED.** B-34 (concrete signature-representation gaps in the C-CP-20 §20.2.1 seam) became buildable when B-36/ADR-D8 (PR #1031) landed a real Ed25519 backend, satisfying its registered "once a real backend exists" gate. Built per the row's own close-out steps verbatim: (1) reversible base64 field-serializer/validator on `CPSignedAuditLedgerEntry.audit_signature_value` — real signature bytes are arbitrary binary, so pydantic's default utf-8 bytes-to-JSON encoding raised `PydanticSerializationError` and a signed entry could not cross any JSON/JSONL persistence boundary (empirically reproduced pre-fix); JSON round-trip is now lossless, the Python-domain contract stays raw `bytes`, and non-canonical base64 spellings are rejected so each signature has exactly one accepted JSON representation (out-of-family Codex round-1 P2); consumer sweep confirmed zero serialization consumers of the shared type outside the two edited harness-cp modules. (2) Per-algorithm byte-length enforcement (`{ed25519: 64, ecdsa-p256: 64 raw r||s, rsa-pss-2048: 256}`, matching C-CP-20 §20.4's committed widths verbatim at `Spec_Control_Plane_v1_2.md:1792`): sign-side fail-loud `ValueError` immediately after `backend.sign`; verify-side pre-backend `SIGNATURE_MISMATCH` (a length-invalid stored signature never reaches the backend — proven by a counting-backend witness); the ecdsa-p256 row forecloses B-34's DER-vs-raw ambiguity by contract. 7 new witnesses, every load-bearing line PD-8 mutation-probed (lossy serializer / disabled sign check / removed verify check / 64→72 table flip each fail exactly their paired witness). 2-round out-of-family `just codex-review` (1 real finding, then clean) + 3-lens `merge-gate` (all APPROVE; the test-witness lens's one non-blocking note — unwitnessed ecdsa/rsa table rows — was closed pre-merge in the same PR; the spec-conformance lens noted 2 Class-3 cosmetic residuals: CP spec v1.98 change-note prose is historical-by-convention, and the register's B-36 block still carries a superseded "still open" line about the JSON gap — one-line append candidate for a future register-touching PR). Per §4's derivation rule there is still no *R-NNN ACTIVE-queue* candidate. The standalone `B-*` register now derives **0 open / 2 design_substrate_gated / 7 registered_finding / 1 operator_gated / 1 held / 37 closed** (`just forward-register --check`). Freshly-unblocked forward candidates from the #1031/#1032 cascade: **B-33** (rotation-boundary-proof carrier — now has a real backend to build against, but its own close-out routes through a Class 1 fork for an IS rotation-correlation carrier, i.e. a design-phase leg first) and **B-47** (KMS composition-root wiring — its own close-out notes zero production `sign_audit_entry` call sites exist, so it likely lands together with whatever CP-axis arc first introduces one; ground the call-site question at open). Remaining rows unchanged from the prior investigation: `B-16`/`B-30`/`B-44`/`B-45`/`B-46` register-don't-build residuals, `B-17` operator-gated, `B-39`/`B-43` design-substrate-gated. **Standing note:** operator has granted forward-work merge permission without per-instance HIL, conditioned on CI fully passing green before any merge, re-confirmed per-PR at #1026/#1027/#1029/#1030 (2026-07-15) and broadened to cover the rest of this autonomous loop's PRs — applied at PR #1032.
