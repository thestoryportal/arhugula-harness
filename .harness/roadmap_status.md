...
| Field | Value |
|---|---|
| `workspace_state_hash` | `62778af34279` |
| `last_refreshed` | 2026-08-14T00:00:00Z |
| `git_head` | `7c7baa9d` — ritual smoke merged |
...
**Current next action (post-#1339).** THE PREVENTION ARC AND ITS RITUAL-SMOKE FOLLOW-ON ARE LANDED (#1338, #1339; main GREEN) — the `B-71` impl leg is the next action: **`U-CP-102` + `U-RT-155`, CO-LAND PINNED** (neither is independently observable). Read the plan ACs first, since three encode defects review already caught: derive the basis UNCONDITIONALLY at both `workflow_driver.py` composition sites; the fold must reach ALL THREE `compose_hitl_action_id` invocations; the identity witness is split by venue because `_escalate_to_secondary_channel` is `NoReturn`. **Use what landed:** `just leg-selfcheck` BEFORE EVERY PUSH (per ROUND, not per arc — that is the whole point); install the next-action pointer with `--refresh --next-action`, never by hand; **archive the superseded round with `--archive-superseded` INSIDE the content PR** (`B-168` exit (iii) — archive round N−1; it CANNOT ride a post-merge commit, which would be a second non-refresh commit and red `main`); reclaim headroom with `--trim-drift-log` in a CONTENT commit. **Registered, not fixed:** `B-166` + `B-169` are both grounded-but-unreproduced wall-clock flakes — **do NOT wave away a red in the fan-out driver as "the known flake"**; neither clears a red. `B-167` is a stated bound on leg-selfcheck (claims, never ground truth).
