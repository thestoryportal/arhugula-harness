...
| Field | Value |
|---|---|
| `workspace_state_hash` | `5c8577ce9942` |
| `last_refreshed` | 2026-06-17T10:48:21+00:00 |
| `git_head` | `70ca4628` — #619 R-FS-1 **B4 Slice 3** (per-step **PROMPT** override on `StepOverride`; CP spec **v1.37**). NEW `StepOverride.prompt_version_sha` (additive optional, CP §6.1) + `StepEffectiveBinding.prompt_version_sha` propagation (§6.2) + NEW §6.6 provenance-scope; runtime dispatch precedence **per-step > per-role > run-level default** with fail-loud unauthored-sha (`RT-FAIL-PROMPT-SELECTION-UNAUTHORED`) + **deployment-tier** governance parity (`RT-FAIL-PROMPT-VERSION-UNAPPROVED`). **Bundled-absorption** (fork + clearance v1.37 + spine; X-AL-3 green). Surface=`StepOverride` not `PromptSelectionManifest` (`step_id` is a workflow-scoped instance, not a run-level category); provenance at the WIRED override state-ledger entry (`binding.model_dump`), NOT the run-level §5.2 hash (no IS-spec change). Per-step ROLE OUT of scope = Slice-4 gate (§14.5.3 single-role-source invariant). Decorrelated: advisor (provenance-scope reframe + driver→dispatch seam blocker → closed by `test_per_step_prompt_override_threads_through_driver_to_dispatch`) + **Codex caught a [P1] governance bypass** (keyed on per-workflow tier not deployment tier) → fixed + regression-tested; Codex re-review clean. pyright 0/0/0; ruff clean; harness-cp 1040 passed / 1 xfailed; harness-runtime 1794 passed; CXA-P1 34. Lags HEAD by one commit (this terminating-refresh fixed point, §12.2.1). Full detail: `git show 70ca4628` + Recently completed. |
| `latest_retirement_batch` | `.harness/phase-7d-retirement-events-batch-57.md` |
| `open_fork_doc_count` | 64 |
...
**Next = R-FS-1 → B4 Slice 4** — the B4 tail: **per-step ROLE override** (the foreclosed Slice-3 gate — `StepOverride.agent_role`/equivalent sacrifices the runtime **§14.5.3 single-role-source invariant**; branch role today flows ONLY via `step_context.agent_role`) **+ linear-path role indexing** (optionally thread roles on the straight-line/single-threaded path too; impl-discretion, scope decision at arc-open). **Drive both to their genuine gate** per `[[feedback-operator-labels-are-claude-driven-no-parking]]`: build the impl-discretion linear-path indexing + scaffold the per-step-role surface up to the invariant-sacrifice decision, then surface ONLY the §14.5.3 single-role-source sacrifice as the genuine operator gate (a committed-invariant sacrifice IS gate-worthy per `[[feedback-gate-only-on-meaningful-architecture-change]]`; not mid-run). **B4 Slice 3 ✅ landed (#619)** — per-step PROMPT override on `StepOverride` (CP v1.37; precedence per-step>per-role>default; deployment-tier governance parity; Codex-caught P1 fixed). **Slice 2 ✅ (#618)** — per-role binding catalog (`derive_agent_role` + `validate_per_role_catalog`). **Slice 1 ✅ (#616)** — per-role PROMPT threading + IS v1.9 hash coherence. Frozen order: B1✅ B3✅ E✅ B2✅ R✅ → **B4 (Slice 1✅ 2✅ 3✅; 4 next)** → CA → B5 → B6 → B7 → M. Re-ground at arc-open per `.harness/r-fs-1-remaining-arcs-grounding-sweep-v1.md` Arc B4 + `.harness/beyond-mvp-capability-boundary-ledger.md` (B4 execution-decomposition; LEADS, presence-not-correctness).
