...
| Field | Value |
|---|---|
| `workspace_state_hash` | `1e0b95d29e8f` |
...
**Frontier:** `B-68` CLOSED 2026-07-25 at PR #1103 — the protected result store's opportunistic GC/publish race fixed across 8 out-of-family Codex rounds: gc_sweep() now ages entries off the committed file's filesystem mtime (not a pre-publish timestamp), a process-wide threading.Lock keyed by the store root's filesystem identity ((st_dev, st_ino), unconditionally mkdir'd before stat'ing so the key is always identity-based) serializes publish against GC, read() degrades a missing entry to a typed refusal, and GC only holds the lock for a fast stat()-only candidate-selection phase (decrypt+unlink happen after release, safe because expiry is monotonic and every write uses a fresh uuid4()-derived key -- round 7's whole-body-lock draft was found to defeat run_off_loop_detach_on_cancel's detach design on a long-lived daemon). 5 narrower residuals split out and registered rather than chased further, per the non-convergent-adversarial-hardening-arms-race discriminator (3 consecutive rounds 6->7->8 each surfacing a defect the immediately-prior round's own fix introduced): B-73 (cross-process lock divergence, needs fcntl.flock), B-74 (coarse-mtime-filesystem TTL floor, needs a probed resolution floor), B-75 (the stat-only phase itself could stall under a pathological backlog/hanging filesystem), B-76 (a transient stat() failure during lock-key resolution could fail open), B-77 (a crash between the os.link commit and the os.utime refresh leaves a narrow crash-recovery window). Merge-gate 2 rounds (round 1 fixed one flaky test-witness -- an ~80%-reliable sleep-based race, corrected to a deterministic Event handshake; round 2 all-approve). A mid-arc merge conflict against a concurrent B-72 grounding-pass PR (#1104/#1105) was resolved cleanly (both sides' content preserved). Full detail at `.harness/post-phase-8-forward-register.md` B-68 through B-77.
