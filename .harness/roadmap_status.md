...
| Field | Value |
|---|---|
| `workspace_state_hash` | `42f4ec2d0e85` |
| `last_refreshed` | 2026-09-05T00:00:00Z |
| `git_head` | `5cd9113b` —  |
...
**Current next action (post-#1523).** `U-HE-38` landed the C-HE-28 §1–§3 surface in `arc_metrics summary`: the joint `(concurrent_lanes_at_open, arc_type)` cohort split, the `ROADMAP_STATUS_DRIFT` incidence join, and the correlational header. Three findings from the arc are load-bearing for whoever touches this next. FIRST, the cohort field stores a SIBLING count, not a lane count — `open_with_sensor` sets it from `sibling_open_count(arc_id)` (`tools/reservations.py:786`), which excludes the arc's own reservation, and C-HE-03 §7 defines it that way, so a solo arc stores `0` and a stored `1` means two live lanes. Reading it as lanes (`siblings + 1`) is the only reading under which C-HE-28 §3's two sentences are simultaneously true — "the only populated joint cells are `(N=1, inventing)` n=6 and `(N=1, null)` n=12" at a HEAD where no row carried the field, and "every N ≥ 2 cell starts at 0". The consumer converts; the SPEC TEXT still names the field as the cohort key while counting in lanes, and that wording is registered as `B-239` rather than amended, because an implementation arc may not edit a cleared contract (§4.3/§4.4). This ambiguity survived six review rounds and caused one shipped-then-corrected fix, so it is worth reading `B-239` before touching any lane-count cohort. SECOND, C-HE-28 §2's numerator has no producer: `ROADMAP_STATUS_DRIFT` is raised by `tools/codex_context_guard.py:1515` as an in-memory CI guard `Finding` and is never appended to `merge-gate-log.jsonl` (0 rows carry it). The report therefore renders `--/<cohort size>` rather than a zero, and the verification manifest claims `C-HE-28 §1+§3` and deliberately NOT §2, so no green check stands over a measurement production cannot make. `B-237` carries the emitter, its required `producer` binding, and the activation boundary that must drop pre-activation arcs from the denominator. THIRD, `B-238` holds lever stratification of the joint key: rejected twice on the grounds that §1 names a 2-tuple and §3 enumerates 2-tuple cells, registered because the confound is real once treated arcs exceed a singleton per cell — 6 of 35 ledger rows carry levers across 3 exact sets at this HEAD. The arc converged at 10 review rounds (30 findings, 27 accepted, 3 rejected with grounds, all adjudicated on the gate log); the recurring defect class was a fixture or a record that could not have been produced by the mechanism it stood for, now closed structurally by validating every drift fixture through `finding_record` at construction. Also carried: `just arc-metrics drain` owes the U-HE-38 row inside the next arc's PR, `just roadmap-status --archive-superseded` rides the next substantive PR, and U-HE-37 Step 5 remains blocked on the operator-gated `just main-protection-apply`. The next unit is `U-HE-39` (skill-carrier doc sweep: the N ≥ 2 wording without an N=2 literal cap, the C-HE-14 blocked/rejected table in `two-lane`, live-carrier cites for #5/#14, #16 stated void and "no round cap", and the Part D non-goals plus K5–K8 dispositions in `merge-gate`/`ship-pr`, with a grep witness asserting no numeric round cap in the loop skills), then U-HE-40.
