...
| Field | Value |
|---|---|
| `workspace_state_hash` | `b7c3f481f590` |
| `last_refreshed` | 2026-08-14T00:00:00Z |
| `git_head` | `d75e5ee5` — prevention arc merged |
...
**Current next action (post-#1338).** THE PREVENTION ARC IS LANDED (PR #1338) — the `B-71` impl leg is now unblocked and is the next action: **`U-CP-102` + `U-RT-155`, CO-LAND PINNED** (neither is independently observable). Read the plan ACs first, since three encode defects review already caught: derive the basis UNCONDITIONALLY at both `workflow_driver.py` composition sites; the fold must reach ALL THREE `compose_hitl_action_id` invocations; the identity witness is split by venue because `_escalate_to_secondary_channel` is `NoReturn`. **What this arc shipped:** headroom is mechanically reclaimable via `just roadmap-status --trim-drift-log` (run it in a CONTENT commit, never the refresh); the next-action pointer is TOOL-INSTALLED (`--refresh --next-action`), never hand-edited; superseded rounds archive via `--archive-superseded` riding a CONTENT PR (`B-168` exit (iii) — archive round N−1, so the prior-only invariant, the one-file refresh rule and the one-non-refresh-commit lag tolerance all hold UNCHANGED); `just leg-selfcheck` is the per-ROUND pre-push gate. **Registered, not fixed:** `B-166` — do NOT wave away a red in the fan-out driver as "the known flake" during the impl leg — and `B-167` (leg-selfcheck compares count CLAIMS, never ground truth).
