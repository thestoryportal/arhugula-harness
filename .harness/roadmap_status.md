...
| Field | Value |
|---|---|
| `workspace_state_hash` | `51065fbca078` |
| `last_refreshed` | 2026-06-17T06:30:00+00:00 |
| `git_head` | `38ebeeb2` — #616 R-FS-1 **B4 Slice 1** (per-role prompt threading + procedural-tier hash coherence, IS spec v1.9). A fan-out branch's `agent_role` now selects + injects its per-role prompt at the §14.5.2 translate seam (unbound/linear roles fall through to the default); IS C-IS-05 §5.2 widened 3→4 components (NEW `prompt_selection_manifest_sha`, mirroring `routing_manifest_sha`) so per-role bindings are audit-hash-visible — closing the §14.5.2 coherence gap per-role injection would reintroduce. Recipe reads `config.prompt_selection_manifest` (no new HarnessContext carrier / C-RT-04 row — not stage-enriched; advisor-caught). Bundled-absorption arc (fork + clearance; X-AL-3 guard green). Decorrelated: advisor (2 blocks fixed) + Codex (clean). pyright 0/0/0; 1586 non-integration + 210 integration tests green (+14 new). Lags HEAD by one commit (this terminating-refresh fixed point, §12.2.1). Full detail: `git show 38ebeeb2` + Recently completed. |
...
**Next = R-FS-1 → B4 Slice 2** — the per-role binding **catalog** surface + validation (operator-authored `per_role_bindings` content for both RoutingManifest + PromptSelectionManifest, incl. the `step_id→AgentRole` derivation contract). **B4 Slice 1 ✅ landed (#616)** — per-role PROMPT threading + the IS v1.9 procedural-tier hash-coherence absorption. Remaining B4 slices: **Slice 2 (catalog, next)** → Slice 3 (per-step prompt/role override on `StepOverride` — likely-small CP §6.1 fork; verify the §6.1 extension clause at arc-open) → Slice 4 (linear-path role indexing, impl-discretion). Frozen order: B1✅ B3✅ E✅ B2✅ R✅ → **B4 (Slice 1✅; 2→3→4)** → CA → B5 → B6 → B7 → M. Re-ground at arc-open per `.harness/r-fs-1-remaining-arcs-grounding-sweep-v1.md` Arc B4 + `.harness/beyond-mvp-capability-boundary-ledger.md` (B4 execution-decomposition; LEADS, presence-not-correctness).
