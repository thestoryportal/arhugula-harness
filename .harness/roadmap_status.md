...
| Field | Value |
|---|---|
| `workspace_state_hash` | `7922b97e8ff4` |
| `last_refreshed` | 2026-08-11T15:12:00Z |
| `git_head` | `da34224e` —  |
...
**Current next action (post-#1306).** *Loop run 2026-08-11 (cont.): PR #1306 merged (B-145 GAP-2b — five `retry.terminal` values wired on the dispatch path; escalate branch verified unreachable-by-construction, branch-contract witness, discrepancy routed to the GAP-2a leg; 3-lens ALL-APPROVE first pass, codex round-1 clean).* Next Claude-executable queue: **B-150 atexit half** (impl-only, confirmed warranted: arm the ordered atexit replacement on the standalone `flush_observability` timeout path in `harness-runtime/src/harness_runtime/shutdown.py` — today only `_spawn_deferred_tracer_close` swaps the SDK backstop; witness per the test_shutdown.py:1396 atexit-spy template + subprocess-probe shape; the standalone backstop must close at exit ONLY when no flush worker is live AND the deferred-close machinery does not own the ctx) → **`B-144` step-2 venue fork** (step-1 re-verified this run: `24.1.B` still single-venue at Spec_Control_Plane_v1_2.md:2152/:2167; manifest inconsistency confirmed at HEAD — retry.* 4 vs breaker 9 vs hitl.* span-names-as-attributes; spec leg, council-conditional per the register row) → the remaining spec legs (B-145 GAP-1 stale-alias + GAP-2a control-flow carve-out + B-150 collector half; bundled-absorption with back-flow docs + clearance markers per X-AL-3). **OPERATOR ACTIONS STILL OWED (unchanged from post-#1305): (1) the reversible E7 move — command at `.harness/r-ctx-1-e7-machine-local-orchestrator-step.md`; then a fresh `*/cli` session + `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust ceremony; (3) the `B-124` Class-2 ratification (fork doc at #1283, recommendation D on file).** R-CTX-1 (B-148) stays Wave-5 PENDING-REMEASURE. `held=2` honored. B-145 forward-register build-state note (GAP-2b landed #1306) rides the next content PR per §12.2.1 file-set purity.
