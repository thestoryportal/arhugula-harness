...
**Frontier:** `B-72` reproduction BUILT 2026-07-25 at PR #1107 — a real-stack test (`harness-runtime/tests/integration/test_b72_fanout_sub_agent_dispatch_hitl_gate_resume.py`) empirically confirms the fan-out HITL delivery-mechanism gap the register's round-2 spec-citation reframe predicted: a `PARALLELIZATION` branch's own `SUB_AGENT_BOUNDARY` gate correctly pauses on first dispatch, but a resolved operator answer delivered via `api.resume()` is never consumed — the gate re-fires (2nd webhook POST) instead of dispatching the child, and the run re-pauses cleanly with no state corruption but never converges. Pinned with 6 oracles including a mutation-probed direct spy on `RuntimeSubAgentDispatcher.dispatch` (0 calls before resume, 1 when the gate is bypassed — confirmed non-vacuous), not a blanket `xfail` or a weak Anthropic-client-call-count proxy (both tried and rejected across 5 codex rounds). Merge-gate 3-lens all-approve at round 1. A genuine merge conflict against the concurrently-merging B-68 PR's own terminating refresh was resolved cleanly (kept this PR's round-3 B-72 text, preserved B-68's new B-73-B-77 rows). **A separate, still-open finding**: this single-branch reproduction scopes the liveness gap but not the multi-branch addressing mechanism (the gate fires before any child run exists to key `hitl_response_for(child_run_id)` by) — needs either a multi-peer-branch reproduction or grounding on branch-own pre-dispatch identity before the fix is genuinely scoped. **Next action:** build the deferred fan-out HITL delivery-mechanism wiring itself (call `hitl_response_for(child_run_id)` at the fan-out branch dispatch site, construct a `HITLDeliveryCell`-equivalent, satisfy property 4's fallback-safety invariant) — council-eligible per the register's own framing (property-4-safe resolver design, not a gate-skip tradeoff; a prior draft's counting mechanism already failed review) — OR ground the multi-branch addressing gap first if that's cheaper. Also owed: investigate whether `test_write_once_survives_immediate_sweep_under_slow_fsync_and_short_ttl` (B-68's own test) is genuinely flaky — it failed once on a post-merge CI run touching only `.harness/merge-gate-log.md` (unrelated file), rerun triggered, outcome pending as of this refresh. `B-68` CLOSED 2026-07-25 at PR #1103 (prior frontier) — full detail at `.harness/post-phase-8-forward-register.md` B-68 through B-77.

**Next candidate:** per the file-order registered_finding queue, `B-72` is the next nominally-buildable item (its own close_out prescribes building a reproduction test before any fix design) -- but it is CURRENTLY CLAIMED by a concurrent session (uncommitted work on branch `b72-fanout-hitl-gate-reproduction-test` in the shared main checkout as of this refresh) -- skip to avoid duplicate work. Every other queue item (B-16/B-30/B-44/B-45/B-56/B-57/B-66/B-69/B-71) was re-grounded and confirmed still-accurately-deferred at the most recent sweep (`docs(register): ground remaining forward-register queue post-B-30`, 2026-07-25) -- re-ground before building, per §12.4.1, rather than trusting that snapshot indefinitely.
