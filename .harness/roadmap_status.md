...
| Field | Value |
|---|---|
| `workspace_state_hash` | `da85e0e8f9d4` |
...
**Current next action (post-#1324).** *B-71: **ALL FIVE preconditions CLOSED** — the spec leg is AUTHORIZABLE.* **Next arc: OPEN THE B-71 SPEC LEG** against the design record (`.harness/council-b71-hitl-external-correlation-2026-08-12/DELIVERABLE.md` v6). It is a CP spec amendment (C-CP-28 §25.2 family) adding `escalation_instance_id: str | None = None` to `HITLEscalationBrief` per §3, with the widening landing once inside `compose_hitl_action_id`, the token HASHED before it enters that composer (§4-quater's tracing leak bar), `resolvability` carrying the closed `PauseLocationVariant` CHANNEL (§4-ter), and the palette kept with an INFORMATIONAL `resolvability_note` (the D-2 suppression binding is WITHDRAWN). **Read §5 first** — 8 residuals owed at open + the three binding council conditions (sequencing, scope/`B-158`, the `branch_context` leak bar extended to tracing). Do NOT re-open preconditions 1-5 and do NOT re-run the council. **NEW: `B-161`** (shutdown-test order dependence — reproduce deterministically before touching it) and **`B-160`** (§C-OD-32.3 head=1.0 vs the 19-entry set; depends on `B-137`). Queue per §12.4.1: 31 `registered_finding`. **OPERATOR OWED (unchanged): (1) E7 move — `.harness/r-ctx-1-e7-machine-local-orchestrator-step.md` (re-probed 2026-08-12, re-denied), then `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust; (3) B-124 ratification (rec D at #1283).** B-148 Wave-5 PENDING-REMEASURE. `held=2` honored.
...
|---|---|---|
| 1324 | 2026-08-12 | B-71 precondition 4 CLOSED — **ALL FIVE preconditions closed; the spec leg is AUTHORIZABLE.** Two things landed. **(1) A citation the record had wrong since v2 is corrected:** basis (B)'s token is composed from `pre_dispatch_gate_owning_branch_identity(run_id, branch_index)` at `workflow_driver.py:8346`, where `run_id` is **the run executing the fan-out** — so for a top-level fan-out the load-bearing continuity is `mcp_server.py:371-372` (C-RT-35), NOT `child_workflow_runner.py:230-234`, which applies only when the fan-out is nested in a child run. Both continuities are now witnessed. **(2) The live-resume witness** on the real stack for the exact population — `api.run` → PAUSED → `api.resume` → re-pause, asserting run_id identity across both snapshots plus `HITL_PENDING` and a 2nd webhook POST as discriminators; mutation-probed. **The `durable=False` decision:** the window is TWO-MODE, differing in who closes it — durable=True the harness journals; the default the CALLER persists `RunResult.pause_snapshot`, a shipped tested capability (`test_api_resume_restart_proof_round_trip` under `PauseResumeProtocolConfig.default()`). A draft called the default "vacuous"; codex falsified it. **NEW `B-161`** registered — `test_shutdown.py::test_force_flush_returning_false_surfaces_timed_out` is order-dependent (spurious `('ledger',)` under full-suite ordering); established as a flake not a regression (CI re-run green, 6/6 isolated, 3/3 full runtime local, main green ×8) before registering. Codex 2 rounds. Register 161: 31 registered / 127 closed. CI 18/18; main green. |
| 1323 | 2026-08-12 | B-71 precondition 4 SHARPENED (not closed) — the v5 closure claim was RETRACTED on review: the boundary was wrong at the `durable=False` default and the witness was runner-level, not the live resume asked for. Delivered a mutation-probed witness that the runner runs a resumed child under `snapshot.run_id` (a branch no existing test took) + the corrected two-mode boundary. Codex 3r. CI 18/18. |
| 1322 | 2026-08-12 | B-71 precondition 5 CLOSED — the charter's "not a span attribute" premise is FALSE in effect: §3's widening lands in `compose_hitl_action_id`, whose output rides the tracing export as `webhook.idempotency_key` (witnessed producer→exporter). ONE reachable carrier. Leak bar extends to tracing and binds the token's FORM. NEW **B-160**. Codex 7r. CI 18/18. |
| 1321 | 2026-08-12 | B-71 precondition 3 CLOSED — `resolvability` carries the resolution CHANNEL, never the outcome; the `held-for-sole-resolution` stamp retired on a WITNESSED eligibility flip. Codex 17r — D-2 palette binding WITHDRAWN; B-155 re-scoped. CI 18/18. |
| 1320 | 2026-08-12 | B-71 preconditions 1+2 CLOSED — basis fork RESOLVED to (B); candidate (A) FALSIFIED on an executed collision witness (13 tests). Codex 6r. CI 18/18. |
