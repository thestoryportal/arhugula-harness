...
**Current next action (post-#1337).** *OPERATOR-DIRECTED 2026-08-13: a PREVENTION ARC runs BEFORE the `B-71` impl leg, in a fresh context window. Full brief at `~/.gstack/projects/arhugula-v2/checkpoints/20260813-220000-HANDOFF-prevention-arc-before-u-cp-102.md` — read it first; it carries the diagnosis, not just the tasks.* **(1) Unblock this file — it is at 25.5K/25.6K, 86 bytes of headroom, and every arc close needs a refresh.** Cause is structural: `roadmap_status_refresh.py --refresh` writes the notes into `recently_completed` but NOT the next-action pointer (so the same prose lands twice by hand), and the uncapped archive is unreachable from inside a refresh because §12.2.1 requires the terminating commit to be roadmap-status-only. **(2) Register the deadline flake** — `test_ow_proceed_deadline_stashed_child_completed_not_cancelled`, 4 of ~12 full-suite runs on 2026-08-13, against ZERO-Python diffs; the risk is it MASKS a real red in the impl leg. **(3) Build `just leg-selfcheck`** — per-round, pre-push, wired into `ship-pr`: re-resolve every `file:line` cite at HEAD; cross-check every count claim against its artifact AND every mirror; scan new `§` labels for chain collisions; assert a touched register row renders current-state-first AND has a prose body. **Why:** `B-71` took TEN rounds — r1-r6 scope gaps, r7-r10 defects the ABSORPTION introduced (counts drifted 5×, occupied labels, a false code claim). All LOCAL checks where the property was GLOBAL; memories existed for three and were violated, so the fix is mechanical-and-per-round. ~7 of 12 were mechanically detectable; ROUND COUNT drives wall-clock. **THEN** the `B-71` impl leg (`U-CP-102` + `U-RT-155`, co-land pinned). **Do NOT ground `B-165`** — correctly parked and self-describing. **OPERATOR OWED: (1) E7 move then `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust; (3) B-124 ratification (rec D at #1283); (4) `#1292` open since 2026-08-10.** `held=2`; `B-137` deferred 2026-08-13.
