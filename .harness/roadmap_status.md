...
| Field | Value |
|---|---|
| `workspace_state_hash` | `e40123de541d` |
...
**Frontier:** `B-30` step 4 BUILT 2026-07-25 at PR #1102 — threaded the workflow's real declared `run_engine_class` into the 3 live, step-scoped cost-attribution composers (`attribute_llm_dispatch_cost`/`attribute_tool_dispatch_cost`/`attribute_validator_dispatch_cost`), resolving `SpanCostRecord.engine_replay_disposition` via the existing closed-total `REPLAY_DISPOSITION_MAPPING[EngineClass]` table instead of the pre-fix hard-coded `NO_REPLAY` — grounded before building via `advisor()`'s decision rule (no live reader of the field, but `run_engine_class` is non-vacuous — it already drives production durable-auto-fence logic — so the hard-code was a genuine persisted-audit-ledger correctness defect, not a no-op). Webhook (4th composer) deliberately deferred to the FM-2 webhook-config arc — its composer is a bootstrap-stage-5 process singleton with cost substrates unbound at HEAD (fully dormant in production). Also re-grounded and closed the finding's other two open steps: `ingest_span`'s ADR-D6 consumer re-confirmed absent (register-don't-build, mirrors B-19/B-42), and the suspected double-billing risk confirmed NOT live (`workflow_driver.py`'s `resume_at` skip logic already excludes the committed step prefix from re-dispatch). 2 merge-gate 3-lens rounds (round 1: BLOCK on spec-conformance [the schema-backed `.harness/forward-register.yaml` B-30 row wasn't updated alongside the prose register, plus a test-count miscount] + BLOCK on test-witness [every new test called the composer functions directly with a manual `run_engine_class` kwarg — none exercised the 3 real production call sites, so a silent revert of the threading would leave the suite green] — both fixed: targeted YAML patch + 3 new integration tests each mutation-probed against the real call sites; round 2: all-approve). 15 new parametrized unit tests + 3 integration witnesses; full harness-runtime suite 3067 passed; pyright/ruff clean. Register row stays `registered_finding` (not closed) — step (3)'s within-invocation-retry accumulator check and webhook's per-call threading remain open residuals. **Next action** per the §4 derivation rule: continue the file-order forward-register `registered_finding` sweep — B-44, B-45, B-56, B-57, B-66, B-68, B-69, B-71, B-72 (B-16 and B-30 already covered this cadence) — ground each candidate's premise at HEAD before picking (`[[full-register-ground-every-arc-before-blocking]]`). `R-600-pattern-bake-in-sweep` cadence-9 CLOSED 2026-07-24 at PR #1100 — next run only after the cadence trips again (~10 PRs) or a concrete new candidate reaches instance-cardinality ≥2. B-70 CLOSED 2026-07-25 (prior frontier) — impl leg PR #1101 landed; full detail at `.harness/post-phase-8-forward-register.md` B-70.
