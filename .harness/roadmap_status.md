...
| Field | Value |
|---|---|
| `workspace_state_hash` | `00ca3cf06bcb` |
| `last_refreshed` | 2026-06-17T01:50:58+00:00 |
| `git_head` | `317929d5` — #606 R-FS-1 arc R / L2: EMBEDDING Layer-2 (Option B, light in-process `fastembed` k-NN classifier). L2-sync = pure Phase-7 impl, no spec amendment; BUILT + PROVEN but production-inert until the routing-activation gate (see Remaining forward work → `B-L2-EMBEDDING-ACTIVATION`). Lags HEAD by one commit (the #607 terminating-refresh fixed point, §12.2.1). Full detail: `git show 317929d5` + Recently completed. |
...
| PR #600 | 2026-06-16 | **R-FS-1 — arc R / R-plan-1: LLM_AS_ROUTER L3 atomic-unit decomposition (CP plan v2.37 + runtime v2.48).** 4 atomic units — U-CP-99 (R-routing types: `RouterResolutionFn` + `RouterResolution` + `ProviderDispatchFn` Protocol-ization) + U-CP-100 (the `infer()` L3 router branch + `wait_for` timeout wrap + 4 no-regress paths) + U-RT-132 (rationale-consume + span) + U-RT-133 (router-injection binding, prod `router=None` + a test-only `layer_decisions` seam). Load-bearing (pyright spike): the Protocol-ization is a cross-package TYPE-RIPPLE CO-LAND → all 4 land in ONE R-impl-1 arc. SPEC→PLAN only; ZERO spec amendment / new contract ID; X-AL-3-clean. Decorrelated: advisor + Codex ×3 to convergence (R1 runtime-e2e-buildability [P2] → the test-only `layer_decisions` seam; R2 the spec-under-satisfaction the relocation introduced; R3 coherent) + 14/14 CI; 2 clearance markers + companion + pointer bumps + SPINE-ledger R registration. **Next = R-impl-1.** |
| PR #598 | 2026-06-16 | **R-FS-1 — arc R / R-spec-1: LLM_AS_ROUTER L3 resolution surface (CP spec v1.36 §2.5).** Additive C-CP-02 §2.5 binds Layer 3 at the async `infer()` (Reading B) via an injected OPTIONAL `RouterResolutionFn`; absent-router / L3-exhausted → preserved `RoutingCandidateUnresolvedError`; router await timeout-bounded by the 200 ms L3 budget; rationale via an additive optional `binding_rationale` dispatch-seam carrier (frozen 4-field trace un-widened; passed only on the router path → legacy 4-arg callables unbroken); no-regress direct-dispatch. SPEC-ONLY (impl at R-impl-1/2; L2 not bound). Decorrelated: advisor + Codex ×3 to convergence (R1 2×[P2], R2 1×[P2] the R1-fix contradiction, R3 clean) + 14/14 CI; clearance marker discharges the R-DESIGN §7(a) fork-to-file. |
