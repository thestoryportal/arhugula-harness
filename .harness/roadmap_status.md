...
| Field | Value |
|---|---|
| `workspace_state_hash` | `4786441b49bf` |
| `last_refreshed` | 2026-07-22T20:02:00Z |
| `git_head` | `a1729666` — drift-log archive trim (post-#1082 refresh companion) |
...
**Frontier:** B-33-A SPEC LEG CLOSED at PR #1082 (IS spec v1.11→v1.12: NEW §5.6 D-derivative sidecar `rotation_correlation_id` on `StateLedgerEntry`/`EntryPayload` — C-IS-05, hash-covered typed sidecar per the §5.1/§5.4 idiom, malformed-value rejected at construction via canonical-round-trip check; NEW §7.7 read-side invariants — non-emptiness/presence/uniqueness — C-IS-07, reframed as a JOIN-KEY contract, not a trust anchor, after 5 rounds of out-of-family codex review: real trust requires the window boundary be authenticated against the OD-side dual-signature pair, and a chain-head rotation entry needs independent OD-signature coverage since `verify_chain` has no successor to detect head-of-chain tampering — both constraints REGISTERED as impl-leg-(ii) requirements, not solved at this leg) + IS plan v2.7→v2.8 (NEW foundational unit U-IS-20, one public composed non-emptiness→presence→uniqueness validator, `Depends on: (none)`, IS 0-outbound). Clearance markers filed both sides; `.harness/forward-register.yaml` B-33 row flipped `design_substrate_gated` → `open` (B-33 itself stays open — this leg closes only the IS-side half). 5 codex rounds to convergence — round 4 re-raised the carrier-shape question a 2nd time (a labeling non-fork per `advisor()`; the artifact is identical either way and IS has no literal open-dict mechanism), rounds 4-5 found 2 genuine security-depth gaps (chain-head tamper coverage; OD-anchored window-boundary trust) both correctly scoped as impl-leg-owned rather than spec-scope-creeping. **Next: B-33-A impl leg — TWO SEPARATE ARCS** (per U-IS-20 AC #9 / the plan header): (i) the U-IS-20 IS-axis code arc (`harness-is/src` only — the carrier + validation + canonicalization + the composed validator; buildable now, no operator gate, no external dependency); (ii) a LATER, separate CP-axis arc (its own new CP spec + plan delta, not yet decomposed) extending `verify_rotation_6_steps` per the fork's §2 points 2-4 + the two registered constraints at U-IS-20 Note (d) — `B-22`/`B-36`/`B-47` (seam/backend-selection/production-wiring) are ALL closed, so nothing external gates arc (ii) except arc (i)'s completion. Start with arc (i).
