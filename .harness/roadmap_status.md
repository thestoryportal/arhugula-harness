...
| Field | Value |
|---|---|
| `workspace_state_hash` | `3f15173f22a5` |
...
**Current next action (post-#1521).** `B-235`'s operator-facing half landed at #1521: `forward_register.py --detail` now prints the canonical `close_out` from the YAML row ABOVE the prose block, separated by `PROSE_DELIMITER`, so the surface routing actually reads carries the authority instead of the hand-maintained copy alone, and `close_out` is always stated rather than rendered as silence. The row stays `registered_finding`: the detection half is deliberately not built, because the shape `B-235` originally prescribed (extract the bound letters from both bodies and compare SETS) would NOT have caught the r6 defect it was written for — at r6 the YAML asserted (c)/(d) as blocking while MENTIONING (a)/(b), so letter extraction reports agreement, and a pattern matching nothing on both sides passes VACUOUSLY. Rendering the canonical field removes the need to parse prose for the fact at all; the residual is that the two bodies can still diverge undetected, materially less dangerous now that the canonical text reaches the reader. Two things this arc produced that forward work must not lose. FIRST, `B-236` is newly registered: `leg_selfcheck`'s dirty-carrier guard snapshots `git status --porcelain` ONCE and then renders the live working tree once per row, so a concurrent write in that window is invisible — the round-11 fail-open reopened from the TIMING side. It is pre-existing on `main` (line 1122, using `_run`) and #1521 changed that one line to `_run_checked`, which is strictly safer, so it was registered rather than absorbed; its row records that the two obvious remedies do NOT hold (`git show HEAD:<path>` breaks `--uncommitted` mode, and a per-call re-check narrows without closing) and that the closure shape is DETECT-AFTER: re-run the dirty check after the row loop and fail HARD if the carriers changed. SECOND, this arc ENDED ON ITS ROUND BUDGET, NOT ON CONVERGENCE: codex r16+ and the gemini failover are both refused `GATE_REFUSED (BUDGET_EXHAUSTED)` at 15/15 because the budget is producer-agnostic, so the last three commits (the `_run_checked` fail-open fix, the `_one_line` frame-spoof fix, and the merge-gate absorption) carry NO out-of-family review and rest on the three merge-gate lenses alone — r14 and r15 each returned a real P2, one falsifying the arc's central promise and one falsifying a claim in its own PR body, so late rounds were still productive when the budget ran out; extending it is operator-gated and is the one open judgment call from this arc. The next arc is unit `U-HE-38` (cohort report joint on `(concurrent_lanes_at_open, arc_type)`, the `ROADMAP_STATUS_DRIFT` join by `lane_id`/`arc_id`, and the correlational header; cited by unit id, NOT by line, because the previous pointer's `:6770` drifted the moment the U-HE-37 record was inserted above it), and its plan skeleton is already grounded: `GATE_LOG` exists at `tools/arc_metrics.py:54` with an env override, the skeleton's `except Exception: drift = []` would swallow the `RecordError` that `finding_record.read_rows` raises deliberately on a corrupt log and print `0/6` for "could not look" — the same empty-versus-unlooked confusion `B-235` exists to record — and its `json.dumps(t)` renders `(N=2, "applying")` while its own test asserts `(N=2, applying)`. Also carried: `just arc-metrics drain`, `just roadmap-status --archive-superseded`, and U-HE-37 Step 5 still blocked on the operator-gated `just main-protection-apply`.
