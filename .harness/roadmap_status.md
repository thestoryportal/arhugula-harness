...
| Field | Value |
|---|---|
| `workspace_state_hash` | `4b90df207756` |
...
**Current next action (post-#1322).** *B-71 preconditions 1+2+3+5 CLOSED on executed witnesses; **precondition 4 is the SOLE remaining gate**.* **Next arc: B-71 precondition 4 — close or explicitly scope the `entry_version` crash window.** It owes (a) an explicit scope statement and (b) a **live-resume witness** for the CITED `snapshot.run_id` reuse (`child_workflow_runner.py:230-234`) — currently the one unexecuted link under the basis decision. Closing it authorizes the spec leg. Also owed (residuals, not gates): the dispatch-level round-trip for both escalation venues, `materialize_span_processor_stage` end-to-end (which also settles **B-160**), the mixed keyed-peer fan-out path (shared with `B-155`), and the OD `redact_span` question. **NEW: `B-160`** — §C-OD-32.3 head=1.0 vs the 19-entry always-sampled set; its fix depends on `B-137`. Queue per §12.4.1: 30 `registered_finding`. **OPERATOR OWED (unchanged): (1) E7 move — `.harness/r-ctx-1-e7-machine-local-orchestrator-step.md` (re-probed 2026-08-12, re-denied), then `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust; (3) B-124 ratification (rec D at #1283).** B-148 Wave-5 PENDING-REMEASURE. `held=2` honored.
...
|---|---|---|
| 1322 | 2026-08-12 | B-71 precondition 5 CLOSED — **precondition 4 is now the SOLE remaining gate**. Witness `test_b71_escalation_token_rides_the_tracing_export.py` (8 tests, mutation-probed). **The charter's premise is FALSE in effect:** it declined to convene C7 because "the identity is not a span attribute", but §3 lands the widening inside `compose_hitl_action_id`, whose output rides the tracing export as `webhook.idempotency_key` on the OD-canonical `hitl.webhook.deliver` span — **witnessed producer→exporter** (the real `_escalate_to_secondary_channel` composes its own key; nothing in the test supplies it). ONE reachable carrier, not two: `hitl.invocation.audit_ledger_entry_id` is DEFAULT-ON but unreachable in **both** venues (the helper is `NoReturn`, raising before `:2238`) — asserted by observation over exported spans. Leak bar extends to tracing and binds the token's FORM: basis (B)'s raw identity carries the run_id verbatim, so hashing must occur BEFORE the composer. C1's `hitl.escalation.instance_id` **DECLINED** (second carrier; homeless on durable-async). **NEW `B-160` REGISTERED** — a contract-vs-impl drift on an inviolable §9.3 floor: §C-OD-32.3 declares webhook spans head=1.0 always-sampled, but `hitl.webhook.deliver` is absent from the 19-entry `ALWAYS_SAMPLED_EVENT_CLASSES`; its fix depends on `B-137` (ParentBased never consults a non-root child's name). Codex **7 rounds**, every one substantive — two reversed my own conclusions (one carrier not two; the drift, not "conditional egress"). Register 159→160: 30 registered / 127 closed. CI 18/18; main green. |
| 1321 | 2026-08-12 | B-71 precondition 3 CLOSED — `resolvability` carries the resolution CHANNEL (the existing closed `PauseLocationVariant`), never the outcome. The `held-for-sole-resolution` stamp retired on a WITNESSED eligibility flip; a channel is time-invariant so it cannot assert a false negative. v1's pause-view routing FALSIFIED (`project_pause_locations` takes the snapshot alone; `read_paused_workflow_state` is ratified NOT a liveness claim). Codex 17r — D-2 palette binding WITHDRAWN (time-invariant field cannot disarm a time-varying harm); B-155 re-scoped, premise UNVERIFIED. CI 18/18. |
| 1320 | 2026-08-12 | B-71 preconditions 1+2 CLOSED — identity-basis fork RESOLVED to (B) on an executed collision witness (13 tests, 4 mutation probes). (A) FALSIFIED: `_parallelization_fanout_action_id` takes `workflow_id` alone, so two sibling child runs of one `child_workflow_id` compose the identical triple. v1's ORCHESTRATOR ESCALATION CORRECTED: verbatim inheritance is WITHIN-run; nesting is CROSS-run. Codex 6r. CI 18/18. |
| PR #1319 | 2026-08-12 | B-71 council arc closed as DESIGN RECORD v1 — spec leg NOT authorizable (5 hard preconditions), and that IS the result. 5 voices to internal zero; 3 decorrelated reviewers falsified the load-bearing uniqueness claim. Basis left an OPEN FORK — resolved at #1320. Register 153→159. CI 18/18. |
| PR #1318 | 2026-08-12 | B-141 CLOSED — validator.fail.class OD/D6 cascade under B-138 (a): OD spec v1.41 + ADR-D6 v1.3 + CP plan v2.52 + OD plan v2.35 (FIFTH venue). Codex 4r; LEAN witness-lens APPROVE. CI 19/19. |
