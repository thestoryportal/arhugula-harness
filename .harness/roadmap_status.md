...
| Field | Value |
|---|---|
| `workspace_state_hash` | `bf1ca81f162f` |
...
**Current next action (post-#1341).** **`B-71` IS CLOSED — both legs (#1337 spec, #1341 impl); main GREEN at `0af632f4`.** `U-CP-102` + `U-RT-155` co-landed per their pin: the token is minted once at §14.8.8.1 step 1, folded once inside `compose_hitl_action_id` (reaching ALL THREE live invocations), persisted at §26.9, and projected at §2.2.A — one identity family across the webhook key, the CP audit `action_id`, the F2 ledger key and the pause view. **The next action is `B-165`**, which THIS arc registered and deliberately did not touch: whether two placements at the SAME position can both mint a token, in which case they collide, because the ratified basis hashes the position and not the declaration. **Its premise is UNVERIFIED — grounding it is step 1, not building it**: CP spec v1.119 §0.4(2) says so explicitly, and the one test previously cited as proof the shape is live actually constructs the composer WITHOUT `pause_resume_protocol`/`webhook_delivery_composer`, so it exercises the SYNCHRONOUS loop, which mints no token at all. If the shape IS reachable, repairing it means revisiting the ratified identity basis through the design record — that is **back-flow, not spec application** — so the honest outcome may be a fork filing rather than code. **Process lessons this arc paid for, all load-bearing forward:** (1) a review loop on an INVENTING leg does not front-load — eight out-of-family rounds yielded findings at r2/r3/r4/r5/r6/r7 and a merge-gate P1 after r8 came back clean, so do NOT stop at the first quiet round; (2) rounds 6-7 found defects the round-5 FIX introduced, so give every late absorption its own witness rather than trusting it; (3) an "advisory" diagnostic that can raise is worse than none — `warnings.warn` under `-W error`, a raising log handler, and a raising tree walk each turned one into a run-ending failure, and the guard must wrap COLLECTION as well as emission; (4) a diagnostic must never sit before the drain gate or snapshot validation, or it preempts the error handling it fronts; (5) reading a discriminated-union tag off a HAND-CONSTRUCTED member proves nothing — only the classifier can witness the classifier; (6) a probe that mutates the LHS cannot demonstrate non-circularity of an RHS. **Tooling gotcha worth keeping:** a mutation-probe driver whose anchored `FAILED` grep is not ANSI-stripped reports a fully-working probe set as fully broken — cross-check the pass/fail TALLY so the gate can tell EMPTY from UNLOOKED. **Do not run `just check` concurrently with a source-mutating probe driver or a mutation-testing subagent in this shared checkout** — it produces false reds (it did, twice). **Owed, not done:** `--archive-superseded` for the superseded round belongs in the NEXT content PR (it writes the archive, so it cannot ride a roadmap-status-only terminating refresh). **Registered, not fixed:** the ring-buffer wall-clock flake `test_flush_to_sqlite_100_span_batch_under_100ms_per_ac_5` went red once on #1341 and passed on a same-sha rerun; it is NOT registered by name alongside `B-166`/`B-169` and should be.
