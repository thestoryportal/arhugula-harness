...
| Field | Value |
|---|---|
| `workspace_state_hash` | `5cd716e35a62` |
| `last_refreshed` | 2026-06-17T05:46:23+00:00 |
| `git_head` | `2fe3da54` — #614 dashboard: per-arc atomic-unit map. A new "Arc & unit map" in the Build-progress section renders every R-FS-1 arc + its atomic units in plain language — real as-built units for the 5 done arcs (B1=14, B3=8, E=9, B2=8, R=4), anticipated slices for the 6 remaining (marked "units decomposed at arc-open"; no fabricated U-* per X-AL-3) — with before/parallel deps. NEW `.harness/r-fs-1-arc-and-unit-map.md` is the single arc→unit source ("Remaining forward work" here is now a pointer). Verified by in-browser render + codex (caught a maybe-serial [P3]); 14 dashboard tests + ruff green. Lags HEAD by one commit (this terminating-refresh fixed point, §12.2.1). Full detail: `git show 2fe3da54` + Recently completed. |
