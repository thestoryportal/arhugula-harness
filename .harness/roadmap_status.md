...
| Field | Value |
|---|---|
| `workspace_state_hash` | `b2584d827284` |
| `last_refreshed` | 2026-08-12T00:00:00Z |
| `git_head` | `cf0f2af3` —  |
...
**Current next action (post-#1327).** *B-161 CLOSED. The B-71 spec leg remains SCOPED but not landed — a reviewed draft sits at **draft PR #1325** / branch `b71-spec-leg-v1-119`.* **Next arc: build the B-71 leg as the SIX artifacts the scoping found**, in order — (1) CP spec delta with **four** carrier amendments (brief field; pre-hash basis carrier on `StepExecutionContext`; persisted echo on the per-branch pre-dispatch resume state; and the read path for the POST-hash echo back to the composer); (2) the **Runtime spec delta** (§14.8.8.1 step 2's two-argument `compose_hitl_action_id` + the driver's signal handling); (3) CP plan delta; (4) Runtime plan delta; (5) clearance markers; (6) then impl. **Start FROM the draft branch**, and carry its four settled sub-decisions (pinned sha256 digest; ordinal-in-prose carve-out; advisory ingress diagnosis; palette-suppression withdrawn) — read `.harness/forward-register.yaml` B-71 close_out first. **A caution the operator should weigh:** that draft took 4 review rounds and one caught regression; a fresh session may be the right venue for canonical-contract authoring. Alternative forward item if preferred: **`B-160`** (§C-OD-32.3 head=1.0 vs the 19-entry always-sampled set; its fix depends on `B-137`, and its close-out says ground first whether this is a CLASS across C-OD-3x rather than an instance). Queue per §12.4.1: 30 `registered_finding`. **OPERATOR OWED (unchanged): (1) E7 move — `.harness/r-ctx-1-e7-machine-local-orchestrator-step.md` (re-probed 2026-08-12, re-denied), then `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust; (3) B-124 ratification (rec D at #1283).** B-148 Wave-5 PENDING-REMEASURE. `held=2` honored.
...
|---|---|---|
| 1327 | 2026-08-12 | **B-161 CLOSED** — root-caused and fixed with the deterministic repro its own close-out demanded (not a retry marker, which that close-out warned against). **Not state leakage**, as registered: a **timing race on the shared flush budget**. `flush_observability` gives the ledger fsync only what is LEFT of the budget (`shutdown.py:943-947`), so with a tight 100 ms total a loaded runner's real fsync overran the remainder, raised `TimeoutError`, and appended `"ledger"` — reddening `failures == ()` for a reason unrelated to the tracer under test. **Repro:** fast False tracer + slow fsync yields the EXACT CI shape every time; slowing the TRACER instead does not (it is bounded too and tags itself) — that asymmetry identified the fsync as the overrunning half. **Fix:** widen that test's budget; the assertion stays EXACT so a real tracer regression still fails. NEW mechanism witness pins that an overrunning fsync IS tagged (mutation-probed). **No production change** — the shared-budget behaviour is correct and already covered. Two self-inflicted follow-ons caught in review and fixed: the witness first reintroduced the same tight-deadline race, then its oversized injected sleep left a daemon thread loitering and reddened a neighbouring mtime-sensitive GC test — both margins now balanced with the reasoning recorded inline. Register 161: 30 registered / 128 closed. CI 18/18; main green. |
| 1326 | 2026-08-12 | B-71 spec leg SCOPED, NOT LANDED — the non-landing IS the result. A full CP draft took 4 out-of-family rounds (14 findings absorbed) at **draft PR #1325**; unmerged because each round opened new STRUCTURAL surface and r3 caught a **regression it would have landed** (composed over v1.18 while v1.19 had widened `fail_detail_hash`). TRUE SHAPE now on the row — SIX artifacts incl. a **Runtime spec delta** and the POST-hash echo read path. 4 sub-decisions settled. B-71 stays `registered_finding`. CI 18/18. |
| 1324 | 2026-08-12 | B-71 precondition 4 CLOSED — ALL FIVE closed. Corrected a citation wrong since v2: the token's run component is the run EXECUTING the fan-out, so continuity is `mcp_server.py:371-372` (C-RT-35), not the child runner. Live-resume witness on the real stack. `durable=False` window resolved TWO-MODE (the CALLER persists). NEW **B-161**. Codex 2r. CI 18/18. |
| 1323 | 2026-08-12 | B-71 precondition 4 SHARPENED (not closed) — the v5 closure claim RETRACTED on review. Delivered a mutation-probed witness on a branch no test took. Codex 3r. CI 18/18. |
| 1322 | 2026-08-12 | B-71 precondition 5 CLOSED — the charter's "not a span attribute" premise is FALSE in effect; the token rides the tracing export. NEW **B-160**. Codex 7r. CI 18/18. |
