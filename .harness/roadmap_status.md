...
| Field | Value |
|---|---|
| `workspace_state_hash` | `8553b91839d9` |
| `last_refreshed` | 2026-08-14T00:00:00Z |
| `git_head` | `f5e0cb75` — B-169 registered |
...
**Current next action (post-#1338).** THE PREVENTION ARC IS LANDED (PR #1338, main GREEN) — the `B-71` impl leg is the next action: **`U-CP-102` + `U-RT-155`, CO-LAND PINNED** (neither is independently observable). Read the plan ACs first, since three encode defects review already caught: derive the basis UNCONDITIONALLY at both `workflow_driver.py` composition sites; the fold must reach ALL THREE `compose_hitl_action_id` invocations; the identity witness is split by venue because `_escalate_to_secondary_channel` is `NoReturn`. **Use what the arc shipped:** `just leg-selfcheck` BEFORE EVERY PUSH (not once per arc — that is the whole point); reclaim headroom with `just roadmap-status --trim-drift-log` in a CONTENT commit; install the next-action pointer with `--refresh --next-action`, never by hand; archive superseded rounds with `--archive-superseded` riding a CONTENT PR (`B-168` exit (iii), archive round N−1). **Do NOT wave away a red in the fan-out driver as "the known flake"** — `B-166` and `B-169` are registered precisely so a real regression is not dismissed; both are grounded but unreproduced, and neither clears a red.
