...
| Field | Value |
|---|---|
| `workspace_state_hash` | `d449092765ad` |
...
**Current next action (post-#1412).** S4c opens: U-HE-21 is on main (#1412). The recommended next action / next implementable unit is `U-HE-22` (tools/merge_door.py lease primitive — acquire fail-fast + rate limit + holder invariant, transition marker family, release/reclaim/self-resume/unblock, poison-pill completion, GC; C-HE-06 §2/§3/§6/§7, C-HE-02 §1–§3; deps U-HE-14 + U-HE-17 landed; plan §S4c), then U-HE-23 per plan §3. U-HE-22 now carries a REGISTERED CARRIER LINE in its Scope (from #1412 codex r2/r6): resolve the acquire-time state contract — a normal arc is still `pending` at ship-pr's final gate, so either acquire() admits the pending holder-elect or the final gate opens pre-acquire — reconciling C-HE-03 §4/§6 vs C-HE-06 §4(vi) together with the U-HE-19 item-(vii)/(ix) + U-HE-20 held classes (merged-headless-capture stall, stale-MERGED_REF bound, C-HE-04 (vi) sentence, spec §8.1 count). Other registered residuals: guard allowlist additions exact-shape at U-HE-25 (reservation CLI verbs selectable|show|reserve|update|mint-lane-id, HARNESS_ARC_ID=/HARNESS_LANE_ID= env-prefix strip, git merge-tree — headless lanes cannot reserve until it lands; degradation clause is live in roadmap-continue); lane-id atomic exclusive-create mint at U-HE-31; detached-launch supervision, RESOLVED-HIL minting, emission dedup, §0-vs-§1 ordering contradiction, activation-gate go-live witness (U-HE-29). Alternatives if the operator defers the lanes program: B-186 step (1), B-160, B-148 (unchanged).
