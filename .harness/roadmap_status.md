...
| Field | Value |
|---|---|
| `workspace_state_hash` | `8f810dfc8694` |
...
**Current next action (post-#1397).** **The S1 cluster is on main (PR #1397, merge `3b3e3a10`)** — `U-HE-03` (verdict schemas) → `U-HE-02` (`tools/review_wrapper_common.py`: terminal triple, classifier table reading error-shaped lines only, schema parse + six-field byte-compare, retry loop under the shared 1260 s deadline, `emit_outcome` via `finding_record.append_observations` with `round_n` minted under the log lock) → `U-HE-04` (`tools/codex_review.py`: `codex exec -o` rooted at a `git archive` export of the bound tree, -o file → stdout → stderr → session-artifact sources with a per-invocation nonce, HEAD-moved guard, `--failover`, HITL routing; `just codex-review` routes through it) → `U-HE-06` (`agy_review.py` under the core: bound-bytes payload, fenced block on the artifact-complete output, per-stage retry, `--outcome-json` envelope written after the rows; `just gemini-review` under `uv run`) → `U-HE-07` (`just review-with-failover`; `ship-pr` + `roadmap-continue` carriers; invariant #3 restated) → `U-HE-08` (`arc_metrics.ci_is_green`, CANCELLED named) → `U-HE-09` (`loop_lib.sh::loop_hil_ttl_resurface` under a flock, called at SessionStart). **Spec `Spec_HE_Loop_Lanes` is v1.2** (X2: C-HE-16 §3 per-attempt timeout is a 1200 s cap — 550 s killed real codex reviews mid-flight; marker `spec-he-loop-lanes-v1.2-cleared-2026-08-19.md`; operator may reverse via v1.3). The chain reviewed its own PR live for 13 rounds (codex primary under `codex exec`, the gemini failover carrying rounds 2/3/12) and the 3-lens merge-gate was unanimous. **Interface facts later units MUST absorb:** `codex review --base` rejects a PROMPT and review-mode ignores output contracts on real diffs (hence `codex exec`); the reviewer sees only the exported bound tree; `HARNESS_GATE_LOG` redirects the gate log for fixtures; `just codex-review`/`gemini-review` carry no preflights (the wrappers classify + record a stale login / missing binary). **The recommended next action is the S2 cluster as ONE PR in plan §3 order — `U-HE-05` (`tools/lanes_verify.py` manifest runner + `.harness/mutation-probe-log.jsonl` emitter; add the S1 rows: `test_review_wrapper.py` phase0 probe, `test_agy_review.py`, `test_arc_metrics.py::test_ci_state_cancelled_incomplete`, `shell:tools/hooks/test_loop_lib.sh`) → `U-HE-10` → `U-HE-11` → `U-HE-12` → `U-HE-13` (the production consumer of `merge-gate.schema.json`: lenses emit/parse the six binding fields; jsonl-first + consistency reducer).** Carry-forwards for `U-HE-23`/`U-HE-29`/`U-HE-44`: the tracked gate-row-after-binding vs exact-head landing rule needs a gate-row-only-delta predicate in C-HE-06 §4's door (U-HE-23); `loop_log` writers are unlocked so a stale NOTIFY can follow a concurrent resolve (U-HE-29, notification-only); the nonzero-exit asymmetry (parsed BLOCK stands, parsed APPROVE does not) is a recorded reviewer disagreement resolved fail-closed; U-HE-01's three residual classes and the X1/X2 notes. Alternatives if the operator defers the lanes program: B-186 step (1), B-160, B-148 (unchanged).
