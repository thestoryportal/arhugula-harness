...
**Frontier:** B-59-A IMPL LEG CLOSED at PR #1081 (U-RT-146: the frame-ledger accounting is factored out of `SubAgentDispatchExecutor` into a separately-adoptable `FrameLedger` — the composition root at `bootstrap/stage_5_loop_init.py:620-622` now adopts the process-lifetime ledger via `adopt_or_create_process_capacity_ledger()` instead of always constructing a fresh budget, while the executor's worker-pool + `_draining` flag stay per-bootstrap; all admission methods share the ledger's own lock object — never a second, separately-acquired lock; new `CapacityAuthorityBudgetShrinkError` typed refusal on an invalid shrink; all 7 acceptance criteria satisfied with 14 PD-8 mutation-probed tests incl. a real two-bootstrap integration test + an AST-based lock-nesting structural witness added after a merge-gate round-1 BLOCK found the original substring-count check + 2 constructor guard branches untested — fixed + re-verified at round 2; 2 codex rounds + 2 merge-gate rounds to clean convergence; full suite 3030 green, pyright clean). B-59 is now fully closed (spec leg #1080 + impl leg #1081). **Next: B-33-A** — RATIFIED, Option A (open-dict additive IS carrier + 3-step verifier extension + injected OD-join evidence DTO from the composition root) — `verify_rotation_6_steps` cannot prove a rotation boundary occurred (`.harness/class_1_fork_b33_rotation_correlation_carrier.md` §3). Spec leg first: author the IS spec delta (C-IS-05/C-IS-07 additive rotation-correlation carrier convention + the 3-step verifier-contract extension, mirroring OD §24.7) + U-IS plan widening + clearance markers; mirrors the B-59-A/B-65-A spec-leg→impl-leg precedent — start fresh, not mid-session.
