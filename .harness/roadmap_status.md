...
**Frontier:** B-39 Class 1 fork FILED 2026-07-23 (PR #1090) — `.harness/class_1_fork_b39_nested_hitl_response_threading.md`. Grounds the reverted per-child HITL-response-routing attempt's root cause (`ResumeContextHolder` is one mutable sidecar threaded by reference to every recursion level; `api.resume()`'s own public signature is singular too) and proposes threading `resume_context` BY VALUE through the existing `pause_snapshot_input` recursive plumbing instead of a shared keyed map — closes the reverted attempt's 2 defects (never-one-shot-consumes; `workflow_id` not branch-unique) structurally. Surfaces 2 operator-gated decisions (Q1: retire `ResumeContextHolder` for one top-to-bottom mechanism vs keep it at depth-0; Q2: how `api.resume()` accepts multiple per-child responses in one call, if at all, this arc) — no code lands until ratified, per X-AL-3. Docs-only PR, CI green, merged clean. B-33 CLOSED 2026-07-23 (prior frontier) — all three ratified-fork (Option A) legs landed: (i) IS carrier, PR #1083; (ii) CP+OD+Runtime spec+plan rider, PR #1084 (4 codex rounds, 24 findings); (iii) the real CODE — `verify_rotation_6_steps`'s `rotation_window_entries`/`evidence_provider`/`key_identity_resolver` extension, OD's `find_rotation_pair_evidence` accessor, the Runtime composition-root adapter, PR #1087 (2 codex rounds to clean convergence + 3-lens merge-gate all-approve). Two residuals REGISTERED as separate forward dependencies (NOT part of B-33's own scope): (a) `execute_key_rotation`'s real ledger-write path + a real `rotation_window_entries` producer; (b) the OD §21.2.2 rotation-period-aware cryptographic signature verifier. **Next: B-39 RATIFIED 2026-07-23 (operator `AskUserQuestion`) — Q1=(A) retire `ResumeContextHolder` entirely; Q2=(A) batched `Mapping[branch_path, ResumeContext]` at the `api.resume()` boundary (the heavier reading on both). Opens a spec leg next (CP §26 + Runtime §14.8 deltas + clearance markers + plan units), per the B-33/B-59 spec-leg/impl-leg precedent — not yet authored.** Meanwhile no auto-`ACTIVE` queue item; per §12.4.1 the remaining forward-register candidates (`tools/forward_register.py --open`, file order): B-16 (registered_finding, open architectural/speculative arcs survey), B-17 (operator_gated, ICM workspace-methodology adoption), B-30 (registered_finding, cost-attribution disambiguator replay-semantics inconsistency), B-44 (registered_finding, no production bootstrap site injects a non-default time_source clock), B-45 (registered_finding, cross-process ledger lock has no genuine Windows serialization backend), B-56 (registered_finding, corrected-relabel repair for already-tagged audit history), B-57 (registered_finding, direct-writer caller-supplied timestamp capture-order inversion), B-66 (registered_finding, B-60 fence-consult order has no single-site witness), B-68 (registered_finding, protected result store's opportunistic GC can race a concurrent in-flight publication under a short TTL — advisor()-reviewed this session as likely dormant/register-don't-build, same criterion as B-44/B-45, pending confirmation). None of these labels is a park — ground each candidate's premise at HEAD before picking (`[[full-register-ground-every-arc-before-blocking]]`), then drive the highest-value one to a genuine gate or a closed PR.
