...
| Field | Value |
|---|---|
| `workspace_state_hash` | `d0f57fa9ec27` |
...
**Current next action (post-#1372).** The register stands at 37 `registered_finding` + 1 `open` (B-148) + 2 `design_substrate_gated` + 2 `held` / 142 closed of **184** rows. **`B-137` is DEFERRED to the operator this run** (its step-3 choice changes a committed per-cell base-rate envelope; both council halves grounded at #1362/#1364, measurement withdrawn with its requirement at #1366). **`B-184` close-out (1) and (2) are DONE at #1372:** `tools/tools_test_coverage_guard.py` is now a blocking standing invariant — every `tools/test_*.py` must be **executed** (workflow `run:` block, parity script, or justfile) or carry an `EXCLUSIONS` entry **with a written reason**; it is two-way (stale exclusions fail) and **parses** commands rather than scanning text, so `cat X.py`, a `#` comment inside a run block, `--ignore X.py` and `echo pytest X.py` all correctly count as NOT executed. Guard reports **44 modules / 44 executed / 0 excluded**; parity collection **597 → 611**. `test_codex_loop.py` was NOT excluded from execution — it got its own CI job — and CI then caught what no local run could: it shells out to **`just`, absent on the runner**, an undeclared host dependency invisible for exactly as long as the module ran nowhere. Declared via `skipif`; 15 of 16 now execute in CI. **`B-184`'s only residual is close-out (3):** `tools/` still mixes bare imports (need `cwd=tools` or a `sys.path` insert) with `tools.`-package imports (need `cwd=ROOT`) — the split that let an earlier arc record a 4-FAILED result that does not exist. Normalise it, or document the required `cwd` per module. Also live: **`B-167` step 3** (false-positive measurement before either recount helper is wired into a gate) and **`B-182`/`B-183`** dispositions. **A LOCAL-ONLY BLOCKER, unchanged and operator-owned:** `just codex-check` reds on `tools/test_codex_stop_gate.py` because `.codex/hooks/stop_gate.py` inspects the ROOT checkout, dirty with four untracked `.claude/skills/` dirs standing since session start; verified pre-existing by stashing, CI clean-checkout green. Committing or removing those four restores a green local gate. **STANDING CAUTION, six consecutive arcs, and the single most useful line here:** every failure was a claim asserted from a LABEL or a convenient surface instead of the thing that determines it — an enum instead of the canonical matrix, a baseline instead of the effective head, a hand-assembled processor chain instead of the cell-selected one, nine filenames instead of the tests inside them, a substring instead of a parsed command, and **a tail of `gh pr checks` instead of the full list, which reported MERGEABLE while a blocking job was RED**. Read the whole result; a name is not a gate; numbers that do not reconcile are the signal.
