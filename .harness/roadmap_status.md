...
| Field | Value |
|---|---|
| `workspace_state_hash` | `958b7091cbe8` |
...
**Current next action (post-#1323).** *B-71: 1+2+3+5 CLOSED; **4 SHARPENED but still BOUNDED** — the sole gate. A v5 draft claimed it closed; retracted on review.* **Next arc: close precondition 4 — it needs TWO things, both named at §4-quinquies.4.** (1) A **live-resume witness through the real entry point** — the delivered witness is runner-level (`compose_child_workflow_runner` + captured `execute_workflow`), so it would stay green if the api/driver → `RuntimeSubAgentDispatcher` → child-runner chain never threaded the recovered snapshot; the fan-out real-stack harness at `test_b72_fanout_sub_agent_dispatch_hitl_gate_resume.py` is what to build it on. (2) A **decision on the `durable=False` default** — restrict the scope statement to durable mode, or extend the boundary through caller persistence and state the default's real guarantee. Closing both authorizes the spec leg; §5 carries the 8 residuals it opens with. **NEW `B-160`** (§C-OD-32.3 head=1.0 vs the 19-entry always-sampled set; fix depends on `B-137`). Queue per §12.4.1: 30 `registered_finding`. **OPERATOR OWED (unchanged): (1) E7 move — `.harness/r-ctx-1-e7-machine-local-orchestrator-step.md` (re-probed 2026-08-12, re-denied), then `just context-budget --sessions 1` → flip B-148 if ≤76k; (2) Codex `/hooks` re-trust; (3) B-124 ratification (rec D at #1283).** B-148 Wave-5 PENDING-REMEASURE. `held=2` honored.
...
|---|---|---|
| 1323 | 2026-08-12 | B-71 precondition 4 **SHARPENED but NOT closed** — it remains the sole gate, and that retraction is the arc's main result. A first draft claimed the closure; out-of-family review falsified it twice and the claim was withdrawn. **(1)** The window boundary was wrong at the DEFAULT config: `PauseResumeProtocolConfig.durable` defaults to `False`, where capture yields an in-memory snapshot on `RunResult.pause_snapshot` and only the CALLER's later persistence makes it recoverable — so the boundary is **mode-dependent** (`durable=True` closes at journaling; the default only at caller persistence, which the harness does not do). **(2)** The witness is runner-level, not the live resume precondition 4 asks for by name. **Delivered:** `test_b71_child_resume_reuses_the_snapshot_run_id.py` (4 tests, mutation-probed) pinning that the runner runs a resumed child under `snapshot.run_id` — not a fresh id, not the seed even when supplied alongside (ordering pinned) — on a branch **no existing test took** (all others pass `pause_snapshot_input=None`, the CRASH shape); plus the corrected two-mode boundary where v2's was vague. **To close:** a live-resume witness through the real entry point AND a decision on the `durable=False` default (§4-quinquies.4). Codex 3 rounds. Register 160: 30 registered / 127 closed. CI 18/18; main green. |
| 1322 | 2026-08-12 | B-71 precondition 5 CLOSED — the charter's "not a span attribute" premise is FALSE in effect: §3's widening lands in `compose_hitl_action_id`, whose output rides the tracing export as `webhook.idempotency_key` (witnessed producer→exporter). ONE reachable carrier — the default-ON audit attr is unreachable in both venues. Leak bar extends to tracing and binds the token's FORM. C1's dedicated attribute DECLINED. NEW **B-160** registered (§C-OD-32.3 head=1.0 vs the 19-entry set; depends on B-137). Codex 7r. CI 18/18. |
| 1321 | 2026-08-12 | B-71 precondition 3 CLOSED — `resolvability` carries the resolution CHANNEL (existing closed `PauseLocationVariant`), never the outcome; the `held-for-sole-resolution` stamp retired on a WITNESSED eligibility flip. v1's pause-view routing FALSIFIED. Codex 17r — D-2 palette binding WITHDRAWN; B-155 re-scoped. CI 18/18. |
| 1320 | 2026-08-12 | B-71 preconditions 1+2 CLOSED — identity-basis fork RESOLVED to (B) on an executed collision witness (13 tests). (A) FALSIFIED: `_parallelization_fanout_action_id` takes `workflow_id` alone, so two sibling child runs of one `child_workflow_id` compose the identical triple. Codex 6r. CI 18/18. |
| PR #1319 | 2026-08-12 | B-71 council arc closed as DESIGN RECORD v1 — spec leg NOT authorizable (5 hard preconditions), and that IS the result. Basis left an OPEN FORK — resolved at #1320. Register 153→159. CI 18/18. |
