...
| Field | Value |
|---|---|
| `workspace_state_hash` | `614cff52b767` |
| `last_refreshed` | 2026-06-17T04:34:18+00:00 |
| `git_head` | `de3bf3f7` — #612 dashboard legibility: R-FS-1 build progress is now the headline (masthead ticker + a done/next/remaining arc strip), fixing the misleading "build closure 100%" (the 54/54 substitution metric is demoted + captioned). Surface legend added; the post-Phase-8 "25" clarified to "2 open of 25 (23 resolved)". Verified by in-browser render (caught + fixed a blanking `rf1`-scope bug). Lags HEAD by one commit (this terminating-refresh fixed point, §12.2.1). Full detail: `git show de3bf3f7` + Recently completed. |
