...
> **STATUS UPDATE 2026-06-15 (R-FS-1 E-impl-2) — DP-2 re-open trigger FIRED + discharged.** DP-2's re-open condition ("a real … WAL-segment … recovery loop lands") is satisfied: the WAL_SEGMENT engine class is materialized end-to-end (U-CP-94 driver + U-RT-121 durable WAL segment-log substrate + U-CP-95 engine-layer firing branch + U-RT-122 durable factory bind & by-execution go-live e2e). `RuntimeEngineRecoveryLoop.capture_pause`/`.attempt_resume` now fire `cp.pause-captured` (C-CP-49) / `cp.resume-attempted` (C-CP-50) from a real production WAL_SEGMENT driver against a crash-survivable store — NOT a `workflow_driver.py` engine impersonation (the foreclosed anti-pattern), but a genuine hand-rolled durable engine per I-6. **DP-3(a) now applies in production** (the recovery loop exists; opaque identities flow from it). The H_T-CXA-2 substitution row transits BOUNDED_RESIDUAL → SUBSTANTIVE_RETIRED (batch-57). **DP-1 (HITL model-driven inner loop) stays bounded-residual** — that re-open condition (a real model-emitted multi-tool inner loop) is a separate arc, untouched here. Record: `.harness/r-fs-1-e-impl-2-finding.md`.
