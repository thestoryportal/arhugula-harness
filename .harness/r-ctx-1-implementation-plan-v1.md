...
- **Agent C · Opus 5 · worktree — Arc 4 heads table (PR-4):**
  ...
  - AC: generator output == committed table; completeness gate green; stale-AS-head grep → zero hits *(errata E6: the original root-wide `rg 'Action_Surface_v1_4|v1\.13.*Action_Surface'` gate is UNATTAINABLE — it matches immutable `design-substrate/**` history (edits prohibited) and cross-matches lines where an IS `v1.13` cite precedes `Action_Surface`. The gate is: `rg 'Action_Surface_v1_4|Spec_Action_Surface_v1\b.*v1\.13|v1\.13.{0,40}Spec_Action_Surface'` restricted to the LIVE head venues — root `CLAUDE.md`, `.harness/artifact-pointers/`, `harness-*/CLAUDE.md`, `AGENTS.md`, `CONTEXT.md` — with the version pattern bound adjacent to the AS artifact name, `design-substrate/**` explicitly excluded)*.
...
- **Agent E · Sonnet 5 · worktree — Arc 7 skills (PR-6):**
  ...
  - **U-CTX-17** R3: 12 bmad entries `.git/info/exclude` → `.gitignore`; documented reversible local removal; catalog assertion test: 21 names absent for this repo; `~/.codex/superpowers`, CodeRabbit surfaces (justfile, test_codex_loop.py), `~/.agents/skills` UNTOUCHED. *(errata E7: SPLIT by execution venue — Agent E's worktree PR carries only the TRACKED surfaces: tracked deletions, the `.gitignore` additions, tests asserting against tracked state. The MACHINE-LOCAL half — deleting the gitignored/untracked payload directories that exist only in the root checkout, and removing the 12 entries from the SHARED `.git/info/exclude` — is ORCHESTRATOR-side, ordered strictly AFTER PR-6 merges: (1) merge lands `.gitignore` entries; (2) orchestrator deletes the local payload dirs in the root checkout; (3) orchestrator removes the now-redundant `.git/info/exclude` entries — never before (2), or the root copies surface as untracked. The 21-name absence AC is verified twice: the tracked half in PR-6 CI, the machine-local half as an orchestrator acceptance step in the root checkout post-merge.)*
