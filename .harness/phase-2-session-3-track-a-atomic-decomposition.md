...
### L9-ter — Sub-agent dispatch composer + StepKindDispatcherRegistry + ChildWorkflowRunner (new at v1.6 / v2.5; in-CLI spec growth — **HALT-marked**)

> **⚠️ HALT marker — Class 1 fork open at C-RT-17 (Spec_Harness_Runtime_v1.md §14.7 head).** v1.6 spec authoring pre-survey at U-RT-59 implementation entry surfaced a structural gap: the `StepDispatcher` Protocol per C-CP-25 §25.3.3.4 lacks the per-step parent context surface that C-CP-12 gate-level descent + C-CP-13 §13.5 audit-trail-link composition require. **U-RT-59 implementation MUST NOT proceed until operator ratifies a resolution path** (4-path set at the fork record). Fork record: `.harness/class_1_tension_c_rt_17_step_dispatcher_parent_context_gap.md`. ACs below preserved as the post-resolution implementation target; the path resolution determines the binding shape changes needed at AC #4 (HandoffContext composition) and AC #5 (gate-descent + admissibility invocation).
...
- AC #4: HandoffContext composition verified — composer constructs C-CP-13 §13.1 7-field `HandoffContext` from step inputs + parent binding; verify the 7 fields populated per spec §14.7.3 v1.6 MVP shape (proposed_action from brief.objective; agent_confidence None; failed_attempts empty; alternatives_considered empty; state_summary with parent_entry_ref + empty summary; audit_trail_link from binding; retry_history empty or carried from binding).
- AC #5: gate-level descent + topology admissibility verified — composer calls `ctx.handoff_registry.dispatch(...)` and receives `SubAgentGateLevelDescent`; composer calls `ctx.topology_dispatcher.dispatch(child_manifest_entry)` and receives `TopologyPattern`; composer calls `is_admissible(topology, workload_class)` and respects the verdict (proceeds on True; raises `SubAgentDispatchTopologyInadmissibleError` mapping to `RT-FAIL-SUB-AGENT-TOPOLOGY-INADMISSIBLE` on False).
...
- AC #9: audit-entry composition + emission verified — composer calls `ctx.handoff_registry.compose_dispatch_audit(...)` per spec §14.7.6; resulting `CPAuditLedgerEntry` is appended to `ctx.audit_ledger_writer`; verify entry contains the dispatch-response-hash join key per C-CP-13 §13.5; verify entry's `child_result_status` matches the §14.7.2 step 7 mapping result.
...
Canonical at `design-substrate/Spec_Harness_Runtime_v1.md` v1.6 §15 (Spec-to-plan traceability) — coverage matrix mapping every U-RT-NN unit to ≥1 C-RT-NN contract, including per-bucket C-RT-12 §12.1–§12.6 rows, the C-RT-14 cross-cutting row, the v1.2-introduced U-RT-52 row (C-RT-15), the v1.4-introduced U-RT-58 row (C-RT-16), and the v1.6-introduced U-RT-59 row (C-RT-17).
