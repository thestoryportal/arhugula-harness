...
**Status:** **RESOLVED at implementation-arc landing 2026-05-20** — full spec arc closed across 8 commits (discovery → Path D → Path B-revised-a → ADR-D5 v1.4 close-out → runtime spec v1.7 + CP spec v1.8); implementation arc materialized the 4-substep sequence at `harness_runtime.lifecycle.sub_agent_dispatch.RuntimeSubAgentDispatcher._compose_and_persist_audit` (8a `compose_dispatch_audit` → 8b `ledger_writer.append` F2-write of dispatch action → 8c `cp_audit_to_od_audit` converter at `harness_cxa.cp_audit_conversion` → 8d `audit_writer.append`). AC #9 write half UN-STRUCK at atomic-decomp v2.6. New typed error `SubAgentDispatchAuditComposeError` + new fail class `RT-FAIL-SUB-AGENT-AUDIT-COMPOSE` added at runtime spec v1.7 §14. Converter moved from `harness-runtime/lifecycle/cp_audit_conversion.py` to `harness-cxa/src/harness_cxa/cp_audit_conversion.py` per Q5 ratification. CP plan v2.14 + OD plan v2.12 + workspace CLAUDE.md §2.3 absorptions all co-published. 2283 workspace tests green at close (up from 2278 — +6 step-8 unit tests + 1 multi-dispatch IS-chain integration test − 2 stale AC #9 partial tests; net +5; spec/plan deltas only on the other 3-test gap). Adversarial-review pass owed per runtime spec v1.7 status posture; H_T-CP-10/13/14 retirement-event prerequisite from AC #12 deferred to Phase 7d batch 4 per operator-ratified retirement audit cadence.
