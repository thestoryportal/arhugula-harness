...
| B-4 | **[proposed]** | V Deployment | TIER_3 microVM + TIER_4 full-VM execution (R-411/R-412) | host-fit gate explicit; current macOS x86_64 host is ineligible for the reviewed R-411 runtimes; R-421 now proves the managed-cloud surface for the future R-412 lane | impl + infra | ⚖️ yes |
...
| B-9 | **[blocked]** | VI Multi-tenant | OD-4: per-session redaction toggle (§13.1) + opaque-token tokenization (§13.2) | §13.1 toggle closed; OD opaque-token substrate present; durable audit-ledger token map + eval-grade classifier remain cross-axis-gated (R-008) | impl + cross-axis classifier | ⚖️ yes |
...
- **Current state.** The §13.1 solo-developer per-session toggle is closed. The 2026-06-07 R-008 slice adds an OD-owned provider-free opaque-token substrate: `RedactionSpanProcessor(tokenizer=OpaqueRedactionTokenizer(...))` can replace default-off content attributes with opaque `[REDACTED:CONTENT:<id>]` placeholders while storing raw values behind a token-map sink. Default strip-not-tokenize behavior remains unchanged when no tokenizer is injected.
- **Close-out steps.** (1) Back the token-map sink with durable audit-ledger-only persistence; (2) add the eval-grade semantic classifier/category pipeline needed for `[REDACTED:PII]`-style tokens; (3) advance OD-4 PARTIAL → RETIRE-READY only after those cross-axis gates are proven.
