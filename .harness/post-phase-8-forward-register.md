...
| **CXA-2** (CP→IS, 36–43 edges) | **STILL-BOUNDED** | `cp_is_wiring.py` PARTIAL-LAND (1 of 17 spec §12.3 edges); the U-RT-35 wiring unit landed (batch-46) but the full typed contract stays STILL-BOUNDED. The three production-materialized §16.5 firing methods are `emit_pause_resume_state_ledger_entry`, `emit_override_state_ledger_entry`, and `emit_workload_class_selection`; all 3 pause/resume workflow-driver caller sites, including the `HITLPauseRequestedSignal` path, have direct integration coverage. | Remaining close-out is not caller-site coverage for the materialized methods; it is the still-unmaterialized CP→IS surface: HITL tool-call rewriting remains bounded on a real production rewrite caller/spec disambiguator, and engine-layer pause/resume emissions remain bounded on future recovery-loop producers. |
