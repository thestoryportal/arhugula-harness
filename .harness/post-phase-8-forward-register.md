...
| B-10 | IX External | Real external MCP server connection | host materialized; `start()`/`shutdown()` lifecycle unwired at bootstrap | impl | no |
...
- **Current state.** The host is **materialized** (`materialize_mcp_client_host_stage`) and the converter (spec v1.40, Reading B) + sandbox-decision-resolver (spec v1.41 §14.9.8, Reading B) **landed**, but the bootstrap **never calls `host.start()`** (`stage_3a_cp_clients.py`) so the tool registry stays empty, and `host.shutdown()` has **zero callers** — the lifecycle is unwired at the bootstrap path. The skipif-gated echo-MCP-via-ollama e2e is the operator's live AC#2 close for `R-100-mvp-real-workflow-execution`.
- **Close-out steps.** (1) Add `await ctx.mcp_client_host.start()` after host construction at stage-3a; (2) wire `host.shutdown()` at stage-7 teardown; (3) operator configures a real `MCPClientConfig.connection_url` + per-server default tier/blast-radius/sandbox fields; (4) live e2e against a real external server. **Council: no** (mechanical wiring + operator config).
...
| **CXA-4** (OD→IS/AS/CP, 26 edges) | **PARTIAL** | All 3 OD-outbound wiring modules (`od_is_wiring` / `od_as_wiring` / `od_cp_wiring`) **exist + stage into bootstrap**, and the OD audit-write seam is **exercised at production** (6 `audit_writer.append` callers via `cp_audit_to_od_audit`). But only **~5 of 26 edges materialized**; the broader contract is PARTIAL (batch-42). | Materialize the remaining ~21 edges at the runtime composition layer (or operator-discretion canonical-scope narrowing of the 26-edge enumeration, parallel to AS-8e/8f). |
