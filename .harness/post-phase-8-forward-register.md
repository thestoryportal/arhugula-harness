...
|---|---|---|---|---|---|
| A-1 | ~~closed~~ | ~~R-700 Phase-8 substitution review + final integer ratification~~ | Phase 8 declared closed at 46/54 retired + 49/54 pipeline-advanced; batch-53 live ledger now reports 51/54 + 52/54 | `R-700-phase-8-substitution-accounting` RESOLVED; batch-53 | no (AUQ) |
...
| B-14 | **[partial]** | CXA | Cross-axis seam completion (CXA-1/2/3) | CXA-1 PARTIAL (no producer), CXA-2 STILL-BOUNDED (engine/upstream loops), CXA-3 STILL-BOUNDED (no composer). **CXA-4 is closed by batch-53** after 0-wireable grounding; placeholders resolved at v2.3/v2.11; v2.18 matrix defect fixed at v2.19/PR #226 | impl (mechanical) | no |
...
- **Current state.** CLOSED. The operator lifted the hold and Phase 8 was declared closed on 2026-06-02 at 46/54 RETIRED and 49/54 pipeline-advanced. Batch-52 and batch-53 are forward live-ledger supersessions, not rewrites of that declaration: R-810/R-820 evidence moves AS-8e, AS-8f, and CP-17 to RETIRED, and the OD-4/CXA-4 accounting back-flow moves two more rows to RETIRED, so the live ledger now reports 51/54 RETIRED and 52/54 pipeline-advanced.
...
- **Current state.** R-411 is closed for the selected no-taxonomy-change path. `GVisorRunscToolRunnerExecutionDriver` extends the R-410 Docker runner substrate, requires `SandboxTier.TIER_3_MICROVM`, and invokes Docker with `--runtime=runsc`. The approved live e2e targets the operator-provisioned Lima Linux VM through `R411_GVISOR_DOCKER_COMMAND`; host readiness passed, the TOOL_STEP executed under `runsc`, network egress was blocked, and the host repo path was not visible. Firecracker and QEMU `microvm` remain tracked as R-412 FULL_VM lanes, not R-411 lanes. E2B (`e2b-dev/e2b`) remains a managed-cloud candidate for R-421/R-412, not the local R-411 runtime.
- **Close-out steps.** R-411 is closed by the gVisor path. Remaining B-4 work is R-412 full-VM provider execution when a compatible Firecracker/QEMU/KVM or managed-cloud full-VM path is selected. **`⚖️ Council (C10 ⊥ C11)`** inherited from B-3.
...
- **Close-out steps.** None for R-420/R-421. R-420 **unblocks R-430** and removes the self-hosted prerequisite for R-421; R-421 removes the managed-cloud prerequisite for the future R-412 lane, which still remains gated on R-411/provider host fit. **`⚖️ Council (R-421 only)`** — closed by the selected E2B + GCP path.
...
> **Status spine = the live substitution ledger after batch-53, with R-700 dispositions preserved as historical provenance.** CXA edge counts per `Cross_Axis_Composition_Document_v2_18.md` §2.3.x. These are mostly **mechanical wiring** (runtime composer landings + production caller sites), not cross-domain design tensions → **Council: no** for all.
...
| **CXA-2** (CP→IS, 36–43 edges) | **STILL-BOUNDED** | `cp_is_wiring.py` PARTIAL-LAND (1 of 17 spec §12.3 edges); the U-RT-35 wiring unit landed (batch-46) but the full typed contract stays STILL-BOUNDED. The two production-materialized §16.5 firing methods are `emit_pause_resume_state_ledger_entry` and `emit_override_state_ledger_entry`; as of 2026-06-07, all 3 pause/resume workflow-driver caller sites, including the `HITLPauseRequestedSignal` path, have direct integration coverage. | Remaining close-out is not caller-site coverage for the materialized pause/resume method; it is the still-unmaterialized CP→IS surface: workload-class selection and HITL tool-call rewriting remain blocked by the U-RT-111 stage-ordering/spec-disambiguator gaps, and engine-layer pause/resume producers remain `NotImplementedError` stubs. |
| **CXA-3** (CP→AS, 24 edges) | **STILL-BOUNDED** | **No `lifecycle/cp_as_wiring.py` module** — consistent with spec §12 (no CP→AS runtime stage); typed edges anchored only at the 7c Pattern-P1 import surface. *(NOTE: this is a real open seam — not "N/A". The substrate-consumption relationship is genuine even though there's no composition stage.)* | Per ledger §11.1b: either **(α)** author a CP→AS runtime composer when a real CP→AS composition stage is opened, or **(β)** operator AskUserQuestion ratifying a scope narrowing of the CXA-3 retirement criterion. Files-arc substitution rows AS-8e/CP-17 are now separately closed by R-810 + batch-52; they no longer carry this CXA-3 analogy. |
