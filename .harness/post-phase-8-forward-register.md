...
|---|---|---|---|---|---|
| A-1 | ~~closed~~ | ~~R-700 Phase-8 substitution review + final integer ratification~~ | Phase 8 declared closed at 46/54 retired + 49/54 pipeline-advanced; batch-55 live ledger now reports 53/54 + 54/54 | `R-700-phase-8-substitution-accounting` RESOLVED; batch-55 | no (AUQ) |
...
| B-14 | **[partial]** | CXA | Cross-axis seam completion (CXA-1) | CXA-1 PARTIAL after must_pass #1: the workflow-time scoped secret-fetch producer now emits through AS→IS, but must_pass #2 / remaining AS source-unit callbacks still need edge-scope audit/back-flow. **CXA-2 is closed by batch-55** as a counted bounded residual after provider-turn HITL continuation landed and durable recovery was recorded as post-MVP hardening; **CXA-3 is closed by batch-54** after the CP→AS runtime composer landed; **CXA-4 is closed by batch-53** after 0-wireable grounding. | impl (mechanical) | no |
...
- **Current state.** CLOSED. The operator lifted the hold and Phase 8 was declared closed on 2026-06-02 at 46/54 RETIRED and 49/54 pipeline-advanced. Batch-52, batch-53, batch-54, and batch-55 are forward live-ledger supersessions, not rewrites of that declaration: R-810/R-820 evidence moves AS-8e, AS-8f, and CP-17 to RETIRED; the OD-4/CXA-4 accounting back-flow moves two more rows to RETIRED; the CP→AS runtime composer moves CXA-3 to RETIRED; the CP→IS seam moves CXA-2 to counted bounded residual. The live ledger now reports 53/54 RETIRED and 54/54 pipeline-advanced.
...
> **Status spine = the live substitution ledger after batch-55, with R-700 dispositions preserved as historical provenance.** CXA edge counts per `Cross_Axis_Composition_Document_v2_18.md` §2.3.x. These are mostly **mechanical wiring** (runtime composer landings + production caller sites), not cross-domain design tensions → **Council: no** for all.

| Seam | Live disposition | Current state | Close-out |
|---|---|---|---|
| **CXA-1** (AS→IS, 13 edges) | **PARTIAL** | `as_is_wiring.py` composer materialized + 7c-tested; the workflow-time scoped secret-fetch producer now calls `emit_secret_fetch_audit_entry` from the active `TOOL_STEP` dispatch site with non-hollow scope and rotation metadata. | Do not retire yet. Run the must_pass #2 / AS→IS edge-scope back-flow audit and either thread the remaining AS source-unit audit-emission callbacks through `AsIsWiring` or narrow/account for the legacy remaining-edge wording. |
| **CXA-2** (CP→IS, 36–43 edges) | **RETIRED-AS-BOUNDED-RESIDUAL** (batch-55) | Workflow-layer pause/resume, override, and workload-selection producers fire from production caller sites; PR #449 added the HITL/recovery producer primitives; PR #452 bound them at stage 5 and proved direct CP→IS emissions; PR #454 wired Anthropic provider-turn HITL continuation through `ctx.hitl_tool_loop`. The remaining durable recovery-loop evidence is explicitly post-MVP hardening under the ratified DP-2 re-open trigger. | **Closed by batch-55.** Do not wire `workflow_driver.py` as a fake engine recovery loop; re-open only when a real event-sourced replay, reconciler-loop, WAL-segment, or engine-native-pause recovery loop lands. |
