...
|---|---|---|---|---|---|
| A-1 | ~~closed~~ | ~~R-700 Phase-8 substitution review + final integer ratification~~ | Phase 8 declared closed at 46/54 retired + 49/54 pipeline-advanced; batch-54 live ledger now reports 52/54 + 53/54 | `R-700-phase-8-substitution-accounting` RESOLVED; batch-54 | no (AUQ) |
...
| B-14 | **[partial]** | CXA | Cross-axis seam completion (CXA-1/2) | CXA-1 PARTIAL (no producer), CXA-2 STILL-BOUNDED (engine/upstream loops). **CXA-3 is closed by batch-54** after the CP→AS runtime composer landed; **CXA-4 is closed by batch-53** after 0-wireable grounding. | impl (mechanical) | no |
...
- **Current state.** CLOSED. The operator lifted the hold and Phase 8 was declared closed on 2026-06-02 at 46/54 RETIRED and 49/54 pipeline-advanced. Batch-52, batch-53, and batch-54 are forward live-ledger supersessions, not rewrites of that declaration: R-810/R-820 evidence moves AS-8e, AS-8f, and CP-17 to RETIRED; the OD-4/CXA-4 accounting back-flow moves two more rows to RETIRED; the CP→AS runtime composer moves CXA-3 to RETIRED. The live ledger now reports 52/54 RETIRED and 53/54 pipeline-advanced.
...
> **Status spine = the live substitution ledger after batch-54, with R-700 dispositions preserved as historical provenance.** CXA edge counts per `Cross_Axis_Composition_Document_v2_18.md` §2.3.x. These are mostly **mechanical wiring** (runtime composer landings + production caller sites), not cross-domain design tensions → **Council: no** for all.
...
| **CXA-2** (CP→IS, 36–43 edges) | **STILL-BOUNDED** | `cp_is_wiring.py` PARTIAL-LAND (1 of 17 spec §12.3 edges); the U-RT-35 wiring unit landed (batch-46) but the full typed contract stays STILL-BOUNDED. The three production-materialized §16.5 firing methods are `emit_pause_resume_state_ledger_entry`, `emit_override_state_ledger_entry`, and `emit_workload_class_selection`; all 3 pause/resume workflow-driver caller sites, including the `HITLPauseRequestedSignal` path, have direct integration coverage. The 2026-06-08 audit at `4baefe0b` found no new producer for the remaining HITL rewrite or engine-layer recovery-loop methods. | Remaining close-out is not caller-site coverage for the materialized methods; it is the still-unmaterialized CP→IS surface: HITL tool-call rewriting remains bounded on a real production rewrite caller/spec disambiguator, and engine-layer pause/resume emissions remain bounded on future recovery-loop producers. |
