...
- **Current state.** Registry + **FILESYSTEM backend landed** (`LocalFilesystemMemoryToolBackend`; `CP-16` closed RETIRED-AS-BOUNDED-RESIDUAL batch-44), **SELF_HOSTED_SERVER `DATABASE` backend landed** (`SqliteMemoryToolBackend`, stdlib sqlite3), and **MANAGED_CLOUD S3 cloud-vault backend live-proven**. The approved live S3 e2e bound `MemoryToolStorageBackend.S3` on `DeploymentSurface.MANAGED_CLOUD`, used AWS CLI login profile credentials, and exercised create/view/str_replace/insert/delete through the Memory tool dispatch seam. The optional managed-DB remainder is now implemented provider-free: `ManagedSqlMemoryToolBackend` handles PostgreSQL-compatible `postgres://` / `postgresql://` `DATABASE` bindings, and the factory lazily constructs `psycopg` only for live use.
- **Close-out steps (live managed-DB gate).** Set `R830_MANAGED_DB_CONNECTION_STRING` to a disposable/provisioned PostgreSQL-compatible managed DB, approve the side-effecting test, then run `just r830-managed-db-live-e2e`. The test creates the `memory_entries` table if needed, writes a unique `/memories/live/...` row, exercises create/view/str_replace/insert/delete through `_invoke_protocol_callback`, and cleans up. Gate is visible in `.harness/codex_credential_gates.jsonl`. **Council: no** (impl).
