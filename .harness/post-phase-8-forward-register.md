...
- **Current state.** Not built; B-3 settled the execution-driver pattern. R-411 should target `google/gvisor` (`runsc`), `kata-containers/kata-containers` (`kata-runtime`), `superhq-ai/shuru`, or `superradcompany/microsandbox` based on host fit. Firecracker is tracked as the R-412 FULL_VM lane, not the R-411 lane. E2B (`e2b-dev/e2b`) is a managed-cloud candidate for R-421/R-412, not a local R-411 runtime, and requires `E2B_API_KEY`. The repo now has a non-mutating `just sandbox-host-check <provider>` readiness probe. Current operator host grounding: macOS x86_64 with no `/dev/kvm`; Shuru/Microsandbox require Apple Silicon on macOS, and Firecracker/Kata require KVM on Linux. R-412 is explicitly deferred until a MANAGED_CLOUD/Linux-KVM surface exists.
