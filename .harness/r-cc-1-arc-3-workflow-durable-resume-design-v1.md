...
Scope (operator full-drive; durable store = step 2): the `api.resume` **public surface (caller-supplied `PauseSnapshot`)** + surfacing PAUSED through `api.run`. Two cleared contracts touched (C-RT-09 RunResult — minor bumps; C-RT-30 NEW sibling to C-RT-08). All grounded at HEAD by direct read.
