...
**CLOSURE 2026-05-28 — foreclosed-by-Reading-H** per `.harness/class_1_fork_webhook_composer_per_workflow_context_threading.md` operator-ratified Reading H apply-pass merged at `8a55259` (runtime spec v1.34 + runtime plan v2.30). The (i) framing as originally written ("future doc-hygiene arc may extend the factory signature analogous to validator factory") is foreclosed at the architectural-pattern layer: the composer's cost-attribution skip condition at `webhook_delivery_composer.py:319-323` requires `workflow_id` non-None (plus parent_action_id + parent_idempotency_key) in addition to {rate_table, cost_chain, audit_writer}. A bootstrap stage-5 LOOP_INIT factory can only thread the 3 bootstrap-time substrates; workflow_id + parent_action_id + parent_idempotency_key are per-workflow context that does NOT exist at bootstrap. Threading 3-of-6 substrates from the factory: skip condition still fires; cost attribution still off; ZERO functional advance.

Reading H ratified per-workflow composer construction as the canonical architectural pattern (operator constructs composer per-workflow at HITL escalation site / workflow_driver entry / etc. with both `webhook_config` AND cost-attribution substrates supplied at ctor). Under this canonical pattern, the bootstrap-singleton path with cost-attribution-off is NOT a deferred state — it IS the canonical end-state for the bootstrap path. Operator-bound deployment-time wiring per H_T-OD-5 RETIRE-READY gate is the per-workflow construction site, not a factory extension.

**Closure-event-class**: **species 5** per workflow v1.10 §7.4.7.2 — *post-authoring stale carry* (SELF-AUTHORED at batch-28 filing, genuine at authoring-commit time; downstream event — Reading H ratification — lands AFTER batch-28 authoring, between authoring-timestamp and next-substantive-amendment-opportunity). Discriminator: §3 (i) was self-authored at batch-28 single-arc filing (no delta-version inheritance lineage) and was genuine at filing time (Reading H not yet ratified). The sibling-arc Reading H ratification ~4 hours later makes the carry's *suggested resolution path* (factory extension analogous to validator) architecturally wrong. Carry-text remains accurate in describing the bootstrap-default state; the "future doc-hygiene arc may extend" framing is what's stale-as-described post-Reading H. Catalogued at workflow v1.10 §7.4.7.2 sub-species candidate (see §3.ii below).

ZERO production code change at closure; ZERO runtime spec v1.26 §14.16 amendment owed (the §14.16 binding chain authoring already states `bootstrap/factories/webhook_delivery_composer_factory.py` opt-in construction without ctx-side cost-substrate thread-through; the spec was right; the (i) framing was the mis-reading).

(ii) **Sub-species catalogue candidate at workflow v1.10 §7.4.7.2 — FIRST species-5 sub-species.** The §3 (i) closure event class maps to **species 5** (post-authoring stale carry) per workflow v1.10 §7.4.7.2 row 5 — distinct from species 3 (resolved-but-carry-stale-inherited; cross-delta-version inheritance lineage) because §3 (i) is self-authored at batch-28 single-arc filing, not inherited across multiple delta versions. Candidate for new sub-species **5.1.same-session-sibling-arc-forecloses-suggested-resolution-path** (first species-5 sub-species catalogued — species 5 sub-species enumeration is empty at v1.10 §1.2 per §1.3 OPEN catalogue authority).

Distinctive closure-event-class statement: *the self-authored carry was genuine at v_n authoring + foreclosed at sibling-arc ratification ~hours later in same session; the carry's suggested resolution path becomes architecturally wrong (not merely "resolved") because the sibling-arc ratification establishes a canonical architectural pattern that the carry's framing did not anticipate.*

Common-ancestor relationship: species 5 with closure at same-session-sibling-arc-architectural-pattern-foreclosing layer. Sibling-relationship to v1.10 species-3 sub-species **3.same-session-immediate-sequel** (different species axis: 3 inherited vs 5 self-authored; same time-axis: same-session sequel) and **3.carry-suggests-foreclosed-reading** (different species axis; different foreclosing-site: 3 originating fork doc vs 5 sibling-arc fork doc).

NOT patched at batch-28 amendment scope (workflow-doc revision out of single-batch-event filing scope); empirical lineage filed here for future workflow-doc revision arc consumption per v1.10 §1.3 OPEN catalogue authority enumeration shape (sub-species number 5.1; closure-event-class statement above; empirical cataloguing arc = batch-28 §3 (ii); common-ancestor statement above).
