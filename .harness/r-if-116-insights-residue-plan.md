...
- `_registry_append()`: one `jq -nc` compact line `{ts, event, session, agent_id, transcript, cwd}` appended under the registry's advisory lock (python-`fcntl` flock on a sibling `.lock`, same pattern as `hook_checkpoint_generation`, `lib.sh:157-189`) — **lock-first, open the registry fd for append only INSIDE the locked region, never carry the fd across a rename** (an fd opened before the lock survives the prune's `mv` and writes to the dead inode — merge-gate lens-1; this lock requirement lives HERE, in the unit that ships first, not only in U-HK-44). Single-`printf` line, guarded `[ ${#LINE} -lt 4000 ]` (bounded-line hygiene; the atomicity guarantee comes from the lock, not PIPE_BUF — which governs pipes, not `O_APPEND` files). Lock contention: `start` rows may skip after ~0.2s (benign loss); **`stop` rows block for the lock inside the hook's existing `hook_bounded` envelope** — a skipped `stop` would manufacture the false "unreconciled" report the feature exists to prevent, and makes AC (5) flaky. Called at the top of both SubagentStart and SubagentStop branches; failure invisible (no behavior change to the existing gate). Correlation key: `agent_id` (documented common subagent hook field, claude-code-hooks.md §hook-input). **Both branches resolve the transcript field identically (`.agent_transcript_path // .transcript_path`)** so the fallback key matches across start/stop (today only the Stop branch reads it — asymmetric keys never reconcile under fan-out); when `agent_id` is absent on either side, the row is low-confidence and must never cross-reconcile another agent's start.
...
- Interface: `uv run tools/mutation_probe.py --file F --lines A-B --test "<cmd>"`. Steps: (0) refuse if `git status --porcelain F` non-empty (exit 2); (1) run test, require PASS (probe against red test is meaningless → exit 2, distinct message); (2) read original bytes to memory, comment out lines A–B (`# ` prefix); (2b) **require the mutated file to remain syntax-valid (`ast.parse` for `.py`, `bash -n` for `.sh`) — a syntax-invalid mutation makes even a vacuous test fail at import/collection, faking probe success; reject the range with exit 2, distinct message** (codex round-5); (3) run test, require FAIL **for a test-level reason, not a parse/collection error**; (4) restore in a `finally` from in-memory bytes — **but first verify the on-disk bytes are still exactly what the probe wrote; on mismatch (a concurrent writer touched F mid-probe) REFUSE to restore and fail loudly, since blind restore would silently clobber the concurrent edit and `git diff --quiet` would still pass** (merge-gate lens-1) — then verify `git diff --quiet F`, hard-error loudly if restore failed; also write an on-disk backup sidecar (`F.mutprobe.bak`) before mutating and reconcile/remove it at startup, so a SIGTERM that skips Python's `finally` (e.g. a `timeout -k` kill) leaves a recoverable original rather than commented-out source; (5) exit 0 probe-pass / exit 1 `PROBE FAILED: test stayed green with F:A-B removed`. **No `git stash`, no `git checkout --` anywhere.**
