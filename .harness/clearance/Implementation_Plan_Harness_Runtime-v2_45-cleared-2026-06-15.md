...
- **U-RT-122** — R-CXA-2 engine-layer activation: bind U-RT-121 in `r_cxa_2_producer_loop_factory.py:208-214` (replace `DeterministicEnginePauseResumeSubstrate`), giving `RuntimeEngineRecoveryLoop` its **first production driver** → `cp.pause-captured`/`cp.resume-attempted` (C-CP-49/50) land → **R-CXA-2 CP→IS engine-layer seam goes LIVE** (a ratified bounded-residual; line-181 trigger fired). AC = by-execution e2e against the DURABLE substrate (foreclosing the cosmetic-Journal-swap anti-pattern).
