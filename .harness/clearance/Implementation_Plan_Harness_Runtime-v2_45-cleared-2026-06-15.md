...
reviewer_chain:
  ...
  - out-of-family Codex review (`just codex-review`, 5 passes to convergence; full provenance at the CP v2.34 marker) — 3 [P2] applied: (1) the `test_u_rt_95` un-skip false-green → strengthened AC; **(2a) the strengthened AC's DAG/AC conflict → relocated the `test_u_rt_95` durable e2e + un-skip + Path-(i) fork flip INTO U-RT-122 (this runtime unit)** + narrowed U-CP-94 to CP/IS-only; (2b) roadmap §12.2 timing (post-merge). Plus pointer-index + U-RT-121 `(none)`-dep-label reconciliations. The U-RT-122 go-live AC was by-execution against the durable substrate throughout. The `roadmap_status.md` `next_action` refresh (→ E-impl-1) follows as the post-merge §12.2 terminating-refresh (the `workspace_state_hash` is computable only post-merge; the §12.1 drift-detector guards the interim).
