...
reviewer_chain:
  - "advisor() pre-substantive review — redirected candidate selection away from the token with the largest internally-documented evidence volume (`merge-gate-catches-what-codex-advisor-miss`, 17+ dated PR instances catalogued inside its file, distinct from its citation cardinality of 3) after identifying its transferable content already has a canonical home (`.claude/skills/merge-gate/SKILL.md`'s own text + root `CLAUDE.md` §13.1's advisor+codex decorrelation framing); pointed at the genuinely uncovered gap instead — four related memories describing distinct adversarial-review-loop non-convergence signatures, none homed in §7.5"
...
- The R-600 sweep artifact (`.harness/R-600-pattern-bake-in-sweep.md`) gains a cadence-9 section in this same PR, including the file-delta-cutoff resolution: git-history reorganization made the recorded cadence-8 PR merge commit (`323dfeae`) unreachable via `merge-base --is-ancestor` against current HEAD, but `git log --all` still resolves that commit's own object timestamp directly (`2026-07-14T18:52:18-06:00`). The memory-file-delta enumeration uses this exact git-object timestamp as cutoff (not the artifact's prose-recorded close date — a first draft used a same-day-midnight approximation and was corrected at codex round 1). The "merge count since cadence-8" metric proved unreconstructable, not merely miscounted: `main`'s first-parent history terminates at `2026-07-17T04:38:40-06:00` (3 days after cadence-8's close), i.e. the reorganization removed reachability further back than the cutoff itself — every commit currently on `main` postdates the cutoff by construction, so no "since cadence-8" sub-count exists to compute (codex round 2 caught an anchor-vs-floating-HEAD instability in an intermediate figure; round 3 traced that instability to this deeper cause). The cadence trigger (~10 PRs) is satisfied regardless, at overwhelming margin (all 121 current commits qualify) — the artifact records this honestly rather than continuing to chase an unreconstructable exact sub-count.
