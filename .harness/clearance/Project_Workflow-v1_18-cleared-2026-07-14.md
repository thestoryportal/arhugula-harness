...
reviewer_chain:
  ...
  - "out-of-family `just codex-review` on this PD-8 promotion's own PR, pre-merge, FIVE rounds — round 1 caught the audit-window misattribution + overstated independent-arc cardinality; round 2 caught round 1's own new errors (PR #972's axis mischaracterized, its probe count uncorrected); round 3 asked for live execution evidence rather than historical self-report; round 4 caught an undercounted merge total + an unreproducible `...::test_*` placeholder in the round-3 evidence; round 5 caught a broken Markdown table (the round-4 recipe's numbered list closed the table early) + unquoted `#`-PR-numbers truncating this very YAML list as comments — all 10 corrected before merge, see Notes"
...
**Ten factual/mechanical corrections made pre-merge, across five rounds of out-of-family Codex review of this promotion's own PR** (PD-8 applied to itself, recursively, until the reviewer ran out of gaps to find). Rounds 1-4 (8 corrections; see `design-substrate/Project_Workflow_v1_18.md` §3(e) for the full per-round detail): audit-window misattribution; overstated arc-count (3→2); PR #972's axis mischaracterized (IS→CP) + probe count (4→2); the "2 probes" claim resting on historical self-report rather than live evidence, closed by literally re-running the mutation-probe against current HEAD; the file-delta audit's own reproducibility caveat once this PR's edits bumped the source memory file's mtime; an undercounted merge total (18→23, via `git log --first-parent --oneline 9cad319e..380ec161 | wc -l`); and an unreproducible `...::test_*` placeholder, fixed with a full 5-step recipe. **Round 5 caught two purely mechanical defects the content-focused rounds missed:** (9) that round-4 recipe was written as a numbered list embedded inside a GFM/CommonMark table cell — a newline inside a table cell closes the table after the first data row, so the §7.5.1 gate table's 2nd and 3rd rows (genuinely-§7.5-shaped; no-canonical-home-elsewhere) were rendering as a stray paragraph instead of table content — fixed by moving the recipe to its own subsection immediately below the table; (10) this very clearance marker's YAML frontmatter had unquoted `#927`/`#972`-style PR references in `reviewer_chain` list items, and PyYAML (confirmed by direct parse) treats an unquoted `#` as a comment start, silently truncating those entries mid-sentence — fixed by quoting the affected list items. The fully corrected, honest, live-verified, and now also *renders and parses correctly* count is **2 genuinely independent instances**: the #927/#933 session + PR #972/`B-26` (2026-07-14, CP-axis, a wholly separate session, 2 discriminating-witness probes in one PR, re-run and confirmed against current HEAD). None of the ten corrections changes the promotion outcome — 2 still clears the literal §7.5.1 gate, at the thinnest margin of any PD promoted so far, recorded honestly rather than overstated. The remaining two gates hold as originally assessed: genuinely §7.5-shaped (verification-sequencing, not stale-carry-text or fidelity grammar), and no canonical home elsewhere (adjacent to but distinct from PD-3's verification-shape-matching and PD-6's composed-chain-witness disciplines — PD-8 is the narrower check that a test, once written at the right depth through the right chain, actually discriminates the fix from its absence).
