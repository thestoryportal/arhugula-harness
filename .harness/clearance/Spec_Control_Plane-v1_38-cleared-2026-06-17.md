...
**The CP-side field add is additive (no operator gate); the committed-invariant relaxation lives in the paired runtime v1.52 §14.5.3 amendment and was operator-gated.** The CP driver folds `binding.agent_role` onto the single `StepExecutionContext.agent_role` source at composition (Option B; precedence per-step > fan-out-derived > default), so the runtime dispatch read is unchanged. **OPERATOR RATIFIED 2026-06-17** (AskUserQuestion "Ratify full"); this marker, the CP v1.38 delta, the runtime v1.52 amendment, and the impl land together; `merge_commit` pinned at the post-merge refresh.
