...
reviewer_chain:
  ...
  - "out-of-family codex round 7 (BLOCK, 1 P1 + 3 P2) -- REGISTER-AND-HOLD TERMINAL (the guard class's 7th consecutive raise, same terminal shape as U-HE-20's): mint-lane-id added to the guard-adjudication matrix + the headless degradation documented as a distinct explicit contract (both new P2s absorbed); the fail-open P1 adjudicated HELD -- the degradation is byte-identical to the pre-U-HE-21 headless baseline (which had NO selection dedup), so the carrier is a strict coverage increase with the remainder registered exact-shape at U-HE-25; lane-id exclusive-create (4th raise) stays at U-HE-31. Final adjudication passes to the 3-lens merge gate"
supersedes: null
superseded_by: null
---
