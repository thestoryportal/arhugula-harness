...
reviewer_chain:
  ...
  - impl-time grounding pass (every AC witnessed by execution; sixteen PD-8 mutation probes, all seen RED)
...
**Fifteen acceptance criteria and seventeen PD-8 mutation probes** (ten and ten at first filing; AC #11 + P11–P14 at review round 1, AC #12 + P15 at round 3, AC #13 + P16 at round 4, AC #14 at round 6, and AC #5's discrimination requirement + AC #15 + P17 at the merge gate), each probe naming the specific witness that must be seen RED. The load-bearing discipline is stated as a rule the unit is measured against: **a green state-machine unit test is NOT the witness** — the state machine was already green and the gap was reachability — so both recovery directions run end-to-end through the real `RetryBreakerFallbackDispatcher.dispatch` with an injected clock, and the concurrency witness GUARANTEES its interleaving with an `asyncio.Event` rather than hoping for it.
...
**Review rounds 1–6 and the merge gate absorbed at §0.7–§0.12**: AC #11 (epoch-guarded trial ownership, witnessed at BOTH the state machine and end-to-end through the composer with the exact A/B interleaving) plus PD-8 probes **P11–P14**; and AC #6 reconciled against cell 4's VOID-BY-CONSTRUCTION status. **FIFTEEN ACs / SEVENTEEN probes**, and the closure criterion is now stated over the SETS rather than over a count, so a further round cannot silently under-specify closure.

**Merge-gate lens 3 BLOCK, and why it matters most.** The gate MEASURED a witness gap the leg's own seventeen-probe PD-8 discipline had missed: deleting `trial_token` from the max-attempts-exhaustion `record_failure` left the whole suite green while `fail_count` went 3 → 2 and `trigger_count` 3 → 0. Cell 5's witness asserted only `state is OPEN` + a fresh `opened_at` — **both equally true of an inconclusive re-arm** — so it could not tell a real exhaustion charge from a stale-dropped one. It was the single cell in the matrix without the discriminating pair. Closed with those two assertions plus probe P17; the escalation site's unwitnessed token threading resolved by determining unreachability from content and pinning the determination; cells 7/8 given the `opened_at`-FRESH assertion AC #6 already required; both `BaseException`-boundary asymmetries witnessed.
