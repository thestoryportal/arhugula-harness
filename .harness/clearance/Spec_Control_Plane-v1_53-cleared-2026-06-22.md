...
**Reviewed during clearance (verified by execution).** NON-VACUITY witness (advisor's blocking check): with the §19.5 persona-floor knob OFF (floor LIVE), a per-step removal skips THIS step's SUB_AGENT_BOUNDARY gate while an unremoved sibling still gates — the genuine per-step delta over §19.5's global policy. Plus: floor-clamp refusal at EXTERNAL_* blast + at an untrusted MCP host + at a deny-tier per_tool; LOCAL_MUTATION removal effective; non-solo no-op; fail-closed on unresolvable floor; the `placement-removed` audit shape (distinct from `approve`); carrier propagation (`StepOverride → resolve_step_binding → binding`, CP-side) + composer consumption (runtime-side) witnessed SEPARATELY + `model_dump` provenance; negative control (empty ⇒ byte-identical). **Seam honesty:** the CP→runtime driver seam — the binding `resolve_step_binding` produces reaching `c_rt_17` in a single live driver run — is NOT a one-execution full-chain witness; it shares the `agent_role`/`prompt_version_sha` composition wiring verbatim (proven by precedent + inspection, both are `StepEffectiveBinding` fields the composer reads via `getattr`). harness-cp 1175 passed + 1 xfailed; harness-runtime non-e2e 2050 passed; CI pytest (all axis packages) green; pyright 0/0/0 (src + changed tests); ruff clean.
