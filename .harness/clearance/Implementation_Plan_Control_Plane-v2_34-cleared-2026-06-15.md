...
reviewer_chain:
  ...
  - out-of-family Codex review (`just codex-review`) — **1 [P2] APPLIED (decorrelation payoff):** the U-CP-94 `test_u_rt_95` un-skip AC was a false-green risk — the skipped test body (`:368-375`) is vacuous (`_ = patched_runtime, tmp_path`, no `execute_workflow` call, no assertions), so un-skipping ALONE proves nothing and must not flip the Path-(i) fork. AC strengthened to require authoring a real `execute_workflow` paused→resume e2e + a contrasting "fails-if-not-materialized" check before removing the skip / flipping the fork (`[[test-bypass-as-runtime-truth-pattern]]`). Propagated to companion §3 + this marker.
...
- **U-CP-94** — WAL_SEGMENT engine materialization (segment replay + per-segment dedup) + **Path-(i) `test_u_rt_95` materialization** (the skipped body `:368-375` is vacuous — un-skipping ALONE is a false green, Codex pre-merge catch — so the AC requires authoring a real `execute_workflow` paused→resume e2e before flipping the skip/fork). WAL_SEGMENT = canonical DURABLE_ASYNC class. Resumption-semantics half (Unit A).
