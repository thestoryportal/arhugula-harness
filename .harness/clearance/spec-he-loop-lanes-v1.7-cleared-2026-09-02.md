...
back_reference:
  ...
  - ".harness/spec/Spec_HE_Loop_Lanes_v1.md (v1.7 change-note: X7 — C-HE-13 §5 lane set = every non-terminal reservation, conforming to C-HE-03 §4; X7b — C-HE-13 §4 O3 rate reported as an interval with the masked-only pair count named)"
...
reviewer_chain:
  - "surfaced by the out-of-family codex reviewer on the U-HE-36 landing PR (r1 P2: a plan-side execution note does not amend the canonical contract; r3 P2: an interval measurement must be what §4 asks for, or §4 must say so) — not silently absorbed; the same PR's remaining codex rounds and its 3-lens merge gate re-review the landed wording"
...
X7b: §4's O3 base rate is measured on squash history, where a later change that
overlaps an intermediate commit's own hunk cannot be placed against the earlier
PR by any replay. The rate is therefore an interval over the 444 window pairs
with the masked-only pair count named, never a point estimate that silently
drops those pairs.
