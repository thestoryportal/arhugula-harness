...
v2.55 absorbs Runtime spec **v1.106 → v1.107** into **ONE NEW unit, U-RT-148**, carrying BOTH ratified surfaces — the §14.14.9 accessor (with its closed **four**-variant projection DTO spanning seven-plus source shapes, five-member cause attribution, and declared postures) and the §30 refusal-only staleness precondition — under the arc's conjunctive closure criterion. ZERO existing unit amended; ZERO new cluster. **TWO new cross-axis DAG edges** — U-RT-148 → U-CP-64 (concrete) and U-RT-148 → a PENDING, not-yet-numbered OD unit.
