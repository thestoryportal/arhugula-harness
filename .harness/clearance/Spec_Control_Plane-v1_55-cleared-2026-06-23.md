...
reviewer_chain:
  ...
  - out-of-family Codex pre-merge review (6 rounds → 2 generators found + closed) — rounds 1-2: per-disposition capture-site + ledger re-materialization patches; round 3 surfaced GENERATOR #1 (output-only store schema → every non-clean-success disposition invisible, at-most-once fail-open). Keystone (advisor): store records terminal DISPOSITION (completed-with-output → fold; completed-no-output → recover-as-terminal; timed_out → FAIL CLOSED) + changed-topology / changed-cardinality / unknown-disposition / fsync-all-ancestors guards. Rounds 5-6 surfaced GENERATOR #2 (recovery is not cascade-policy-aware — "reuse pause-resume verbatim" is correct only for PROCEED; PAUSE + CASCADE_CANCEL produce wrong/less-safe outcomes). The stopping rule TRIPPED (advisor): PR1 gates crash-resume to CascadePolicy.PROCEED + fails closed for PAUSE/CASCADE_CANCEL; cascade-policy-aware recovery is the registered back-flow follow-on (.harness/class_1_fork_fanout_crash_resume_cascade_policy.md)
