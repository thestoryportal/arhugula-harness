...
reviewer_chain:
  ...
  - just codex-review (2026-07-23, round 5, branch-vs-main against the round-4 fix commit) — out-of-family, found the round-4 fix had not propagated to the sibling Runtime spec's own restatement of the same property; this file itself required no round-5 change
  - just codex-review (2026-07-23, round 6, branch-vs-main against the round-5 fix commit) — out-of-family, found (a) a Runtime-plan-side atomicity drift (this file's own §1.4 fix never propagated to the Runtime plan's Files row) and (b) a genuinely NEW gap — operator-facing HITL escalation requests carry no run_id, making hitl_responses addressing impossible for peer branches sharing a child_workflow_id
  - Direct file read (2026-07-23, round 6) — read validator_framework_types.py and hitl_gate_composer.py to empirically confirm HITLEscalationBrief/compose_hitl_action_id carry no run_id before registering B-71
  - just codex-review (2026-07-23, round 7, branch-vs-main against the round-6 fix commit) — out-of-family, found property 4's safety clause too vague for a deterministic acceptance test, plus 2 P2 hygiene items (B-70 close-out repeating a rejected mechanism; B-39 FINAL STATE line omitting B-71)
---

# Clearance — Spec_Control_Plane_v1_106 (B-39 arc, spec leg; SEVEN same-day correction passes)
...
**Round-5 note (branch-vs-main `just codex-review` against the round-4 fix commit).** Found round 4's own gate-owning/container-branch correction had NOT been propagated to the sibling Runtime spec's own restatement of the same property (a drift, not a new design defect) — entirely Runtime-owned; see that marker's own round-5 section. This file required no further content change.

**Round-5 note.** Findings were entirely Runtime-owned (the round-4 gate-owning correction hadn't propagated to the sibling Runtime spec) or documentation hygiene — see `Spec_Harness_Runtime_v1.md`'s own round-5 correction. This file required no further content change.

**Round-6 correction (same-day, branch-vs-main `just codex-review` against the round-5 fix commit).** Two findings: (a) the Runtime plan's own U-RT-94 Files row still asserted `HarnessContext.resume_context_holder` removal as unconditional/standalone-owned — a drift, since this file's own §1.4 atomicity requirement was never propagated there; fixed at the Runtime plan (see that file's own change-note). (b) A genuinely NEW, separate finding, empirically confirmed by direct file read: `HITLEscalationBrief` (C-CP-28 §25.2) and the HITL webhook payload derive their identifiers from `compose_hitl_action_id(parent_action_id, ...)` — the SAME workflow_id-scoped, non-run-instance pattern this arc's own `branch_path` keying-defect note already found collides across repeated same-`child_workflow_id` dispatch. Neither carries a `run_id`, so two peer branches sharing a `child_workflow_id` produce indistinguishable escalation requests — an operator cannot know which `child_run_id` to key a `hitl_responses` response under. This is a genuinely SEPARATE carrier (`HITLEscalationBrief`/webhook payload, an EXISTING C-CP-28 contract this delta never touches) from `ResumeContext` — narrowed per the established B-69 precedent: §0 gains a new scope-limit note; registered as `B-71` (`.harness/forward-register.yaml`), not fixed in this spec leg.

**Round-7 correction (same-day, branch-vs-main `just codex-review` against the round-6 fix commit).** Found property 4's safety clause ("MUST NOT be silently applied... whenever doing so could misattribute") too vague to support a deterministic acceptance test — fixed with an exact rule (uniform fallback resolves a gate-owning branch only when it is the sole unaddressed member of the gate-owning set this cycle), staying at invariant-altitude (a property of the resolution OUTCOME, not a prescribed counting mechanism) precisely because property 5 now scopes "gate-owning" correctly, unlike round 3's unscoped draft. Also fixed 2 P2s: `B-70`'s close-out repeated the round-4-rejected count-based mechanism verbatim without the gate-owning scoping — corrected with a caveat that effect-fence's own container-branch question is unresolved and must be checked first; and the B-39 forward-register FINAL STATE line still said "Two follow-ons" after `B-71` was registered at round 6 — corrected to three.
