...
reviewer_chain:
  ...
  - "round-4 whole-contract resolution, recorded at §0.3-quater — two reviewer findings plus one self-found contradiction, closed together rather than per field, because corner-patching is what produced them. The cause rule is now ONE-WAY (the reverse cannot survive deserialization) with that direction re-homed to a producer-side witness; clause (b) was verified TOTAL at the producers rather than assumed; `CAPTURED ⇒ no cause` is left derived and asserted as such. NEW AC #10 requires the full legality table (14 rows) and a round trip for every legal shape."
  - "acceptance-criteria review — TEN ACs, each traced to an obligation rather than to a code shape. AC #1 carries the four-part determinism definition the spec's row-6 condition demands and pins its LEDGER placement as required-not-incidental; AC #3 is structural (a source-level single-direct-append assertion) because a behavioural sweep cannot cover a fourth append site that does not exist yet; AC #4 asserts the partition in FOUR directions because a one-sided assertion passes against a wholesale family reclassification; AC #6 PRESERVES the `B-84` W-5 `== 2` assertion verbatim so the leg cannot claim a closure it did not deliver; AC #7 enumerates every landed witness touched, with the store-error staircase-charge control explicitly STAYING with unchanged assertions."
