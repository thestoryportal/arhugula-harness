...
back_reference:
  ...
  - PR #513 (cascade step 1 — api.resume + C-RT-30, caller-supplied snapshot)
...
- **⚠ Pre-existing C-RT-30 collision (NOT fixed here).** The identifier `C-RT-30` is used twice — §14.19 (v1.35) `WorkflowManifestLoader` AND §30 (v1.45) `resume()`. v1.45 reused the number. This arc deliberately adds no new contract number (extends §30 + §14.14) to avoid compounding it; renumbering is a separate doc-integrity remediation (touches two cleared contracts + code cites + clearance markers). Surfaced as an R-CL-Q1 doc-hygiene candidate.
