...
| U-CP-55 | CP-axis cross-axis composition manifest + F2-12 carry-forward (`CrossAxisCompositionExport`/`CompositionSurface`, `SessionTarget`/`AxisName`/`SurfaceKind` enums, `CP_CROSS_AXIS_COMPOSITION_MANIFEST` 9-entry, `F2_12_CarryForward`/`RevisionStep` + `F2_12_CARRY_FORWARD`) — **CP axis-stream TERMINAL unit (L8)**; 5 OD-load-bearing + 4 Session-5-load-bearing exports; F2-12 6-step closure path declared; matches U-IS-17 / U-AS-33 terminal-exporter shape | C-CP-24 §24.2/§24.3/§24.4 | ✅ landed | `feat(cp): land U-CP-55` | 2026-05-16 |
| U-CP-54 | CP-axis namespace export manifest (`NamespaceExport`, `SourceAuthorityPosture`/`IngestionTarget` enums, `CP_NAMESPACE_EXPORT_MANIFEST` 11-entry, `CP_EXPORTED_ATTRIBUTE_COUNT`=63) — 6 §24.1.A specialization + 4 §24.1.B F3-lifecycle + 1 §24.1.C inheritance; `harness.breaker.*` SUBSTRATE_ANCHORED_OUTSIDE_CP per F2-16; descriptive manifest, no executable behavior | C-CP-24 §24.1.A/B/C | ✅ landed | `feat(cp): land U-CP-54` | 2026-05-16 |
| U-CP-52 | HITL timeout-degradation + webhook delivery semantics (`TimeoutDegradationKind` 3-enum, `TimeoutDegradationPolicy` + `TIMEOUT_DEGRADATION_TABLE` 3-entry, `WebhookConfig`/`WebhookPayload` 4-field records, `WebhookDeliveryEvent`/`WebhookDeliveryOutcome`, `on_hitl_timeout`/`deliver_webhook`) — v2.9-revised body; `HITLInvocation` opener-side record added at U-CP-17 carrier (v2.9 §2A/§0.4 — U-CP-17 landed pre-v2.9 without it) and consumed cross-cluster; v2.9 AC#6 `audit.policy.webhook_delivery_failed` plan-invented attr carried as the `EXHAUSTED_AFTER_RETRIES` outcome (Class 3) | C-CP-21 §21.6/§21.8 + C-CP-18 §18.5 | ✅ landed | `feat(cp): land U-CP-52` | 2026-05-16 |
| U-CP-51 | Operator-burden eval primitive + tail-keep rules (`OperatorBurdenEval` 4-field, `TailKeepRule` + `HITL_TAIL_KEEP_RULES` 3-entry, `compute_operator_burden`/`evaluate_tail_keep`) — `hitl.gate.evaluated` tail-keep re-anchored to the v2.4-conformed §20.6 `hitl.gate.required` (the v2.1 `audit.gate.computed_level` was dissolved at U-CP-46 v2.4); `hitl.policy.overridden` span is plan-invented vs §20.6 (Class 3, descriptive rule kept) | C-CP-21 §21.3 | ✅ landed | `feat(cp): land U-CP-51` | 2026-05-16 |
...
| U-CP-45 | 5-axis composition + operator-policy override + key-rotation two-row pattern (`FiveAxisCompositionInput`/`Result`, `OverrideKind`/`OverrideScope` enums, `OperatorPolicyOverride` + `OPERATOR_POLICY_OVERRIDE_SCOPE_TABLE`, `KeyRotationStage`/`RotationVerificationStep`, `KeyRotationPattern`/`StepResult`/`KeyRotationOutcome`, `compose_five_axis`/`apply_operator_policy_override`/`execute_key_rotation`/`verify_rotation_6_steps`) — gate-level + sandbox-tier orthogonal axes; CP `OverrideKind`/`OverrideScope` declared (AS `OverrideScopeResult` is a distinct concept) | C-CP-19 §19.3/§19.5 + C-CP-20 §20.3/§20.3.1 | ✅ landed | `feat(cp): land U-CP-45` | 2026-05-16 |
...
| U-CP-17 | Workload-binding engine-class selection (`WorkloadBindingSelectionInput`, `WorkloadBindingSelectionResult`, `select_engine_class`, `WorkloadBindingError`) — **PARTIAL**: §7.3 5-step procedure landed as 4 steps; step 4 (`operator_preferences`) + 1 test struck — `EngineClassPreferences` homed at blocked U-CP-27. **v2.9 carrier delta (2026-05-16):** `HITLInvocation` opener-side record (7 fields, C-CP-17 §17.1.1) added per v2.9 §2A/§0.4 — landed during U-CP-52 (U-CP-17 had landed pre-v2.9 without it) | C-CP-07 §7.3 + C-CP-17 §17.1.1 | ⚠️ partial-landed | `feat(cp): land U-CP-17`, `feat(cp): land U-CP-52` | 2026-05-16 |
...
| U-OD-02 | Per-cell backend class + candidate witness columns (`BackendClass` 7-value) — **HALTED**: single-valued `backend_class` signature cannot carry the cell-4/cell-5 disjunction acc #3 requires (`.harness/class_1_tension_u_od_02_cell_4_5_alternation.md`) | C-OD-02 §2.1-§2.3 | 🛑 halted (Class 1) | — | 2026-05-16 |
...
| U-OD-04 (v2.6) | Carrier-growth: `Span*` OTel-handle alias family (`SpanRef`/`ChildSpanRef`/`SpanAttributes` type-aliases of OTel-SDK span/attr handles, `EventEmission` 4-field frozen record) + acc #9 + 6 tests — additive to landed v2.5 U-OD-04 | C-OD-04 §4 + ADR-F5 + ADR-D6 v1.2 | ✅ applied | `feat(od): apply U-OD-04 v2.6 carrier-growth` | 2026-05-16 |
| U-OD-13 | Per-cell cardinality budget (`PerCellCardinalityBudget` 4-field, `PER_CELL_CARDINALITY_BUDGET` 8-entry one-per-ACTIVE-cell, `EnforcementLayer` 2-value, `PATTERN_P1_DISCIPLINE_ANCHOR` §11.4 verbatim) — stale U-OD-11 edge struck per resolution record; effective deps [U-OD-01, U-OD-05] | C-OD-11 §11.1+§11.4 | ✅ landed | `feat(od): land U-OD-13` | 2026-05-16 |
| U-OD-19 | Sandbox-tier overhead + fan-out rollup (`SandboxOverhead`/`SpanTotalCost` records, `compose_span_total_cost` additive §14.2, `FanOutPattern` 2-value, `FanOutRollupResult`, `rollup_fanout_at_close` — consumes `SpanRef` from U-OD-04) | C-OD-14 §14.2+§14.3 | ✅ landed | `feat(od): land U-OD-19` | 2026-05-16 |
| U-OD-23 | Five operator-burden eval primitives (`OperatorBurdenEvalPrimitive` 5-value, `ComputationKind` 3-value, `EVAL_PRIMITIVE_DECLARATIONS` 5-entry §17.1, `EvalEmissionContract`/`EVAL_EMISSION_CONTRACT` §17.2 child-span commitment, `emit_eval_as_child_span` → `ChildSpanRef`, `reject_span_event_only_emission`) | C-OD-17 §17.1+§17.2 | ✅ landed | `feat(od): land U-OD-23` | 2026-05-16 |
| U-OD-14 | Cardinality-safe + cardinality-prohibited attribute classes (`CARDINALITY_SAFE_ATTRIBUTES` 13-member §11.2 frozenset, `CARDINALITY_PROHIBITED_ATTRIBUTES` 6-member §11.3 frozenset, disjoint; `assert_cardinality_safe_for_dashboard_dimension` / `assert_cardinality_prohibited_not_in_dashboard_dimension` gates, `CardinalityViolation`) | C-OD-11 §11.2+§11.3 | ✅ landed | `feat(od): land U-OD-14` | 2026-05-16 |
| U-OD-08 | F3 lifecycle event-to-span-event mapping | C-OD-06 §6.1-§6.3 | 🛑 HALTED Class 1 | — `.harness/class_1_tension_u_od_08_f3_lifecycle_event_set_divergence.md` | 2026-05-16 |
| U-OD-09 | `harness.breaker.*` 7-attribute canonical schema | C-OD-07 §7.1-§7.3 | 🛑 HALTED Class 1 | — `.harness/class_1_tension_u_od_09_tier_classification_design_gap.md` | 2026-05-16 |
| U-OD-20 | Idempotency-key join + replay-aware dedup + per-attempt cost (`SpanCostRecord` 9-field §14.4/§14.5.2-3, `dedupe_on_replay`/`DedupOutcome` 5-value §14.5.1, `cause_attribution_invariance_check`/`InvarianceCheckResult` + `ReplaySemanticDivergenceError` §14.5.3, `per_attempt_cost_attribution_roll_up`/`ParentOperationTotalCost` §14.5.4, F2-12 ✅ CLOSED notation — `F2_12_NOTATION`/`F2_12_CLOSURE_PATH` 9-entry; consumes CP `ReplayDisposition` read-only, `SpanRef` from U-OD-04; `F2StateLedgerEntry` materialized OD-local read-only — 7c seam) | C-OD-14 §14.4+§14.5.1-4 | ✅ landed | `feat(od): land U-OD-20` | 2026-05-16 |
| U-OD-26 | Eval-vs-runtime-gate distinction (`EvalKindDiscriminator` 2-value §18.3, `EVAL_KIND_ATTRIBUTE_NAME` byte-exact, `EVAL_SPAN_SHAPES` 2-entry, `SamplingPostureF18`, `classify_eval_span` → `Option`, `validate_eval_span_routing` non-mergeable shape gate, `EvalShapeViolation`; consumes `SpanAttributes`/`SpanRef` from U-OD-04) | C-OD-18 §18.3 | ✅ landed | `feat(od): land U-OD-26` | 2026-05-16 |
| U-OD-11 | Per-deployment-surface sampling mode + 18-entry always-sampled set (`SamplingMode` 2-value, `SamplingDecision` 2-value in-unit, `PerDeploymentSurfaceSamplingMode`, `PER_DEPLOYMENT_SURFACE_SAMPLING` 3-entry, `ALWAYS_SAMPLED_EVENT_CLASSES` 18-member §9.2 frozenset, `sampling_decision`) — U-OD-09 dep is event-class-string-only, not blocked by U-OD-09 halt | C-OD-09 §9.1-§9.3 | ✅ landed | `feat(od): land U-OD-11` | 2026-05-16 |
| U-OD-21 | Cross-family `provider_discriminator` rollup + tokenization-version anchor — **HALTED Class 1**: `SpanCostRecord` (U-OD-20 carrier) lacks the provider / model / family-tag fields acc #3's rollup keys require (`.harness/class_1_tension_u_od_21_span_cost_record_missing_rollup_keys.md`) | C-OD-15 §15.1-§15.3 | 🛑 halted (Class 1) | — | 2026-05-16 |
| U-OD-12 | 13-entry base-rate-sampled set + per-cell tuning envelope — **HALTED Class 1**: acc #2 set-disjointness (`BASE_RATE ∩ ALWAYS == ∅`) contradicted by the `files.operation` / `memory.operation` bare-string collision across both sets (`.harness/class_1_tension_u_od_12_disjoint_set_string_collision.md`) | C-OD-10 §10.1-§10.3 | 🛑 halted (Class 1) | — | 2026-05-16 |
| U-OD-17 | Cross-deployment monotonic-tightening invariant (`REDACTION_CLASS_ORDER` 3-tuple, `class_index`, `assert_monotonic_tightening_across_transition`, `reject_class_downgrade` alias, `MonotonicityViolation`; D2/D5 cross-deployment-monotonicity composition anchors — cross-axis edges resolve at 7c) | C-OD-13 §13.3 | ✅ landed | `feat(od): land U-OD-17` | 2026-05-16 |
| U-OD-27 | Local-first OTLP collector at cell-1 (`CELL_1`, `CollectorTopology` 3-value, `InProcessCollectorBinding`, `RingBufferTraceStoragePolicy`, `RingBufferStorageState`, `SpanRow`/`EvictionAction` in-unit M-1, `TuiTraceBrowserSurface`/`TuiQuery`, `bind_in_process_collector`, `evict_oldest_per_ring_buffer_policy`, `query_ring_buffer_via_tui`, `SCOPED_QUERY_CATEGORIES`) — **library surface only**; live collector/TUI/sqlite Class 3 for Phase 2; acc #8 U-OD-21+U-OD-25 query contributions documented integration-time | C-OD-19 §19.1-§19.3 | ✅ landed | `feat(od): land U-OD-27` | 2026-05-16 |
