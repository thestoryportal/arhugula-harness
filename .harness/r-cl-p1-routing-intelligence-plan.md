...
| **`LLM_AS_ROUTER` decision-fn** (C-CP-02 §2.1 Layer 3) | **DEFER — documented; CONFIRM-DEFERRED at R-CC-1 arc #6 (operator Option C, 2026-06-11)** | ⚠️ **Stale-premise correction (re-grounded at HEAD `643f4a8`, R-CC-1 arc #6):** this row originally claimed `infer()` (U-CP-03) is **sync** — that is **false at HEAD**: `infer()` is `async def` (`routing_core_surface.py:129`; R-300 activation made it async). Only `route()` + `LayerDecisionFn` (U-CP-05) are sync. So a faithful Layer 3 does **not** require widening `infer()`; the contract-shape decision is narrower (Reading A widen `route()` to async, vs Reading B resolve at the already-async `infer()` layer). Router-model binding is undecided impl-discretion (§2.4 "Haiku-class typical"). **Disposition:** confirmed bounded-residual (Layer-3 twin of EMBEDDING above) — today only `_declarative_echo` is bound, Layer 3 cleanly falls through, no behavior missing. Full grounding + readings + resolution at `.harness/class_2_fork_llm_as_router_layer3_contract_shape_vs_defer.md`. Re-open trigger: real routing-intelligence traffic + an operator router-model + hot-path-cost decision → build via **Reading B**. |
