...
*Filed 2026-05-29 post-checkpoint item 6 audit-stub remediation arc opening. **Status: ✅ SUPERSEDED-BY-V1.28-READING-D 2026-05-31 at PR #87** (spec v1.28 + 3-site timestamp universal fix). PR #87 absorbed the consumer-chain framing via per-tier-conditional disposition at v1.28 §16.5.6.X: (1) `timestamp` field universal fix at all 3 composer sites (`per_step_override_evaluator.py` + `sub_agent_gate_level_descent.py` + `hitl_gate_composer.py`) closes the universal bug; (2) `prior_event_hash="0"*64` sentinel REFRAMED as solo-developer-tier canonical end-state per ADR-D5 §1.4 row 1 ("no hash chain required by default") — NOT a bug at v1.6 MVP default; (3) team-binding+ tier wiring (chain state holder + signing wiring per C-CP-20 §20.4) DEFERRED as operator-deployment-time opt-in arc (bounded-residual per X-AL-2; mirrors AS-8d/OD-5/OD-6 precedent). PR #87's per-tier reading collapses this fork's "consumer-chain absence" framing into the spec-canonical ADR-D5 §1.4 tier-conditional reading. v1.28 §1 + Change-note §"Adjacent observations (b)" catalogue the multi-site-pattern-not-surfaced-at-original-deferral-arc sub-species candidate. Status refresh at workflow §7.4.7.3.B audit 2026-05-31 (sub-species 3 `resolved-but-carry-stale-inherited`).*

*Original framing (preserved for lineage):* Closes PR #66 Q2=iii "audit-half stub IN-SCOPE-BUT-MARK-DEFERRED" framing as empirically wrong shape: bounded-scope stub remediation forecloses on substrate-consumer-asymmetry that requires Reading D bounded-defer per sub-species 7d catalogue at `.harness/retirement-event-pattern-catalogue-batch-45-addendum.md` §2.1.
