...
**At-most-once *per (run, step) invocation*, NOT exactly-once.** The fence claims the harness's per-invocation `idempotency_key`. **A legitimate loop iteration is NOT conflated with a resume re-fire** (advisor pre-merge catch, verified against HEAD): a looping topology already assigns a distinct `idempotency_key` per invocation — EVALUATOR_OPTIMIZER passes the **monotonic `entry_index`** as `StepExecutionContext.parent_idempotency_key` (`workflow_driver.py:3620-3628`), fan-out uses the branch-scoped `compose_branch_path` key — because it MUST (else the IS `idempotency_key`-only dedup at C-IS-07 §7.5 would drop iteration-2's ledger entry). So a fresh loop iteration / branch gets a NEW key → fires; only a *resume* re-dispatch reuses the key → fenced. The fire-then-crash-before-commit window is ambiguous (did the effect fire?), so the conservative fail-close (a reserved-but-unfired effect also fail-closes its re-dispatch) is the honest answer — exactly the reconciler's §22.1 posture. **The v1.60 interim is a FAILED `RunResult`** (honest, never a silent double-fire); the genuine §22.1 operator-resolvable PAUSE is the `B-EFFECT-FENCE-HITL-ROUTE` follow-on. Single-host (cross-host folds into the deferred F-CC item).
