...
> **⚠ CORRECTION (2026-06-15, R-FS-1 arc #25): the per-slice "FORK-owed" classification below is SUPERSEDED IN PART.** A systems-architect re-grounding (`.harness/architect_recommendation_e_engine_fork_vs_impl.md` — dedicated systems-architect agent; out-of-family **Codex concurred on the substance** in this PR's review, flagging only a status-wording nit since fixed; advisor overloaded across 5 attempts) found, via the **U-CP-56 precedent** (save-point-checkpoint materialized into `_IN_SCOPE_ENGINE_CLASSES` as **impl against cleared spec, "no spec bump required"**; the one fork it filed was a plan-level substrate-primitive gap) + the **byte-exact C-CP-07 §7.4 deferral clause** (`Spec_Control_Plane_v1_2.md:704`), that:
> - **E-1 EVENT_SOURCED_REPLAY + E-2 WAL_SEGMENT are impl-against-cleared-spec (NO fork).** §7.4 *explicitly defers their substrate to implementation discretion* (Temporal/DBOS/Restate at event-sourced; "specific WAL implementation at WAL-segment"); I-6 (committed) selects hand-rolled within that open slot; §7.1/§8.1 already specify their lifecycle + `engine_replay`/`segment_replay` resumption semantics. The `engine_class.py` docstrings self-classify the same split ("candidate/implementation deferred per §7.4"). → **E-1/E-2 go straight to E-plan → E-impl; no spec leg, no clearance marker owed.**
> - **Only E-3 RECONCILER_LOOP carries a genuine (narrow) fork:** reconciler-loop is the **ONE class §7.4 omits** from the impl-discretion clause — its substrate is named concretely + non-deferred ("K8s controller / etcd + CRD events", §7.1 row 4), conflicting with I-6's "no vendored K8s; hand-roll etcd-style." → a **single C-CP-07 §7.4 substrate-deferral reconciliation sentence** (mirror the ESR/WAL wording) + clearance marker, authored at the E-3 spec-leg (operator-ratified per the architect rec §7, since it touches a P5-CK cleared contract). The live K8s e2e is a separate deployment-surface gate, not part of the fork.
>
> **Net: 3 planned spec legs (E-spec-1/2/3) → 1 narrow §7.4 reconciliation (E-3 only).** The substantive design below — engine semantics, hand-roll per I-6, slice sequence, foreclosed fake-producer anti-patterns, R-CXA-2 seam activation, the built-but-unwired recovery-loop producer gap — **SURVIVES unchanged**; only the fork-vs-impl labeling in §0/§3/§5/§7 is corrected. **Read `.harness/architect_recommendation_e_engine_fork_vs_impl.md` as authoritative on fork-vs-impl + the corrected next-action (E-plan, not E-spec-1).**
