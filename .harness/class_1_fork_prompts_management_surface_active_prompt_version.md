...
**Cascade (the owed PR sequence):** #1 runtime injection (load-bearing, standalone-valuable, e2e-proven) → #2 IS versioning/authoring → #3 CP selection → #4 OD per-tier governance → #5 CXA seam registration. Each = bundled-absorption arc (spec amendment + impl + tests + clearance) + adversarial + Codex + advisor review.
