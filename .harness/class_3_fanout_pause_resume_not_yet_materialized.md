...
> **✅ CLOSED for `ORCHESTRATOR_WORKERS` (2026-06-19) by the `B-FANOUT-PAUSE` build arc.** The `_execute_orchestrator_workers` `pause` branch now returns genuine resumable `RunStatus.PAUSED` (with a bound `pause_resume_protocol`) carrying a `FanOutResumeState`-bearing `PauseSnapshot`; `api.resume` re-enters the strategy, skips terminal branches (recovering their outputs), and re-dispatches the rest (§25.15.2 obl. 7). Bundled-absorption: CP spec v1.42 (C-CP-26 §26.2 carriers + `PauseSnapshot.fan_out_resume`) + `.harness/class_2_fork_b_fanout_pause_resumable_fan_out.md`. The deviation below stands as the HISTORICAL interim record (the FAILED-honestly-without-protocol path is preserved as the detect-then-refuse `orchestrator-workers-pause-resume-protocol-not-bound`). **The four other non-linear strategies' fan-out/handoff pause-resume remain interim FAILED** (registered forward arcs `B-FANOUT-PAUSE-EVALUATOR-OPTIMIZER` / `-PARALLELIZATION` / `B-HANDOFF-PAUSE` / `B-HIERARCHICAL-PAUSE`) — this deviation is NOT yet closed for them.
