...
|---|---|---|---|---|---|
| `AttributeValueType` / `Cardinality` | CP U-CP-07/11/21/31/37/46/47 (consume); **AS U-AS-31 (consume — C-AS-14)** | **harness-core** — re-homed 2026-05-16 per the U-AS-31 Class 1 fork (`.harness/class_1_tension_u_as_31_attribute_schema_enums.md`); `harness-cp` re-exports for CP-citation stability | CP + AS consumers `Depends on: [core carrier]` | Plan-introduced auxiliary enums. **Cross-axis: CP + AS** (the prior "all consumers in-axis → CP" determination + operator decision D3 missed U-AS-31, a C-AS-14 consumer). An AS→CP package edge cycles against the 24 CP→AS edges, so the enums belong on `harness-core`. | Decided (re-homed) |
