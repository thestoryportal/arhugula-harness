...
| H_T-CP-20 | HITL primitive + 4-response palette + hitl.* / audit.* | STILL-BOUNDED | `RuntimeHITLPlacementRegistry` materialized as pure decision surfaces only (`hitl_placement.py:23-33`); NotImplementedError CP stubs explicit. Driver never invokes HITL gate |
