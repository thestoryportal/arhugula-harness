...
**v1.111** §14.8.11.1 + Runtime plan **v2.58** U-RT-150). FURTHER SUPERSEDED 2026-08-06 by §11.9 — the IMPL LEG HAS LANDED at PR
#1237 (U-RT-150; merged 2026-08-06 at `39389d09`, post-merge main CI success). The chain is COMPLETE at leg 4 of 4;
NO leg remains owed.** *(The sentence this replaces read "the spec leg and the impl leg are both still OWED; this
filing's chain is at leg 3 of 4" — true when written, false once §11.8 landed. Corrected in place at
out-of-family review round 3 [P2] rather than left for a top-down reader to trip over. The
sentence THAT correction installed — "Only the IMPL leg remains OWED; the chain is at leg 4 of
4" — was itself true when written and false once §11.9 landed, and is corrected in place here
under the same discipline, at the impl leg's own merge-gate round.)***
...
| Carrier | **(C-i)** — a dedicated **candidate-filename + timestamp** sidecar, whose content is council condition #5's **closed two-member set `{candidate filename, first_observed_at}`, keyed on the CANDIDATE FILENAME over BOTH sweep classes — `*.entry` AND `.tmp-*`.** *(Stated as candidate-filename, never "digest": a digest-only key leaves condition #1's second conjunct permanently unsatisfiable for `.tmp-*` crash orphans, forcing first-sight reclaim or unbounded retention. The permission ground is **independence-or-non-invertibility**, not "already a sha256" — that ground is true of `*.entry` and false of `mkstemp`-drawn `.tmp-*` names. Corrected at out-of-family review round 3 [P2].)* Carriers **(C-ii)** and **(C-iii)** are **not** owed by any leg. |
...
| Retention | **CONDITIONAL, never an unconditional bound** (council condition #7). Typical worst case **2×TTL** plus **up to two** sweep-trigger intervals (one to the first post-TTL observation, one more to the post-grace reclaim); the trigger interval is **unbounded in the one-shot process shape** and is **pre-existing at HEAD**, not introduced by this grace; and the figure is conditional on the observation record being **present and readable** — a condition contemporaneously emitted per condition #6 and readable on demand per condition #8(b), the pull surface being authoritative. **Observability is NOT a term in the bound itself.** **No unconditional "bounded by N × TTL" claim is available under either form.** *(Corrected at out-of-family review round 3 [P2] — an earlier draft stated `2×TTL + 2I` flatly, which condition #7 forbids.)* |
...
### §11.9 Impl leg — LANDED 2026-08-06 at PR #1237 (chain cross-stamp)

**Chain status: COMPLETE, leg 4 of 4 landed — filing (#1179) → ratification (#1233, incl. the #1183 convening) → spec leg (#1235) → IMPL LEG (#1237, merged 2026-08-06 at `39389d09`; post-merge main CI success).** The stamp was drafted at the impl leg's merge-gate round (mirroring how §11.8 stamped the spec leg from inside its own PR) and landed as this doc-only follow-on after the merge, because carrying it on the code PR would trip the DESIGN_IMPL_MIX guard — the split the guard's fork-filings-ship-doc-only rule encodes.

**What landed.** `U-RT-150`, against Runtime spec **v1.111 §14.8.11.1** and Runtime plan **v2.58 §1**, at `harness-runtime/src/harness_runtime/lifecycle/protected_result_store.py` plus the Runtime-owned `harness-inspect` admin CLI. The sweep-COUNT grace this filing exists to replace is gone: reclaim is now **CONJUNCTIVE** — past-TTL filesystem age **AND** past-TTL elapsed time since a **durably recorded** first observation — with **no third path** and **no absolute ceiling** (form **C-2** as ratified). The carrier is **(C-i)** exactly as §11.3 tables it: a dedicated `gc-observations.json` in the store root holding council condition #5's **closed two-member set**, keyed on the **candidate filename over BOTH sweep classes**, published by **temp-write + `fsync` + `os.replace` + directory `fsync`** — never the write-once `O_EXCL`/`os.link` primitive, never unlink-then-recreate — with record-absence reading as **no observation**. `first_observed_at` is **wall-clock, sampled at the locked post-re-verification observation point**. All **FIFTEEN** acceptance criteria are green with PD-8 mutation probes (18 killed / 2 inert-by-construction), including AC #13's re-grounding pass over every inverted grace-dependent witness with **no witness weakened or deleted**, and AC #10 / #14(i) driven as **real successive one-shot process invocations** against one store root.

**§8's impl-leg row is discharged, and the §8 dispositions land as tabled.** `B-96` closes; the **`B-77` residual** closes; **`B-74`** closes — and closes in the shape its own `close_out` asks for, with `test_coarse_filesystem_mtime_granularity_does_not_lose_a_live_entry_on_sight` **re-pinned as a POSITIVE witness that the live entry SURVIVES BOTH SWEEPS**, kept rather than deleted. Reading **B** stays retired: AC #10's cross-process witness is what makes per-process elapsed time unbuildable in fact, not merely forbidden in text.

**What this leg did NOT do, per §8's not-owed-by-any-leg list.** No `ttl_seconds` floor or narrowing; no numeric `k` or absolute reclaim ceiling; no fourth reordering of `_publish_atomic`'s two-stamp pipeline; no carrier **(C-ii)** or **(C-iii)**; no new `C-RT-*` number; no `design-substrate/**` edit; no CXA delta (aggregate stays frozen at 111) and no OD delta (`C-OD-05` §5.1 roster 15 → 15). Registered row **`B-110`** disposition **(c)** was **NOT** taken — optional, not owed by any `B-96` leg, and inconsistent with AC #5's witness, which pins the timestamp-derived bound that survives *without* lock-bound identity revalidation.

**Two residuals were REPORTED rather than absorbed at this leg.** **`B-111`** is NEW: the record's own publication temporary is unreclaimable after a kill strictly between `mkstemp` and `os.replace`, because both sweep globs exclude it by construction — a correct finding whose natural fix **AC #15 as written forecloses**, so it is registered with two grounded dispositions instead of being built here. And a family of value-plausibility narrowings raised across three out-of-family rounds is **declined with pins in the code**, on the ground that they act in the retention-**LENGTHENING** direction term 4 explicitly permits, while the two directions that matter — retention-**SHORTENING** and an exception **ESCAPING** term 11's totality — are closed.

**Register disposition at this leg:** `B-96` moves **`open` → `closed`**; `B-74` moves **`registered_finding` → `closed`**; `B-77`'s status was already `closed` and does not move, its **residual** recorded closed in prose; **`B-111`** is added as `registered_finding`. Snapshot re-derived in the same commit (total 110 → 111, open 3 → 2, registered_finding 15 → 15, closed 90 → 92; digest `1fe23cdcb7637014` → `744dbb3f1e034018`), with `tools/forward_register.py --check` and `tools/arc_ledger.py --check` both green.
