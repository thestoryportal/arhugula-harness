...
| Condition B verification | `harness-runtime/src/harness_runtime/lifecycle/sub_agent_dispatch.py:RuntimeSubAgentDispatcher.dispatch` (composer body steps 3-4) calls `topology = self.topology_dispatcher.dispatch(payload.child_manifest_entry)` → returns `TopologyPattern` per C-CP-10 §10.1 → calls `self.topology_dispatcher.is_admissible(topology, payload.child_manifest_entry.workload_class)` advisorially (post-`ff25165` Class 1 fork resolution: strict gate dropped at v1.6 MVP per predicate-semantic-mismatch; composer proceeds regardless of admissibility result). Substitution mechanism (`CLAUDE.md`-prose annotation: "you are working as orchestrator-workers" convention) no longer reachable from runtime composer — runtime owns the dispatch + advisory admissibility predicate at production execution path. **Retirement criterion narrowed:** condition B met for "dispatcher operational + predicate callable"; strict-gate retirement criterion owed per `[[class_1_tension_u_rt_59_topology_admissibility_predicate]]`. Test coverage: `test_lifecycle_sub_agent_dispatch.py::test_topology_advisory_admissibility_does_not_gate_at_v1_6_mvp` (verifies no-raise + span emission + child runner invoked for SINGLE_THREADED_LINEAR + SOFTWARE_ENGINEERING common case) |
