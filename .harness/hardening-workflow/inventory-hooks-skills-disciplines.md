...
- `tools/hooks/lib.sh` — anti-drift keystone. `hook_project_dir()` resolves `$CLAUDE_PROJECT_DIR` or the Git toplevel; `hook_emit()` owns additionalContext JSON; `hook_bounded()` is the portable watchdog; `hook_state_hash()` owns the §12.1 recipe; `hook_roadmap_next()` resolves the live pointer; `hook_write_checkpoint()` owns bounded atomic snapshots; `loop_mode_active()` gates autonomy on `HARNESS_LOOP=1` or `.harness/.loop-active`. Symbol names are the stable anchors because this live library moves as parity hardening lands.
- `tools/hooks/loop_lib.sh` — ledger substrate (U-HK-11). `loop_log/loop_defer/loop_activate/loop_deactivate`, `loop_skip_set()` (run-scoped DEFERRED-HIL item IDs), `loop_pending_hil_summary()`, and `loop_gc_worktrees()` (report candidates at SessionStart; reap only from an explicit post-merge/closeout or loop-start call).
...
| U-HK-12 | `permission-guard.sh` | PreToolUse(*), PermissionRequest(*) | **ENFORCING** | Tri-state: INERT (unless loop)→DENY-LIST→ALLOWLIST→ask. The locked deny-list covers paid LLM routes, recursive/forced removal, force-push/history rewrite, branch/ref deletion, secret or credential-store mutation, paid provider network calls, and credential-gated `just` recipes. The allowlist admits structurally validated safe Git/GitHub/uv/pytest commands; `_safe_path()` and `_bash_args_safe()` own path/argument validation. **Loop-control wrappers (`tools/04-loop/defer.sh` and `tools/04-loop/halt.sh`) short-circuit to ALLOW.** |
...
## B. LOOP RUNNER (`tools/04-loop/`)
- `run.sh` (U-HK-15) — headless overnight runner. `loop_activate` → per-iteration skip-set computation/injection → `claude -p` with `HARNESS_LOOP=1` and `HARNESS_LOOP_MAX`. It checks the halt marker every iteration, clears loop mode on every exit path, and exits only at max iterations or `.loop-halt`.
- `defer.sh` — allowlisted wrapper; `loop_defer <id> <reason>` writes a DEFERRED-HIL row and requires an id plus non-empty reason. The loop advances afterward; defer is not halt.
- `halt.sh` — allowlisted wrapper; raises `.harness/.loop-halt` plus a STOP row. It is called only at true exhaustion, when every forward item is deferred.
