...
| **U-HK-42** | postedit-lint | YAML parse branch (`*.yaml`/`*.yml`, `.venv`-python `safe_load_all`, advisory) | `test_postedit_lint.sh` (cases 7–11) | med | low | — |
| **U-HK-43** | subagent-validate | subagent registry JSONL append (start/stop, atomic single-line) | `test_subagent_validate.sh` (+5) | med | low | — |
| **U-HK-44** | loop-gc | unreconciled-subagent sweep + 7d prune in `[hygiene]` report | `test_loop_gc.sh` (+3) | med | low | U-HK-43 |
