...
| Lease transition marker | `QUEUE_DIR/merge-door/transition.<lease_token>` (one per token, ever; payload `{pid, host, target_action ∈ {release, reclaim, unblock}, created_at}`) | sole carrier (new fact) | "a transition of this token is in flight, by whom, toward what" (C-HE-06 §6) — the marker winner is the only party allowed to move `LEASE`, and a third party completes a dead creator's declared `target_action` from this payload alone; not reconstructible from `LEASE` once it has moved |
| Lease history + per-token side-markers | `QUEUE_DIR/merge-door/released.<token>`, `reclaimed.<token>`, `LEASE.<token>.<suffix>` for suffix ∈ {`attempted`, `blocked`, `refresh`} | derived | the lease (the renamed-aside `LEASE` payload itself = audit history of its transitions; GC 30 d) |
