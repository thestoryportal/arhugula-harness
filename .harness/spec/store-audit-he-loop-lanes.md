...
| `.harness/merge-gate-log.md` + structured sibling `.harness/merge-gate-log.jsonl` | `REPO` | gate verdicts and every finding-class row (human + machine views of ONE fact; the sibling is written FIRST by the same producer — `merge_gate_log.emit` via `rw.emit_outcome` — in the same step under one `flock`; a projection, not a second authority) |
...
- "What was the gate verdict on #N@head?" — the JSONL row (written first) and the md line are one emission; `just merge-gate-log-check` reduces the md against the JSONL (ts,count,round) multiset — a disagreement is a defect in the producer, never a second authority to prefer.
