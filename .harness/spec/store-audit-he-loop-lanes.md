...
The C-HE-30 clearance-fold note adds these families under the invariant "none a new authority for an **existing** fact". Audited row by row, they are of three kinds — a **derived** copy/history of a store above (recomputable from it), a **part of** one of the eight stores (a sidecar file that carries some of that store's own fields), or the **sole carrier of a NEW coordination fact** this spec introduces (exclusive-create registries, counters, allocators, live mode state — no other store holds the fact, so the file *is* its one authority). All three satisfy the invariant; the third is recorded honestly as an authority rather than mislabelled "derived". The eight-store count is the authority set for the facts HE-1 §5 enumerates; new-fact carriers are enumerated here (C-HE-30 Invariant 1) and do not join the eight.

| Family | Path | Relation | Derived from / sole carrier of |
|---|---|---|---|
| Reservation generations | `QUEUE_DIR/reservations/<arc_id>/<gen>.json`, `.<gen>.<pid>.tmp` | derived | the reservation store (history by construction; head = highest gen; GC prunes below head) |
