...
- **X9e — C-HE-06 §4 (viii), the Invariants, the Verification continuation unit, §0.4 and council row G4: the door no longer drives a terminating refresh.** Step (viii) becomes: `lit done <arc_id>` then `lit sync push --remote origin`. A non-zero exit there emits a `NOTIFY` row and never blocks the lease, because the merge is irreversible and a block would lock the single global door. **Durability of the lit step.** A failed `lit done` or sync push never lets a merged arc be selected again: the reservation already reads `merged` at step (vi), `arc_id` is the lit id, and `reservations.py selectable` refuses any existing head, terminal ones included (C-HE-03 §4). The lit ticket's state is repaired durably, not by hope: the session-start reconcile pass (C-HE-03 §5, `reconcile-all`) runs `lit done` for every `merged` reservation whose lit ticket is still open, then `lit sync push --remote origin`, and emits a `NOTIFY` for each repair — so a clone that missed the push converges at the next session start. `REFRESH_BOUND_S` retires, and the worst-case legitimate hold drops from ≈ 92 to ≈ 47 minutes. The invariant "never released while … the terminating refresh is UNCONFIRMED" keeps only its merge-run clause; the X8 exception covers that clause alone. The continuation verification unit and its re-acquire mutation are void; the replacement lit-step unit observes both commands, their order under the held lease, and NOTIFY-not-block on failure. C-HE-06 §10's clean cycle swaps "refresh green" for "`lit done` recorded".
- **X9f — C-HE-01 §3 and its Invariants, C-HE-03 §4, AC#4: `lit` is the state of record for forward work.** Open roadmap items, open forward-register rows, open arcs and the next action live in `lit` (promptctl/links-issue-tracker). Its Dolt store sits in the clone's git common dir, so every worktree of the clone reads one database, and it syncs through `refs/dolt/data` on `origin`. Selection is `lit next` then `lit start <id>`; the reservation is still minted at selection (`(none)→pending`, C-HE-03 §4) with `arc_id` = the lit id, and it keeps the whole merge lifecycle. A lit claim held fresh by another checkout, or a non-terminal reservation, makes a second lane's selection fail. The §12.2.1 terminating-refresh fixed point and the one-file refresh shape retire with `.harness/roadmap_status.md`, which moves to `.harness/archive/roadmap-plainfile/` as frozen history.
