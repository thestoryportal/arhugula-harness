...
| **merge door** | The single-writer landing path: acquire lease → verify base → `gh pr merge` → confirm → release → terminating refresh (`CLAUDE.md` §12.2.1). Depth 1 by construction. | *(v1.9 X9e: the terminating refresh is gone — the door ends `lit done` → release.)*
...
| 6 | Refresh collision | `ROADMAP_STATUS_DRIFT` **[V]** | lane-attributed (C-HE-24 §6) | *(v1.9 X9h: retired — no refresh artifact exists)*
