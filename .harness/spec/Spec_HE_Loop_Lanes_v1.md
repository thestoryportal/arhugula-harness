...
- **X8 — C-HE-06 §6 gains an operator-confirmed release, and the Invariants bullet distinguishes UNCONFIRMED from TERMINAL.** "Unconfirmed" in that bullet means *not yet observed*, which is the state the fence exists for: an outcome still in flight must never be released past. A run that HAS been observed and reached a terminal non-success is confirmed — badly, but confirmed — and the contract previously had no sentence for it, which is what left the door with no exit. The carve-out is gated on the block's REASON, not merely on the fact of a block. Ten distinct reasons reach `mark_blocked`, and only `post_merge_ci_not_green` means the merge SHA's own run was observed and is terminally not green; `base_toctou_first_parent_mismatch`, `containment_refusal:*`, `door_failed_after_attempt:*`, `unreconcilable_at_resume:*`, `refresh_skipped_without_optin` and `refresh_intent_unresolved` all block WITHOUT ever observing that run, so an unblock alone attests nothing about it. Because `unblock` nulls `blocked_reason` on the successor it mints, the reason is carried forward as `unblocked_reason` beside the `unblocked_from` sha it already carries, and that field is the gate. A landing that merely crashed mid-flight carries no block at all and is refused a step earlier. Both of a landing's outstanding effects stay fenced independently: the content merge must be settled, and a MINTED terminating refresh (either sidecar) refuses outright, since step (viii) is mandatory and nothing local proves its outcome.
...
- The lease is never released while the merge SHA's own `main` run or the terminating refresh is UNCONFIRMED — not yet observed. **Exception (X8), operator-confirmed:** a run that has been observed and reached a terminal non-success is confirmed, not unconfirmed, and its lease MAY be released through §6's operator-confirmed release. The gate is the successor's `unblocked_reason` — the block reason `unblock` carries forward — and it admits ONLY `post_merge_ci_not_green`, the one reason of ten that means that run was observed. Every other block reason leaves it unobserved and stays fenced, and a landing that merely crashed mid-flight carries no block at all.
