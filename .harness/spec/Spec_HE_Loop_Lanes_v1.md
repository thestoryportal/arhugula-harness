...
- **X7 — C-HE-13 §5 lane set = every non-terminal reservation.** "every other lane's current head" is the `branch` of every other lane's `pending` or `open` reservation (the C-HE-03 §2 non-terminal states); `merged`/`abandoned` heads are not lanes. A record whose `state` is outside the C-HE-03 §2 domain is unreadable, not terminal: the gate refuses to complete (exit 2) rather than pass. No other sentence of §5 changes; §4 (O3 + prospective), the Verification bullets, and every other contract are untouched.

**Scope + reversal.** One clause of one contract (C-HE-13 §5) conforms to the state machine C-HE-03 §4 already defines; no contract number, terminal state, verdict-validity rule, store surface, or §6 order changes. Clearance is proportionate: the correction was surfaced by the out-of-family reviewer on the landing PR and is re-reviewed by that PR's remaining rounds and its 3-lens merge gate; the executable witness is `tools/test_arc_disjoint_check.py::test_other_lane_heads_are_the_non_terminal_siblings`. **Operator may reverse** by a v1.8 note. Marker: `.harness/clearance/spec-he-loop-lanes-v1.7-cleared-2026-09-02.md`.
