...
**Trigger.** Execution of the `merge-door-release-verb` arc (closing the registered finding that C-HE-06 §6's release had no CLI surface, so the door's documented recovery was unreachable). The verb was built and hardened across three out-of-family review rounds; the fourth round refused it on the contract rather than the code, correctly: the state the verb exists to recover — a landing whose own merge commit reddened `main` — is one §6 permits no exit from, because step (vii) polls that merge SHA's own run and the commit's bytes are immutable, so `land` re-blocks forever, `unblock` mints a successor rather than opening the door, and `gc` skips live leases. Observed twice in one session (#1570 blocked at (viii) on `refresh_pr_ci_not_green`; #1573 blocked at (vii) with a permanently red merge SHA). The reviewer's objection is sound under the contract as written, and a lane may not amend a contract to dissolve its own objection — so the mechanism was surfaced to the operator, who ratified this carve-out.
