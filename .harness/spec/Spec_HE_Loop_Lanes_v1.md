...
- The lease is never released while the merge SHA's own `main` run or the terminating refresh is UNCONFIRMED — not yet observed. **Exception (X8), operator-confirmed:** a run that has been observed and reached a terminal non-success is confirmed, not unconfirmed, and its lease MAY be released through §6's operator-confirmed release. The gate is `unblocked_from`, which is reachable only through `unblock`, which requires `state: blocked`, which §4(ix) writes only after (vii)/(viii) resolved non-success — so the operator has already adjudicated the outcome before this path opens. A landing that merely crashed mid-flight carries no block, no `unblocked_from`, and is refused.
