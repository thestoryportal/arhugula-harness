...
Path: `.harness/spec/Spec_HE_Loop_Lanes_v1.md` · Date: 2026-08-18 · **v1.10** (execution-correction change-notes X1 2026-08-18 + X2 2026-08-19 + X3 2026-08-19 + X4a–X4d 2026-08-20 + X5 2026-08-25 + X6a–X6f 2026-08-26 + X7 2026-09-02 + X8 2026-09-18 + X9a–X9h 2026-09-18 + X10 2026-09-19; marker `spec-he-loop-lanes-v1.10-cleared-2026-09-19.md`) · Repo at `17011f89c` (all `file:line` cites are pinned to this commit) · Namespace `C-HE-*` (H_E dev tooling; see §0.2)
...
## Change-note (v1.10 — merged worktrees are reapable, operator decision 2026-09-19)

**Trigger.** The operator directed that merged worktrees be removed autonomously at session start or end whenever that is safe. #1606 made SessionStart launch the existing reaper (`loop_gc_worktrees reap`, detached). Its end-to-end run showed C-HE-04 §6 refusing every standard arc worktree after its squash merge, so in the standard flow no merged worktree could ever be reaped. The operator chose amending §6 over clearing the branch upstream at closeout and over leaving merged worktrees in place.

- **X10 — C-HE-04 §6: a worktree merged at its exact head holds no unpushed capture.** §6 refuses teardown while `git -C <wt> rev-list @{u}..HEAD` is non-empty, so that disposal can never lose a capture silently. After a squash merge that check is always non-empty: an arc branch created from `origin/main` tracks it, and the branch's own commits never become ancestors of the squashed commit; once the remote branch is deleted, `@{u}` stops resolving and the check fails closed. Every merged arc worktree was therefore refused forever, and autonomous reaping (operator directive 2026-09-19) never removed one in the standard flow. The amendment: the ahead-of-upstream and unresolvable-upstream refusals do not apply when the caller has proved that the worktree's HEAD equals the head of its branch's pull request MERGED INTO THE DEFAULT BRANCH (a merge into any other branch proves nothing), because the merge put that content on the default branch and the reaper leaves the branch ref in place, so no commit loses its reference. The proof is the caller's (the reaper's exact-SHA `headRefOid` match), never an inference inside the teardown; a teardown without it refuses exactly as before, and uncommitted changes, detached HEADs and precious ignored files still refuse unconditionally. Operator decision 2026-09-19 (the spec amendment over an unset-upstream convention).

**Scope + reversal.** One contract gains a tagged clause (C-HE-04 §6). No contract number is added or removed (the 35-contract set is unchanged); no terminal state or verdict-validity rule changes. The C-HE-04 teardown verification row (`safe-worktree-remove.sh` refuses on `rev-list @{u}..HEAD` non-empty) still holds as written: that wrapper supplies no merge proof. The tagged clause keeps its prior text and carries an inline `v1.10 X10` marker. **Operator may reverse** by a v1.11 note. Marker: `.harness/clearance/spec-he-loop-lanes-v1.10-cleared-2026-09-19.md`.

## Change-note (v1.9 — one-hour arcs, operator-ratified 2026-09-18)

**Trigger.** The operator set a wall-clock goal: an arc lands in a mean of ≤ 60 minutes, in one lane or in parallel lanes, while code, doc and repo quality stay reasonably balanced. A measured audit of the last 30 fully reserved arcs (2026-09-03 → 09-18; `.harness/audit/arc-wallclock-optimization-2026-09-18.md`) put the mean at 177 min and the median at 126, with code arcs at a median of 203. Review rounds run to convergence took 64% of every measured hour; the door's hold through a second CI run for the terminating roadmap refresh took 16%. The three-lane pilot `pilot-2026-09-17-a` failed its own C-HE-13 report and added four failure classes: lanes collided on the single next-action pointer, terminating refreshes failed CI five times, lanes merged outside the door, and sessions drained their context. The operator accepted every recommendation of the audit on 2026-09-18, including the known cost: in the same cohort 34% of accepted P1/P2 findings (71 of 209) were first raised at review round 6 or later, so a bounded cycle will ship some of those defects or catch them later as follow-up work. This note is the contract half of that decision; the unit program is the plan's §9 (U-HE-52 … U-HE-71).

- **X9a — C-HE-21 §1: a bounded review cycle replaces review-to-convergence.** Each PR runs ONE cycle, launched on push and concurrent with CI: **pass 1** (out-of-family codex + the three merge-gate lenses, in parallel, full diff) → one fix round (every accepted P1/P2 fixed; an invalid finding rejected with its cited law; every P3/prose finding collected into ONE follow-up work item) → **pass 2** (codex + the witness-adequacy lens, on the fix delta) → **escalation**, at most once and only if pass 2 raised an accepted P1 (a repeat of pass 1) → **pass 3** (one lens on the full diff, the merge gate; it blocks only on P1, and its re-runs are still pass 3). A P1 still unfixed after pass 3 stops the arc and routes to HITL; nothing merges past a known P1. Doc-only arcs run pass 3 alone. Commits added after the cycle get exactly one pass sized to the change. **Every accepted finding gets exactly one disposition, whichever pass raised it:** an accepted P1 blocks until fixed (raised in pass 2 it triggers the escalation; raised in the escalation or in pass 3 it is fixed and pass 3 re-runs); an accepted P2 raised before pass 3 — in pass 1, pass 2 or the escalation — is fixed before pass 3 runs; an accepted P2 first raised in pass 3, and every P3 or prose finding from any pass, goes into the one follow-up work item. A rejected finding carries its cited law and needs nothing further. The v1.5 X5 recorded-decision checkpoint survives only as the escape from that stop (`review-attest-budget`: recorded, never grantable by the loop). The v1.6 X6b launch-admission rule stands — a launch the admission state forbids is never made, and a refused launch never consumes a pass identity — but its sweep-attestation precondition moves to authoring time: the defect-class preflight is attested once, before pass 1, and is no longer re-attested per round.
- **X9b — C-HE-34, C-HE-31 invariant, §8 row 7: the non-goals follow X9a.** "No auto-stopping round cap" and "No collapsing of review layers" are struck from C-HE-34: the cycle is the cap, and passes 2 and 3 are deliberately narrower than pass 1. What stays foreclosed is running review past the cycle without a recorded decision. The C-HE-31 invariant now reads that no mechanized check is cited as grounds for skipping a pass of the cycle.
- **X9c — C-HE-24 §2: the envelope gains `cycle_pass`.** `cycle_pass: <null|1|2|esc|3>` is written by the emitter on every finding-class row of a review invocation. `round_n` cannot carry the pass because `round_n_for` counts per producer, and passes 2 and 3 run fewer producers than pass 1. Historical rows read `null`, under C-HE-25's historical-row rule.
- **X9d — C-HE-29 §2: the scored unit is a cycle.** The shadow lens fires on each cycle's pass-3 terminal verdict. Each arc runs one cycle, so a scored unit is one arc, scored at its FINAL pass-3 terminal before merge — a pass-3 re-run after a P1 fix changes `head_sha`, and scoring each terminal would count one cycle twice. The denominator is `COUNT(DISTINCT arc_id)` over arcs with a final pass-3 terminal — never `round_n` alone, which repeats across arcs. The reducer change (`tools/shadow_trial.py`) is owned by U-HE-53. Everything else in C-HE-29 is unchanged.
- **X9e — C-HE-06 §4 (viii), the Invariants, the Verification continuation unit, §0.4 and council row G4: the door no longer drives a terminating refresh.** Step (viii) becomes: `lit done <arc_id>` then `lit sync push --remote origin`, each bounded at 120 s each call (C-HE-02 §3's bounded-wait rule, as step (iv) bounds the merge call); a timeout is handled like a non-zero exit. A non-zero exit there emits a `NOTIFY` row and never blocks the lease, because the merge is irreversible and a block would lock the single global door. **Durability of the lit step.** A failed `lit done` or sync push never lets a merged arc be selected again: the reservation already reads `merged` at step (vi), `arc_id` is the lit id, and `reservations.py selectable` refuses any existing head, terminal ones included (C-HE-03 §4). The lit ticket's state is repaired durably, not by hope: the session-start reconcile pass (C-HE-03 §5, `reconcile-all`) runs `lit done` for every `merged` reservation whose lit ticket is still open, and runs `lit sync push --remote origin` whenever `lit sync status` reports local lit commits not yet on `origin` — which covers the case where `lit done` succeeded locally and only the push failed — emitting a `NOTIFY` for each repair. The fence against reselection is the reservation store, which is per machine (`QUEUE_DIR`); lanes therefore run on one machine (the C-HE-01 reference machine), and a second machine's clone is out of scope for this fence. `REFRESH_BOUND_S` retires, and the worst-case legitimate hold drops from ≈ 92 to ≈ 51 minutes (120 s merge + 45 min post-merge CI + 2 × 120 s lit calls). The invariant "never released while … the terminating refresh is UNCONFIRMED" keeps only its merge-run clause; the X8 exception covers that clause alone. The continuation verification unit and its re-acquire mutation are void; the replacement lit-step unit observes both commands, their order under the held lease, and NOTIFY-not-block on failure. C-HE-06 §10's clean cycle swaps "refresh green" for "`lit done` recorded".
- **X9f — C-HE-01 §3 and its Invariants, C-HE-03 §4, AC#4: `lit` is the state of record for forward work.** Open roadmap items, open forward-register rows, open arcs and the next action live in `lit` (promptctl/links-issue-tracker). Its Dolt store sits in the clone's git common dir, so every worktree of the clone reads one database, and it syncs through `refs/dolt/data` on `origin`. Selection is `lit next` (read-only) → `reservations.py reserve --arc-id <id>` (the fence, `(none)→pending`, C-HE-03 §4) → `lit start <id>`. The reservation is minted first so that no crash can leave a lit claim without one; a crash after the reserve leaves a `pending` reservation that C-HE-03 §5's aged-pending path already surfaces, and it keeps refusing a second lane. The reservation keeps the whole merge lifecycle, with `arc_id` = the lit id. A lit claim held fresh by another checkout, or a non-terminal reservation, makes a second lane's selection fail. The §12.2.1 terminating-refresh fixed point and the one-file refresh shape retire with `.harness/roadmap_status.md`, which moves to `.harness/archive/roadmap-plainfile/` as frozen history.
- **X9g — C-HE-08 §4 and its §8.1 row: the tiebreaker asserts on a stale content branch.** The load-bearing parameter stays — a PR branched from a since-superseded `main` either fast-forwards cleanly or is caught pre-merge under `strict: true` — but the probe branch is a scratch content branch, since refresh PRs no longer exist.
- **X9h — C-HE-12 §1, C-HE-28 §2 and §4, the §9 hazard row 6, C-HE-33 §3: `ROADMAP_STATUS_DRIFT` retires with its artifact.** Hazard #6 (refresh collision) cannot occur without a refresh artifact, so it leaves C-HE-12's inventory: the target becomes ≥ 3 of the 18 remaining named hazards emitting (#1, #4, #7), and AC#9 reads the same. C-HE-28 §2's correlation is replaced by the per-arc wall-clock measure (**NEW** `tools/arc_wallclock.py`: reservation `reserved_at` → content-merge commit → door release, reported per phase with the rolling mean and median), which is the tracked outcome for this note's goal. The §4 mid-budget lens trial is superseded by X9a's pass 2. `--allow-roadmap-drift` becomes an accepted no-op until its callers are removed, which narrows C-HE-33 §3's parity requirement to the remaining findings.

**Scope + reversal.** Twelve contracts gain tagged clauses (C-HE-01, 03, 06, 08, 12, 21, 24, 28, 29, 31, 33, 34), plus the §0.4 definition, §8 rows 4 and 7, the §8.1 tiebreaker row, the §9 hazard row 6, the surfaces-table rows for C-HE-06 and C-HE-21/22, and council row G4. No contract number is added or removed (the 35-contract set is unchanged); no terminal state or verdict-validity rule changes. Each tagged clause keeps its prior text for history and carries an inline `v1.9 X9<letter>` marker naming what now governs. Clearance: the program was audited, red-teamed against the code for ordering and collision defects before any byte moved (20 corrections folded into the plan), and operator-ratified. **Operator may reverse** by a v2.0 note. Marker: `.harness/clearance/spec-he-loop-lanes-v1.9-cleared-2026-09-18.md`.

## Change-note (v1.8 — execution correction, 2026-09-18)

**Trigger.** Execution of the `merge-door-release-verb` arc, against the registered finding that C-HE-06 §6's release had no CLI surface, so the door's documented recovery was unreachable. **This closes PART of `B-253`, not that row.** B-253 (`.harness/forward-register.yaml`, REGISTERED 2026-09-17) names this same wedge and requires three things: (1) the C-HE-08 §4 tiebreaker emitting a terminating refresh for its own landings, and/or (2) step (vii) distinguishing a drift-only red inherited from prior un-refreshed landings from this arc's own failure, AND (3) a CLI `release` verb. This arc ships (3) only. (1) and (2) remain open, and the row also asks for the change-note at C-HE-08 §4 / C-HE-06 §4(viii) — this note is at §6 and the Invariants bullet instead, because what changed is the release transition and the release invariant, not step (viii)'s obligation. The row itself is not amended here: `.harness/forward-register.yaml` is fenced by the `lane-init-shell-portability` (#1561) and `u-he-40` reservations, so B-253's close_out is updated by whichever arc takes it when the register is takeable. The verb was built and hardened across three out-of-family review rounds; the fourth round refused it on the contract rather than the code, correctly: the state the verb exists to recover — a landing whose own merge commit reddened `main` — is one §6 permits no exit from, because step (vii) polls that merge SHA's own run and the commit's bytes are immutable, so `land` re-blocks forever, `unblock` mints a successor rather than opening the door, and `gc` skips live leases. Observed twice in one session (#1570 blocked at (viii) on `refresh_pr_ci_not_green`; #1573 blocked at (vii) with a permanently red merge SHA). The reviewer's objection is sound under the contract as written, and a lane may not amend a contract to dissolve its own objection — so the mechanism was surfaced to the operator, who ratified this carve-out.

- **X8 — C-HE-06 §6 gains an operator-confirmed release, and the Invariants bullet distinguishes UNCONFIRMED from TERMINAL.** "Unconfirmed" in that bullet means *not yet observed*, which is the state the fence exists for: an outcome still in flight must never be released past. A run that HAS been observed and reached a terminal non-success is confirmed — badly, but confirmed — and the contract previously had no sentence for it, which is what left the door with no exit. The carve-out is gated on the RUN ITSELF, read from ground truth at release time — not on any proxy for it. Two proxies were tried and both admitted states they should not: the fact of a block (ten reasons reach `mark_blocked` and most never poll that run), then the block's reason (`land` persists `post_merge_ci_not_green` for a 45-minute TIMEOUT too, where the run was equally unobserved). So the release path queries the merge SHA's own `main` run and admits ONLY a run that has COMPLETED with a non-success conclusion. A run still pending, one that never appeared, and a ground truth that cannot be read all refuse — unreadable is unconfirmed — and a GREEN run refuses too, routing the operator to resume the landing through `land` rather than free the door. The sha queried is the RESERVATION's `merge_sha`, recorded at step (vi) after `gh pr view` confirmed the merge — never `blocked_at_sha`/`unblocked_from`, which names the merge commit on only two of the ten block paths and the PR head or a refresh sha on the rest. `unblocked_from` remains required alongside it as the operator-confirmation half; a landing that merely crashed mid-flight carries no block at all and is refused a step earlier. The admitted conclusions are C-HE-19 §1's declared domain minus SUCCESS — `{FAILURE, CANCELLED}` — so this carve-out widens no terminal-state contract; anything outside that domain fails closed. The absence of all three refresh sidecars is what proves the refresh was never minted, and is admissible only under the both-clauses carve-out recorded at the Invariants bullet; it is never read as the refresh having been confirmed. `unblocked_reason` is carried onto the successor as the durable record of why the door blocked, and appears in the refusal message, but decides nothing. Both of a landing's outstanding effects stay fenced independently: the content merge must be settled, and a MINTED terminating refresh (either sidecar) refuses outright, since step (viii) is mandatory and nothing local proves its outcome.

**Scope + reversal.** One contract (C-HE-06) gains one admissible transition and one clarified invariant; no contract number, terminal state, verdict-validity rule, store surface, or §4 step order changes, and the in-process landing driver's own step-(ix) release is untouched. The 35-contract set is unchanged. Clearance is proportionate: the carve-out was surfaced as a halt after the r4 refusal and ratified by the operator before any spec byte moved; the executable witnesses are `tools/test_merge_door.py::test_release_verb_frees_the_door_from_the_cli`, `::test_release_verb_refuses_a_lease_no_operator_has_adjudicated`, `::test_release_verb_refuses_an_unreconciled_in_flight_merge`, `::test_release_verb_refuses_an_in_flight_refresh_merge`, `::test_release_verb_refuses_a_recorded_but_unattempted_refresh`, `::test_release_verb_refuses_a_blocked_lease` and `::test_unblock_then_release_frees_a_wedged_door_from_the_cli`, each mutation-probed. **Operator may reverse** by a v1.9 note. Marker: `.harness/clearance/spec-he-loop-lanes-v1.8-cleared-2026-09-18.md`.

## Change-note (v1.7 — execution correction, 2026-09-02)

**Trigger.** U-HE-36 execution (`tools/arc_disjoint_check.py`, the C-HE-13 §5 selection-time gate): §5 as written took "every other lane's current head" from "the `open` reservations' `branch`", but C-HE-03 §4 keeps a lane `pending` for its whole build and flips it `open` only at drain start (C-HE-04 §2) — an `open`-only read sees no sibling until that sibling is already landing, so the gate would fence nothing during the window it exists for. The out-of-family reviewer on the U-HE-36 PR (codex r1 P2) refused a plan-side execution note as the amending carrier: the contract text is the authority, so the contract text changes.

- **X7 — C-HE-13 §5 lane set = every non-terminal reservation.** "every other lane's current head" is the `branch` of every other lane's `pending` or `open` reservation (the C-HE-03 §2 non-terminal states); `merged`/`abandoned` heads are not lanes. A record whose `state` is outside the C-HE-03 §2 domain is unreadable, not terminal: the gate refuses to complete (exit 2) rather than pass. No other sentence of §5 changes.
- **X7b — C-HE-13 §4 O3 rate is an interval with its masked count named (codex r3 P2 on the same PR).** The 172 historical pairs are squash merges: the lane branches that would make a pair's textual-conflict status exact no longer exist, so the replay rebuilds the later PR's change onto the earlier one (per-file `apply --3way` onto A's tree, merge-tree against A at base A^). A file whose change overlaps an intermediate commit's own hunk cannot be placed against A by any replay — its status is unmeasurable from squash history — and is reported MASKED, never dropped or counted. The "real textual-conflict rate" is therefore reported as `certain/N .. (certain + masked-only)/N` over the window-pair denominator N (444), with the masked-only pair count named; a point estimate is admissible only when that count is zero. The semantic-conflict rate stays unmeasured. §4's O3 sentence gains the parenthetical below and its U-WT-07 cite is dated (the "deliberately unbuilt" carrier note it quoted was retired by the U-HE-36 PR that built the tool — spec-conformance lens on that PR); the prospective-check requirement and the Verification bullets are untouched.

**Scope + reversal.** Two clauses of one contract (C-HE-13 §4 measurement shape, §5 lane set) conform to what the state machine C-HE-03 §4 defines and to what squash history can measure; no contract number, terminal state, verdict-validity rule, store surface, or §6 order changes. Clearance is proportionate: both corrections were surfaced by the out-of-family reviewer on the landing PR (r1 P2, r3 P2) and are re-reviewed by that PR's remaining rounds and its 3-lens merge gate; the executable witnesses are `tools/test_arc_disjoint_check.py::test_other_lane_heads_are_the_non_terminal_siblings` and `::test_pair_conflicts_names_a_masked_file_instead_of_dropping_it` / `::test_historical_reports_interval_and_unmeasured_semantic_line`. **Operator may reverse** by a v1.8 note. Marker: `.harness/clearance/spec-he-loop-lanes-v1.7-cleared-2026-09-02.md`.

## Change-note (v1.6 — U-HE-35 workflow-repair absorption, 2026-08-26)

**Trigger.** The two U-HE-35 session audits — the findings audit `.harness/session-audit-2026-08-26-u-he-35-preflight-suite.md` (cited **[A]** below: all 29 codex findings across 10 rounds classified; a faithful application of knowledge already held reaches all-approve in 4–6 rounds instead of 10) and the cost audit `.harness/session-audit-2026-08-26-u-he-35-workflow-efficiency.md` (cited **[B]**: 418 main-session API calls ≈ 21.0M input-equivalent tokens (IET) + 286 subagent calls ≈ 4.5M; 13 review rounds — 10 codex + 3 merge-gate — to all-approve) — measured that the loop's own measurement substrate is mis-recording the quantities any repair program will be judged by. Concretely: the arc-metrics round snapshot counted the arc's two `GATE_REFUSED` transcripts as review rounds and mis-numbered the P1 rounds ([B] F15: `review_rounds: 12`, `p1_rounds: [1, 11]` against a ground truth of 10 rounds with P1s at r1 and r10); the C-HE-24 attribution fields are null on all 44 finding rows ([B] F16), so the which-layer-uniquely-caught-what question is unanswerable from the ledger; the C-HE-27 verify/absorb/edit spans were never emitted ([B] §3 item 4: 0 of the 60 edges expected across the 10 codex BLOCK rounds — the emission instructions live in `ship-pr/SKILL.md:95-108` prose while those rounds ran under `roadmap-continue`, which was the only skill in context); and two launches were made that the admission state already forbade ([B] F13: r9's first launch was refused `SWEEP_MISSING` and its relaunch reused `r9.log` → `PUBLISH FAILED (exit 4)` under the round-log write-once rule; r11 was launched into `BUDGET_EXHAUSTED`). Each X6 item amends a contract this spec already owns; no contract number is added or removed (the 35-contract set is unchanged), and the companion skill-suite/governance repairs land separately under the workflow-repair charter (`.harness/spec/Workflow_Repair_Charter_v1.md`), which cannot amend these contracts — a second authority would violate the one-authority rule this spec's C-HE-23 §1 encodes.

- **X6a — C-HE-27 span-emission site (new §5; [B] a1).** The verify/absorb/edit span edges MUST be emitted by the review-wrapper path itself (`just review-with-failover-logged`), not left to skill prose: verify start/end are the wrapper invocation's own process boundaries; absorb start/edit edges key off the first fix edit and the absorption commit. Until the wrapper emits, the interim carrier is the emission block copied into `roadmap-continue/SKILL.md` (the skill actually in context during review rounds). Evidence: 0 of the 60 edges expected across the 10 codex rounds while the instructions sat in `ship-pr/SKILL.md:95-108` ([B] §3 item 4) — an instrument carried as agent discipline in a skill that is not loaded during the phase it measures is not an instrument.
- **X6b — C-HE-21 §1 launch admission + per-attempt round-log names ([B] a3 + c5).** Before any review launch the loop MUST evaluate the admission state the checkpoint mechanism enforces — the spent-round budget and every launch precondition (sweep attestation) — and MUST NOT make a launch that would be refused; a refused launch MUST NOT consume a round identity, so round-log destinations are named per attempt (a refused or failed attempt never claims the write-once round name). Evidence: [B] F13 — both failure shapes occurred in one arc.
- **X6c — C-HE-25 round derivation ([B] b2).** The arc row's per-round records MUST be derived from round-log content (the wrapper's terminal lines), never from file position; `GATE_REFUSED` transcripts are refused launches, not rounds, and MUST be excluded; rounds are keyed by round id. Acceptance witness (asserting the shape, not a recalled number): the U-HE-35 round directory `.harness/tmp/u-he-35-rounds/` (12 files: `r1..r11.log` + `r9-verdict.log`, two of them `GATE_REFUSED`) MUST reduce to 10 rounds with P1s at r1 and r10 ([B] F15).
- **X6d — C-HE-24 §2 attribution writers ([B] b3).** `cause_attribution` and `unique_catch` MUST be populated at emission time on merge-gate lens rows (the emit path knows exactly which codex rounds preceded it), and the absorption step MUST append the `finding_adjudication` rows (`disposition`) that §5 already defines. [B] b3 offered two arms — write the fields or drop them; the drop arm is foreclosed by the existing contract set, not by preference: `unique_catch` is C-HE-29's operational definition (fold G15) and `disposition` is §5's adjudication core. Null-everywhere ([B] F16: 44/44 rows) is the worst state and stops being legal for new merge-gate rows. Match predicate for the gate-lens `unique_catch` join, fixed here so the emitter invents nothing: `unique_catch = true` iff no same-arc finding row from a codex round matches on `(location, finding_type)` — an intra-arc heuristic join keyed on location+type, never `finding_id` (C-HE-24 §4's cross-`head_sha` identity exclusion stands), and distinct from C-HE-29 §2's shadow-vs-blocking definition, which is unchanged.
- **X6e — C-HE-25 per-arc cost fields ([B] d1).** Arc rows gain additive-nullable cost fields derived by a deterministic extractor (`just arc-cost <transcript>`, **NEW** `tools/arc_cost.py`): usage deduplicated by `requestId` (naive sums double-count 1.9× — [B] Evidence and method), the IET index (input + 1.25×cache-write + 0.1×cache-read + 5×output), and stage windows cut at transcript event timestamps — feeding the B-211/B-212 lever report so it can show cost per arc, not only rounds per arc. C-HE-25's historical-row rule applies unchanged (absent fields read as `null`).
- **X6f — C-HE-28 mid-budget lens trial (new §4; [B] b9, F3).** One measured trial on the next code arc: run the concurrency and witness-adequacy lenses once after codex round 4; spec-conformance stays at ship only, with its findings-per-IET recorded (this arc: 0 findings per 1.34M IET across three clean-approve runs — a second zero can retire or shrink it). The measurable claim is rounds-to-all-approve against this arc's 13 (10 codex + 3 gate); the result is correlational per §3's own discipline — a trial to run, not a proven saving ([B] F3: every gate finding landed on code codex had already reviewed ten times, but the concurrency/witness defects existed in code present by mid-budget).

**Scope + reversal.** Five contracts gain tagged clauses (C-HE-21 §1, C-HE-24 §2, C-HE-25, C-HE-27 new §5, C-HE-28 new §4); §5's files table gains the X6a–X6e carrier entries; no contract number, terminal state, verdict-validity rule, store surface, or §6 order changes. Clearance is proportionate: every item conforms a contract to defects two evidence audits measured on the arc that exercised it ([A]/[B], both filed in-repo alongside this note), the repair program's sequencing (instruments before levers) and unit decomposition live in the implementation plan's workflow-repair section, and the doc set carries a `harness-adversarial-reviewer` pass before its clearance marker. **Operator may reverse** by a v1.7 note. Marker: `.harness/clearance/spec-he-loop-lanes-v1.6-cleared-2026-08-26.md`.
...
| **merge door** | The single-writer landing path: acquire lease → verify base → `gh pr merge` → confirm → release → terminating refresh (`CLAUDE.md` §12.2.1). Depth 1 by construction. *(v1.9 X9e: the terminating refresh is gone — the door now runs acquire lease → verify base → `gh pr merge` → confirm → post-merge `main` CI → `lit done <arc_id>` → `lit sync push --remote origin` → release.)* |
...
3. Coordination MUST NOT use a daemon, spawner, coordinator process, or merge-queue lock (L-2). Depth-1 is enforced by the merge-door lease (C-HE-06), **not** by drift detection alone (R-8: `ROADMAP_STATUS_DRIFT` fires against already-landed history, rolls nothing back, blocks no merge — `codex_context_guard.py:774-781` **[V]**). *(v1.9 X9h: `ROADMAP_STATUS_DRIFT` is retired; the merge-door lease is the only depth-1 fence.)*
...
- The merge lane's `§12.2.1` fixed point is preserved: a terminating refresh PR touches only `.harness/roadmap_status.md`; other files are never folded into it (`roadmap_status_refresh.py` enforces one-file shape — v1 AC#4 **[C]**). *(v1.9 X9f: retired — no refresh PR exists; `lit` is the state of record for forward work.)*
...
4. **Transitions.** `(none)→pending` at **arc open** — the instant a lane selects the roadmap unit (`roadmap-continue`), before any work; *(v1.9 X9f: selection is `lit next` → reserve → `lit start <id>`, reservation first; the reservation is minted then, with `arc_id` = the lit id, and a fresh lit claim by another checkout also fails the second selection. Because `arc_id` = the lit id, a `merged` reservation keeps refusing that ticket even if its lit state was never closed; §5's `reconcile-all` closes it.)* a `pending` or `open` reservation for the same unit MUST make a second lane's selection fail (selection-time reservation; duplicate *scheduling* is prevented here, duplicate *append* by §6). This is NOT the existing `arc_metrics.py queue` artifact (which records capture inputs at arc **closure** and already requires `--arc-type` there **[V]**) — the reservation is a new open-time record; the term "queue entry" in this spec always means the closure-time `queue`/`.taken` files. `pending→open` at drain start (C-HE-04 §2: the flip precedes `append()`; holder = the draining lane's `lane_id`) — and, on the merge lane, BEFORE door acquisition (C-HE-06 §1/§7: acquisition verifies an `open` holder, so the landing flow — ship-pr's final gate or the land driver — performs the flip pre-acquire; drain-start remains the opener for the closure-capture/bootstrap path); `open→merged` on **confirmed** merge (`gh pr view <pr> --json state,mergedAt` = MERGED); `open→abandoned` on **confirmed** abandonment (PR CLOSED with a superseding pointer, or an operator `RESOLVED-HIL` row naming the arc). Every transition is a §1 CAS.
...
- The lease is never released while the merge SHA's own `main` run or the terminating refresh is UNCONFIRMED — not yet observed. *(v1.9 X9e: the terminating-refresh clause is void — only the merge-run clause and its X8 exception remain.)* **Exception (X8), operator-confirmed, covering BOTH clauses:** a run that has been observed and reached a terminal non-success is confirmed, not unconfirmed, and its lease MAY be released through §6's operator-confirmed release. The terminating-refresh clause is carved out with it and for the same landing only: a landing blocked at (vii) never reached (viii), so its mandatory refresh was never minted, and holding the door shut until a refresh that cannot now be produced is confirmed would make the fence permanent. The obligation is not waived, it MOVES — `main` is left un-refreshed, which CI's `ROADMAP_STATUS_DRIFT` fails on independently, and the repair PR lands as its own reserved arc whose own step (viii) produces the refresh. A refresh that WAS minted or declared is NOT covered: any of the three sidecars refuses the release and routes to `record-refresh` / `clear-refresh-intent`. The gate is the RUN, queried from ground truth at release time: admissible only when the merge SHA's own `main` run has COMPLETED with a non-success conclusion. Pending, absent, unreadable and green all refuse, and a landing that merely crashed mid-flight carries no block and is refused earlier still.

### Verification

- **AC#2(c) crash-resume** (**mutation-probe**; `subprocess.Popen` + env plumbing; `MERGE_DOOR_TEST_KILL_AFTER=<step>` makes `merge_door.py` `os._exit(137)` immediately after the named step): kill after `merge_attempted_at` write and before merge → restart (self-resume via marker) sees OPEN → exactly one merge call; kill after merge success and before release → restart sees MERGED → continues to (vii), merge mock call-log length == 1; kill after release, before the refresh PR → refresh proceeds under a fresh cycle. *(v1.9 X9e: this sub-case is VOID — no refresh PR exists. Replacement: kill after (vii) and before the lit step → restart resumes at the lit step under the held lease; kill after release → the session-start `reconcile-all` repairs the lit ticket and any unpushed lit state.)* Plus: kill after (vi) and before (vii)/(viii) → lease held, restart resumes the CI wait.
...
- Continuation unit: refresh PR merge under the same lease — no second acquire call recorded *(v1.9 X9e: this unit and its re-acquire mutation are VOID. Replacement — **lit-step unit** (**mutation-probe**): with a green (vii), the door records, in order and under the held lease, `lit done <arc_id>` then `lit sync push --remote origin`, then release — no second acquire call; a non-zero exit from either command emits a `NOTIFY` row and the lease still releases, never `blocked`; mutations that must go RED: delete the `lit done` call, delete the sync push, swap their order, or route a `lit` failure to `mark_blocked`.)* (**mutation-probe**: force a re-acquire → deadlock/timeout observed).
...
4. **Tiebreaker before enforcing (HE-1 O4; C10-T8).** *(v1.9 X9g: the stale branch is a scratch content branch; refresh PRs no longer exist.)* `just main-protection-tiebreaker` runs one `gh pr merge --squash --match-head-commit` against a scratch PR under `strict: true` **and** asserts that a refresh PR branched from a since-superseded `main` either fast-forwards cleanly or is caught pre-merge — the load-bearing parameter — not merely that some refresh run succeeds once.
...
1. Every Phase-0 fix in this Part MUST pair with a **finding-row emission** on the C-HE-24 record, not only a pass/fail gate. Detection count today: **1 of the 19 named hazards** (`ROADMAP_STATUS_DRIFT`, *(v1.9 X9h: `ROADMAP_STATUS_DRIFT` is retired with its artifact and hazard #6 can no longer occur, so it leaves the inventory: the target becomes ≥ 3 of the 18 remaining named hazards emitting — #1 split-brain, #4 stale reservation and #7 base TOCTOU — and AC#9 reads the same.)* `codex_context_guard.py:774-781` **[V]**, JSON-emitted `:928-960` **[V]**); the target after this item is ≥ 4/19 (R-2) — concretely hazards #1 split-brain, #4 stale reservation, #6 refresh collision, #7 base TOCTOU of the §9 table each have an emitting code.
...
4. **O3 + prospective check (R-12).** Keep O3 (`git merge-tree --write-tree` over the 172 historical colliding pairs → real textual-conflict rate vs the 38.7% upper bound (v1.7 X7b: reported as the interval `certain .. certain+masked-only` over the 444 window pairs with the masked-only count named — a squash-merged pair whose later change overlaps an intermediate commit's own hunk is unmeasurable and is MASKED, never dropped); report semantic-conflict rate as **unmeasured, not zero**) as a Phase-1 base-rate prior, **and** add the prospective merge-tree check on the actual chosen lane set — `tools/arc_disjoint_check.py` (U-WT-07; at v1.0 named in `two-lane/SKILL.md:19` **[V]** as deliberately unbuilt and absent at HEAD **[V]** — built at U-HE-36, v1.7, which retired that carrier note in the same PR).
5. **Scope is a hint, not a gate (E17).** `two-lane/SKILL.md` instructs picking arcs whose `scope.files` do not overlap, but the forward register carries **zero** such keys across 186 rows **[V]**. Declared scope MUST be treated as a non-authoritative scheduling hint; enforcement MUST be actual-write based: before a lane opens an arc, `arc_disjoint_check.py` computes `git merge-tree --write-tree` of the candidate head against every other lane's current head (from the `branch` of every other lane's non-terminal — `pending` or `open` — reservation; v1.7 X7: C-HE-03 §4 keeps a lane `pending` for its whole build, so an `open`-only read would see no lane until it was already landing; a `state` outside the C-HE-03 §2 domain makes the check refuse to complete rather than pass) and refuses selection on a non-empty conflict set; at landing, the `BASE_TOCTOU` first-parent detection in C-HE-12 catches what selection-time could not.
...
1. **No auto-stopping round cap anywhere** (v1.5 X5). *(v1.9 X9a: SUPERSEDED — each PR runs one bounded review cycle, pass 1 → fix → pass 2 → escalation at most once → pass 3; see the v1.9 change-note. The checkpoint survives only as the recorded escape from an unfixed P1.)* Mechanization removes defect classes; it never licenses shortening review (R1 #11/#17/#20; PR #1034 produced genuine findings through round 48 of 49; round-2 P1-rate 75% > round-1 62% **[C]**). A **periodic recorded-decision checkpoint** is NOT a cap and is admitted (v1.5 X5, operator-ratified 2026-08-25, U-HE-33/B-216 grounding): at every N spent rounds the loop MAY refuse further review invocations until a recorded decision continues (unbounded, auditable, never grantable by the loop itself) or holds; review is punctuated, never shortened, and the refusal MUST carry the late-round-productivity counter-evidence for the decider. *v1.6 X6b: the admission state is evaluated BEFORE any launch — a launch the checkpoint or a launch precondition (sweep attestation) would refuse is not made, and a refused launch never consumes a round identity: round-log destinations are named per attempt, so a refused attempt cannot claim a write-once round name; see the v1.6 change-note.*
...
5. **Emission site (v1.6 X6a).** The verify/absorb/edit span edges are emitted by the review-wrapper path (`just review-with-failover-logged`): verify start/end at the wrapper invocation's own process boundaries; absorb/edit keyed off the first fix edit and the absorption commit. Skill prose is not an emission site — until the wrapper emits, the interim carrier is the emission block in `roadmap-continue/SKILL.md`, the skill actually in context during review rounds (U-HE-35: 0 of 60 expected edges while the block lived only in `ship-pr/SKILL.md:95-108`); see the v1.6 change-note.
...
2. Refresh-collision incidence (`ROADMAP_STATUS_DRIFT` findings, now lane-attributed) MUST be correlated against `concurrent_lanes_at_open` (R-15). *(v1.9 X9h: retired; the tracked outcome is per-arc wall-clock from `tools/arc_wallclock.py`, correlated against `concurrent_lanes_at_open`.)*
...
4. **Mid-budget lens trial (v1.6 X6f).** *(v1.9 X9h: superseded by X9a's pass 2.)* One measured trial on the next code arc: the concurrency and witness-adequacy lenses run once after codex round 4; spec-conformance stays at ship only, with its findings-per-IET recorded (U-HE-35 baseline: 0 findings per 1.34M IET across three clean-approve runs — a second zero can retire or shrink it). The measurable claim is rounds-to-all-approve against the U-HE-35 baseline of 13 (10 codex + 3 gate); the result is correlational per §3 — a trial to run, not a proven saving; see the v1.6 change-note.
...
2. **Rows.** Every scored round emits at least one row *(v1.9 X9d: the scored unit is a cycle, and each arc runs one cycle, so the unit is the arc: only the arc's FINAL pass-3 terminal before its merge is scored (a pass-3 re-run after a P1 fix changes `head_sha`, and scoring each would count one cycle twice), and the denominator is `COUNT(DISTINCT arc_id)` over arcs with a final pass-3 terminal. The per-lens `COUNT(DISTINCT round_n)` below is void: round numbers repeat across arcs, and the existing reducer already keys `(arc_id, round_n)` for that reason.)* with `producer=<second_reviewer_identity>`: finding rows for what it caught, or exactly one `record_kind=no_finding` marker row when it caught nothing — so the scored-round denominator is `COUNT(DISTINCT round_n WHERE producer=<lens>)` from `merge-gate-log.jsonl` alone. **`unique_catch=true`** iff (a) the finding's `location`+`finding_type` appears in no row from the blocking reviewers for the same `head_sha` AND (b) its last-row `disposition = accepted`; a `unique_catch=true` row later disposed `rejected` MUST NOT count. The adjudicator's identity is `disposition_actor` (C-HE-24 §2), named as `operator` or a specific third-party reviewer identity of **neither model family** — never a placeholder.
...
- No mechanized check is ever cited as grounds for a round cap (K4; #20). *(v1.9 X9b: now — no mechanized check is cited as grounds for skipping a pass of the bounded cycle.)*
...
- No auto-stopping round cap; no shortening of review generically *(v1.9 X9b: STRUCK — the bounded cycle is the cap; running review past it without a recorded decision is what stays foreclosed.)* (#11/#17/#20; PR #1034, #1338 **[C]**). A periodic recorded-decision checkpoint with unbounded auditable continuation is not a cap (v1.5 X5).
...
- No collapsing of review layers to cut the 68% *(v1.9 X9b: STRUCK — passes 2 and 3 are deliberately narrower than pass 1.)* (93.4% of 679 findings across 146 PRs were single-tool catches; merge-gate BLOCKed 46% of 141 gated PRs **[C]**).
...
| C-HE-21 / 22 | `.claude/skills/merge-gate/SKILL.md`, `ship-pr/SKILL.md` (live-carrier cites, no round cap) *(v1.9 X9a: bounded cycle)* — doc; **NEW** `tools/reviewer_concurrency_probe.py`; `tools/review_loop_gate.py` + `tools/round_log_publish.py` + the launch step in `roadmap-continue/SKILL.md` (v1.6 X6b admission guard + per-attempt log names) |
| C-HE-23 / 24 / 25 | `tools/arc_metrics.py` (row schema; v1.6 X6c content-derived rounds); `.claude/skills/merge-gate/SKILL.md` (emit `.harness/merge-gate-log.jsonl` **NEW** file; v1.6 X6d emit-time `cause_attribution`/`unique_catch`); **NEW** `tools/finding_record.py` (8-field core + envelope + `Finding` projection); **NEW** `tools/arc_cost.py` + `just arc-cost` (v1.6 X6e); `tools/codex_context_guard.py` (consume projection); tests |
| C-HE-26 | `tools/arc_metrics.py` (`arc_type` required at open; `arc_type_declared_at`); `.claude/skills/roadmap-continue/SKILL.md` |
| C-HE-27 | `tools/arc_metrics.py` (`phases` map); emitters in `.claude/skills/{roadmap-continue,ship-pr}/SKILL.md` and `tools/hooks/*`; `justfile` `review-with-failover-logged` (v1.6 X6a §5 emission site; interim carrier `roadmap-continue/SKILL.md`) |
...
| 4 | §12.2.1 refresh fixed point preserved *(v1.9 X9f: SUPERSEDED by row 4′ — no refresh PR exists)* | C-HE-01 | one-file refresh shape unchanged |
| 4′ (v1.9 X9f) | forward-work state lives in `lit`; no refresh PR exists | C-HE-01, 03, 06 | no landing opens a refresh PR; `lit next` drives selection |
...
| 7 | No auto-stopping round cap | C-HE-21, 34 | *(v1.9 X9a/X9b: superseded — one bounded cycle per PR; nothing merges past a known P1)* no mechanism ends review by round count alone; the B-215 checkpoint refuses-until-recorded-decision with unbounded continuation (v1.5 X5) |
| 8 | Rebase tax addressed or priced | C-HE-01 §4 | priced as "well under N×", measured by AC#10 |
| 9 | Failure-mode inventory with detections | C-HE-12, §9 | ≥ 4/19 emitting after S4d *(v1.9 X9h: ≥ 3/18 — hazard #6 left the inventory with its artifact)* |
...
| C-HE-08 | `just main-protection-tiebreaker` (scratch PR under `strict:true` + stale-refresh-branch fast-forward) *(v1.9 X9g: stale content branch)* · `apply` | operator-gated live | loop, live — operator answers one decision | — | recorded in the plan's evidence log, not a pytest row |
...
## 9. Failure modes (19; 18 live since v1.9 X9h) → detection / contract
...
| 6 | ~~Refresh collision~~ *(v1.9 X9h: retired — no refresh artifact exists; not counted)* | ~~`ROADMAP_STATUS_DRIFT` **[V]**~~ | ~~lane-attributed (C-HE-24 §6)~~ |
...
| Council G4 | Lease held through post-merge CI + terminating refresh as a continuation; *(v1.9 X9e: the refresh half is retired; the post-merge-CI hold stands)* `main` CI concurrency keyed by SHA | council 2026-08-18 (C10/C9 deadlock fix; Codex C1-04) | C-HE-06 §4 |
