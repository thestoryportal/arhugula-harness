...
- **X9d — C-HE-29 §2: the scored unit is a cycle.** The shadow lens fires on each cycle's pass-3 terminal verdict. A scored unit is one `(arc_id, head_sha)` that reached a pass-3 terminal, and the denominator is `COUNT(DISTINCT (arc_id, head_sha))` over those terminals — never `round_n` alone, which repeats across arcs. The reducer change (`tools/shadow_trial.py`) is owned by U-HE-53. Everything else in C-HE-29 is unchanged.
...
1. Every Phase-0 fix in this Part MUST pair with a **finding-row emission** on the C-HE-24 record, not only a pass/fail gate. Detection count today: **1 of the 19 named hazards** (`ROADMAP_STATUS_DRIFT`, *(v1.9 X9h: `ROADMAP_STATUS_DRIFT` is retired with its artifact; hazard #6 cannot occur, and the count is re-derived from the live emitters.)* `codex_context_guard.py:774-781` **[V]**, JSON-emitted `:928-960` **[V]**); the target after this item is ≥ 4/19 (R-2) — concretely hazards #1 split-brain, #4 stale reservation, #6 refresh collision, #7 base TOCTOU of the §9 table each have an emitting code.
