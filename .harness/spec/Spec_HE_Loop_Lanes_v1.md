...
Path: `.harness/spec/Spec_HE_Loop_Lanes_v1.md` · Date: 2026-08-18 · Repo at `17011f89c` (all `file:line` cites are pinned to this commit) · Namespace `C-HE-*` (H_E dev tooling; see §0.2)
...
2. **Order and holder rule.** At drain, for each claimed entry: (i) flip the reservation `pending→open` with holder = this `lane_id` (C-HE-03 §4, a §1 CAS); (ii) `append()` — which MUST refuse unless this lane holds the `open` reservation; (iii) restore/hold the queue entry. A lane killed after (ii) therefore cannot be silently superseded by a second appender: B sees `open` held by A → not appendable; A's `open` + no PR + dead claim → the recovery path in §4, never auto-abandonment.
...
7. **Lease-holder invariant (P2).** The lease MAY be held only by an arc whose reservation is `open`. Acquisition MUST verify the reservation state; a mismatch fails the acquire.
...
- No lease exists without `pr`, `head_sha`, `reservation_id`, `lease_token`; no lease exists whose reservation is not `open`.
...
| C-HE-03/04 | `tools/test_arc_metrics_lanes.py::test_ac2_a_same_instant[*]` (5 interleavings) · `::test_ac2_b_cross_latency` | phase0 | local + CI | yes | needs C-HE-05; real subprocesses; **no skip** |
