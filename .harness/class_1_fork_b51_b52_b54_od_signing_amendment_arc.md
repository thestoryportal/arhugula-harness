...
(design-substrate spec deltas required; two legs cross-axis OD↔CP). **Status: RATIFIED 2026-07-18 — the operator ratified ALL TEN gate items AS RECOMMENDED** (1: leg-1 option 2 with the full tenant-bearing signing-API rider; 2: leg-2 two-spec shape with the redaction path unconditionally fail-closed; 3: leg-3 §21.2.2 API with all contract requirements, non-blocking default; 4: explicit `false` INVALID at MTC — no §20.1 forward relaxation; 5: §20.3.1 brought into the CP rider aligned with the new API; 6: `sign_rotation_pair` PROHIBITED at MTC until B-33; 7: B-53 rides the Runtime rider as the `harness migrate-audit-sidecar` subcommand; 8: prewarm/keepalive DISABLED at MTC under fail-closed; 9: narrow cutover-scoped §20.1 historical exception; 10: one bundle — OD v1.33→v1.34 + CP v1.100→v1.101 all-four-sections rider + Runtime v1.100→v1.101 rider + OD/CP/Runtime plan deltas). The apply pass proceeds per the final section. No `design-substrate/**` file is edited by this filing; the three register rows stay
`design_substrate_gated` until the ratified OD/CP/Runtime spec + plan deltas are applied (X-AL-3). Source recommendations: `.harness/b-47-pr-b2-design-disposition-v1.md`
