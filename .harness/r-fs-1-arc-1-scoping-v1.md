...
**Result: 5 doc-hygiene (Q1, NOT build) + 2 substantive BUILD (→ engine-classes) + 1 thin contract-authoring/wiring arc (C-RT-28 — capability already built + live-proven at R-820). Zero net-new from-scratch arc from this half.**
...
| **C-RT-28** | managed_agents runtime executable-consumer **contract** — never authored (the *capability* is a separate question, below) | **Capability BUILT + live-proven:** `managed_agents.py` is a **real Anthropic Managed Agents consumer** (`AnthropicManagedAgentsClient` SDK adapter + `ManagedAgentSession` lifecycle + `managed_agents_runtime_span` + beta header `managed-agents-2026-04-01`); **R-820 live-proved it** (batch-52: session `sesn_019aMgaF8sAW2cXhhpMTYij4`→idle, `managed_agents.*` exported to Cloud Trace) ⟹ **H_T-AS-8f SUBSTANTIVE_RETIRED**. **Two real gaps:** (i) the formal **C-RT-28 contract** was never authored (runtime spec line 579: *"no §14.18 C-RT-28 sibling to v1.32 §14.17 C-RT-27"*); (ii) the consumer has **zero production workflow callers** — reached only by the R-820 proof script `tools/r820_managed_agents_live_e2e.py` (`just r820…`), not from `api.run` (`[[r-cxa-seam-wiring-is-producer-discovery]]`). | proof-only | **THIN ARC (not from-scratch build): author C-RT-28 contract + wire the consumer into a production managed-cloud workflow step + cite-bind the R-820 carriers.** SDK/session/telemetry exist + are proven. |

**Provably-complete claim for Part 1:** the overlay `contract_without_code` set decomposes with **zero unexplained entries** into `{phantom: C-IS-11} ∪ {built-but-uncited→Q1: C-CP-30, C-CP-37, C-CP-43, C-OD-3} ∪ {dormant-driver→BUILD: C-CP-49, C-CP-50} ∪ {capability-built-at-R-820, contract-unauthored + unwired→thin arc: C-RT-28}`. No from-scratch build arc arises from this half — C-CP-49/50 are subsumed by the engine-classes build; C-RT-28 is a thin contract-authoring + production-wiring arc on an already-built, already-live-proven consumer (Part 3 §M). **Asymmetric-rigor correction (advisor pre-done, `[[subagent-landscape-reports-need-regrounding]]`):** C-RT-28 was initially mis-classified as "never built → new managed-cloud build arc" by reading the spec's "contract never authored" as "capability absent"; reconciling against R-820 (H_T-AS-8f SUBSTANTIVE_RETIRED) + reading `managed_agents.py` corrected it — *contract*-unauthored ≠ *capability*-unbuilt.
...
| **M** | **managed_agents contract + production wiring** (thin — from Part 1; NOT a from-scratch build) | The consumer is **already built + R-820 live-proven** (`managed_agents.py`; H_T-AS-8f SUBSTANTIVE_RETIRED). Owed: (i) author the **C-RT-28** contract (formalize the managed_agents executable-consumer contract; design-phase spec delta) + (ii) **wire the consumer into a production managed-cloud workflow step** (today proof-script-only) + (iii) cite-bind. Managed-cloud is committed scope (3-tier deployment; team→multi-tenant persona arc). | AS-8f managed_agents indefinite-defer re-opened; R-820 carriers | a *production* managed-cloud run re-touches the IAM/credential boundary (surface, don't auto-fire) — but the SDK/session/telemetry are built + proven, so this is NOT a from-scratch build |
