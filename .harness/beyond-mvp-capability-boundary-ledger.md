...
2. **First arc = make scope provably complete (the scoping pass). ✅ DONE — `.harness/r-fs-1-arc-1-scoping-v1.md`.** Resolved the 8 `contract-w/o-cite` candidates per-contract (5 implemented-but-uncited/phantom → Q1 hygiene; 3 genuinely-unbuilt → `C-CP-49/50` engine-layer emits fold into the engine-classes build, `C-RT-28` managed_agents consumer = new managed-cloud arc) + the fuller spec-body pass (live-at-head filter: no net-new *major* arc beyond Bucket A/B; circuit-breaker found BUILT, SE single-step CLOSED, +2 minor field-level scopes — breaker config-tuning, idempotent MCP-host restart). Output: the **FROZEN ordered child-arc list** `B1→B3→E(engines)→B2→R(router/embedding)→B4→CA(cost)→B5→B6→B7→M(managed-cloud)` + minor field-level + Bucket C→Q1.
