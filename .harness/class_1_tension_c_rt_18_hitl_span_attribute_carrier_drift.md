...
**Status:** **AUTHORING → PROPOSING → RATIFIED → APPLIED** 2026-05-21 (Q5 single-arc co-publication landed: spec v1.10 → v1.11 at HEAD `904a4ec` + plan v2.8 → v2.9 at HEAD `8e08e9e`) — status-line refreshed 2026-05-30 per workspace `[[stale-carry-text-disposition]]` discipline. U-RT-60 implementation arc unblocked against re-clearance state — canonical 4-span shape per ADR-D5 v1.3 §1.8 + CP carrier `HITL_SPAN_NAMESPACE_SCHEMA` 4-entry shape is now spec-authoritative + plan-authoritative. Q6 systemic-pattern follow-on arc remains operator-scheduled independently. **Downstream LANDED (status refresh 2026-05-30):** workspace `CLAUDE.md` §2.3 + §2.4 runtime spec row bump v1.10 → v1.11 long since absorbed at the apply arc + runtime spec subsequently advanced to v1.39 per delta-only-spec-file lineage absorbing multiple subsequent revisions (`v1.12..v1.39`); runtime plan analogously advanced to v2.42 per delta-only-plan-chain lineage. The "downstream owed" framing pre-status-refresh was stale-carry-text per workflow v1.9 §7.4.7 species 5 (post-authoring stale carry) — workflow §7.4.7.3.B audit-template-driven refresh at this update. Per-axis CLAUDE.md no change owed (CP carrier untouched; ADR-D5 untouched) — preserved from original framing.
