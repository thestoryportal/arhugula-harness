...
**Status:** ✅ BUILT 2026-06-22 (BUILD-not-gate; runtime spec v1.73 §14.22.9 + CP spec v1.52 §26.8/§26.2 + harness-runtime/harness-cp impl + by-execution witnesses + 2 clearance markers + spine ledger; bundled-absorption PR). advisor (2 passes — the reframe that flipped the disposition + the key-bound/consume-once + clear_claim/ABORT-claim witnesses) + out-of-family Codex. The §14.22 effect-fence family is now COMPLETE (DURABLE-AUTO + PER-TOOL + HITL-ROUTE + PAUSE-RESOLUTION). The vetted design below held verbatim. (Was: 📐 DESIGN-VETTED, advisor-concurred, BUILD-not-gate.) The resume-side completion of the operator-recoverability story `B-EFFECT-FENCE-HITL-ROUTE` (#702) opened: #702 ships the ambiguous-pause CAPTURE (a labeled, non-terminal, hash-valid `WorkflowPauseReason.EFFECT_FENCE_AMBIGUOUS` PAUSE); a naive `api.resume` re-pauses identically (INERT — pinned by `test_ambiguous_pause_resume_re_pauses_until_resolution_follow_on`). This arc wires the resume to actually RESOLVE the pause via an operator-supplied resolution.
...
**As-built witness note (honest scope, `[[full-chain-witness-not-half-proofs]]`).** The §5 plan below described an `api.resume` round-trip; the LANDED witnesses are seam-stopping + read-verified, NOT a single end-to-end `api.resume` call. Specifically: the **dispatcher** branches (RE_FIRE / SKIP_AS_FIRED / ABORT / key-mismatch) are witnessed through the REAL `RuntimeToolDispatcher.dispatch` (no proxy — `test_effect_fence.py`); the **driver** carrier-population + key-bound directive-threading through the REAL `execute_workflow` with a `ResumeContextHolder` **stand-in** (`test_workflow_driver_effect_fence_pause.py`); the `api.resume → mcp_server `holder.set` → execute_workflow` holder handoff is **READ-VERIFIED** at `mcp_server.py:364` (the production-proven HITL one-shot delivery mechanism this reuses), NOT exercised e2e. A future `api.resume` RE_FIRE round-trip e2e would close the one remaining seam (the real holder api.resume sets == the one the driver peeks); low-risk (reuses the proven HITL delivery), registerable if it ever bites. The original §5 plan (the intended shape):
