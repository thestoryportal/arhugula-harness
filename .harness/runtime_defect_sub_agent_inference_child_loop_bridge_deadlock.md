...
| Status | **RESOLVED 2026-07-19** — the B-48 sync sub-agent dispatch offload executor (Runtime spec v1.102 §14.8.10; `harness_runtime.lifecycle.sub_agent_dispatch_executor.SubAgentDispatchExecutor`) moves the sync `SUB_AGENT_DISPATCH` worker off the loop-bound HITL-gate bridge thread onto a dedicated executor thread, so the nested `run_coroutine_threadsafe` bridge for the INFERENCE child no longer contends with the outer bridge for the same loop slot. The anchor test flipped XFAIL(strict=False) → **XPASS** against real Ollama (`llama3.2:3b`) and was promoted to a plain `PASS` (xfail marker removed) per filing §4 item 8 of `.harness/class_2_fork_b48_sync_subagent_dispatch_offload.md`. |
...
| Anchored test | `harness-runtime/tests/integration/test_u_cp_89_hierarchical_delegation_live_e2e.py::test_u_cp_89_hierarchical_delegation_depth2_live_ollama` — was `@pytest.mark.xfail(strict=False)`; **XPASSed and promoted to plain PASS 2026-07-19** (xfail marker removed) as the resolution signal. |
