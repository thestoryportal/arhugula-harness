...
- The 3 translate fns gain a system parameter — `_payload_to_{provider}_kwargs(payload, system: str | None)` — and inject **per-provider**:
  ...
  - **OpenAI / Ollama** → if `system` and no leading system message, **prepend** `{"role": "system", "content": system}` to the messages list.
...
|---|---|---|---|
| **#1** | **Runtime injection** — runtime spec new `C-RT-NN` §14.5-adjacent (translate-time system injection) + `HarnessContext` active-content resolution; impl at `_payload_to_*_kwargs` + dispatcher; **e2e proof (Ollama/mock)** the content reaches the provider. | runtime | The load-bearing gap. Closes "nothing reaches the model"; everything else is plumbing into it. Standalone-valuable (a single operator-supplied prompt injects even before selection/versioning land). |
| **#2** | **Versioning/authoring** — IS spec amendment: `PromptManifest`/`PromptVersion` identity → content + versioned store on `PROMPTS` path-class; content-addressed sha. | IS | Gives injection real content to resolve; extends the #496 carrier. |
