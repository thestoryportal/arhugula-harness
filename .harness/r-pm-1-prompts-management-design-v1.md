...
2. **Injection mechanism = bounded `HarnessContext` channel + translate-time per-provider injection** [HIGH] — NOT a change to the frozen `ProviderAgnosticPayload`. Resolves nameable **tension (ii)** (C10 contract-stability ⊥ C11 operator-burden) on **primary-source grounds**: a system prompt is *not uniformly representable* in the provider-neutral 3-tuple (Anthropic = top-level `system=` kwarg; OpenAI/Ollama = `role:"system"` message-list entry), so the per-provider translate functions are the correct seam. **ADR-F1 (read directly this session) is not merely untouched but honored** — §Decision commits to a thin core + *per-provider feature use at every call site*, and §Rationale(b) forecloses exactly the LCD-union move that lifting `system` into the neutral record would be → this is a fork-resolution + distributed spec amendments, **not a new ADR**.
...
**Probe-first resolution (the primary source decides — §10.9 amendment 5):** the `claude-api` skill confirms a system prompt is **not uniformly representable** in the neutral 3-tuple — Anthropic takes a **top-level `system=` kwarg**; OpenAI/Ollama take a **`role:"system"` entry in `messages`**; a `role:"system"` array entry is *not honored as a base system prompt by Anthropic* (it is a separate `mid-conversation-system-2026-04-07` beta, model-gated, cannot be `messages[0]`, 400s on unsupported models). Therefore a single `system` field on `ProviderAgnosticPayload` would be **provider-leaky** (the same field maps to a kwarg for one provider and a message-list entry for the others).

**ADR-F1 read directly (advisor-flagged; the pivot for the "no new ADR" call) [HIGH]:** `ADR-F1.md` §Decision commits to "a **thin core surface (generation, streaming, tool-use) plus a per-provider capability-introspection API at call sites**" — *not* to the complete provider-neutral input flowing through the 3-tuple. §Rationale(b) names the failure mode the ADR exists to prevent: "**LCD abstraction (LiteLLM-style portable API normalizing per-provider features into the union)** … erases the persona's feature affinity." §Consequences(a): "**Per-provider feature use … at every call site**, exposed via capability introspection." The C-CP-01 code-realization docstring agrees: "*No provider-specific field is lifted into the record*; C-CP-01 §1.4 defers the provider-adapter binding library to implementation discretion." So the discriminating question (does the committed surface require the *complete* input through the 3-tuple, or only that *no provider-specific field is lifted into the record*?) resolves to the **latter**. **This makes Option 1 not merely ADR-safe but ADR-faithful, and Option 2 the anti-pattern ADR-F1 forecloses:** placing `system` at the per-provider translate seam IS "per-provider feature use at every call site"; lifting a provider-divergent `system` into the neutral record IS the LCD-union feature-erasure move §Rationale(b) prevents.

**RESOLUTION: Option 1** — bounded `HarnessContext` channel + translate-time per-provider injection. **`surfaced + probe-resolved`.** ADR-F1 **preserved and honored** (verified at `ADR-F1.md` §Decision/§Rationale(b)/§Consequences(a), not from the code cite); this is a fork-resolution + distributed spec amendments, **not a new ADR**. [HIGH]
...
- **Content source for PR #1 (self-contained — Codex finding):** PR #1 cannot prove injection e2e if the content store is deferred to PR #2. So **PR #1 adds a minimal inline content carrier** — an optional `content: str` on `PromptVersion` — so a single operator-supplied active prompt injects + proves e2e *within PR #1*. PR #2 (versioning/authoring) then **generalizes** this inline field into the multi-version `PROMPTS`-path-class store + content-addressing. The inline carrier is the minimal thing that makes PR #1 standalone-valuable and self-provable; #2 is a superset, not a redo.
- **`content` ↔ `version_sha` derive-invariant (advisor-flagged correctness gap) [HIGH]:** `PromptVersion.version_sha` **already participates in the C-IS-05 §5.2 procedural-tier content hash** (cleared at IS v1.5 last session). So `version_sha` MUST be **derived from `content`** (`version_sha = digest(content)`) — otherwise injected content can change while the procedural-tier hash reports "unchanged" (a silent provenance/replay-integrity gap). PR #1 specs this invariant: content and its sha move together; the sha is not an independent operator-set field.
- **#496-carrier amendment wrinkle (advisor-flagged) [HIGH]:** `PromptVersion` is frozen / `extra="forbid"` and was **cleared last session (IS v1.5)**. Adding `content: str` is a **forward amendment to a just-cleared contract** — so "the #496 binding is the foundation, not a redo" carries a precise wrinkle: PR #1 *modifies the #496 carrier's shape*, it doesn't merely consume it. → a **clearance marker is owed for the carrier-shape amendment** at PR #1 (IS spec bump), and the field is **optional with an empty/None default** so existing empty-carrier configs are forward-compatible (no hash rebase for configs with no content; configs that newly supply content rebase forward, as #496's recipe-widening already established).
...
- **Conflict precedence — fail-loud / detect-then-refuse (Codex finding):** an active prompt is the harness-owned **base** system prompt. If an active prompt is configured **AND** the payload already carries a competing system source — an OpenAI/Ollama leading `role:"system"` message, or an Anthropic `params["system"]` (the opaque escape hatch) — the two-source ambiguity is **raised as a fail-loud error**, not silently resolved (consistent with the arc-#1 `detect-then-refuse` posture, `RT-FAIL-SANDBOX-DRIVER-UNAVAILABLE`, and `[[conformance-validator-disciplines]]`). v1 does **not** silently replace/merge — silently dropping either the operator's payload system message or the configured active prompt is the failure mode.
  - **Known, possibly-common consequence — frame honestly (advisor-flagged):** for OpenAI/Ollama a leading `role:"system"` message is the **idiomatic** way a workflow step supplies its own system prompt, so this is **not** a rare edge case — **configuring an active prompt will hard-error any workflow that already carries its own system message.** That is the *intended* v1 contract (surface the collision, don't silently pick), but it must be documented as a known operational consequence at the runtime amendment, not buried as edge handling. The escape valve is OQ-5 (an explicit `merge`/`replace` policy) if a real workload needs both sources simultaneously.
  - With **no** active prompt configured, behavior is byte-identical to today (existing payload system messages pass through untouched).
