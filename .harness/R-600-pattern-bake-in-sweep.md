...
**Five rounds of out-of-family Codex review, ten corrections, culminating in live, reproducible, and correctly-rendered/parseable re-verification.** Round 1 caught an audit-window misattribution + overstated arc-count (3→2). Round 2 caught round 1's own new errors (PR #972's axis + probe count). Round 3 asked for something neither round had provided: actual execution evidence that the PD-8 technique was performed on PR #972, not just historical self-report — closed by literally re-running the mutation-probe against current HEAD; it also flagged the file-delta audit's own reproducibility caveat once this PR's edits bumped the source memory file's mtime past the cutoff. Round 4 caught the trigger paragraph's hand-enumerated PR list undercounting (18 vs. the true 23, per `git log --first-parent --oneline 9cad319e..380ec161 | wc -l`) and the round-3 live-verification evidence's unreproducible `...::test_*` placeholder — fixed with a complete 5-step recipe. **Round 5 caught two purely mechanical defects in that recipe and its own surrounding artifacts:** the recipe's numbered list had been embedded inside a GFM/CommonMark table cell, whose embedded newlines silently closed the §7.5.1 gate table after its first data row (fixed by moving the recipe to its own subsection below the table); and the clearance marker's YAML frontmatter had unquoted `#`-prefixed PR references that PyYAML truncates as comments (confirmed by direct parse; fixed by quoting the affected list items). See `design-substrate/Project_Workflow_v1_18.md` §7.5.1 + §3(e) for the complete, exact, reproducible recipe and full per-round detail.
