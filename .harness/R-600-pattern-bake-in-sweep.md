...
**Cadence trigger.** `/loop run R-600-pattern-bake-in-sweep` (explicit operator/loop directive; the ~10-PR interval had already tripped well past threshold by the time this ran). **File-delta cutoff.** `main`'s git history was reorganized after cadence-8 landed, so its recorded merge commit (`323dfeae`) is unreachable via `merge-base --is-ancestor`; the cutoff instead uses that commit's own object timestamp (`2026-07-14T18:52:18-06:00`, resolvable via `git log --all`). A precise "commits since cadence-8" count is not reconstructable at all — the reorganization point (`2026-07-17T04:38:40-06:00`) postdates the cutoff itself, so every commit currently reachable via first-parent already postdates it by construction. This is immaterial to the gate: the ~10-PR trigger is satisfied at overwhelming margin regardless of the exact count, and no number is recorded here.

**Method.** Read-only enumeration of the auto-memory store at `/Users/robertrhu/.claude/projects/-Users-robertrhu-Projects-arhugula-v2/memory/` (146 files): grep raw `[[wiki-link]]` tokens across all file bodies; count per-token cardinality (total refs + distinct referencing files). 617 total refs / 114 distinct tokens → **79 at cardinality ≥2**, 35 at cardinality 1. File-delta set: **32 files** with mtime `>= 2026-07-14T18:52:18-0600` (new or substantively edited since cadence-8's true close-commit timestamp).
...
**Candidate 1 — `merge-gate-catches-what-codex-advisor-miss.md` (citation cardinality 3; internally documents 17+ dated named PR instances of the 3-lens `merge-gate` skill catching a defect codex+advisor missed — a distinct, much larger evidence-volume metric than its citation count).** Initial read treated this as the leading promotion candidate on that evidence volume alone. `advisor()`, consulted before drafting, corrected this: the volume is a **tool-usage catch-log for one specific workspace skill**, not independent recurrence of a *transferable* SDLC discipline — the same shape §7.5.1 gate 3 already rejected for `new-surface-audit-hash-and-config-not-carrier` (cadence-5) and Class-B'd for `codex-out-of-family-reviewer` / `fable5-fallback-reviewer` (cadence-6/7). Direct read of `.claude/skills/merge-gate/SKILL.md` confirmed its two genuinely transferable claims are already stated verbatim there (lens-decorrelation in the "Honesty caveat" section; the bare/missing-verdict fail-closed rule in "Parsing — fail closed"), and the lens-decorrelation framing is independently already codified at root `CLAUDE.md` §13.1. **Gate 3 FAILS for this token as a standalone candidate.** Not promoted — Class B, already homed.
...
**Other touched files (spot-checked by direct read, not assumed from filename/cardinality):** the remaining 27 delta files (32 total, minus 1 for candidate 1, minus 4 for candidate 2's family) are project-state bookkeeping (forward-register/roadmap/checkpoint edits from this session's own B-39 impl-leg Slice B arc), single-instance operator/tooling guidance already Class B, or edits to already-canonical entries at `MEMORY.md` that don't change domain-fit — including `branch-deletion-hard-blocked-by-permission-guard.md` and `full-register-ground-every-arc-before-blocking.md`, both already-indexed and requiring no fresh gate evaluation. None of the 27 cleared the §7.5 candidacy bar independently of the two candidates evaluated in full below.
...
**`merge-gate-catches-what-codex-advisor-miss` — deliberately NOT promoted.** See C9-§1 candidate 1 above; a large evidence volume is a salience proxy, same as citation count — it failed gate 3 once its content was checked against existing homes rather than counted.
