...
**Latest status:** **ACTIVE-SURVEYED (cadence-9, 2026-07-24)** — cadence-9 survey complete (both `must_pass` met); promoted a 4-signature family (`reviewer-oscillation-register-and-hold`, `non-convergent-adversarial-hardening-arms-race`, `over-correction-away-from-mostly-right-baseline`, `self-referential-review-loop-discriminator`) as **PD-9 adversarial-review-loop non-convergence discriminators** in `design-substrate/Project_Workflow_v1_19.md`, clearance marker `.harness/clearance/Project_Workflow-v1_19-cleared-2026-07-24.md` — `advisor()` redirected the initial candidate pick away from the highest-raw-citation token (`merge-gate-catches-what-codex-advisor-miss`, already homed elsewhere, NOT promoted) toward this genuinely uncovered family. Prior: cadence-8 (2026-07-14) promoted `mutation-probe-load-bearing-witness` as **PD-8 mutation-probe-as-load-bearing-witness** in `design-substrate/Project_Workflow_v1_18.md`, clearance marker `.harness/clearance/Project_Workflow-v1_18-cleared-2026-07-14.md`; cadence-7 was a legitimate zero-promotion cadence; cadence-5 promoted `disposition-label-is-a-claim-verify-against-spec` as **PD-7 disposition-label-is-a-claim** in `design-substrate/Project_Workflow_v1_17.md`, clearance marker `.harness/clearance/Project_Workflow-v1_17-cleared-2026-07-12.md`. R-600 remains a recurring lane; next run only after the cadence trips again (~10 PRs) or a concrete new candidate reaches independent instance-cardinality >=2.
...
1. **Instance-cardinality >=2 of independent arcs — PASS, with an honesty caveat (mirrors the cadence-2 PD-5 precedent).** The memory file's own prose listed 3 named PRs (#927, #933, #972); verified against `gh pr view --json mergedAt` + each commit's `Claude-Session` trailer rather than taken at face value, per **PD-7's own discipline applied to itself**. PR #927 and PR #933 (both 2026-07-11) share one `Claude-Session` trailer — same session, same underlying lesson, applied to two sibling PRs in the `B-18-3C-PREWARM` family — and collapse to ONE instance per the cadence-2 same-session discriminator. PR #972/`B-26` (2026-07-14, CP-axis, `fix(cp)` wiring real IS hash-chain verification into `harness_cp/five_axis_composition.py`, a CP→IS seam) is a wholly separate session with 2 discriminating-witness probes on direct read of the test diff. The fully-corrected count is **2 genuinely independent instances**, clearing the literal ">=2" gate at the thinnest margin of any PD promoted so far — flagged honestly rather than overstated.
...
Evidentiary claims were corrected across five rounds of out-of-family Codex review before merge — see `design-substrate/Project_Workflow_v1_18.md` §7.5.1 + §3(e) for the reproducible recipe and commit history for per-round detail.
...
# Cadence-9 run — 2026-07-24 (HEAD `e5529e8a`, post-#1099)

**Cadence trigger.** `/loop run R-600-pattern-bake-in-sweep` (explicit operator/loop directive, not a `~10-PR` auto-trip check — the cadence had already tripped well past the interval by the time this ran). **File-delta cutoff resolution (corrected at codex round 1 of this arc's own PR):** the natural approach — `git merge-base --is-ancestor <cadence-8-merge-commit> HEAD` — returned NOT-ANCESTOR for the cadence-8 PR's recorded merge commit (`323dfeae`) despite `gh pr view` confirming that PR as merged, consistent with a git-history reorganization after cadence-8 originally landed. A first draft substituted a same-day-midnight approximation (`--since="2026-07-15T00:00:00"`) and treated exact-SHA reconstruction as out of scope — codex round 1 caught that this leaves a same-day gap and that `git log --all` still resolves `323dfeae`'s own commit timestamp directly even though ancestor-chain reconstruction fails: `323dfeae` = `2026-07-14T18:52:18-06:00`. **Merge count — further correction at codex round 3, resolving a deeper obstacle than rounds 1-2 addressed.** Round 3 reproduced the round-2 command against the fixed anchor `e5529e8a` and got **121**, not the 50 this artifact had recorded — re-running it live confirms round 3's number. Root cause, verified directly: `git rev-list --first-parent --count e5529e8a` (no `--since` filter at all) ALSO returns **121**, and `main`'s own first-parent history terminates at `2ab2ef0b` (`2026-07-17T04:38:40-06:00`) — i.e. `main`'s reachable git history does not extend back to cadence-8's close (`2026-07-14`) at all. This is the SAME git-history-reorganization phenomenon already surfaced by the `merge-base --is-ancestor` failures on `323dfeae` above, now shown to be more extensive than "cadence-8's specific merge commit is unreachable" — the reorganization point (`2026-07-17`) is itself 3 days AFTER cadence-8's close, meaning no commit-count "since cadence-8 close" is reconstructable from current `main` at all; every commit currently reachable via first-parent already postdates the cutoff, by construction. **Honest, defensible statement: all 121 commits on current `main` postdate cadence-8's close** (satisfying the ~10-PR cadence trigger with enormous margin regardless of the exact pre-cutoff count), and the earlier "50" and "122" figures were both artifacts of transient/inconsistent local git state while probing an unreconstructable range — neither is trustworthy and both are superseded here rather than reconciled further, per this very PD-9's own self-referential-drift discriminator (chasing an unreconstructable exact count past the point it changes the outcome is the failure mode PD-9 itself warns against).

**Method.** Read-only enumeration of the auto-memory store at `/Users/robertrhu/.claude/projects/-Users-robertrhu-Projects-arhugula-v2/memory/` (146 files): grep raw `[[wiki-link]]` tokens across all file bodies; count per-token cardinality (total refs + distinct referencing files). 617 total refs / 114 distinct tokens → **79 at cardinality ≥2**, 35 at cardinality 1. File-delta set: **32 files** with mtime `>= 2026-07-14T18:52:18-0600` (new or substantively edited since cadence-8's true close-commit timestamp — corrected from an initial 30-file, midnight-cutoff count per the codex round-1 finding above).

> **Metric caveat (unchanged from cadence-1 through cadence-8, checked fresh, not inherited).** Citation-count is a salience proxy, not instance-cardinality. The `>=2` filter is over-inclusive by design; true promotion-cardinality is confirmed per-candidate by direct content read against named, dated instances — never assumed from the raw count.

---

## C9-§1 — `must_pass` #1: file-delta audit against the cadence-8 cutoff

Of the 32 files touched since the corrected cutoff, the overwhelming majority are Class A/B (project-state bookkeeping, single-instance session/tooling guidance, or edits reinforcing an already-dispositioned home) — consistent with every prior cadence. Two Class-C candidates stood out on first pass and were evaluated in full:

**Candidate 1 — `merge-gate-catches-what-codex-advisor-miss.md` (card. 3; internally documents 17+ dated named PR instances of the 3-lens `merge-gate` skill catching a defect codex+advisor missed).** The highest raw-evidence-volume candidate in R-600's 9-cadence history. Initial read treated this as the leading promotion candidate on citation strength alone. `advisor()`, consulted before drafting per the standing pre-substantive-work discipline, corrected this: the evidentiary volume is a **tool-usage catch-log for one specific workspace skill** (`merge-gate`), not independent recurrence of a *transferable* SDLC discipline — the same shape §7.5.1 gate 3 already rejected for `new-surface-audit-hash-and-config-not-carrier` (cadence-5, domain-specific runtime mechanics) and Class-B'd for `codex-out-of-family-reviewer` / `fable5-fallback-reviewer` (cadence-6/7, "reviewer-ladder tooling guidance, not a §7.5 SDLC discipline"). Direct read of `.claude/skills/merge-gate/SKILL.md` confirmed its two genuinely transferable claims are **already stated verbatim** in that file's own text: the "Honesty caveat" section states lens-decorrelation explicitly ("These three subagents are... lens-decorrelated, not vendor-decorrelated... This gate does not replace either — it runs alongside them"), and the "Parsing — fail closed" section states the bare/missing-verdict discipline explicitly ("Missing, malformed, or ambiguous → treat as BLOCK... Never read a silent/truncated/off-format response as approval"). The lens-decorrelation framing is independently ALSO already codified at root `CLAUDE.md` §13.1 (advisor+codex "the two are decorrelated ... the strongest signal is when they disagree"). **Gate 3 (no canonical home elsewhere) FAILS for this token as a standalone candidate.** Not promoted — Class B, already homed, consistent with the codex-out-of-family-reviewer / fable5-fallback-reviewer precedent rather than the PD-8 precedent.

**Candidate 2 — the review-loop non-convergence family (4 files, 2 new this window + 2 substantively extended):** `reviewer-oscillation-register-and-hold.md`, `non-convergent-adversarial-hardening-arms-race.md`, `over-correction-away-from-mostly-right-baseline.md`, `self-referential-review-loop-discriminator.md`. `advisor()` pointed at this cluster as the genuinely uncovered gap once candidate 1 was disqualified: four memories, each naming a *distinct signature* for why an iterative adversarial review loop fails to converge — but none of them is covered by PD-3 (verification-shape depth), PD-6 (composed-chain witness), or PD-8 (mutation-probe test-discrimination), all three of which verify whether **one finding, once made, is correct**, not whether **continued iteration on the loop itself** is still producing signal. Full evaluation at C9-§2.

**Other touched files (spot-checked by direct read, not assumed from filename/cardinality):** the remaining 27 delta files (32 total, minus 1 for candidate 1, minus 4 for candidate 2's family) are project-state bookkeeping (forward-register/roadmap/checkpoint edits from this session's own B-39 impl-leg Slice B arc), single-instance operator/tooling guidance already Class B, or edits to already-canonical entries that don't change domain-fit. Two of these 27 were missed by the file-delta audit's first (uncorrected) cutoff — `branch-deletion-hard-blocked-by-permission-guard.md` and `full-register-ground-every-arc-before-blocking.md`, both edited between cadence-8's true close (18:52:18) and the midnight-approximation cutoff originally used (see cadence trigger note above) — both are pre-existing, already-indexed entries at `MEMORY.md` (Class B operational recipe; Class A feedback already codified into `CLAUDE.md` §12.4.1 respectively), requiring no fresh gate evaluation once correctly included. None of the 27 cleared the §7.5 candidacy bar independently of the two candidates evaluated in full below.

## C9-§2 — `must_pass` #2: candidates evaluated against §7.4.7 and §7.5

**§7.4.7 (stale-carry-text):** ZERO new sub-species owed. This is the **9th independent surfacing** of the same null result across cadence-1 through cadence-9, checked fresh against this cadence's file-delta set.

**§7.5 (process-discipline) — one candidate (a 4-signature family) clears the gate:**

**Review-loop non-convergence family → PD-9 PROMOTED (as one entry, four signatures).** Run against the §7.5.1 inclusion gate (verified per-candidate, not by citation count):

1. **Instance-cardinality >=2 of independent arcs — PASS, wide margin (PD-7's own clearance recorded 8 independent arcs; not claimed to exceed that here, a codex round-3 correction of an initial overstated superlative).** At least **6** genuinely independent arcs, verified by direct read of each memory file's own cited evidence: (a) the prewarm/keepalive-disable scope fork (5+ oscillating rounds → registered as `B-55`, confirmed closed at `.harness/forward-register.yaml:1225`) — cited from BOTH `reviewer-oscillation-register-and-hold` AND `self-referential-review-loop-discriminator`'s "Hold-the-fork remedy" note, same underlying arc, deduped to ONE instance per the standing same-arc-collapse discriminator; (b) the concurrent-resume-witness fork (rounds 22/33/35 → moved wholly into `B-39` scope) — a separate arc, separate register row; (c) PR #1079 — 6 rounds of `just codex-review` hardening a permission-guard exclusion against an inexhaustible finding supply; (d) the `B-33-A` spec-leg Q4-without-incoherent-threat-model instance — a separate arc from (c) sharing the same discriminator shape; (e) `B-40` (2026-07-15) — 3 rounds of a register-row "correction" each REVERSING rather than narrowing the prior disposition; (f) this very workflow doc's own PD-8/v1.18 promotion arc (2026-07-14) — rounds 5-6 of its own out-of-family review drifted from substance to auditing its own correction-narrative's changelog count. A 7th, weaker-fit supporting instance — this session's own `B-39` impl-leg Slice B `B-72` finding, re-raised identically at codex rounds 6/7 and correctly resolved via register-and-hold (2026-07-24) — is cited as a recent application of remedy (a), not counted toward the cardinality total (it is a confirmed re-raise of an already-grounded finding, not a flip-flopping fork).
2. **Genuinely §7.5-shaped — PASS.** A review-execution SDLC discipline: recognizing *why* an adversarial review loop is failing to converge (four distinct signatures) and choosing the *matching* remedy, transferable across all four H_T axes and both postures — not domain-specific runtime mechanics.
3. **No canonical home elsewhere — PASS with cite-don't-relocate to PD-3, PD-6, and PD-8.** All three verify a single finding's correctness once made; none addresses whether continued iteration on the loop itself is still producing signal. Direct read of the full v1.18 §7.5.2 catalogue (PD-1 through PD-8) confirmed no existing entry covers loop-convergence recognition. Root `CLAUDE.md` §13.1/§13.2 covers *which* reviewer mechanism to invoke, not *when to stop* invoking one — also checked directly, no overlap.

**Promotion applied this arc:** `design-substrate/Project_Workflow_v1_19.md` adds **PD-9 adversarial-review-loop non-convergence discriminators** (4 lettered signatures: oscillation / arms-race hardening / over-correction / self-referential drift, one catalogue entry — kept as a single entry rather than fragmenting into four standalone PDs, per `advisor()`'s explicit steer against catalogue fragmentation). Clearance marker filed at `.harness/clearance/Project_Workflow-v1_19-cleared-2026-07-24.md`. Root `CLAUDE.md` §2.1 + §10.2 and `.harness/claude-artifact-pointers.md` now point to workflow head v1.19 — both pointers were verified current at v1.18 (no staleness regression since the cadence-8 fix) before this bump.

**`merge-gate-catches-what-codex-advisor-miss` — deliberately NOT promoted.** See C9-§1 candidate 1 above; this is the sharpest empirical instance yet of the standing "citation count is a salience proxy, not proof of a gap" caveat — the highest raw-instance-count candidate surveyed across all 9 cadences failed gate 3 outright once its content was checked against existing homes rather than counted.

## C9-§3 — Closure disposition (cadence-9)

- **`must_pass` #1** (cardinality>=2 frontier identified + file-delta audit): ✅ C9-§1.
- **`must_pass` #2** (candidates evaluated against §7.4.7 + §7.5): ✅ C9-§2 — result: ZERO new §7.4.7 owed (9th surfacing); **one §7.5 PD-9 promoted at workflow v1.19** (4-signature family), one high-citation candidate correctly rejected at gate 3 after `advisor()` redirection.
- **Entry status:** **ACTIVE-SURVEYED (cadence-9)**. Cadence-9's PD-9 promotion is discharged in the same arc; both governance pointers verified non-stale before bumping. R-600 remains an ACTIVE recurring lane; next run only after the cadence trips again (~10 PRs) or a concrete new candidate reaches independent instance-cardinality >=2 as a named, consolidated entry.
