...
1. **Instance-cardinality >=2 of independent arcs — PASS.** At least **5** genuinely independent arcs, verified by direct read of each memory file's own cited evidence: (a) the prewarm/keepalive-disable scope fork (5+ oscillating rounds → registered as `B-55`, confirmed closed at `.harness/forward-register.yaml:1225`) — cited from BOTH `reviewer-oscillation-register-and-hold` AND `self-referential-review-loop-discriminator`'s "Hold-the-fork remedy" note, same underlying arc, deduped to ONE instance per the standing same-arc-collapse discriminator; (b) the concurrent-resume-witness fork (rounds 22/33/35 → moved wholly into `B-39` scope) — a separate arc, separate register row; (c) PR #1079 — 6 rounds of `just codex-review` hardening a permission-guard exclusion against an inexhaustible finding supply; (d) `B-40` (2026-07-15) — 3 rounds of a register-row "correction" each REVERSING rather than narrowing the prior disposition; (e) this very workflow doc's own PD-8/v1.18 promotion arc (2026-07-14) — rounds 5-6 of its own out-of-family review drifted from substance to auditing its own correction-narrative's changelog count. Two supporting, not-counted data points: `B-33-A`'s Q4-without-incoherent-threat-model observation motivated the arms-race predicate's mandatory-(i) condition rather than counting as its own (b)-instance; this session's own `B-39` impl-leg Slice B `B-72` finding, re-raised identically at codex rounds 6/7 and correctly resolved via register-and-hold (2026-07-24), applied remedy (a) correctly but is a confirmed re-raise, not a flip-flopping fork.
