...
**Four rounds of out-of-family Codex review, eight corrections, culminating in live and reproducible re-verification.** Round 1 caught an audit-window misattribution + overstated arc-count (3→2). Round 2 caught round 1's own new errors (PR #972's axis + probe count). Round 3 asked for something neither round had provided: actual execution evidence that the PD-8 technique was performed on PR #972, not just historical self-report (commit message + test docstrings) — closed by literally re-running the mutation-probe against current HEAD; it also flagged that this PR's own corrections had bumped `mutation-probe-load-bearing-witness.md`'s mtime past the cadence-7 cutoff, meaning a fresh file-delta re-run would no longer reproduce the 6-file result above — clarified in the file-delta audit note as the original snapshot. Round 4 caught two final gaps: the trigger paragraph's hand-enumerated PR list undercounted (18 vs. the true 23, per `git log --first-parent --oneline 9cad319e..380ec161 | wc -l` — missing #975/#977/#979/#980/#981, each a terminating refresh the hand-list skipped; fixed by citing the reproducible command rather than hand-listing again, the same fix-shape as the failure it was correcting); and the round-3 live-verification evidence used a `...::test_*` placeholder instead of the complete runnable pytest command + exact mutation/restore procedure, making it unreproducible as written — see `design-substrate/Project_Workflow_v1_18.md` §7.5.1 gate table's "Live re-verification" entry for the complete, exact, reproducible 5-step recipe (full test node paths, exact insertion point and line, exact expected output at each step).
