...
**Status:** APPLIED — CP spec v1.107 → v1.108 (§1 property 6) + CP plan v2.43 → v2.44 + 2 clearance markers landed 2026-07-25 on branch `b72-cp-spec-leg-property-6`. 5 rounds of out-of-family `just codex-review` (base=main) to convergence — round 1 caught the plan row omitting delivery-cell construction + an incomplete canonicalization bundle (both fixed); round 2 caught property 6's internal identity lacking tree-wide uniqueness + a stale `child_run_id`-based close-out recipe + stale pointer-table rows (all fixed); round 3 caught property 6 permitting "counted but never delivered" for the sole-branch case (fixed at CP spec v1.108 §1.1(b), which now REQUIRES delivery as an explicit outcome, not merely counting + never-keying) plus this status field lagging the applied state (fixed by this line) plus an overbroad `B-71` dependency label on the register (fixed — only item (2) is `B-71`-gated, not (3)/(4)); round 4 caught the new carrier's existence/byte-compat treatment being left to unauthorized impl discretion despite `snapshot_hash` participation (fixed — NEW §1.3a pre-authorizes ONE additive, drop-when-empty/`None` hash-strip-scoped field, mirroring the `v1.97`/`v1.99` precedent); round 5 caught this dashboard's own staleness (fixed at `.harness/roadmap_status.md`'s round-8 update) plus property 6's text overclaiming topology-agnostic scope when only `PARALLELIZATION`/`ORCHESTRATOR_WORKERS` are grounded by the round-3 repro (fixed — NEW §1.2b explicitly scopes property 6 to the two fan-out topologies; `EVALUATOR_OPTIMIZER`/`DECENTRALIZED_HANDOFF`'s sequential carriers are a distinct, ungrounded, out-of-scope gap for a future arc). This Class 1 halt is CLEARED; downstream sessions should treat CP spec v1.108 / CP plan v2.44 as canonical, not as still-pending this record's recommendation.
