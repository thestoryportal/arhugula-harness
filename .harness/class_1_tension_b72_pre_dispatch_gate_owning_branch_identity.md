...
**Status:** APPLIED — CP spec v1.107 → v1.108 (§1.2 property 6) + CP plan v2.43 → v2.44 + 2 clearance markers landed 2026-07-25 on branch `b72-cp-spec-leg-property-6`. 3 rounds of out-of-family `just codex-review` (base=main) to convergence — round 1 caught the plan row omitting delivery-cell construction + an incomplete canonicalization bundle (both fixed); round 2 caught property 6's internal identity lacking tree-wide uniqueness + a stale `child_run_id`-based close-out recipe + stale pointer-table rows (all fixed); round 3 caught property 6 permitting "counted but never delivered" for the sole-branch case (fixed at CP spec v1.108 §1.1(b), which now REQUIRES delivery as an explicit outcome, not merely counting + never-keying) plus this status field lagging the applied state (fixed by this line) plus an overbroad `B-71` dependency label on the register (fixed — only item (2) is `B-71`-gated, not (3)/(4)). This Class 1 halt is CLEARED; downstream sessions should treat CP spec v1.108 / CP plan v2.44 as canonical, not as still-pending this record's recommendation.
