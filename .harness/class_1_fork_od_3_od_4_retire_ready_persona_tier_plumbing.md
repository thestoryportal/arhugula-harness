...
**Status:** ✅ APPLIED-AS-READING-(α) (operator AskUserQuestion 2026-05-28 ratified Q1=A + Q2=A + Q3=a + Q4=i + Q5=α; single bundled binding-lift arc). Runtime spec v1.36 → v1.37 + OD spec v1.25 → v1.26 + OD plan v2.24 → v2.25 + production binding at `harness-runtime/types.py` + `harness-runtime/lifecycle/tracer_provider.py` + `harness-runtime/lifecycle/span_processor.py` + `harness-od/redaction_span_processor.py` + 16 NEW tests at `tests/test_persona_tier_plumbing.py` + workspace `CLAUDE.md` §2.3 + §2.4 row bumps + this Status update co-published 2026-05-28. H_T-OD-3 + H_T-OD-4 remain **PARTIAL (refined)** per X-AL-2 (one-and-a-half-of-four gates closed across two rows; both rows stay at PARTIAL). OD-3 gate (b) §10.3 base_rate envelope CLOSED; OD-3 gate (a) §9.1 tail-keep still deferred. OD-4 gate (a) §13.1 PARTIALLY CLOSED (deployment-level persona_tier + multi-tenant non-toggleability); per-session toggle + gate (b) §13.2 still deferred. Tier transit to RETIRE-READY requires follow-on arcs. Workflow v1.12 §7.4.7.3.C audit applied at `harness-od/CLAUDE.md` §4.1 + cumulative-counts line refreshed; pipeline-advanced 6/8 = 75.0% UNCHANGED. 3367/3367 tests pass + 10 skipped.
