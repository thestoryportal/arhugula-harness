...
| CI wall clock after Task 4 — content-merge `main` run 33796026184 (the #1507 squash touched `tools/`, so nothing was skipped) | 489 s (`createdAt` 19:22:13Z → `updatedAt` 19:30:22Z); 22 jobs, 0 `skipped` | `gh run view 33796026184 --json createdAt,updatedAt,jobs`
| CI wall clock after Task 4 — refresh-merge `main` run 33796976912 (#1508) | 62 s (`createdAt` 19:31:58Z → `updatedAt` 19:33:00Z); 17 jobs, 5 `skipped` | `gh run view 33796976912 --json createdAt,updatedAt,jobs`
| Loop cost baseline at `3c7f74448` (main after #1508; the b-230-task-5 branch point, before this arc's review rounds) | 2605 rows / 79 arcs (77 reviewed); median 6, max 24; gate rounds with findings 50, single-lens 36; accepted-only unique catches witness-adequacy 20 / spec-conformance 10 / concurrency 0 (raw 34; rejected-or-suppressed 4; unadjudicated 0); lease-acquire events 2 | `uv run python tools/loop_cost_baseline.py`
| CI wall clock after Task 5 — content-merge `main` run 33810518768 (the #1509 squash touched `tools/`, so nothing was skipped) | 515 s (`createdAt` 21:55:37Z → `updatedAt` 22:04:12Z); 22 jobs, 0 `skipped` | `gh run view 33810518768 --json createdAt,updatedAt,jobs`
...
- **Design picked at the b-230-task-2 arc.** The tests live in the suites that already own both functions — `tools/hooks/test_lib.sh` (its `hook_roadmap_next` cases) and `tools/hooks/test_loop_lib.sh` (§23 NOTIFY cases) — not the two new files named under Files: one home per function's tests, and both suites are already globbed by `tools/codex-parity-check.sh`. The precedence in `hook_roadmap_next` is a rule table (`scope|carrier regex|strip program`, first non-range match wins), not a branch ladder — ordered plan pointer → the explicit `next implementable unit is` phrase → first bold unit → FIRST `then ` → section-wide backticked, NOT the (r3) order above, which ranks the most explicit marker last and misfires on two live-adjacent shapes (a bold LANDED unit before the phrase returns the landed unit; an unquoted phrase with a `then ` tail returns the successor's successor) — both are fixtures now. The dedupe is the first awk's `END` block, keyed on the rendered `[lane] detail` line, so the same detail from two lanes stays two notices. The plan's rows-1-and-8-of-nine notify fixture passes on the UN-deduped reducer (row 1 falls off the `tail -5` cap regardless — the fourth plan-skeleton defect, in a test this time) and was replaced by a repeat INSIDE the cap window (rows 3 and 6 of seven), which also witnesses dedupe-before-cap: B-2, which a post-cap dedupe would hide, surfaces. Codex r1 (P1, absorbed): `tools/test_roadmap_status_consumers.py` (CI-run) mirrored the parser's contract in Python and asserted an empty token on the live file whenever the current line has no backticked unit — a second copy of the contract that drifted the moment the carriers widened; its accepted set is now every unit id or backticked plan pointer on the current line (membership, not a re-implementation of precedence). That pre-existing live-file test stays; the "no assertion against the live file" constraint above governs the new cases only.

- [x] **Step 1: Write the failing tests** — landed as four `hook_roadmap_next` cases appended to `tools/hooks/test_lib.sh` and four NOTIFY cases (§23d) appended to `tools/hooks/test_loop_lib.sh`; the sketch below is the plan's original shape.
...
- [x] **Step 2: Run them to verify they fail** — `test_lib.sh` 3 FAIL (bold, first-then, plan pointer; the quoted case already passed), `test_loop_lib.sh` FAIL on the three-row repeat.
- [x] **Step 3: Extend the parser and the reducer** per the constraints.
- [x] **Step 4: Run the tests** — `test_lib.sh` PASS=156 FAIL=0; `loop_lib: 150 passed, 0 failed`; live witness printed `plan:loop-optimization-plan-2026-09-03` (§0 row above). Original expectation: `ok` twice. Witness on the live file: `bash -c 'source tools/hooks/lib.sh; hook_roadmap_next .harness/roadmap_status.md'` prints the unit the pointer paragraph names as next (the plan pointer once Task 9's refresh installs it; `U-HE-37` for the post-#1497 prose), and a fresh prompt shows the same in `[roadmap] next=`; a new session with a `.codex-worktrees/` lane present shows its notice once.
- [x] **Step 5: Commit** — as `fix(hooks): prompt-context next= recognizes plan pointers and bold units; notify reducer dedupes in order` over `tools/hooks/{lib,loop_lib,test_lib,test_loop_lib}.sh`. Original: `git add tools/hooks/lib.sh tools/hooks/loop_lib.sh tools/hooks/test_lib_roadmap_next.sh tools/hooks/test_loop_lib_notify.sh` and `git commit -m "fix(hooks): prompt-context next= recognizes plan pointers and bold units; notify reducer dedupes in order"`.
...
- **codex r1 on b-230-task-4 (absorbed).** [P1] `verify_all` checked two of the four facts the ship-pr close-out block requires before ONE delete; it now checks all four per branch — PR `MERGED`, `baseRefName` equal to `gh repo view`'s default branch, `headRefName` equal to the row's branch, and the merge commit's own `main` run green through `arc_metrics.ci_is_green` (the one C-HE-19 predicate) — so a deferral naming a PR merged into a side branch, or one whose post-merge run was cancelled, cannot become a force-delete. [P2] the row grammar consumes the WHOLE pair list (`A (…) and B (…)`, optional ` — <note>`): a truncated or malformed second pair is `UnreadableRow`, never a one-branch item whose refresh branch would be hidden once the first is resolved. **Residual (recorded, not absorbed):** [P2] `--resolve` appends `RESOLVED-HIL` for an item id read from the reducer snapshot at the start of the run; a `DEFERRED-HIL` for the same id appended by another lane between the snapshot and the append would be cleared by last-write-wins. Item ids are arc ids of merged arcs, so the window needs a re-deferral of a closed arc's id for a different gate during the seconds of one interactive run; the lean protocol records it as the residual class rather than adding a ledger-lock re-read (every bound added is new defect surface).
...
**Files:** modify `tools/merge_gate_log.py` (an `emit-all` subcommand), `justfile` (a `merge-gate-emit-all *ARGS` forwarder next to `merge-gate-emit` at line 359), `tools/hooks/permission-guard.sh` and `tools/hooks/test_permission_guard.sh` (the loop-mode allowlist), BOTH merge-gate carriers — `.claude/skills/merge-gate/SKILL.md:171-195` and `.agents/skills/merge-gate/SKILL.md` (the plan as written said the Codex directory holds no `SKILL.md`; it has held one since #1479 — the third plan-skeleton defect in this program, after Task 3's phantom guard entries and Task 4's hostile-name test) — with the parity witness extended in `tools/test_codex_workflow_parity.py`; test `tools/test_merge_gate_emit_all.py` (wired into `tools/codex-parity-check.sh`).
...
- Two skill carriers, not one (the constraint as first written was wrong — see Files): both name `emit-all` as the one recording call and the per-lens `merge-gate-emit` ONLY as the repair path, and the parity suite pins both.
- **Design picked at the b-230-task-5 arc: (b), no resumption.** The JSONL is the only record; a resume file would be a second representation that can drift from it, and the `emit-all` loop then has no branch at all — the same three operations every run, exit `max` of the three. A single unrecorded lens is repaired with the existing per-lens `emit` (already guard-allowlisted), never by re-running `emit-all`, which would mint a fresh round for the two lenses that were fine. Under (b) the r5 P1 split-verdict rule moves from code to the carriers' prose: after a repair the gate outcome is the worst of the three RECORDED rows at the reviewed head, never the last exit seen. Plan tests (c) and (d) were (a)-shaped and are replaced by a real-path witness that a second run re-emits all three (six rows) and a mixed 0/1/2 → 2 table.

- [x] **Step 1: Write the failing test** — monkeypatch the single-emit function and record calls: (a) concurrency returns 1, the others 0 → all three emitted in order, exit 1; (b) the middle lens returns 2 → the third is still emitted, exit 2; (c) no resumption, real path: three lens files bound to HEAD, the first a BLOCK → three rows in lens order, exit 1; a second identical run appends three MORE rows (six, `LENSES * 2`) — nothing is skipped on the JSONL's say-so (`test_real_path_records_three_rows_per_run_and_never_skips`; replaces the (a)-shaped resume case the plan first listed here, superseded by the design (b) pick above); (d) a verdict file that cannot be read → that lens exits 2 and leaves no row, the other two ARE recorded, exit 2 (`test_a_missing_verdict_file_is_not_recorded_but_the_rest_still_are`; replaces the (a)-shaped `base_sha`-mismatch case); (e) `just --show merge-gate-emit-all` names the subcommand.
- [x] **Step 2: Run it, expect FAIL.** (12 failed: no `emit_verdict_file`, no `emit-all`, no recipe.)
- [x] **Step 3: Add the subcommand, the recipe, and the guard allowlist entry + tests.** The single `emit` body was lifted into module-level `emit_verdict_file(...) -> int`; `emit` calls it once, `emit-all` three times.
- [x] **Step 4: Run the tests, expect PASS** (including `bash tools/hooks/test_permission_guard.sh`).
- [x] **Step 5: Wire the test into `tools/codex-parity-check.sh`, update the skill text, commit** — `git commit -m "feat(just): merge-gate-emit-all"`.
...
- [x] **Step 6: One AskUserQuestion** with the table and the fail-open bound stated: (a) open the C-HE-34 spec leg for a detector-gated concurrency lens with the typed skip path above, so C-HE-29 accounting stays whole; (b) keep three lenses always. On (a): spec + marker PR first (doc-only), then the code PR (typed skip path + guard entry + tests) and the skill edit. On (b): Steps 1–5 stay as a measurement instrument and this task closes. **Answered (b) — operator, 2026-09-03:** keep three lenses always; no C-HE-34 spec leg; `tools/concurrency_surface.py` stays a measurement instrument. Task 6 CLOSED.
...
**Landed** at the b-230-task-8 arc (2026-09-04). Step 1: `wait_for_door` takes a required `lane_id` and, at the first `held` observation only, appends `NOTIFY` / `merge-door-lease-acquire:lease_held_yield` / `holder=<arc id> backoff=0` through `_notify` → `rs.emit_loop_row` (the holder read from `read_lease()` at that moment; `-` when the lease vanished in between); witnesses in `tools/test_merge_door.py` — three held observations then success → exactly one row and no gate-log write, a vanished holder → `holder=-`, and the CLI contention probe now asserts one yield row naming the foreign holder across its twelve held observations. Mutation probe: an every-retry emitter turns both witnesses red. Step 2: `B-232` (`held`, trigger-gated) with the half-open-window trigger text and the three cites; B-230 CLOSED in the same register edit. Step 3: `rolling_window_max` (two-pointer over the parsed `ts` column, half-open `[t, t + 30 d)`) → `lease_held_yields_30d_max`; boundary tests: six rows over 29 days → 6, six over 31 days → 5, exactly 30 days apart → 1, six spread over years → 1; the cause literal is pinned equal to `merge_door.LEASE_HELD_YIELD_CAUSE`. Not done here, by design: nothing in acquire, release, the gate log or the C-HE-06 invariants changed; `.harness/.lane-id` stays lane-init's. **Codex r1 (b-230-task-8), 3 P2 / 0 P1:** (1) `_notify` swallows a ledger write failure so a lost yield row undercounts the trigger — REJECTED with grounds: the row is telemetry, undercounting is the conservative direction, and a raise would let telemetry gate a landing (B-199 carries the persistent-failure residual); the chosen behaviour is now witnessed (`test_wait_for_door_survives_a_lost_yield_row`). (2) `design_substrate_gated` is a schedulable status, so the loop could select B-232 before its trigger — ACCEPTED: B-232 is `held` (a grounded, trigger-gated defer), flipped back when the trigger fires. (3) "re-evaluated at each roadmap refresh" had no operational carrier — ACCEPTED: `tools/lease_yield_trigger.py` (stdlib-only, imports the baseline's `yield_stamps` / `rolling_window_max` / `TRIGGER_THRESHOLD`) is run by `tools/roadmap-audit/session-start.sh` every session by its own path, printing `[b-232] lease_held_yields_30d_max=<n>/5` beside the HIL/NOTIFY segments and `TRIGGER FIRED` past 5; witnesses in `tools/test_lease_yield_trigger.py` and three `test_session_start.sh` cases (0/5, 6/5 fired, 5/5 not fired).
