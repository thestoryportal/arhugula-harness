...
**Status:** ✅ APPLIED-AS-READING-A 2026-05-29. Single-token edit at `harness-runtime/src/harness_runtime/cli/app.py:368-373` dropped `os.getpid()` from `_default_daemon_socket_path()`; now resolves to `${tempfile.gettempdir()}/harness-daemon.sock` (stable across processes). Docstring rewritten to disclose the single-daemon-per-host invariant + cite this fork doc. Adjacent finding (d) (silently-wrong docstring) closed via the rewrite. Test `test_ac1_default_socket_path_uses_pid_namespacing` renamed → `test_ac1_default_socket_path_is_pid_independent_well_known` (the prior test codified the defect per `[[test-bypass-as-runtime-truth]]` pattern) + NEW sibling `test_ac1_default_socket_path_matches_across_simulated_processes` asserting cross-process stability. Empirical cross-process verification: two distinct `uv run python` invocations (different PIDs) compute identical `/var/folders/.../T/harness-daemon.sock` path. 1309/1309 harness-runtime tests pass + 10 skipped (+1 NEW from +2 NEW minus AC #1 rename). mech-beta-ac3 integration test still passes against real Anthropic in 4.39s (it threads explicit `--socket-path` so behavior is unaffected). ZERO spec amendment; ZERO cross-axis cascade. Reading B (pidfile discovery) + Reading C (spec narrow + required flag) NOT applied; preserved as future routing if multi-daemon-per-host operator scenarios surface.
