...
**Status:** ✅ APPLIED 2026-05-28..2026-05-29 (status-line refreshed 2026-05-31) — Gate G2 en-bloc ratification (operator AskUserQuestion 2026-05-28, all 6 Q-S sub-decisions Q-S1..Q-S6); **all downstream gates since LANDED:** G4 (runtime spec v1.35 NEW §14.18 C-RT-29 HarnessRunCLI + §14.19 C-RT-30 WorkflowManifestLoader; current spec at v1.39), G5 (plan v2.31 NEW L9-sedecies cluster; current plan at v2.42), G6 (Phase 2b U-RT-102..U-RT-109 LANDED). Production loader at `harness-runtime/src/harness_runtime/lifecycle/workflow_manifest_loader.py`. Two later canonical-reading deltas applied: YAML scalar-coercion Reading A (`class_1_fork_yaml_loader_step_payload_scalar_coercion_gap.md`, spec v1.39) + topology-admissibility defer-to-runtime (`class_1_fork_topology_admissibility_check_load_time_vs_runtime_asymmetry.md`, spec v1.38). See also the canonical-reading addendum below (2026-05-28, U-RT-104 keying).

**Canonical-reading addendum (2026-05-28, post Phase 2b U-RT-104 implementation arc open).** Per `.harness/class_1_fork_u_rt_104_admissibility_keying_and_carrier_defaults.md` operator-ratified Q1=(β) + Q2=(i), TWO canonical-reading clarifications applied at this fork doc body:

  **(I) §3.3 row 5 + §4.2 row 7 — `engine_class` U-CP-16 admissibility cite shape.** The "U-CP-16 candidate mapping" cited at §3.3 + §4.2 is `ENGINE_CLASS_CANDIDATES` at `harness-cp/src/harness_cp/engine_class_candidate.py:57` keyed by `deployment_surface` (NOT `workload_class`). Manifest-layer admissibility check at U-RT-104 reduces to enum-validity only (covered by AC #9 enum-strictness invariant); deployment-surface-keyed admissibility check `engine_class in ENGINE_CLASS_CANDIDATES[deployment_surface].candidate_set` DEFERS to U-RT-106 one-shot dispatch site per runtime spec v1.36 §14.19.2 row 7 + §14.19.4 invariant 2 canonical-reading amendment (v1.35 body PRESERVED VERBATIM; canonical reading at v1.36 maps the trigger text). Body text at §3.3 + §4.2 PRESERVED VERBATIM at SF-1 doc layer; the canonical reading at v1.36 spec + v2.32 plan supersedes the cite shape for downstream consumers.

  **(II) §3.1 + §3.3 — OPTIONAL/Default columns vs `WorkflowManifestEntry` carrier reality.** §3.1 declares `engine_class` / `topology_pattern` / `layer_budgets` / `fallback_chain` / `hitl_placements` / `per_step_overrides` as "OPTIONAL"; §3.3 "Default" column lists `()` empty tuples / `FallbackChain.default()` / `{}` empty dict / `None` for those fields. Empirically, `WorkflowManifestEntry` Pydantic carrier at `harness-cp/src/harness_cp/workflow_manifest_entry.py:68-160` has NO defaults for 4 of those 6 fields (`engine_class` + `topology_pattern` §3.3 self-consistently declare "(none — caller-supplied; no Pydantic default at v1.20 carrier)" — those 2 are accurate). The 4 remaining drift: `layer_budgets`, `fallback_chain`, `hitl_placements`, `per_step_overrides` carry NO Pydantic default at the carrier; §3.1 "OPTIONAL" + §3.3 default-column entries are documentation drift (Class 3 informational). Additionally, `FallbackChain.default()` cite at §3.3 row 10 is a PHANTOM CITE — no `default()` method exists at `harness-cp/src/harness_cp/cross_family_fallback_chain.py:52-67` (verified via grep). Per runtime spec v1.36 §14.19.4 invariant 6 "loader does NOT inject defaults; absence passes None / omitted-kwarg to carrier; Pydantic carrier defaults apply OR validation rejects" + Q2=(i) operator ratification: the canonical reading is that those 4 fields are REQUIRED at the manifest layer (carrier-required reading wins). §3.1 OPTIONAL annotations + §3.3 default-column drift entries are aspirational shorthand for a future carrier extension (not authored at this arc per (i) doc-only routing). Phantom `FallbackChain.default()` cite STRIKES at the canonical reading. Body text at §3.1 + §3.3 PRESERVED VERBATIM at SF-1 doc layer per Q2=(i) "doc-only reframe" routing.

  ZERO body text amendment at §3.1 / §3.3 / §4.2 — canonical-reading addendum is the operative interpretation going forward. Downstream readers MUST apply this addendum when consulting SF-1 §3 + §4. Co-published with spec v1.36 + plan v2.32 + fork doc `.harness/class_1_fork_u_rt_104_admissibility_keying_and_carrier_defaults.md` status PROPOSING → ✅ APPLIED-AS-READING-β + Q2=APPLIED-AS-(i) + workspace `CLAUDE.md` §2.3 + §2.4 row bumps. 2026-05-28.
