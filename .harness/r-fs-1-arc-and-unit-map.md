...
Headline (HEAD): **14 closed · 0 remaining · 1 gated** (the build-ready standalone arcs) **+ 2
...
B-FANOUT-PAUSE: `B-FANOUT-PAUSE-EVALUATOR-OPTIMIZER` / `-PARALLELIZATION` / `B-HANDOFF-PAUSE` /
`B-HIERARCHICAL-PAUSE`). Full per-arc disposition: the spine ledger
(`.harness/beyond-mvp-capability-boundary-ledger.md`).
...
| B-LAYER-BUDGET-OVERRIDE | closed · 2026-06-19 | CP + runtime | Enforce **per-layer time budgets** honoring per-workload/persona overrides (built: the L3 router timeout now uses the §3.1 OVERRIDE-RESOLVED effective budget keyed on the request's workload_class/persona_tier — §3.1 commits the tuning surface explicitly on `llm_as_router` — not the flat field; the `effective_layer_budget_ms` resolver + a dormant `RuntimeLLMDispatcher.budgets` threading seam; CP spec v1.43 §2.5.3 amendment, NO operator gate). Capability-built + production-dormant (live firing gated by the UNOWNED routing-activation gate). Deterministic-layer wall-clock enforcement + the 200 ms recalibration are separate registered concerns. |
...
| B-HITL-PLACEMENT-PER-STEP-PRODUCER | closed · #657 | CP + runtime | Bind workflow-level `hitl_placements` onto **per-step** steps so the wrap-time human-approval gates (inference / sub-agent / tool) actually **fire in production** (built: the CP driver surfaces the workflow's declared placements onto `StepExecutionContext.hitl_placements` at every per-step context; the composer reads that surface — runtime §14.8.2-step-1 reconciled `step`→`step_context`; the B3 placement-composition residual closed for the workflow-level tuple). The per-step `StepOverride.hitl_placement` override fold is the registered follow-on `B-HITL-PLACEMENT-PER-STEP-OVERRIDE-FOLD`. |
| B-L2-EMBEDDING-ACTIVATION | closed · 2026-06-19 | runtime | **Switch on §2.2 layered routing** — the LAST standalone arc. Opt-in `routing_activation` flag (default-off, zero blast radius) makes the DECLARATIVE layer §2.2-faithful: it declines on a manifest-miss → `route()` reaches the (built) EMBEDDING classifier (then L3); the factory builds the real fastembed classifier when flag-on (fail-loud on a missing `[embedding]` extra). impl-to-cleared-spec (C-CP-02 §2.2, the #653 pattern — no spec bump). Non-dormant stub witness + a real-fastembed factory e2e. Production-dormant behind the operator-creds second-provider deployment gate; the `[embedding]` dep-promotion stays deferred. **Originally-enumerated standalone B-* frontier BUILT; this arc SPAWNED 2 follow-ons (B-ROUTING-MANIFEST-MODEL-FOLD, B-L2-FALLBACK-COMPOSITION) so the register grew. routing_activation is NOT production-reachable until B-L2-FALLBACK-COMPOSITION + a 2nd provider; `R-CL-Q1` admission is an OPERATOR scoping call, not auto-routed.** |
