...
| B-L2-EMBEDDING-ACTIVATION | closed · 2026-06-19 | runtime | **Switch on §2.2 layered routing** — the LAST standalone arc. Opt-in `routing_activation` flag (default-off, zero blast radius) makes the DECLARATIVE layer §2.2-faithful: it declines on a manifest-miss → `route()` reaches the (built) EMBEDDING classifier (then L3); the factory builds the real fastembed classifier when flag-on (fail-loud on a missing `[embedding]` extra). impl-to-cleared-spec (C-CP-02 §2.2, the #653 pattern — no spec bump). Non-dormant stub witness + a real-fastembed factory e2e. Production-dormant behind the operator-creds second-provider deployment gate; the `[embedding]` dep-promotion stays deferred. **ALL standalone B-* arcs now BUILT → `R-CL-Q1` quality track unblocks.** |
