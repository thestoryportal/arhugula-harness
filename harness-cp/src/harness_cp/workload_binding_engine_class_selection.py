...
async def emit_workload_class_selection_state_ledger_entry(
    *,
    ...
    payload = EntryPayload(
        ...
        # v2.9 §2.1 row 7). This §16.5 composer's `timestamp` means WHEN THE
        # ENTRY WAS APPENDED — the selection's own outcome is carried in the
        # canonicalized bytes behind `idempotency_key`, not in this field —
        # so §7.6.1's eligibility rule permits electing. Per call site, never
        # a default.
