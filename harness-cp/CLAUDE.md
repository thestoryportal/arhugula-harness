...
|---|---|---|
| **RETIRED** | 17/22 (77.3%) | (12 prior RETIRED rows preserved verbatim per row-history at batch-17 close); H_T-CP-22 (PauseResumeProtocol + state_summary — batch 18 PARTIAL → RETIRED workflow-layer composer close); H_T-CP-19 (D5 cross-deployment monotonicity — batch 22 RETIRE-READY → RETIRED close via Layer 3 e2e reframed in-process scope per advisor 2026-05-27; same-day same-session close from batch-21 PARTIAL → RETIRE-READY transit; `resolve_parent_gate_level()` helper extracted at `workflow_driver.py:359` from inline v1.20 §6.1.Y composition expression — ZERO behavior change; 12 NEW e2e tests at `harness-cp/tests/test_default_gate_level_monotonicity_e2e.py`; ADR-D5 §1.5.2 monotonicity contract verified in-process — multi-deployment substrate NOT required (Q3 framing was over-conservative read of contract scope); FIFTH RETIRE-READY → RETIRED close in ledger history; ZERO cross-axis cascade); H_T-CP-14 (multi-agent span hierarchy + `subagent.*` + `topology.*` — batch 29 PARTIAL → RETIRED via operator-discretion ratification of v1.6 MVP single-sub-agent slice bounded scope per runtime spec v1.6 §14.7.2 step 5 line 2546 explicit ratification path; v1.6 MVP narrow subset emission at `sub_agent_dispatch.py:613` IN COMPLIANCE — `topology.pattern` + `topology.workload_class` at open + 7 `subagent.*` attrs across open + close; 8 fan-out-specific `topology.*` attrs (`fan_out_cap`, `cascade_policy`, `results_collected`, `results_failed`, `cascade_applied`, `synthesis_token_budget`, `cascade_decision_audit_ledger_id`, `concurrent_token_budget_at_dispatch`) deferred to v1.7+ parent-topology-expansion arc (Phase 6 substrate); SIXTH RETIRE-READY → RETIRED close in ledger history + SECOND same-session joint single-batch transit (CP-19 batch-22 first) + SECOND closure of sub-species 7.operator-explicit-deferred-close-gate; ZERO production code change; ZERO spec amendment; ZERO cross-axis cascade); **H_T-CP-11 (D4 multiplicative tunable + per-workload commitment table + per-engine overlay + 2D matrix per C-CP-11 §11; carriers U-CP-23 + U-CP-24 + U-CP-25 — batch 30 PARTIAL → RETIRED via operator-discretion ratification of v1.6 MVP single-sub-agent slice cascade_policy carve-out per runtime spec v1.6 §14.7.2 step 5 explicit ratification path; U-CP-23 admissibility + default-pattern consumed at runtime via `topology_dispatcher.py:38, 113` + `workload_engine_class_matrix.py:77`; U-CP-24 consumed via U-CP-25 matrix + U-CP-53 T-perm-3 composition at `t_perm_3_composition.py:275-312`; U-CP-25 2D matrix + `d4_tunable` carriers landed but `d4_tunable` runtime invocation structurally unreachable at v1.6 MVP single-sub-agent scope (no siblings, no cascade-decision firing point, no `ParentFanoutCloseEntry` construction); `cascade_policy` attribute explicitly carved out at runtime spec v1.6 §14.7.2 step 5 in the 8-attr fan-out deferred set (same v1.6 MVP scope ratification that closed CP-14 at batch-29); deferred to v1.7+ parent-topology-expansion arc (Phase 6 substrate — same gate as CP-14); SEVENTH RETIRE-READY → RETIRED close in ledger history + THIRD same-session joint single-batch transit (CP-19 batch-22 + CP-14 batch-29 prior) + THIRD closure of sub-species 7 catalogue (sibling carve-out family to CP-14 under the SAME §14.7.2 step 5 v1.6 MVP scope statement); ZERO production code change; ZERO spec amendment; ZERO cross-axis cascade)** |
| **RETIRE-READY** | 0/22 (0.0%) | (empty — CP-11 + CP-14 transits straight through PARTIAL → RETIRE-READY → RETIRED at batch-30 + batch-29; CP-19 transit 1 → 0 within day at batch-22; three fastest-transit precedents in ledger history all via sub-species 7 operator-discretion ratification) |
| **PARTIAL** | 3/22 (13.6%) | H_T-CP-8 (F2-substrate-join — 1 of 17 edges wired; 7 of 9 source units lack composers; Phase 6 CP plan revision-pass required per `[[fork-cp-is-wiring-gaps]]`); H_T-CP-9 (ResumptionKind 5-class — driver emits binary only per CP spec v1.23 §25.5 v1.4 scope carve-out; expansion to all 5 EngineClass cases requires Phase 6 substrate); H_T-CP-17 (files.* CP-side consumer — batch 11, U-AS-28+U-AS-31 landed at AS axis; Files arc deferred indefinitely per runtime spec v1.17 §14.C fork-doc ratified scope) |
...
Totals: 17 RETIRED + 0 RETIRE-READY + 3 PARTIAL + 2 STILL-BOUNDED = 22 ✓.

**Pipeline advanced (RETIRED + RETIRE-READY + PARTIAL):** 20/22 = **90.9%** CP-axis advanced post-batch-30 (unchanged from post-batch-29; within-tier promotion CP-11 PARTIAL → RETIRED this batch). Workspace-wide pipeline advanced: 41/54 = 75.9% (unchanged from batch-29 footer cite via within-tier CP-11 transit). **Workspace 35/54 = 64.8% RETIRED post-batch-30** (+1 from batch-29). **Cardinality check at batch-30 close: 35 + 2 + 4 + 11 + 2 = 54 ✓.**
...
**PARTIAL → RETIRE-READY gates (post-batch-30: 3 PARTIALs remain at CP-axis):** all gate on workflow_driver / sub_agent_dispatch composer invocation of carrier primitives that already landed at library layer. Specifically: CP-8 on CP-IS-wiring 17-edge completion (`[[fork-cp-is-wiring-gaps]]`); CP-9 on driver emission of 5-class ResumptionKind beyond binary; CP-17 on Files-arc design-phase opening (currently deferred indefinitely per runtime spec v1.17 §14.C). CP-11 (D4 multiplicative tunable / cascade_policy carve-out) + CP-14 (parent-topology-expansion) both transited to RETIRED via sub-species 7 operator-discretion ratification at batches 30 + 29 respectively — sharing the SAME runtime spec v1.6 §14.7.2 step 5 v1.6 MVP scope carve-out. Each remaining PARTIAL is bounded by a specific finite gap, not by a missing major composer. Operator-discretion timing per existing 7d retirement-event cadence.
