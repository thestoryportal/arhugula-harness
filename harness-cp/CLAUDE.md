...
**Retirement status (post 7d batch 15 Reading-D′ DOWN-classification of H_T-CP-21, 2026-05-24).** Per `.harness/phase-7d-retirement-events-batch-{1..15}.md` cumulative + `.harness/phase-7d-retirement-ledger-v2.md` (operator-ratified runtime-only substitution-site reading + line-33 strict-reading discipline: "bootstrap-materializes-but-driver-never-invokes ≠ RETIRE-READY"). This block refreshes the prior post-batch-14 update with the Reading-D audit at fork doc `.harness/class_1_fork_validator_composer_arc_stage_4_absence.md` §3.4 (HEAD `3adacc0`) — H_T-CP-21 RETIRE-READY → PARTIAL per batch-15 §1 (FIRST DOWN-classification in ledger history; the batch-11 promotion claimed structural-criterion-B on driver-hook landing but skipped empirical bootstrap-binding-chain verification — production `HarnessContext.validator_framework` has no stage factory + no RuntimeConfig field, so the driver branch is dead in production). **CP-axis 50% RETIRED milestone preserved (11/22 from batch-14 H_T-CP-16 close).** Forward-only ledger discipline preserved — prior batch records stand verbatim per workspace `CLAUDE.md` §4.3.

| Status | Count | Substitutions |
|---|---|---|
| **RETIRED** | 11/22 (50.0%) | H_T-CP-1 (multi-LLM routing core — batch 2, `llm_dispatch.py:RuntimeLLMDispatcher.dispatch`); H_T-CP-2 (layered routing strategy — batch 2); H_T-CP-3 (per-layer time-budget + `retry.*` 6-attr namespace + dual-emission — batch 3, `retry_breaker_fallback.py:_run_per_candidate_attempts`); H_T-CP-4 (fallback chain + cross-family fallback — batch 3, `_advance_or_exhaust` + `fallback.exhausted`); H_T-CP-5 (routing attribute namespaces inheritance + per-class sampling — batch 3); H_T-CP-6 (workflow manifest schema — batch 1, `routing_manifest.py:143-145` + `workflow_driver.py:360-364`); H_T-CP-10 (TopologyPattern + admissibility predicate — batch 4, `sub_agent_dispatch.py:RuntimeSubAgentDispatcher.dispatch`); H_T-CP-13 (HandoffContext + SubAgentBrief + StateSummary + LedgerEntryRef typed handoff — batch 4, `sub_agent_dispatch.py:_compose_handoff_context`); H_T-CP-16 (memory.* CP-side consumer + Anthropic Memory tool client-side primitive consumption — batch 14 at `03025cb`, U-RT-82 e2e empirical exercise against real Anthropic API `claude-haiku-4-5` with `ANTHROPIC_API_KEY` supplied; 3/3 e2e tests pass; criterion-A MET (U-AS-28 + U-AS-31 at AS axis) + structural criterion-B MET (runtime composer at `llm_dispatch.py:319` + `memory_tool_dispatch.execute_with_memory_callbacks` inner loop + `memory.operation` span emission per AS spec v1.5 §14.7) + operational criterion-B MET (U-RT-82 e2e verifies `create` callback invoked + file written verbatim at `LocalFilesystemMemoryToolBackend` root + OTel span with `memory.backend=filesystem` / `memory.operation.kind=write`)); H_T-CP-20 (HITL primitive + 4-response palette + `hitl.*` / `audit.*` — batch 9 at U-RT-62 close, end-to-end via FastMCP server + `run_workflow` tool + `ctx.elicit` HITL delivery); H_T-CP-24 (authoring-close at v1 §1, substrate seam exports + F2-12 closure manifest) |
| **RETIRE-READY** | 1/22 (4.5%) | H_T-CP-18 (MCP integration + per-server trust + `mcp.*` — batch 10, L9-septies cluster `00da5ef` materializing 5 cited units; criterion-A MET + structural criterion-B MET (verified at `materialize_mcp_client_host_stage` U-RT-73 stage 3a + `RuntimeConfig.mcp_servers` config field + driver MCP-tool-dispatch invocation path); operational e2e gated on operator-supplied `mcp_servers` config non-empty + external MCP server availability) |
| **PARTIAL** | 8/22 (36.4%) | H_T-CP-8 (F2-substrate-join — `cp_is_wiring.py` 1 of 17 edges); H_T-CP-9 (ResumptionKind 5-class — driver emits binary only); H_T-CP-11 (D4 multiplicative tunable not surfaced at runtime); H_T-CP-14 (multi-agent span hierarchy + `subagent.*` + `topology.*` — batch 4 single-sub-agent slice; 8 fan-out-specific `topology.*` attrs deferred to parent-topology-expansion arc); H_T-CP-17 (files.* CP-side consumer — batch 11, U-AS-28+U-AS-31 landed at AS axis; Files arc deferred indefinitely per runtime spec v1.17 §14.C fork-doc ratified scope); H_T-CP-19 (D5 cross-deployment monotonicity — batch 11, U-CP-26+U-CP-27+U-CP-43 landed; workflow_driver uses static GateLevel.AUTO only); H_T-CP-21 (ValidatorFailClass 5-class + operator-burden eval — DOWN-classified at batch-15 from RETIRE-READY following Reading-D audit per fork doc `.harness/class_1_fork_validator_composer_arc_stage_4_absence.md` §3.4; criterion-A MET (U-CP-47+U-CP-48+U-CP-51+U-CP-58+U-CP-59+U-CP-60+U-CP-61 all landed) BUT criterion-B NOT MET — production `HarnessContext.validator_framework` field has no bootstrap stage factory + no `RuntimeConfig.validator_framework_config` field; `workflow_driver.py:668` branch `if ctx.validator_framework is not None:` is a dead branch in production; restoration to RETIRE-READY requires validator-composer arc design-phase opening per fork doc §3 Reading A or B); H_T-CP-22 (PauseResumeProtocol + state_summary — batch 11, U-CP-49/50/62/63/64/65 landed at `harness-cp/pause_resume_protocol.py`; no workflow_driver invocation of capture_pause_snapshot/attempt_resume per hitl_placement.py:18-23 deferral) |
...
Totals: 11 RETIRED + 1 RETIRE-READY + 8 PARTIAL + 2 STILL-BOUNDED = 22 ✓.

**Pipeline advanced (RETIRED + RETIRE-READY + PARTIAL):** 20/22 = **90.9%** CP-axis advanced post-batch-15 (within-advanced-tier DOWN shift vs post-batch-14; vs 14/22 = 63.6% post-batch-9; vs 9/22 = 40.9% post-batch-5). Workspace-wide pipeline advanced: 35/49 = 71.4% (per batch-15 §3 cumulative table — unchanged from batch-14, within-advanced-tier DOWN shift).

**Operator-opt-in RETIRE-READY pattern (post-batch-15: 3 members — CP-18, CP-16-CLOSED-AT-BATCH-14, AS-2; CP-21 REMOVED at batch-15 per Reading-D audit).** Three substitutions advanced to RETIRE-READY under the pattern when production execution path branches on operator-supplied config or operator-supplied step payload (`ctx.mcp_servers=[]` default for CP-18 + AS-2 via shared MCP-client substrate; `step.step_payload.tools` lacks `memory_20250818` default for CP-16 (CLOSED RETIRED at batch-14)). CP-21 was prematurely included in the pattern at batch-11; removed at batch-15 per `[[advisor-before-substantive-work-for-cross-axis-blockers]]` empirical-binding-chain verification discipline catalogued at batch-15 §6(a). Full RETIRED gates on operator-config-non-default + external-substrate-exercise per ledger-v2 + batch-10/11/12/13/14/15 §1 reconciliation discipline. **Close pattern (per batch-14 §6(a) catalogue) + DOWN-classification pattern (per batch-15 §6(a) catalogue):** the UP-classification close pattern (RETIRE-READY → RETIRED) and the DOWN-classification corrective pattern (RETIRE-READY → PARTIAL) are symmetric counterparts; both rest on empirical end-to-end binding-chain verification.
