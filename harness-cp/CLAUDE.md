...
| **PARTIAL** | 6/22 (27.3%) | H_T-CP-8 (F2-substrate-join — `cp_is_wiring.py` 1 of 17 edges); H_T-CP-9 (ResumptionKind 5-class — driver emits binary only); H_T-CP-11 (D4 multiplicative tunable not surfaced at runtime); H_T-CP-14 (multi-agent span hierarchy + `subagent.*` + `topology.*` — batch 4 single-sub-agent slice; 8 fan-out-specific `topology.*` attrs deferred to parent-topology-expansion arc); H_T-CP-17 (files.* CP-side consumer — batch 11, U-AS-28+U-AS-31 landed at AS axis; Files arc deferred indefinitely per runtime spec v1.17 §14.C fork-doc ratified scope); H_T-CP-19 (D5 cross-deployment monotonicity — batch 11, U-CP-26+U-CP-27+U-CP-43 landed; workflow_driver uses static GateLevel.AUTO only) |
...
Totals: 14 RETIRED + 0 RETIRE-READY + 6 PARTIAL + 2 STILL-BOUNDED = 22 ✓.

**Pipeline advanced (RETIRED + RETIRE-READY + PARTIAL):** 20/22 = **90.9%** CP-axis advanced post-batch-18 (within-tier promotion of CP-22 vs post-batch-17; vs 14/22 = 63.6% post-batch-9; vs 9/22 = 40.9% post-batch-5). Workspace-wide pipeline advanced: 35/49 = 71.4% (per batch-18 §3 cumulative table — unchanged from batch-13..17, within-tier promotion + DOWN compositions). **Workspace 27/49 = 55.1% RETIRED post-batch-18.**
