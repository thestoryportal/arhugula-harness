...
|---|---|---|
| `Spec_Control_Plane_v1_31.md` | **v1.31 — canonical HEAD** (delta-only chain; this row's prior `v1.3` was the Phase-6.5 authoring-era pin, pre-dating the Phase-7 v1.4→v1.31 deltas — latest delta = NEW §29 / C-CP-29 `PromptSelectionManifest` (R-PM-1 cascade PR #3): per-role/workload prompt-version selection mirroring C-CP-01 §1.3 `RoutingManifest`, resolving `(role, workload) → version_sha` (the IS `PromptManifest.versions` store, IS spec v1.7 §5.3, resolves sha→content; runtime injects via v1.44 §14.5.2); §29 additive, §1/§16.5.x/§25–§28 PRESERVED VERBATIM. Prior delta v1.30 = §16.5.12 composer-signature collapse per Reading C. Per delta-only convention each version is canonical-at-authoring for its scope; the chain head is the current contract authority) | Contract authority — 29 contracts C-CP-01 through C-CP-29 *(authoring-era count was 24; deltas grew the enumeration — C-CP-25 ValidatorFramework / C-CP-26 PauseResumeProtocol / C-CP-27 PerServerTrust / C-CP-28 validator-hook / C-CP-29 PromptSelectionManifest — see the canonical spec)* |
