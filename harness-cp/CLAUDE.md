...
**Retirement status (post 7d batch 11 doc-hygiene-pass refresh, 2026-05-23).** Per `.harness/phase-7d-retirement-events-batch-{1..11}.md` cumulative + `.harness/phase-7d-retirement-ledger-v2.md` (operator-ratified runtime-only substitution-site reading + line-33 strict-reading discipline: "bootstrap-materializes-but-driver-never-invokes ≠ RETIRE-READY"). This block refreshes the prior post-batch-3 + post-batch-5 + post-batch-8 incremental updates with the cumulative current state at HEAD `2bc775c` (batch-11 landing). Forward-only ledger discipline preserved — prior batch records stand verbatim per workspace `CLAUDE.md` §4.3.
...
| **PARTIAL** | 8/22 (36.4%) | H_T-CP-8 (F2-substrate-join — `cp_is_wiring.py` 1 of 17 edges); H_T-CP-9 (ResumptionKind 5-class — driver emits binary only); H_T-CP-11 (D4 multiplicative tunable not surfaced at runtime); H_T-CP-14 (multi-agent span hierarchy + `subagent.*` + `topology.*` — batch 4 single-sub-agent slice; 8 fan-out-specific `topology.*` attrs deferred to parent-topology-expansion arc); H_T-CP-16 (memory.* CP-side consumer — batch 11, U-AS-28+U-AS-31 landed at AS axis; no CP-side runtime invocation); H_T-CP-17 (files.* CP-side consumer — batch 11, same shape); H_T-CP-19 (D5 cross-deployment monotonicity — batch 11, U-CP-26+U-CP-27+U-CP-43 landed; workflow_driver uses static GateLevel.AUTO only); H_T-CP-22 (PauseResumeProtocol + state_summary — batch 11, U-CP-49/50/62/63/64/65 landed at `harness-cp/pause_resume_protocol.py`; no workflow_driver invocation of capture_pause_snapshot/attempt_resume per hitl_placement.py:18-23 deferral) |
...
Totals: 10 RETIRED + 2 RETIRE-READY + 8 PARTIAL + 2 STILL-BOUNDED = 22 ✓.

**Pipeline advanced (RETIRED + RETIRE-READY + PARTIAL):** 20/22 = **90.9%** CP-axis advanced post-batch-11 (vs 9/22 = 40.9% post-batch-5; vs 14/22 = 63.6% post-batch-9). Workspace-wide pipeline advanced: 32/49 = 65.3% (per batch-11 §7 cumulative table).

**Operator-opt-in RETIRE-READY pattern (stable across CP-18 + CP-21).** Both substitutions advance to RETIRE-READY when production execution path branches on operator-supplied config (`ctx.mcp_servers=[]` default for CP-18; `ctx.validator_framework=None` default for CP-21). Full RETIRED gates on operator-config-non-default + e2e exercise per ledger-v2 + batch-10 §1.4 + batch-11 §1.4 reconciliation discipline. See batch-11 §9(b).
...
**PARTIAL → RETIRE-READY gates (the 8 PARTIALs):** all gate on workflow_driver / sub_agent_dispatch composer invocation of carrier primitives that already landed at library layer. Specifically: CP-8 on CP-IS-wiring 17-edge completion (`[[fork-cp-is-wiring-gaps]]`); CP-9 on driver emission of 5-class ResumptionKind beyond binary; CP-11 on workload commitment table runtime exposure; CP-14 on parent-topology-expansion arc; CP-16/17 on CP-side composer landing for memory.* / files.* consumption; CP-19 on multi-deployment runtime scenario; CP-22 on workflow_driver pause-event handler invoking PauseResumeProtocol.capture_pause_snapshot/attempt_resume. Each is bounded by a specific finite gap, not by a missing major composer. Operator-discretion timing per existing 7d retirement-event cadence.
