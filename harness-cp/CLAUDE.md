...
**Retirement status (post 7d batch 14 U-RT-82 e2e empirical exercise, 2026-05-24).** Per `.harness/phase-7d-retirement-events-batch-{1..14}.md` cumulative + `.harness/phase-7d-retirement-ledger-v2.md` (operator-ratified runtime-only substitution-site reading + line-33 strict-reading discipline: "bootstrap-materializes-but-driver-never-invokes ≠ RETIRE-READY"). This block refreshes the prior post-batch-13 update with the U-RT-82 e2e empirical exercise at `03025cb` against the real Anthropic API (`claude-haiku-4-5`; 3/3 e2e tests pass) — H_T-CP-16 RETIRE-READY → RETIRED per batch-14 §1. **FIRST RETIRE-READY → RETIRED close in ledger history; CP-axis crosses 50% RETIRED threshold (11/22).** Forward-only ledger discipline preserved — prior batch records stand verbatim per workspace `CLAUDE.md` §4.3.
...
| **RETIRE-READY** | 2/22 (9.1%) | H_T-CP-18 (MCP integration + per-server trust + `mcp.*` — batch 10, L9-septies cluster `00da5ef` materializing 5 cited units; criterion-A MET + structural criterion-B MET; operational e2e gated on operator-supplied `mcp_servers` config non-empty + external MCP server availability); H_T-CP-21 (ValidatorFailClass 5-class + operator-burden eval — batch 11, cluster 10-CP-A `9b009d3` wiring `validator.*` post-dispatch hook at workflow_driver.py:668; criterion-A MET + structural criterion-B MET; operational e2e gated on operator-supplied `validator_framework` non-None at HarnessContext) |
...
Totals: 11 RETIRED + 2 RETIRE-READY + 7 PARTIAL + 2 STILL-BOUNDED = 22 ✓.

**Pipeline advanced (RETIRED + RETIRE-READY + PARTIAL):** 20/22 = **90.9%** CP-axis advanced post-batch-14 (within-tier promotion vs post-batch-13; vs 14/22 = 63.6% post-batch-9; vs 9/22 = 40.9% post-batch-5). Workspace-wide pipeline advanced: 35/49 = 71.4% (per batch-14 §4 cumulative table — unchanged from batch-13, within-tier promotion).

**Operator-opt-in RETIRE-READY pattern (close pattern established at batch-14 H_T-CP-16 — first RETIRE-READY → RETIRED close in ledger history).** All four originally-RETIRE-READY substitutions (CP-18, CP-21, CP-16, AS-2) advance to RETIRE-READY when production execution path branches on operator-supplied config or operator-supplied step payload (`ctx.mcp_servers=[]` default for CP-18 + AS-2 via shared MCP-client substrate; `ctx.validator_framework=None` default for CP-21; `step.step_payload.tools` lacks `memory_20250818` default for CP-16). Full RETIRED gates on operator-config-non-default + external-substrate-exercise per ledger-v2 + batch-10/11/12/13/14 §1.4 reconciliation discipline. **Close pattern (per batch-14 §6(a) catalogue):** RETIRE-READY ledger entry enumerates operator-opt-in operational sub-conditions → operator supplies gating substrate → test infrastructure landed alongside RETIRE-READY transition is exercised → empirical evidence captured at new batch §1.1 evidence block → RETIRE-READY → RETIRED transition recorded. H_T-CP-18 + H_T-CP-21 follow the same close shape at future batches.
