...
**Retirement status (post 7d batch 13 L9-octies absorption, 2026-05-23).** Per `.harness/phase-7d-retirement-events-batch-{1..13}.md` cumulative + `.harness/phase-7d-retirement-ledger-v2.md` (operator-ratified runtime-only substitution-site reading + line-33 strict-reading discipline: "bootstrap-materializes-but-driver-never-invokes ≠ RETIRE-READY"). This block refreshes the prior post-batch-11 update with the L9-octies cluster close at `42c9a30` materializing the CP-side runtime composer for Memory tool primitive consumption (H_T-CP-16 PARTIAL → RETIRE-READY per batch-13 §1). Forward-only ledger discipline preserved — prior batch records stand verbatim per workspace `CLAUDE.md` §4.3.

| Status | Count | Substitutions |
|---|---|---|
| **RETIRED** | 10/22 (45.5%) | H_T-CP-1 (multi-LLM routing core — batch 2, `llm_dispatch.py:RuntimeLLMDispatcher.dispatch`); H_T-CP-2 (layered routing strategy — batch 2); H_T-CP-3 (per-layer time-budget + `retry.*` 6-attr namespace + dual-emission — batch 3, `retry_breaker_fallback.py:_run_per_candidate_attempts`); H_T-CP-4 (fallback chain + cross-family fallback — batch 3, `_advance_or_exhaust` + `fallback.exhausted`); H_T-CP-5 (routing attribute namespaces inheritance + per-class sampling — batch 3); H_T-CP-6 (workflow manifest schema — batch 1, `routing_manifest.py:143-145` + `workflow_driver.py:360-364`); H_T-CP-10 (TopologyPattern + admissibility predicate — batch 4, `sub_agent_dispatch.py:RuntimeSubAgentDispatcher.dispatch`); H_T-CP-13 (HandoffContext + SubAgentBrief + StateSummary + LedgerEntryRef typed handoff — batch 4, `sub_agent_dispatch.py:_compose_handoff_context`); H_T-CP-20 (HITL primitive + 4-response palette + `hitl.*` / `audit.*` — batch 9 at U-RT-62 close, end-to-end via FastMCP server + `run_workflow` tool + `ctx.elicit` HITL delivery); H_T-CP-24 (authoring-close at v1 §1, substrate seam exports + F2-12 closure manifest) |
| **RETIRE-READY** | 3/22 (13.6%) | H_T-CP-18 (MCP integration + per-server trust + `mcp.*` — batch 10, L9-septies cluster `00da5ef` materializing 5 cited units; criterion-A MET + structural criterion-B MET; operational e2e gated on operator-supplied `mcp_servers` config non-empty + external MCP server availability); H_T-CP-21 (ValidatorFailClass 5-class + operator-burden eval — batch 11, cluster 10-CP-A `9b009d3` wiring `validator.*` post-dispatch hook at workflow_driver.py:668; criterion-A MET + structural criterion-B MET; operational e2e gated on operator-supplied `validator_framework` non-None at HarnessContext); H_T-CP-16 (memory.* CP-side consumer + Anthropic Memory tool client-side primitive consumption — batch 13, L9-octies cluster `42c9a30` materializing 7 cited units U-RT-76..U-RT-82; criterion-A MET (U-AS-28 + U-AS-31 at AS axis) + structural criterion-B MET (runtime composer at `llm_dispatch.py:319` + `memory_tool_dispatch.execute_with_memory_callbacks` inner loop + `memory.operation` span emission per AS spec v1.5 §14.7); operational e2e gated on operator-supplied `memory_tool_backend_config` non-default + step payload with `memory_20250818` tool + `ANTHROPIC_API_KEY` for U-RT-82 integration test exercise) |
...
Totals: 10 RETIRED + 3 RETIRE-READY + 7 PARTIAL + 2 STILL-BOUNDED = 22 ✓.

**Pipeline advanced (RETIRED + RETIRE-READY + PARTIAL):** 20/22 = **90.9%** CP-axis advanced post-batch-13 (within-tier promotion vs post-batch-11; vs 14/22 = 63.6% post-batch-9; vs 9/22 = 40.9% post-batch-5). Workspace-wide pipeline advanced: 35/49 = 71.4% (per batch-13 §4 cumulative table — unchanged from batch-12, within-tier promotion).

**Operator-opt-in RETIRE-READY pattern (stable across CP-18 + CP-21 + CP-16, plus AS-2 at AS axis).** All four substitutions advance to RETIRE-READY when production execution path branches on operator-supplied config or operator-supplied step payload (`ctx.mcp_servers=[]` default for CP-18 + AS-2 via shared MCP-client substrate; `ctx.validator_framework=None` default for CP-21; `step.step_payload.tools` lacks `memory_20250818` default for CP-16). Full RETIRED gates on operator-config-non-default + e2e exercise per ledger-v2 + batch-10/11/12/13 §1.4 reconciliation discipline. See batch-13 §1.4.
