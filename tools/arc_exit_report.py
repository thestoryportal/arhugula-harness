...
Emitted at the FINAL `ship-pr` step, AFTER the reflect / `/context-save-lean` block (U-WT-04).
...
prose tail — it never crashes and never fabricates a value. In particular:
  ...
  * `checkpoint` is BOUND, not guessed. `--checkpoint PATH` names the exact file the
    arc's own `/context-save-lean` just wrote; only a bound checkpoint can ever be
    ...
    PARALLEL frontier — another live session can `/context-save-lean` between this arc's save
...
def _checkpoint(
    ...
    * `explicit` (from `--checkpoint`) is a BINDING: the caller names the file its own
      `/context-save-lean` step just reported, so ownership is known and `confirmed` is a real
    ...
    * Without it, the workspace-newest checkpoint is a HEURISTIC and `confirmed` is
      ...
      another live session's `/context-save-lean` can be the newest file here; mtime cannot
...
def render(data: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "reflect / `/context-save-lean` block. Regenerating for the same PR overwrites this file.*",
        "",
    ]
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "the checkpoint file this arc's /context-save-lean just wrote. Required for "
