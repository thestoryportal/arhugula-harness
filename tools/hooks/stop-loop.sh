...
If the item is GATED (needs a paid call / secret / vendor selection / missing credential / infra you cannot provide): do NOT force it and do NOT raise .loop-halt. Build whatever slice is possible WITHOUT the gated input, then run \`source tools/hooks/lib.sh tools/hooks/loop_lib.sh && loop_defer <ITEM-ID> '<exactly what operator input is needed> — built without it: <slice or none>'\` and ADVANCE to the next forward item. The hard-stop deny-list still blocks the dangerous TOOL; this is the item-level disposition.
ONLY when EVERY forward item per §12.4.1 is already in the deferred set (no buildable slice remains anywhere) do you raise the stand-down: \`source tools/hooks/lib.sh tools/hooks/loop_lib.sh && loop_log STOP 'forward menu exhausted — N items awaiting operator input' && : > .harness/.loop-halt\`. Then the run ends for operator review."
