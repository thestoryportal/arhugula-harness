...
# Pre-lease availability guard (merge-gate r1, all three lenses): the refresh continuation
# flag `--emit-refresh-pr-json` is U-HE-28's to land. merge_door.land() runs the refresh
# AFTER the irreversible merge (check=True), so an unsupported flag would mark_blocked()
# the shared global lease post-merge (operator-only unblock). Until the CLI actually
# supports the flag, abort HERE — before any lease is taken or merge attempted. This guard
# auto-passes the moment U-HE-28 lands; the delegated invocation below stays plan-verbatim.
if ! uv run python tools/roadmap_status_refresh.py --help 2>/dev/null | grep -q -- --emit-refresh-pr-json; then
  echo "safe-merge: refresh producer flag --emit-refresh-pr-json not available yet (lands at U-HE-28); aborting pre-lease" >&2
  exit 69
fi
