...
- **advisor_required** — `yes` = run the §13.1 transcript-brief review before substantive work; `conditional:<reason>` = run it if the condition holds; `satisfied:<YYYY-MM-DD>` = a `yes` entry whose review already happened on that date and resolved the cross-axis blocker — re-run only if a NEW cross-axis question surfaces. Used on a paused entry (with `resume:`) so a fresh session does not redundantly re-consult. (The field keeps its legacy name — recorded entries predate the U-SR-06 rename of the legacy reviewer prose to the transcript-brief review.)
...
**When you pause mid-execution (checkpoint-on-pause rule).** If a substantive entry (especially `phase-7` / multi-PR) is paused after orientation but before completion — the §13.1 transcript-brief review + scope-ratification done, implementation not started — make the stop seamless across a session `/clear`: (1) write `.harness/<R-NNN>-checkpoint.md` capturing the resolved decisions + a "read this FIRST; do not re-derive" instruction; (2) set the entry's `resume:` field to that path and keep `status: ACTIVE` (downgrade `advisor_required: yes` → `satisfied:<date>` if the transcript-brief review is banked); (3) put an explicit "➡️ ON RESUME: …" instruction + checkpoint pointer in the dashboard Next-action. A fresh session then resumes via `/clear` → SessionStart hook (`next=R-NNN`) → `continue` → checkpoint → execution, with zero re-derivation. Remove `resume:` and the checkpoint pointer when the entry RESOLVES.
...
R-IF-116:
  ...
  blocks: []
  posture: mode-agnostic
  scope: { files: [tools/hooks/**, tools/*.py, .claude/skills/**, .agents/skills/**, justfile, .gitignore, .harness/hardening-workflow/HARDENING_PLAN.md, .harness/wave5-hooks-status.md, .harness/r-if-116-insights-residue-plan.md, Project_Roadmap_v1.md, tools/codex-parity-check.sh], contracts: [], cross_axis: no }
...
R-600-pattern-bake-in-sweep:
  ...
  next_pointer: null
  resume: .harness/R-600-pattern-bake-in-sweep.md   # ACTIVE-SURVEYED (cadence-9) 2026-07-24 HEAD e5529e8a: both must_pass met. ZERO new §7.4.7 owed (9th surfacing); ONE new §7.5 promotion — PD-9 adversarial-review-loop non-convergence discriminators (4-signature family: oscillation / arms-race hardening / over-correction / self-referential drift) at design-substrate/Project_Workflow_v1_19.md, clearance .harness/clearance/Project_Workflow-v1_19-cleared-2026-07-24.md. advisor() redirected the initial candidate pick away from merge-gate-catches-what-codex-advisor-miss (large internally-documented evidence volume, distinct from its citation cardinality of 3; already homed at .claude/skills/merge-gate/SKILL.md + CLAUDE.md §13.1 — gate 3 FAILS, not promoted) toward the genuinely uncovered family, verified at >=5 independent arcs. Cadence-8 (2026-07-14, HEAD 380ec161) promoted PD-8 mutation-probe-as-load-bearing-witness at v1.18; cadence-7 was a legitimate zero-promotion; cadence-6 (2026-07-12) was also zero-promotion. Next run only after the cadence trips again (~10 PRs) or a concrete new candidate reaches independent instance-cardinality >=2.
  notes: >
