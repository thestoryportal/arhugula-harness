...
r420-self-hosted-stack-up:
    @bash -c 'source tools/hooks/lane-init.sh >/dev/null || exit 1; lane_stack_allowed || { echo "self-hosted stack skipped: RAM floor (C-HE-11 5; see stderr for the NOTIFY row status)"; exit 0; }; _e="$(uv run python tools/lane_ports.py --shell)" || exit 1; eval "$_e"; docker compose -p "$R420_PROJECT" -f deploy/self-hosted-local/compose.yaml up -d'
