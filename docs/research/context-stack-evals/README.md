...
| E2b | E2 on code with callers (handoff-s2 §2 D) | PRs #1341 and #1330 (non-empty `graft blast` at depth 2: 2 and 1 dependents), four fresh Sonnet reviewers via `e2b_assemble.py` (repo path = worktree at the reviewed commit), plus one `codex exec` run with the pack → `E2b-results.md` | 5 runs, 6 findings reported; 4 verified in full, 2 verified with a qualification (one spec-directed change whose test gap stands; one with a reviewer trace step not re-traced), 0 refuted. Pack vs no-pack: same count on both PRs (1 vs 1); the pack runs cited the exact file/lines (no-pack cited a neighbouring file on 1330) and reached the operator-facing consequence on 1341. Codex + pack: 2 verified test-adequacy findings, PACK_USED yes, 294 s. | **Supported for precision, not recall, at n=2.** Attach the ~3 KB blast markdown to the out-of-family prompt and judge it by citation accuracy. Two verified findings against merged code are handed to the register (E2b-results "Residue"). |
...
| E3b | E3 reconfigured: drift only where the text claims identity (handoff-s2 §2 E) | `e3b_claimed_identical.py` → `E3b-results.md`; four claim sets: (A) packs "relocated byte-verbatim" tested at the relocation commit against the root body they name; (B) the 30 root pointer stubs vs the pack section each names, by cosine and by lexical containment; (C) the artifact-pointers "split byte-preservingly" tested paragraph- and line-wise at the split commit; (D) the Workflow deltas' "preserved verbatim" blocks | A: 33 sections claimed, 32 byte-identical at relocation, **1 claimed but absent** — `project-framing.md`'s header claims §7, which was relocated to `skills-and-subphases.md` (nine sections took 1–3 line edits since, as the live body). C: 7 paragraphs of the original are not in the union; at line level all 7 lost lines are the source's own title, headings and preamble the stub replaced; lineage lines lost: 0. D: 1 flagged, read as a mention, not a claim (v1.14's block says "§7.5 does NOT touch" the preserved sections; §7.5 is that delta's new section); true violations 0. B: no stub below both read thresholds; cosine and lexical containment rank the stubs differently (Spearman rho 0.46, bottom-5 overlap 3 of 5). | **One real drift where identity is claimed (an over-claiming relocation header), found by a diff; embeddings add nothing over a normalised diff here.** A, C and D are identity questions a diff answers exactly. B is the one semantic question; the two scores disagree on ranking, and only the lexical one names the missing words, so the cosine is not a substitute for reading. E3's 76 high-cosine pairs were structure, as E3 said. |
