...
| E3b | E3 reconfigured: drift only where the text claims identity (handoff-s2 §2 E) | `e3b_claimed_identical.py` → `E3b-results.md`; four claim sets: (A) packs "relocated byte-verbatim" tested at the relocation commit against the root body they name; (B) the 30 root pointer stubs vs the pack section each names, by cosine and by lexical containment; (C) the artifact-pointers "split byte-preservingly" tested paragraph- and line-wise at the split commit; (D) the Workflow deltas' "preserved verbatim" blocks | A: 32 of 32 sections identical at relocation (nine took small edits since, as the live body). C: 7 paragraphs of the original are not in the union; at line level the 7 lost lines are the source's own title, headings and preamble that the stub replaced; lineage lines lost: 0. D: no re-tabled pair exists to diff. B: no stub below both read thresholds; the lowest-cosine stubs are one-line abstracts whose containment score already says which nouns are absent. | **No drift found where identity is claimed; embeddings add nothing over a normalised diff here.** A, C and D are identity questions a diff answers exactly; B is the one semantic question, and the lexical containment ranks the same rows and names the missing words. E3's 76 high-cosine pairs were structure, as E3 said. |
