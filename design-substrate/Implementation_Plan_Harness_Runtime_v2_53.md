...
4. **(Physical-key-distinctness input threading — REQUIRED, not optional, per CP spec v1.105 §2 row 5's corrected posture.)** The factory (criterion #3) REQUIRES a `key_identity_resolver` input (e.g. `AwsKmsSigningBackend.key_arns` when that backend is configured) to construct a usable adapter pairing — a deployment whose `SigningBackend` exposes no physical-identity mapping cannot get a `succeeded=True` `PROBE_VERIFY_AT_READ` from this adapter pairing (CP spec v1.105 §2 row 5); this unit does NOT fabricate a resolver where none exists, and does NOT silently degrade to skipping the attestation.

**Tests (mutation-probed per PD-8):** **Integration witness — real OD accessor through the adapter through the CP step, driven DIRECTLY (not through a live CLI/production call site, per criterion #3):** `test_rt147_adapter_real_od_find_rotation_pair_evidence_through_verify_rotation_6_steps` (the test constructs the adapter via the factory and calls `verify_rotation_6_steps` directly with it — a genuinely valid OD-side pair drives `PROBE_VERIFY_AT_READ` to succeed through the FULL real chain — OD `find_rotation_pair_evidence` → adapter → CP `verify_rotation_6_steps`; an ABSENT pair drives the step to the explicit-incomplete failure, never a silent pass; a TAMPERED pair drives a `RotationPairIntegrityBreach` raise through the adapter unchanged — mutation probe: reverting the adapter's exception re-raise to a swallow-and-return-false makes the tampered-pair case silently report `succeeded=False` instead of raising, failing the test). **Independence witness:** `test_rt147_and_rt138_factories_independent` (constructing this unit's adapter via its factory does not require the audit-walk verifier's inputs, and vice versa). **Required-resolver witness:** `test_rt147_factory_without_key_identity_mapping_yields_adapter_pairing_that_cannot_succeed` (mutation probe: a factory that silently omits the resolver requirement passes this test incorrectly and must be rejected).
