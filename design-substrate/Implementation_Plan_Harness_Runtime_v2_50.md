...
3. **Atomic whole-fan-out admission (refined apply-PR rounds 45-48).** A fan-out's FULL N+S requirement is acquired in ONE atomic allocation call (the Protocol's fan-out-level operation) and split into per-branch leases; if the full allocation does not fit, the deterministic excess branches receive immediate per-branch rejections and only the fitting prefix is acquired — atomically either way, never incremental hold-and-wait, never partial-then-raise (per-branch-only atomicity lets two racing fan-outs mutually degrade when either could have run whole). Single-frame non-fan-out dispatch admission stays all-frames-or-fail-fast per reservation. Witness `test_simultaneous_fanout_admission_no_mutual_degradation` (two 3-frame fan-outs vs 4 free — one whole, one deterministically degraded; mutation probe: per-branch incremental acquisition degrades both and fails).
