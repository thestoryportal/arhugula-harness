...
Per (F) ratification 2026-05-29, v2.33 establishes a cross-axis cascade owed at CP plan v2.29 → v2.30 revision-pass arc:

| Element | Detail |
|---|---|
| CP plan v2.30 NEW units owed | 6 firing-site invocation units (one per §16.5.2 composer); each widens the corresponding CP-axis production function's signature to thread the runtime wiring callable + inserts the canonical post-resolve-pre-return invocation per CP spec v1.26 §16.5.7 firing-site discipline |
| Per-composer firing-site target | (i) `resolve_step_binding` at `per_step_override_evaluator.py:179` → `cp_is_wiring.emit_override_state_ledger_entry`; (ii) `select_engine_class` at `workload_binding_engine_class_selection.py:142` → `cp_is_wiring.emit_workload_class_selection_state_ledger_entry`; (iii) `PauseResumeProtocol` class methods at `pause_resume_protocol.py:214+` → `cp_is_wiring.emit_pause_resume_state_ledger_entry`; (iv) `rewrite_tool_call_to_hitl` at `hitl_as_tool_call_rewriting.py:149` → `cp_is_wiring.emit_hitl_tool_call_rewriting_state_ledger_entry`; (v) `capture_pause_snapshot` at `pause_resume_protocol.py:106` → `cp_is_wiring.emit_pause_captured_state_ledger_entry`; (vi) `attempt_resume` at `pause_resume_protocol.py:128` → `cp_is_wiring.emit_resume_attempted_state_ledger_entry` |
| Threading-shape resolution at CP plan v2.30 | Implementation-discretion per implementation-planner §10 — the runtime wiring callable arrives at the CP-axis function either via (a) new kw-only parameter on each production function widening the existing signature, (b) new field on `StepEffectiveBinding` / `StepExecutionContext` carrying the callable as part of step-resolution-time context, or (c) a thread-local / ContextVar substrate. CP plan v2.30 authors the choice; runtime plan v2.33 does NOT pre-commit to one shape (cross-axis scope discipline) |
| CXA v2.16 §0.4 forward-tracking | 6 PENDING entries transit to 6 LANDED upon paired CP plan v2.30 firing-site units landing; CXA narrow-scope revision-pass at v2.16 → v2.17 absorbs the §2.3.2 CP→IS bucket canonical enumeration refresh per v2.9 / v2.15 / v2.16 narrow-scope-CXA-revision precedent |
| H_T-RT-35 transit gate | PARTIAL → RETIRE-READY transit eligible when BOTH U-RT-110 (v2.33) LANDED + 6 CP plan v2.30 firing-site units LANDED + e2e verification per `[[verification-shape-sharpened-grep-vs-e2e]]` discipline shows production-callsite-fires-composer-fires-ledger-write at the full 6 source-unit surfaces. Retirement-event filing at workspace retirement-event ledger per `[[h-t-cp-19-default-gate-level-spec-extension]]` filing precedent |
...
(b) **CP plan v2.29 → v2.30 paired revision-pass arc owed per (F) ratification 2026-05-29.** Per §0.3 cross-axis cascade: 6 NEW CP plan v2.30 firing-site invocation units author the production-callsite widening (the threading shape for the runtime wiring callable arriving at the CP-axis function per implementation-discretion (a) / (b) / (c) at §0.3). Authoring is `implementation-planner` skill scope at the CP-axis stream (sa-cp sub-agent at Phase 7 7b cluster open). The paired revision is the LOAD-BEARING half for H_T-RT-35 transit; U-RT-110 LANDED at v2.33 + paired CP firing-sites LANDED + e2e verification COMPLETE = H_T-RT-35 PARTIAL → RETIRE-READY transit-eligible per `[[verification-shape-sharpened-grep-vs-e2e]]` discipline.

(c) **CXA v2.16 §0.4 forward-tracking 6 PENDING → 6 LANDED at paired-cascade-completion.** Per §0.3 cross-axis cascade row 4: CXA narrow-scope revision-pass at v2.16 → v2.17 absorbs the §2.3.2 CP→IS bucket canonical enumeration refresh at the same arc that retires H_T-RT-35. NOT owed at v2.33 publication (U-RT-110 is the runtime-side half only). Owed at CP plan v2.30 firing-site landings + retirement-batch filing arc.

(d) **H_T-RT-35 batch-filing precedent.** Per `[[h-t-cp-19-default-gate-level-spec-extension]]` filing precedent: when both halves of a paired-cascade transit-arc land, the retirement-event filing collapses the dual-axis arc into a single retirement-batch entry (e.g., "batch-46: H_T-RT-35 PARTIAL → RETIRE-READY via U-RT-110 wiring + 6 CP plan v2.30 firing-site units + CXA v2.17 §2.3.2 enumeration refresh"). v2.33 publication is the FIRST half; batch filing awaits the SECOND half (CP plan v2.30 + impl arc closures).
...
(i) **NO retirement event filing at v2.33 publication.** v2.33 is the runtime-side authoring half of the paired (F) arc; retirement-tier transit at H_T-RT-35 awaits BOTH halves LANDED + e2e verification COMPLETE per `[[verification-shape-sharpened-grep-vs-e2e]]` discipline. Filing transit at v2.33 alone would be X-AL-2 second-conjunct violation (substituted H_E surface still invoked: the 6 §16.5.7 firing sites at CP-axis production functions still bypass the runtime wiring).
...
| Successor consumption | U-RT-110 implementation arc against runtime plan v2.33; paired CP plan v2.29 → v2.30 firing-site authoring arc (`implementation-planner` skill at CP-axis; sa-cp sub-agent at Phase 7 7b cluster open); CXA v2.16 → v2.17 narrow-scope revision at paired-cascade-completion |
| Cross-axis cascade | NON-ZERO — 6 cross-axis dependency edges declared at §2.1 (U-RT-110 → U-CP-74..U-CP-79 — all LANDED); 6 CP plan v2.30 firing-site units owed per §0.3 (cascade-OWED, not cascade-LANDED); CXA v2.17 §2.3.2 enumeration refresh owed at retirement-batch filing arc |
