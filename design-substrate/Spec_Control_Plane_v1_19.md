# Specification — Control Plane v1.19

## Change-note (v1.18 → v1.19)

**Scope of revision.** Narrow-scope `HITLEscalationBrief.fail_detail_hash` field-type widening at C-CP-28 §25.2 (the v1.10-lineage canonical field-set declaration). v1.19 amends the field type from `fail_detail_hash: str` (non-Optional, no default) to `fail_detail_hash: str | None = None` (Optional, defaults to None). The amendment resolves CP spec v1.18 change-note **adjacent defect (i)** — `HITLEscalationBrief.fail_detail_hash` field-type at durable-async pause-trigger callsite. v1.18 surfaced 3 resolution options ((α) parallel widening / (β) sentinel value / (γ) carrier split); operator routed to **(α) parallel widening** at session 2026-05-25 ("Proceed" disposition on `fail_detail_hash` widening). v1.19 executes option (α) — the spec v1.18 change-note explicitly stated "if (α) is chosen at a follow-on arc, the amendment is structurally identical to v1.18." v1.19 is that structurally-identical follow-on. Operator-ratified 2026-05-25. ZERO change to other `HITLEscalationBrief` fields (`parent_step_id`, `parent_action_id`, `fail_class`, `escalation_reason`, `proposed_response_palette` all preserved verbatim). ZERO change to `ValidatorFailClass` enum body. ZERO change to `ValidatorResult.fail_detail_hash: str | None` (the field at v1.10 §25.2 ValidatorResult was already Optional per §25.2 line 170 area — v1.19 makes `HITLEscalationBrief.fail_detail_hash` parallel to that pattern). ZERO change to ValidatorFramework Protocol signature. ZERO new fail class. ZERO new contract. ZERO cross-axis cascade — `HITLEscalationBrief` is intra-CP-axis; consumed at runtime composer body per §14.8.8.1 step 1 (already in v1.24 spec text via the durable-async pause-trigger construction) + §14.15 C-RT-25 (validator-escalation gate composer); OD spec / CXA v2.10 / ADR / ADD / PRD unaffected.

**v1.18 substantive content preserved verbatim.** All v1.18 NEW §25.2.X (`HITLEscalationBrief.fail_class` field-type widening) preserved unchanged including §25.2.X.1 canonical-reading amendment + §25.2.X.2 rationale + §25.2.X.3 consumer-side semantics + §25.2.X.4 verbatim-layer integrity + §25.2.X.5 ValidatorResult.fail_class parallel posture preservation. All v1.17 NEW §6.5 (`StepEffectiveBinding.persona_tier` extension + `resolve_step_binding` signature widening) preserved verbatim. All v1.16 NEW §26.8 (`ResumeContext` carrier + `attempt_resume` signature widening) preserved verbatim. All v1.15 §19.1.1 (NEW) canonical 4-axis statement preserved verbatim. All v1.14 4-cite-cell amendments preserved. All v1.13 §28 ValidatorFramework rename preserved. All v1.12 §25.2.1 9th-field `workflow_id` amendment preserved. All v1.11 §26.2 `PauseReason` → `WorkflowPauseReason` rename + §26 NEW NOTE preserved. All v1.10 §25 / §26 / §27 substantive content preserved verbatim except the single-field-type widening recorded at §25.2.X canonical-reading amendment at v1.18 §25.2.X (`fail_class`) + §25.2.Y canonical-reading amendment at v1.19 §25.2.Y below (`fail_detail_hash`). All v1.6 / v1.2 substantive content preserved verbatim.

**Source of fix.** CP spec v1.18 change-note **adjacent defect (i)** ("`HITLEscalationBrief.fail_detail_hash` field-type at durable-async pause-trigger callsite") + runtime impl arc at `hitl_gate_composer.py:985` carrying sentinel value `fail_detail_hash="0" * 64` (option (β)) per U-RT-94 composer body landing at `06ed03d` (2026-05-24) + operator routing decision at session 2026-05-25 ("Proceed" disposition selecting option (α) parallel widening per pre-arc orientation menu). Empirical-verification at HEAD `ea437b4`: production composer site at `harness-runtime/src/harness_runtime/lifecycle/hitl_gate_composer.py:981-988` constructs `HITLEscalationBrief(... fail_class=None, fail_detail_hash="0" * 64, ...)` at durable-async cell HITL pause-trigger context. Post-v1.19, the sentinel value is dropped — composer constructs `fail_detail_hash=None` directly. ZERO production reads of `brief.fail_detail_hash` exist at HEAD per grep verification (`grep -rn "brief\.fail_detail_hash\|escalation_brief\.fail_detail_hash"` returns empty; only `validator_namespace.py` declares its OWN field with the same name, NOT a reader of HITLEscalationBrief's field).

**Authority basis for fix direction.** The runtime spec v1.24 §14.8.8.1 step 1 construction site is the canonical consumer of `HITLEscalationBrief` at the durable-async cell HITL pause-trigger composition arc; the construction context has no validator outcome (the cell is firing on synchrony class alone — `binding.persona_tier × binding.engine_class → SynchronyClass.DURABLE_ASYNC` per C-CP-18 §18.1 matrix + CP spec v1.17 §6.5.2 carrier field-set extension authoring the canonical `persona_tier` field). The validator-failure construction context (the v1.10 original intent at C-CP-28 §25.2 — `ValidatorFramework.evaluate(...)` returning `ValidatorOutcome.ESCALATE` with a typed `HITLEscalationBrief` carrying the fail context including `fail_detail_hash = sha256(fail_reason_text)` per §25.8 deferred-to-discretion shape) remains valid; v1.19 broadens the carrier shape to accommodate the durable-async pause-trigger construction context as well. The parallel precedent is `ValidatorResult.fail_detail_hash: str | None` at v1.10 §25.2 (already Optional with `# None if outcome=PASS` semantic at parallel field); v1.19 aligns `HITLEscalationBrief.fail_detail_hash` with the same Optional posture. The two contexts produce semantically distinct construction shapes (validator-failure: `fail_class` populated + `fail_detail_hash` populated with sha256 of fail-reason; pause-trigger: `fail_class=None` + `fail_detail_hash=None`); consumers MUST disambiguate via the construction-context discipline at §14.8.8.1 step 1 vs §14.15 C-RT-25 — and `fail_class is None` + `fail_detail_hash is None` jointly form the durable-async-pause-trigger discriminator post-v1.19 (vs `fail_class is None` alone as the implicit discriminator at v1.18 per §25.2.X.3). The structural-symmetry argument at v1.18 §25.2.X.5 holds identically for `fail_detail_hash`: both `HITLEscalationBrief` Optional fields now have parallel `ValidatorResult` Optional precedents.

**Single amendment site (1 field-type widening canonical-reading amendment).**

| Site | Amendment shape |
|---|---|
| **§25.2.Y (NEW at v1.19) — `HITLEscalationBrief.fail_detail_hash` field-type widening (canonical-reading amendment)** | Authors a canonical-reading amendment over the v1.10 §25.2 `HITLEscalationBrief` dataclass body (preserved verbatim in v1.10 file): `fail_detail_hash: str` → `fail_detail_hash: str | None = None`. The v1.10 file body is NOT edited — delta-only spec-chain preservation discipline preserved per v1.13 §1.3 + v1.14 §1.3 + v1.15 §19.1.1.4 + v1.16 §26.8.4 + v1.17 §6.5.5 + v1.18 §25.2.X.4 verbatim-layer-integrity precedent. Consumers reading the delta chain interpret the v1.10 §25.2 `HITLEscalationBrief` carrier field-set AS canonically supplemented at v1.18 §25.2.X (`fail_class` widening) + v1.19 §25.2.Y (`fail_detail_hash` widening) per these change-notes. |

**Adjacent harmonization sites.** None — the amendment is surgical at one field. §25.2 `ValidatorResult.fail_detail_hash` field-type (already `str | None` at v1.10) preserved verbatim. `ValidatorFailClass` enum body preserved verbatim. `ValidatorOutcome` enum body preserved verbatim. `ValidatorNextAction` enum body preserved verbatim. §25.3-§25.8 (lifecycle / invocation / span / failure-mode / invariants / deferred-discretion) preserved verbatim. The other 4 non-amended fields on `HITLEscalationBrief` (`parent_step_id`, `parent_action_id`, `escalation_reason`, `proposed_response_palette`) preserved verbatim with non-Optional types — these fields have canonical values at both construction contexts (`parent_step_id` + `parent_action_id` populated at both; `escalation_reason` populated at both with the construction-context label (`"durable_async_cell_synchrony"` at pause-trigger vs validator-derived prose at escalation); `proposed_response_palette` populated at both with the operator-effective palette).

**Sections preserved verbatim from v1.18.** All v1.18 NEW §25.2.X (`fail_class` widening) + all v1.17 NEW §6.5 + all v1.16 NEW §26.8 + all v1.15 §19.1.1 + all v1.14 / v1.13 / v1.12 / v1.11 / v1.10 / v1.6 / v1.2 substantive content preserved verbatim.

**Status posture.** Proposed (v1.18) → **Proposed (v1.19)**. v1.19 is a fidelity-pure field-type widening amendment — single existing carrier field changes from non-Optional to Optional with backward-compatible default. NO v1.18 contract removed; NO v1.18 contract re-decomposition; NO v1.18 field-set added or removed. Contract count unchanged at 28. Fail-class count unchanged. Signature change at any Protocol: ZERO (the `HITLEscalationBrief` is a typed payload, not a Protocol). Field-set change at any field set: ONE — `HITLEscalationBrief.fail_detail_hash` widened from `str` to `str | None = None` (backward-compatible at validator-failure callsites — existing callers passing `sha256(fail_reason_text)` continue to work unchanged; new callers at durable-async pause-trigger callsites can pass `None` or omit the parameter entirely per the default). Acceptance criterion change at any contract: NONE at spec-side (CP plan U-CP-59 absorption AC re-decomposition is downstream absorption per (b) below; runtime plan U-RT-94 AC #13 re-evaluation is Phase 3 step 4). Behavior change: NONE at canonical validator-failure callsites — `fail_detail_hash` is a typed-payload field; widening to Optional preserves all existing semantics. Behavior change at durable-async pause-trigger callsites: composer body at §14.8.8.1 step 1 can construct `HITLEscalationBrief(..., fail_detail_hash=None, ...)` without sentinel-value synthesis (was: composer carried `"0" * 64` sentinel per option (β) at v1.18 deferral).

**Downstream absorption owed (post-v1.19).**

(a) Workspace `CLAUDE.md` §2.3 CP spec row version bump (v1.18 → v1.19); co-published this arc OR next bookkeeping commit.

(b) **CP plan v2.22 → v2.23** — single-unit-body amendment at U-CP-59 (the cluster 10-CP-A unit landing `HITLEscalationBrief` dataclass body per C-CP-28 §28.3 implementation). Add new AC covering the field-type widening. Files-column unchanged (the dataclass body file already exists; only the field type annotation changes). NO new unit; NO new cluster; NO DAG topology change. Co-published this arc.

(c) **harness-cp impl** updates: `harness-cp/src/harness_cp/validator_framework_types.py:148` — AMEND `fail_detail_hash: str` annotation to `fail_detail_hash: str | None = None`. Backward-compatible: existing validator-escalation construction callsites passing sha256(fail_reason_text) continue to work unchanged. New durable-async pause-trigger construction callsites at runtime composer body per §14.8.8.1 step 1 can pass `fail_detail_hash=None` or omit. Co-published this arc.

(d) **harness-runtime impl** updates: at `harness-runtime/src/harness_runtime/lifecycle/hitl_gate_composer.py:985` durable-async cell composer construction site — DROP `fail_detail_hash="0" * 64` sentinel; AMEND to `fail_detail_hash=None` per v1.19 widening. Helper test docstring at `harness-runtime/tests/test_lifecycle_hitl_pause_trigger_helpers.py:123` references the sentinel pattern; UPDATE docstring text to reflect v1.19 widening. Co-published this arc.

(e) **runtime plan v2.25 → v2.26** — U-RT-94 AC #13 amendment per CP spec v1.19 (advisor-flagged staleness post-v1.19: AC #13 at v2.25 text "no parallel widening at v1.18" becomes stale; AMEND in-place per `[[advisor-before-substantive-work-for-cross-axis-blockers]]` precedent — option (β) absorption). U-RT-93 helper test docstring may be referenced via its test landing AC; verify and amend. Co-published this arc.

(f) **OD spec / OD plan / OD impl**: ZERO — `HITLEscalationBrief.fail_detail_hash` is intra-CP-axis (consumed by runtime composer at construction site only; no production reader of the field exists). `validator_namespace.py:220` declares its OWN `validator_fail_detail_hash: str` field for OD-side namespace ingestion attribute set; NOT a reader of HITLEscalationBrief's field. ZERO OD cascade. The OD-side `validator.fail.detail_hash` span attribute emission per C-OD-NN remains canonical and is populated from `ValidatorResult.fail_detail_hash` (already Optional at v1.10) at the validator-failure construction context; pause-trigger context does NOT emit `validator.fail.*` attributes (no validator outcome exists).

(g) **CXA v2.10**: ZERO — no new cross-axis edge; the field-type widening is internal to C-CP-28; no consumer crosses the axis boundary at v1.19.

(h) **ADR-D1 / ADR-D5 / ADD / PRD**: ZERO retag owed — `HITLEscalationBrief.fail_detail_hash` Optional posture is a derivative narrowing within C-CP-28's contract surface (already ADR-D3 v1.2 §1.1 #1 validation-contract territory; the Optional posture aligns with the parallel `ValidatorResult.fail_detail_hash: str | None` already canonical at v1.10 §25.2). ZERO upstream-artifact revision triggered.

**Adjacent defects surfaced (NOT patched per FM-2 no-extension discipline).**

(i) **CP spec v1.18 change-note adjacent defect (ii) "Construction-context discipline at consumers."** v1.18 noted that consumers MUST disambiguate via `fail_class is None` as the implicit durable-async-pause-trigger discriminator. Post-v1.19, the discriminator is more precisely `fail_class is None AND fail_detail_hash is None`. The construction-context-discipline burden on consumers is unchanged at v1.19 (still implicit via Optional-field-None-check; no explicit `construction_context: Literal[...]` discriminator field added). v1.18 adjacent defect (ii) preserved as OPEN; the v1.19 amendment narrows the discriminator predicate but does not change the construction-context-discipline shape.

(ii) **CP spec v1.18 change-note adjacent defect (iii) "`ValidatorFailClass` enum extension foreclosed."** v1.18 foreclosed enum extension at FM-2; v1.19 preserves the foreclosure verbatim. Future arcs requiring an explicit pause-trigger discriminator carry option (γ) carrier split per v1.18 adjacent defect (i) — the Optional posture is the canonical pattern for the dual-context construction at v1.19.

(iii) **`HITLEscalationBrief.escalation_reason` field NOT widened.** The field carries an operator-readable summary string at both construction contexts (validator-failure: validator-derived prose; pause-trigger: `"durable_async_cell_synchrony"` literal per runtime impl at `hitl_gate_composer.py:986`). The field is meaningful at both contexts and was NOT identified as needing widening at v1.18 nor at v1.19. Field-set parity at the brief is now: `fail_class` Optional (v1.18), `fail_detail_hash` Optional (v1.19), all other 4 fields non-Optional with canonical values at both construction contexts. Surfaced; no action owed.

(iv) **U-CP-59 cluster-boundary edge to runtime composer body.** The U-RT-94 composer body amendment per (e) consumes `HITLEscalationBrief` per the v1.19 widened type. The cluster-boundary edge from runtime plan v2.25 → v2.26 L9-terdecies cluster to CP plan v2.23-cluster-10-CP-A is implicit via the typed carrier import at `harness-runtime/src/harness_runtime/lifecycle/hitl_gate_composer.py:141` (the composer body imports `HITLEscalationBrief` from harness-cp). v1.19 does NOT amend the cluster-boundary edge declaration at the plan-level; implementation-planner absorption per (b) + (e) handles the cluster-boundary edge at the AC text only (the cluster-boundary edge declaration in the plan's §3 §3.1 DAG topology unchanged at v2.23/v2.26). Surfaced; routed to implementation-planner revision-pass per CP plan + runtime plan amendments this arc.

---

## §25.2.Y (NEW at v1.19) — `HITLEscalationBrief.fail_detail_hash` field-type widening (canonical reading)

### §25.2.Y.1 Canonical-reading amendment

The v1.10 §25.2 `HITLEscalationBrief` dataclass body at v1.10 file lines 201-210, AS canonically supplemented at v1.18 §25.2.X.1, is further canonically read at v1.19 as:

```python
class HITLEscalationBrief(BaseModel):
    parent_step_id: str
    parent_action_id: str
    fail_class: ValidatorFailClass | None = None         # WIDENED at v1.18 §25.2.X.1 (was: ValidatorFailClass, non-Optional, no default)
    fail_detail_hash: str | None = None                  # WIDENED at v1.19 §25.2.Y.1 (was: str, non-Optional, no default)
...
(Note: the implementation at HEAD uses Pydantic v2 `BaseModel` with `model_config = ConfigDict(frozen=True)` per `harness-cp/src/harness_cp/validator_framework_types.py:133-150`; the spec's original `@dataclass(frozen=True)` shape at v1.10 §25.2 is honored as a typed-payload contract — implementation discretion per Pydantic-v2-vs-dataclass selection at U-CP-59 landing.)

The v1.10 file body at lines 201-210 is NOT edited — delta-only spec-chain preservation discipline preserved per v1.13 §1.3 + v1.17 §6.5.5 + v1.18 §25.2.X.4 verbatim-layer-integrity precedent. Consumers reading the delta chain interpret the v1.10 §25.2 `HITLEscalationBrief` carrier field-set AS canonically supplemented at v1.18 §25.2.X.1 + v1.19 §25.2.Y.1 per these change-notes.

### §25.2.Y.2 Field-type widening rationale

The widening from non-Optional to Optional-with-default-None resolves the dual-context construction discipline at:

| Construction context | Callsite | `fail_detail_hash` value |
|---|---|---|
| Validator-failure escalation | `ValidatorFramework.evaluate(...)` returning `ValidatorOutcome.ESCALATE`; §14.15 C-RT-25 ValidatorEscalationGateComposer composition | Populated per the validator outcome — `sha256(fail_reason_text)` per CP spec v1.10 §25.8 deferred-to-discretion shape; 64-char hex string |
| Durable-async cell HITL pause-trigger | Runtime spec v1.24 §14.8.8.1 step 1 composer-body construction | `None` (no validator outcome at construction site; no fail-reason text exists; cell is firing on synchrony class alone per C-CP-18 §18.1 + CP spec v1.17 §6.5.2) |

The Optional posture aligns with the parallel `ValidatorResult.fail_detail_hash: str | None` at v1.10 §25.2 (already Optional). Pre-v1.19, the `HITLEscalationBrief.fail_detail_hash` field was inconsistent with the parallel `ValidatorResult.fail_detail_hash` Optional posture — v1.19 harmonizes the two. Pre-v1.19, the durable-async pause-trigger construction site at `hitl_gate_composer.py:985` carried sentinel value `"0" * 64` per option (β) at v1.18 deferral — the sentinel is dropped post-v1.19 absorption.

### §25.2.Y.3 Consumer-side semantics

Post-v1.19, consumers of `HITLEscalationBrief` (the HITL gate composer at runtime spec v1.24 §14.8.2 step 4 cluster + the validator escalation gate composer at runtime spec v1.22 §14.15 C-RT-25 + the audit-payload composers at OD spec §C-OD-30) MUST treat `fail_class is None AND fail_detail_hash is None` jointly as the durable-async-pause-trigger construction discriminator (v1.18 used `fail_class is None` alone; v1.19 narrows the discriminator to the joint check). Future spec amendments adding explicit construction-context discriminator (e.g., `construction_context: Literal["validator_failure", "pause_trigger"]`) remain foreclosed at v1.19 per FM-2 no-extension discipline.

Empirical verification at HEAD: ZERO production reads of `brief.fail_detail_hash` exist (`grep -rn "brief\.fail_detail_hash\|escalation_brief\.fail_detail_hash"` returns empty). The widening is consumer-safe — only the construction site at `hitl_gate_composer.py:985` is affected by the v1.19 amendment, and only as a sentinel-drop transformation.

### §25.2.Y.4 Verbatim-layer integrity

The v1.10 file (`Spec_Control_Plane_v1_10.md`) §25.2 `HITLEscalationBrief` dataclass body at lines 201-210 is NOT edited at v1.19 — delta-only spec-chain preservation discipline preserved per v1.13 §1.3 + v1.14 §1.3 + v1.15 §19.1.1.4 + v1.16 §26.8.4 + v1.17 §6.5.5 + v1.18 §25.2.X.4 verbatim-layer-integrity precedent. The §25.2.Y.1 canonical-reading amendment at v1.19 is the canonical interpretation of the `HITLEscalationBrief.fail_detail_hash` field-type going forward; consumers reading the delta chain interpret the v1.10 §25.2 body AS canonically supplemented at v1.18 §25.2.X.1 (`fail_class`) + v1.19 §25.2.Y.1 (`fail_detail_hash`).

### §25.2.Y.5 ValidatorResult.fail_detail_hash parallel posture preservation

The v1.10 §25.2 `ValidatorResult.fail_detail_hash: str | None` field (already Optional with `# None if outcome=PASS`-area semantic at v1.10) is preserved verbatim at v1.19. The amendment at `HITLEscalationBrief.fail_detail_hash` brings the parallel carrier to the same Optional posture; the two carriers now have structurally-identical `fail_detail_hash` field shapes, completing the symmetry that v1.18 began for `fail_class`. The structural symmetry is intentional and load-bearing — future amendments to either carrier's `fail_*` field-types SHOULD preserve the symmetry per FM-2 no-extension discipline.

Post-v1.19 carrier shape parity table:

| Field | `ValidatorResult` (v1.10) | `HITLEscalationBrief` (v1.18+v1.19) |
|---|---|---|
| `fail_class` | `ValidatorFailClass \| None` (v1.10) | `ValidatorFailClass \| None` (v1.18 widened) |
| `fail_detail_hash` | `str \| None` (v1.10) | `str \| None` (v1.19 widened) |

The two carriers' `fail_*` fields now have identical Optional postures — full structural parity achieved at v1.19.

---

## §1.5 — v1.18 change-note preserved verbatim

The following sections preserve the v1.18 change-note + §25.2.X content verbatim. All v1.18 amendments stand at v1.19.
