...
DAG: ZERO new within-axis edges; NEW edges (atomic, unit-to-unit): within-axis U-CP-85/86/88 → U-CP-101 (the Protocol + default-authority declaration unit, codex round-18 — the CP consumers need only the declaration, never the runtime adapter, breaking the would-be CP↔Runtime cycle); cross-axis U-CP-101 → U-CORE-03 and U-CP-82 → U-IS-11 (the writer-owned drain-timestamp API its call-site change consumes — a DAG edge, not only the co-land pin; codex rounds 14/16). Acyclicity preserved (Kahn-verifiable, codex round-17: the new edges point CP → Runtime AND CP → IS — U-CP-82 → U-IS-11; IS is declared 0-outbound (no IS unit depends on any CP or Runtime unit, verified at IS plan v2.7), and no Runtime v2.50 unit depends on any U-CP-* unit, so every new edge leaves an axis with no return path — acyclic). Co-land pins recorded (not DAG edges): U-CP-86/U-CP-88 ⊕ U-RT-141; U-CP-85/U-CP-86 ⊕ U-RT-144; U-CP-82 ⊕ IS plan v2.7 U-IS-11 — all land in the one B-48 impl arc.
