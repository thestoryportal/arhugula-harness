...
- **(§1 row 3 — refined rounds 45/46.)** the fan-out's FULL N+S requirement is acquired in ONE atomic admission then split into per-branch leases (per-branch release lifecycle unchanged); if the full allocation does not fit, the deterministic excess branches fail-fast IMMEDIATELY and only the fitting prefix is acquired — never incremental hold-and-wait (codex rounds 45/46: per-branch-only atomicity lets two racing fan-outs each start some branches and mutually degrade when either could have run whole); witness: the simultaneous-admission scenario; witness `test_simultaneous_fanout_admission_no_mutual_degradation` (two 3-frame fan-outs vs 4 free — one runs whole, the other degrades deterministically; mutation probe: per-branch incremental acquisition degrades both and fails).
