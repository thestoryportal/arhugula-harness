...
21. **(§21.2.1 row 7 — enforcement site, codex round-8 P2.)** `sign_rotation_pair` is PROHIBITED at MULTI_TENANT_COMPLIANCE until `B-33`'s rotation-aware message binding lands (the v1.33 out-of-scope deferral STRENGTHENED to an explicit prohibition); non-MTC tiers unchanged. The function itself takes no tier input and has ZERO production callers on `main` (verified at apply time — its API is NOT amended here; adding a tier parameter would be an unratified contract change), so the enforceable slice at THIS arc is a CALLER-REGRESSION GUARD: a static witness asserting `sign_rotation_pair` has no production caller ANYWHERE in `harness-runtime`/`harness-cp`/`harness-cxa`/`harness-od` source — including intra-module callers inside the defining module (only the `def` site itself and tests excluded; codex rounds 29/36). A tier-aware API-boundary guard is STRUCTURALLY impossible without an unratified signature change (the function has no tier input) — that guard is `B-33`'s carried obligation (B-33 threads rotation-aware binding AND the tier signal); until then the spec prohibition + this widened static guard are the bounded enforceable slice (a new caller must confront the prohibition and this test), replacing any vacuous no-call runtime test; the tier-aware refusal at the first real call site is `B-33`'s carried obligation (recorded at that row).
