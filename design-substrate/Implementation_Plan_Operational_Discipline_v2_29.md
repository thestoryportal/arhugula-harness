...
18. **(§21.2.1 row 2 — dyad-1 pin; normalizer HOMED in `harness_od`, codex round-2 P2.)** The impl AUTHORS the tenant-tag normalizer as a `harness_od` export (the ONE authority; rules: `None` → no segment; empty string and the reserved `"_single"` literal REFUSED with a raise, never silently normalized; any other value passes through unchanged — `harness-od` cannot import the runtime writer, so the rules live HERE and the writer delegates, per the U-RT-137 enforcement criterion). Signing applies this normalizer to its `tenant_id` before composing the fifth segment — the signed segment and the sidecar `tenant_tag` join key are the SAME token by construction (the U-OD-55 verifier applies the identical normalizer to its tenant-scope input).
