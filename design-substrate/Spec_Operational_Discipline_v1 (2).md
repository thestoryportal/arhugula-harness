...
| Source-set | `PRD_v1.0.md` §4 (R-OD-01 through R-OD-08); `Architectural_Design_Document_v1.md` v1.2 §3.4.1 + §5.1.1 + §5.1.3 + §5.3.1 + §6.3.1 + Appendix B; `ADR-D6.md` v1.1 (§Decision + §1.1 + §1.2 + §1.2.1 + §1.3 + §1.4 + §1.5 + §1.6 + §1.7 + §1.8 + §1.9); secondary-axis ADRs by citation only at expected versions (F1 v1.2 / F2 v1.2 / F3 v1.1 / F4 v1.1 / F5 v1.1 / D1 v1.1 / D2 v1.1 / D3 v1.2 / D4 v1.1 / D5 v1.3); `Persona_Document_v1.md` §2 + §4 + §6 + §9 + §10.2 + §10.4 + §11.4 + §11.10 + §11.12; `Spec_Information_Substrate_v1.md` (C-IS-05 + C-IS-06 + C-IS-07 + C-IS-10); `Spec_Action_Surface_v1.md` (C-AS-14 + C-AS-15 + C-AS-16); `Spec_Control_Plane_v1.md` (C-CP-03 + C-CP-05 + C-CP-09 + C-CP-14 + C-CP-20 + C-CP-21 + C-CP-24) |
...
| Revision | v1.1 → v1.2 (P5-CK iter-2 close final-revision-pass per modified `Project_Workflow_v1_2.md` §4.1.2 path — F-iter2-03 / Pattern P2-PHASE-5 use-latest-version body-citation discipline applied at fourteen live-body sites: C-OD-05 §5.1 rows 1/2/3/4/12/13 (six sites at lines 359-371; bounded-sed); C-OD-14 §14.4 cost-attribution table rows 1-2 (two sites at lines 507-508; `anthropic.cache_creation_input_tokens` + `anthropic.cache_read_input_tokens`); C-OD-14 §14.8 always-sampled list cells (five sites at lines 556/558/559/561/562 — `mcp.tool.call`, `files.operation`, `memory.operation`, `managed_agents.runtime`, `skill.activation` via-citations through C-AS-14 §14.8); C-OD-?? `cache-hit-rate-alignment-floor` cell (line 989 — citation `ADR-D3 v1.1 §1.5 + §1.8` → v1.2 form); Status block Source-set field secondary-axis citation (D3 v1.1 → D3 v1.2 at line 13). Scope expanded from session-prompt-enumerated 8 sites to 14 sites under Stage-3 scope adjudication Option A per operator selection at session-prompt §3.4. Cited content materially unchanged at ADR-D3 v1.2 §1.8.1 + §1.5 + §1.8 per `P5-CK_Iteration_2_Close_Handoff.md` §3.5; token-level alignment only. §[coherence pass] audit rows at lines 1540 and 1569 preserved verbatim per discipline. All other contracts preserved verbatim.) |
| Revision date | 2026-05-13 |
...
**Scope of revision.** Single-finding final revision pass clearing `Adversarial_Review_5_iter2.md` F-iter2-03 / Pattern P2-PHASE-5 (Class 1 mechanical — body-citation drift `ADR-D3 v1.1` → `v1.2` across fourteen live-body sites in OD spec). Cited content materially unchanged at ADR-D3 v1.2 §1.8.1 + §1.5 + §1.8 per `P5-CK_Iteration_2_Close_Handoff.md` §3.5; token-level alignment only under use-latest-version discipline. Stage-3 scope expanded from session-prompt-enumerated 8 sites to 14 sites under operator Option A selection at session-prompt §3.4 (unenumerated sites at lines 508, 556, 558, 559, 561, 562, 989 were structurally identical Pattern P2-PHASE-5 occurrences not captured in `P5-CK_Iteration_2_Close_Handoff.md` §3.5 cross-spec revision impact summary; deferral of these sites would have re-introduced the systemic drift this revision pass exists to resolve and surfaced as §4.1.2 escalation at Phase 6 entry-gate verification).

**Fourteen-site amendment inventory.** Sites grouped by execution mechanic:

1. **Status block Source-set field (line 13; one site).** Secondary-axis version manifest citation `D3 v1.1` → `D3 v1.2` (within parenthetical list `F1 v1.2 / F2 v1.2 / F3 v1.1 / F4 v1.1 / F5 v1.1 / D1 v1.1 / D2 v1.1 / D3 v1.1 / D4 v1.1 / D5 v1.3`).
2. **C-OD-05 §5.1 namespace-source-version-manifest rows (six sites; lines 359/360/361/362/370/371; bounded sed).** Rows 1 (`anthropic.*`), 2 (`mcp.*`), 3 (`skill.*`), 4 (`managed_agents.*`), 12 (`files.*`), 13 (`memory.*`); each row's Source-ADR cell carries `ADR-D3 v1.1 §1.8.1` → `ADR-D3 v1.2 §1.8.1`.
3. **C-OD-14 §14.4 cost-attribution table rows 1–2 (two sites; lines 507–508; bounded sed).** `anthropic.cache_creation_input_tokens` and `anthropic.cache_read_input_tokens` rows; each carries `ADR-D3 v1.1 §1.8.1 per C-AS-14 §14.2` → v1.2 form.
4. **C-OD-14 §14.8 always-sampled list via-citations (five sites; lines 556/558/559/561/562; bounded sed).** `mcp.tool.call`, `files.operation`, `memory.operation`, `managed_agents.runtime`, `skill.activation` rows; each carries `C-AS-14 §14.8 (D3 v1.1 §1.8.1 ...)` → `(D3 v1.2 §1.8.1 ...)` (no `ADR-` prefix at these cells per their cite convention).
5. **C-OD-?? cache-hit-rate-alignment-floor cell (one site; line 989).** Citation `ADR-D3 v1.1 §1.5 + §1.8 (declared at C-AS-14 §14.2)` → `ADR-D3 v1.2 §1.5 + §1.8 (declared at C-AS-14 §14.2)`.

**§[coherence pass] preservation discipline.** Two `D3 v1.1` occurrences in §[coherence pass] (lines 1540 in Fabricated citations audit row, 1569 in ADR secondary citation manifest) are preserved verbatim as v1 + v1.1 historical record. The §[coherence pass] section captures the point-in-time audit state at v1/v1.1 filing; v1.2 final-revision-pass mechanical body-citation bumps do not re-run the audit per `P5-CK_Iteration_2_Final_Revision_Pass_Session_Prompt.md` §5.2 §[coherence pass] preservation discipline.

**Sections preserved verbatim at v1.2.** §Front-matter (Axis declaration; Axis-grounding note; Persona summary; PRD requirement scope; ADR commitment scope; cross-axis citation table; Deferred to implementation discretion — only Source-set field bumped at line 13); §1 C-OD-01 (per-deployment-surface × per-persona-tier matrix); §2 C-OD-02 through §4 C-OD-04 (cell-bounded contracts); §5 C-OD-05 (all contract content; only the six §5.1 namespace-source-version-manifest cells at lines 359/360/361/362/370/371 bumped — table structure, ingestion postures, invariants all preserved); §6 C-OD-06 through §13 C-OD-13 (all contracts); §14 C-OD-14 (all contract content; only §14.4 cost-attribution rows at lines 507–508 and §14.8 always-sampled list rows at lines 556/558/559/561/562 bumped — schema structure, sampling discipline, cost-attribution dashboarding all preserved); §15 C-OD-15 through §22 C-OD-22 (all contracts; only the cache-hit-rate-alignment-floor cell at line 989 bumped — surrounding C-OD-?? primitive structure preserved); §23 C-OD-23; §[traceability] matrix (D3 v1.1 → v1.2 in cross-spec traceability if present per use-latest-version discipline); §[carry-forwards]; §[coherence pass] (preserved verbatim as v1 + v1.1 historical record per discipline).

**Changes inline.** Status block (Status row revised; second pair of Revision row + Revision date row appended for v1.1 → v1.2; Source-set field D3 v1.1 → D3 v1.2 at secondary-axis citation list). This Change-note section (new; appended between Status block and previous Change-note v1 → v1.1). C-OD-05 §5.1 rows 1/2/3/4/12/13 (six sites). C-OD-14 §14.4 rows 1–2 (two sites). C-OD-14 §14.8 always-sampled list rows for `mcp.tool.call` + `files.operation` + `memory.operation` + `managed_agents.runtime` + `skill.activation` (five sites). Cache-hit-rate-alignment-floor cell at line 989 (one site). Fourteen amendment sites total + Status block + Change-note discipline; no other content modified; §[coherence pass] preserved verbatim.
...
|---|---|---|---|---|---|
| 1 | `anthropic.*` | ADR-D3 v1.2 §1.8.1; declared at `Spec_Action_Surface_v1.md` C-AS-14 §14.2 | 10 | C-AS-14 §14.2 | Ingest verbatim |
| 2 | `mcp.*` | ADR-D3 v1.2 §1.8.1; declared at C-AS-14 §14.3 | 7 | C-AS-14 §14.3 | Ingest verbatim |
| 3 | `skill.*` | ADR-D3 v1.2 §1.8.1; declared at C-AS-14 §14.4 | 6 | C-AS-14 §14.4 | Ingest verbatim |
| 4 | `managed_agents.*` | ADR-D3 v1.2 §1.8.1; declared at C-AS-14 §14.5 | 3 | C-AS-14 §14.5 | Ingest verbatim |
...
| 12 | `files.*` | ADR-D3 v1.2 §1.8.1; declared at C-AS-14 §14.6 | 8 | C-AS-14 §14.6 | Ingest verbatim |
| 13 | `memory.*` | ADR-D3 v1.2 §1.8.1; declared at C-AS-14 §14.7 | 6 | C-AS-14 §14.7 | Ingest verbatim |
...
| `anthropic.cache_creation_input_tokens` | Cache-tier breakdown — tokens charged at 1.25× rate (5-min TTL creation) | ADR-D3 v1.2 §1.8.1 per C-AS-14 §14.2 |
| `anthropic.cache_read_input_tokens` | Cache-tier breakdown — tokens charged at 0.10× rate (cache hit) | ADR-D3 v1.2 §1.8.1 per C-AS-14 §14.2 |
...
| `mcp.tool.call` | C-AS-14 §14.8 (D3 v1.2 §1.8.1 + F2-09 closure) | C10 audit requirement; head=1.0 with tail-keep-on-trust-tier-floor-violations |
...
| `files.operation` at `kind ∈ {upload, delete}` | C-AS-14 §14.8 (D3 v1.2 §1.8.1) | Files API mutation operations |
| `memory.operation` at `kind ∈ {write, update, delete}` | C-AS-14 §14.8 (D3 v1.2 §1.8.1) | Memory tool mutation operations |
...
| `managed_agents.runtime` | C-AS-14 §14.8 (D3 v1.2 §1.8.1) | Cost attribution ($0.08/hr non-trivial) |
| `skill.activation` | C-AS-14 §14.8 (D3 v1.2 §1.8.1) | head=1.0 design-time; base-rate at production |
...
| `cache-hit-rate-alignment-floor` | ADR-D3 v1.2 §1.5 + §1.8 (declared at C-AS-14 §14.2) | Ratio `cache_read.input_tokens / (cache_read.input_tokens + cache_creation.input_tokens)` rolled up per agent role and per session; alignment-floor threshold operator-tunable |
