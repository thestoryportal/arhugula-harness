...
`team-binding × local-development` is head-based at a default base-rate of **0.5**, and the tail-keep processor is not engaged at that surface at all (the production materializer wraps the BSP only when `deployment_surface != LOCAL_DEVELOPMENT`). The head half is therefore a real, non-vacuous exposure, and it cannot be repaired at candidate (a)'s venue for a structural reason: a span's events do not exist at span creation, so the head sampler has nothing to inspect. It is stated below as a **declared bound**, not as a discharged one.

**A first drafting of this delta then made a second error in the opposite direction, and it is corrected here rather than shipped.** It concluded from the table above that the exposure sat at *"exactly one live cell"* and that *"every `TAIL_BASED_PROD` cell, which is every production cell, resolves at the tail and is covered by terms 1–3."* **That is false**, and it was caught by out-of-family review and then falsified by measurement rather than by argument. Production resolves the head sampler from the §10.3 envelope **unconditionally in both §9.1 modes** — `tracer_provider.py` binds `build_default_sampler(base_rate=PER_CELL_BASE_RATE_ENVELOPE[cell].default_rate)` and states in the same breath that *"the current default sampler ignores the mode"* — so a `TAIL_BASED_PROD` cell at base-rate 0.1 drops ~90% of event carriers **before** the tail consumer exists to classify them. The corrected statement is term 4's table; the residual is the same head-half bound, now stated to bite at **five of eight** ACTIVE cells rather than one. *(Recorded because "the exposure is one dev cell" would have been a comforting and wrong claim to leave in a contract.)*
...
**One further structural fact the measurement supplies, and it bears directly on F-08.** At the two multi-tenant cells the head admits only ~20% of event carriers (term 4's table), so the keep-volume this arm adds at those cells is bounded **above** by one-fifth of the exhausted-dispatch rate — still a population figure nobody has measured, and still not asserted here, but the measurement does establish that the arm cannot admit more than the head already admitted. **Row `B-137` inverts exactly this**: if the head were made unconditionally admitting in `TAIL_BASED_PROD` mode, that ×5 headroom disappears and the F-08 question becomes materially larger. **The two rows must therefore be measured together, and `B-137`'s close-out says so** — F-08 measured against today's admission would not survive `B-137`'s repair.
...
**Term 4 — the HEAD consumer is a DECLARED BOUND, not a discharged one, and the bound reaches FIVE of the eight ACTIVE cells.** The head sampler resolves at span **creation**, before any event exists; it therefore cannot resolve an event-shaped member and is UNCHANGED by this delta. Terms 1–3 realize the floor **for carriers the head ADMITTED** — they cannot realize it for carriers the head never recorded, because a dropped span never reaches `on_end` at all.

**This is stated as a measured fact rather than a coverage claim, because the coverage claim would be false.** Production binds the head sampler from the §10.3 per-cell envelope **unconditionally**, at every cell and in both §9.1 modes — the mode is resolved for the composer's audit trail only and does not select a sampler. So a `TAIL_BASED_PROD` cell with a sub-1.0 base rate applies that ratio **at span creation**, and the tail consumer sees only the admitted fraction:

| Cell | §9.1 mode | head base-rate | event-shaped carriers reaching the tail |
|---|---|---|---|
| solo-developer × self-hosted-server | `TAIL_BASED_PROD` | 1.0 | **100%** — floor fully realized by terms 1–3 |
| solo-developer × managed-cloud | `TAIL_BASED_PROD` | 1.0 | **100%** — floor fully realized by terms 1–3 |
| solo-developer × local-development | `HEAD_BASED_DEV` | 1.0 | 100% (nothing is dropped; no tail consumer needed) |
| team-binding × local-development | `HEAD_BASED_DEV` | 0.5 | ~50%, **and no tail consumer engages at all** |
| team-binding × self-hosted-server | `TAIL_BASED_PROD` | 0.1 | **~10%** (measured 10.4% over 4,000 carriers) |
| team-binding × managed-cloud | `TAIL_BASED_PROD` | 0.1 | ~10% |
| multi-tenant-compliance × self-hosted-server | `TAIL_BASED_PROD` | 0.2 | ~20% |
| multi-tenant-compliance × managed-cloud | `TAIL_BASED_PROD` | 0.2 | **~20%** (measured 20.9% over 4,000 carriers) |

**Of the carriers that DO reach the tail, terms 1–3 deliver 100%** (measured 420/420 and 837/837) — the arm is complete for what it can see. What bounds it is admission, not classification.

**The bound is SHAPE-specific, and this asymmetry is the load-bearing finding.** At the SAME cell and the SAME base rate, a §9.2 member realized as a **root span name** reaches the tail at **100%** and exports at **100%** (measured 4,000/4,000 for `sandbox.violation` at base-rate 0.1) — because the head sampler resolves `is_always_sampled` against the span name and returns `RECORD_AND_SAMPLE`. **The floor's realization therefore depends on the member's emission SHAPE**: name-shaped members are delivered at every cell; event-shaped members are delivered at the cell's base rate. That shape-dependence is precisely what this delta names, and terms 1–3 close only the tail half of it.

Closing the head half requires giving the members a span shape at emission — a NEW emission primitive that no cleared contract authorizes, routing through workspace `CLAUDE.md` §4.3 back-flow per X-AL-3 — and is **NOT** authorized by this delta. A second, architecture-level route (making the head unconditionally admitting in `TAIL_BASED_PROD` mode and moving the §10.3 ratio into the tail consumer, which `tracer_provider.py`'s own docstring already names as an unbuilt surface) is **larger than this delta and larger than this contract**: it would change the admitted volume at every production cell and it affects **every** class the tail preserves, not only these three. It is registered as forward row **`B-137`** and is not decided here.
...
| Corrections recorded | **TWO, in opposite directions, both by measurement rather than argument.** (1) `B-133` close-out step (2)'s hypothesis that a head-based-dev cell *"samples at 1.0 anyway"* is **FALSE** — `team-binding × local-development` is `HEAD_BASED_DEV` at §10.3 default **0.5** with no tail consumer. (2) This delta's own first drafting then over-corrected, claiming the exposure sat at *"exactly one live cell"* and that every `TAIL_BASED_PROD` cell *"is covered by terms 1–3"* — also **FALSE**, caught by out-of-family review and falsified by a 4,000-carrier measurement per cell: production binds the §10.3 ratio at the head **in both §9.1 modes**, so a `TAIL_BASED_PROD` cell at base-rate 0.1 starves the tail of ~90% of event carriers. The bound reaches **five of eight** ACTIVE cells, and term 4 now states it with the per-cell table |
...
| Residual | v1.37's realization residual **DISCHARGED at the TAIL for head-ADMITTED carriers**, and **NARROWED to a declared bound** for head-dropped carriers and for the HEAD consumer generally (term 4). The v1.37 paragraph itself is PRESERVED VERBATIM. `B-133` close-out step (3) — the F-08 per-tenant keep-volume measurement — stays **explicitly OPEN**, carried as a named residual on the closed row per the `B-104` precedent, and must be measured **jointly with `B-137`**: today's ~20% multi-tenant admission is the ceiling on the arm's added volume, and `B-137`'s repair would remove that ceiling |
| New forward rows | **`B-136`** — the pre-existing name-matching arm leaves an always-sampled ROOT's trace buffered until `force_flush`; observed at this leg's probe, NOT repaired here (repairing it would change shipped behaviour for name-shaped always-sampled roots, outside this delta's authority). **`B-137`** — head sampling starves the tail-keep mechanism for every preserved class that is NOT a root span name (event-carried §9.2 members AND non-root §10.2 triggers, the latter measured at ~9% preservation at base-rate 0.1), because `ParentBased` consults the inner sampler only for roots and the shipped head sampler ignores the §9.1 mode. An architecture-level question spanning the whole §10.2 mechanism; **surfaced by out-of-family Codex round 1 against this leg's commit** and registered rather than absorbed |
| Plan delta | `Implementation_Plan_Operational_Discipline_v2_33.md` — NEW **U-OD-59**, filed in the SAME arc |
| Impl leg | **BUNDLED** — the arm and its sixteen witnesses (40 cases, split across the two axis-homed modules) land in this same PR under U-OD-59 |
