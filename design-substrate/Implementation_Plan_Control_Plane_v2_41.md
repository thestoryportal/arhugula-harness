...
1. **(CP v1.105 §1 — `WRITE_DUAL_VERIFY_ENTRY` presence/uniqueness + id derivation.)** `verify_rotation_6_steps` gains ONE new optional keyword-only parameter `evidence_provider: RotationPairEvidenceProvider | None = None` — deliberately NO separate `rotation_correlation_id` parameter (out-of-family review [P1]: an independently-supplied id could diverge from the window's own id, letting the verifier join two UNRELATED rotation events). `WRITE_DUAL_VERIFY_ENTRY` calls the ALREADY-LANDED `harness_is.rotation_window_verification.verify_rotation_window` (IS spec v1.12 §7.7, U-IS-20) over `audit_ledger_entries` — a `VALID` result (non-empty, every entry's `rotation_correlation_id` present and identical) makes the step succeed AND extracts the single distinct id as the ONLY id used downstream; any `INVALID` result (empty window / presence failure / uniqueness failure) makes the step FAIL with the specific `RotationWindowFailureType` named in the detail, and NO id is available for `PROBE_VERIFY_AT_READ`. Witness: `test_write_dual_verify_entry_valid_window_extracts_the_single_correlation_id` (the extracted id used downstream is asserted to equal the window's own value, never a separately-threaded one — mutation probe: accepting a caller-supplied id parameter instead of deriving it fails this test by construction, since no such parameter exists to supply).
2. **(CP v1.105 §1 — `PROBE_VERIFY_AT_READ` OD-anchored evidence + required physical-key attestation.)** When `evidence_provider` AND `key_identity_resolver` (U-CP-44 below) are BOTH supplied AND `WRITE_DUAL_VERIFY_ENTRY` derived a valid id, `PROBE_VERIFY_AT_READ` calls `evidence_provider.evidence_for(<the window-derived id>)`. `pair_present=True` → the physical-key-distinctness comparator (U-CP-44) runs next; both passing → step succeeds with an "OD-anchored structural pair evidence confirmed" detail naming the returned `outgoing_key_period`/`incoming_key_period` — NEVER worded as "signature verified" (OD v1.35 §24.8 row 8 — structural evidence is not cryptographic signature verification; this precision is itself an acceptance criterion, not just documentation, per witness below). `pair_present=False` → step FAILS with an EXPLICIT "no OD-anchored evidence for this rotation boundary" detail — distinct wording from a `RotationPairIntegrityBreach` raise (criterion #4). Witness: `test_probe_verify_at_read_pair_present_true_succeeds_with_structural_evidence_wording_not_signature_verified` (asserts the success detail string does NOT contain "signature" and DOES contain "structural") + `test_probe_verify_at_read_pair_present_false_fails_with_explicit_absence_detail` (mutation probe: treating `pair_present=False` as `succeeded=True` fails the second test).
...
**Tests (mutation-probed per PD-8):** the 7 witnesses named at criteria #1-#5 above.
