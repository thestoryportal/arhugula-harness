...
| Version | v1.106 |
| Filing event | B-39 spec leg (Q1=(A) + Q2 revised-at-grounding=(keyed-field), per the fork's own §0 risk (2) deferral to this leg); SIX same-day correction passes after out-of-family review — rounds 1-2 (wiring-correctness class, `-uncommitted`) + round 3 (addressability/safety class, branch-vs-main `just codex-review`) + round 4 (round-3's OWN fix converted from a prescribed mechanism to a black-box invariant) + round 5 (propagated the round-4 fix to the sibling Runtime spec, which this file did not itself need to change) + round 6 (a Runtime-plan-side atomicity drift, corrected there; a genuinely NEW operator-facing escalation-request correlation gap, registered as `B-71`, not fixed here) — see change-note |
