...
| Version | v1.106 |
| Filing event | B-39 spec leg (Q1=(A) + Q2 revised-at-grounding=(keyed-field), per the fork's own §0 risk (2) deferral to this leg); FOUR same-day correction passes after out-of-family review — rounds 1-2 (wiring-correctness class, `-uncommitted`) + round 3 (addressability/safety class, branch-vs-main `just codex-review`, run against open PR #1092) + round 4 (round-3's OWN fix converted from a prescribed mechanism to a black-box invariant, after review found it stranded transitively-paused branches and was unassignable to any unit — see change-note) |
