...
1. **Per-branch-distinct resolution.** For any two DISTINCT concurrently-paused GATE-OWNING branches (branches whose OWN dispatch is itself paused at a HITL gate — peer siblings under `PARALLELIZATION`/`ORCHESTRATOR_WORKERS`, or nested descendants under `HIERARCHICAL_DELEGATION` recursion; round-4 correction: this explicitly EXCLUDES a transitively-paused container/ancestor branch, defined at property 5 below, which is not itself gate-owning and never itself consumes a keyed `hitl_responses` entry), the mechanism MUST deliver each gate-owning branch its OWN `hitl_response_for(child_run_id)` resolution (keyed by the paused child's own `run_id`, not `branch_path` — see §0's keying-defect note), computed against the SAME unmutated, operator-supplied `resume_context` object — never a value another branch already consumed, and never a value narrowed/mutated by an ancestor's own resolution (the exact grandchild-fallback-corruption bug a first draft of this section introduced and review caught — recorded so it is not reinvented: DO NOT narrow `resume_context.hitl_response` per recursion hop via e.g. `.model_copy(update={"hitl_response": resolved})`; `hitl_response_for`'s unaddressed-branch fallback must always resolve against the operator's TRUE original uniform default, at any depth).
...
4. **Multi-child fallback-safety invariant (round-3 correction; round-4-REVISED from a mechanism to a black-box invariant, out-of-family review).** Two guarantees the replacement mechanism MUST satisfy — the WHAT, deliberately not the HOW (round-4 correction: a round-3 draft of this property prescribed a specific mechanism — "the resolver MUST first count HITL-paused children... an absent key MUST re-pause INERT" — which out-of-family review found both wrong (it would strand a transitively-paused container/ancestor branch that is never itself gate-owning and so never itself has a `hitl_responses` key to be "absent," see property 5) and unassignable to any declared unit (the counting/traversal logic is resolver-internal, and the resolver is itself deferred to the impl leg, §1.3/§5 — no unit can test a mechanism it doesn't own). Restated as an invariant: **(a) Safety — an EXACT, deterministic rule (round-7 correction, out-of-family review: the original round-4 wording — "whenever doing so could misattribute" — was found too vague to support a deterministic acceptance test; sharpened below WITHOUT reintroducing the round-4-rejected mechanism, since the rule is scoped to gate-owning branches only, per property 5, which is exactly what round 4's draft lacked).** For a given resume cycle, let the "unaddressed gate-owning set" be every gate-owning branch paused this cycle whose own `child_run_id` is NOT a key in `hitl_responses` (an absent map counts every gate-owning branch paused this cycle as unaddressed). The uniform `hitl_response` fallback MAY resolve a gate-owning branch's `hitl_response_for(child_run_id)` call ONLY when that branch is the SOLE member of the unaddressed gate-owning set this cycle. When the unaddressed gate-owning set has 2+ members, EVERY member of it MUST re-pause INERT rather than receive the uniform default (never an auto-re-fire, never a cross-branch misattribution — possibly an EDIT payload with audit-trail attribution stakes — of a response intended for a sibling). This is a property of the RESOLUTION OUTCOME, not a prescription of implementation technique — HOW the resolver determines set membership (counting, a set-difference computation, or any equivalent) is impl discretion; the rule only fixes WHAT the outcome must be. **(b) Liveness** — resume MUST still reach every gate-owning branch the operator addressed a `hitl_responses` entry to; a transitively-paused container/ancestor branch (property 5) MUST be traversed en route, never treated as itself requiring an addressed response to proceed, and is NEVER a member of the unaddressed gate-owning set above (it is not gate-owning). HOW the resolver distinguishes an addressed gate-owning branch, an unaddressed gate-owning branch, and a transitively-paused container branch, and enforces both guarantees, is impl discretion — deferred to the SAME impl-leg bucket as properties 1-3 (CP plan v2.42 §5's deferred-scope row), verified by execution against the real recursion structure, not asserted here. **Deliberately NOT extended to `effect_fence_resolution_for`/`effect_fence_resolutions`** (the sibling mechanism, §26.8.1, unamended by this delta): it ships the identical uniform-fallback-to-every-unaddressed-branch safety gap, already SHIPPED and CLEARED prior to this arc. Fixing it is registered as a separate, pre-existing finding (`B-70`, not this arc's scope — different blast radius: an idempotency SKIP/RE_FIRE/ABORT judgment call, not a human-authorized decision with audit-attribution stakes) rather than silently left unflagged.
5. **Transitively-paused container/ancestor branches are not gate-owning (round-4, definitional).** Under `HIERARCHICAL_DELEGATION` recursion, an ancestor branch can be "paused" only in the sense that ITS OWN execution cannot complete until an unresolved DESCENDANT resolves — it never itself dispatched into a HITL gate, never itself needs a `hitl_responses` key, and MUST always be traversable (re-entered/recursed into) regardless of what `hitl_responses` does or does not contain, so that recursion can reach the actual gate-owning descendant the operator addressed. Property 4's safety/liveness invariant applies ONLY among gate-owning branches — never to a container/ancestor branch's own re-entry, which this arc does not gate on `hitl_responses` content at all.
...
| `hitl_response_for`'s uniform-fallback safety scope | Round-3 added a safety requirement here; **round-4-REVISED** from a prescribed counting/INERT mechanism (found to strand transitively-paused container branches, and unassignable to any declared unit) to a black-box safety+liveness invariant (§0, §1.2 property 4) — deferred to the impl-leg resolver alongside properties 1-3, NOT a carrier-level guarantee and NOT owned by U-CP-64 |
| Gate-owning vs. transitively-paused container branches | **NEW definitional distinction at round 4** (§1.2 property 5) — a container/ancestor branch paused only because an unresolved descendant exists below it is NEVER itself subject to `hitl_responses` addressing; it is unconditionally traversable. Clarifies property 1 + property 4 scope; not itself a carrier/behavior change |
| `hitl_responses`/`child_run_id` addressing — resume-mode coverage | SCOPED at round-3 correction to the `pause_snapshot`-return resume path only; the `resume_handle` crash-recovery path is a registered, NOT-YET-SUPPORTED limitation (§0 scope-limit note) pending a follow-on durable-pause-state read accessor (`B-69`) |
...
| Version | v1.106 |
| Filing event | B-39 spec leg (Q1=(A) + Q2 revised-at-grounding=(keyed-field), per the fork's own §0 risk (2) deferral to this leg); SEVEN same-day correction passes after out-of-family review — rounds 1-2 (wiring-correctness class, `-uncommitted`) + round 3 (addressability/safety class, branch-vs-main `just codex-review`) + round 4 (round-3's OWN fix converted from a prescribed mechanism to a black-box invariant) + round 5 (propagated the round-4 fix to the sibling Runtime spec) + round 6 (a Runtime-plan-side atomicity drift, corrected there; a genuinely NEW operator-facing escalation-request correlation gap, registered as `B-71`) + round 7 (property 4's safety clause sharpened from a vague qualifier to a deterministic outcome rule, staying at invariant altitude; 2 P2 hygiene fixes to B-70's close-out and the FINAL STATE follow-on count) — see change-note |
