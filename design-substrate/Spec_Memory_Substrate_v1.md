...
Revision: v1.2 -> v1.3 (`B-88` spec-leg apply pass - C-MEM-19 gains `input_validation_failure` as a SEVENTH failure class (RATIFIED reading A), plus the boundary subsection and invariant that separate it from provider adapter failure and serialization failure, the ratified A-ii type-boundary rule, the coverage statement that scopes the class to the classify-routed population, and the re-typed sites' retry exit classification. The Memory threat model is unchanged. Detail at the change-note below.)

Revision date: 2026-08-06

## Change-note (v1.2 -> v1.3)

**Trigger and back-flow authority.** RATIFIED Class 2 fork `.harness/class_2_fork_b88_memory_failure_input_validation_class.md` (filed 2026-08-05 at PR #1220, eight out-of-family review rounds with SOUNDNESS EXIT declared at R8; ratified 2026-08-05 at PR #1233; register row `B-88` at `.harness/forward-register.yaml`, status `design_substrate_gated` **at the ratification** - historical-at-ratification, not live: the row transits to `open` within this same PR, since this spec leg plus its paired plan delta is what clears the design gate. Consult the register for live status). The operator selected **reading A** - C-MEM-19's failure vocabulary gains a seventh member for input-validation faults, named `input_validation_failure`, with the stated fallback name `input_error` **not** taken - and resolved the fork's sub-decision **A-ii in its RE-TYPE form**: the `lifecycle/llm_dispatch.py` sites that raise the memory-tool input-error type for **harness-internal** faults are re-typed rather than recorded as mis-typed-at-birth and left in place. Readings B (bless the `provider_adapter_failure` stopgap in contract text) and C (defer under the D-0..D-3 demand test) were **not** selected and are **not** partially adopted; the dyadic C7/C8 convening the fork made conditional on reading B is therefore not owed, and the fork's own council question was probe-resolved before that (fork §7). Amendments 1 through 3 below are the mechanical apply of the ratified position against the fork §4 reading-A drafting targets and the §11.1 ratification text.

**Amendment 1 - C-MEM-19: the failure-vocabulary invariant gains a seventh value.** The single must-distinguish sentence in `### Invariants` now reads *"policy denial, path violation, IO failure, serialization failure, provider adapter failure, input validation failure, and retrieval empty-result."* This is a **vocabulary addition, not a schema widening** - the same category as C-MEM-10's `cross_family_capture` at v1.2 (`### Cross-family-captured promotion candidates`), and the nearest sibling precedent this contract family has. `memory.failure_class` is a required attribute already declared in this contract's `Required attributes` block; the amendment extends one list and adds no attribute, no field, no record type, and no operation kind. The fork established by direct measurement that this contract **never closed** the vocabulary: C-MEM-19 carries no value-domain subsection, no enum table, no "exactly six", and no closure statement, so the sentence is a MUST-DISTINGUISH **floor**, not a ceiling, and the seventh member contradicts no sentence of this contract (fork §3(ii)). The honest converse is stated with it: the pre-amendment implementation was already conformant, because all six required distinctions were being made. This is a precision improvement, not a conformance repair, and it does not claim otherwise.

**Amendment 2 - C-MEM-19: NEW subsection "Input validation failure" plus two appended invariants.** A seventh member with no boundary would be re-litigated by the next arc, so the class is defined against the two neighbours it is most easily confused with. Input validation failure names a **refusal of caller- or model-supplied arguments at the memory tool boundary** - a malformed, out-of-domain, or missing argument; provider adapter failure names a fault of the provider adapter or its transport; serialization failure stays reserved for record-codec faults. The **decisive ground for separating the first two is not that the old label was imprecise**, and it is recorded here because it is what the ratified reading actually rests on: an implementation is free to reach `provider_adapter_failure` as the **residual** for an unclassified failure, and where it does, routing a large, deliberately-classified, well-typed population into that same value makes a determination and a failure-to-determine indistinguishable at the telemetry surface. **The rule this yields is scoped, and the scope is what keeps it consistent with the residual's legitimate double duty:** *this* population - argument refusals at the memory tool boundary, large, well-typed, and deliberately classified - must not be reported under the value that also carries the unclassified residual, because then an operator cannot tell a triaged failure from an untriaged one. The prohibition is stated over that population rather than as a general ban on any value serving two roles; a general ban would forbid the residual itself, which this contract permits. **This delta does not touch the residual itself**: the fork's variant A-prime (a dedicated `unclassified` member) is recorded as dominated-and-separable there (fork §5) and is neither adopted nor foreclosed here; it belongs to its own row if it is ever taken up.

**Amendment 3 - C-MEM-19: the A-ii type boundary, the coverage statement, and the re-type's retry disposition, all inside the same new subsection.** Three obligations the ratification placed on this leg by name (§11.1, §11.5), each stated at contract altitude rather than as an implementation instruction.

*(a) The boundary is a TYPE boundary where the class is keyed on a type.* An implementation may determine the failure class from the raised exception type - and the shipped one does. Where it does, the type that carries `input_validation_failure` must be raised **only** for argument refusals as defined above; a harness-internal fault, such as an unset runtime memory context or a missing or malformed entry in the harness's own tool-schema table, is not a caller- or model-supplied argument and must not be raised as that type. This is exactly the ratified A-ii RE-TYPE, stated as the rule that makes it checkable: without it a single type spans both populations and the boundary sentence is unfalsifiable at the emission site, which is the condition A-ii was raised to end. The rule constrains the **type-to-class relation**, not the number or shape of exception types an implementation declares, and it changes nothing about what any fault *does* - only about which class it is reported as.

*(b) Coverage, narrowed honestly, and NOT promised beyond what this contract can hold.* The new member is emitted wherever memory telemetry already classifies a failure, and nowhere else. The fork and the ratification both grounded the specific consequence: the harness's dispatch **prepare** path - the step that composes a memory tool context and injects the tool schemas, before the standard memory tool call's own telemetry span is opened - raises argument refusals that reach **no** classifying surface today, so it emits no `memory.failure_class` at all, and neither the seventh member nor the A-ii re-type changes that. **This delta therefore states the class for the classify-routed population and does not promise a dispatch-side emission it does not wire.** The gap is recorded rather than closed, and closing it would be a separate arc against this contract's `Contract` list, not an implementation's discretion. What is **not** licensed by that narrowing is the converse: `memory.failure_class` remains a **required** attribute for every operation this contract's `Contract` list covers, "Standard memory tool call" among them, so an implementation may not *withdraw* an emission that exists today by relocating a check off a classified path. Such a withdrawal is a change to this contract and back-flows as one.

*(c) The re-typed sites' exit classification, recorded because a re-type that leaves it unstated ships a control-flow change nobody chose.* Where an implementation composes memory-tool failures with a retry/fail-fast classifier, the harness-internal faults moved off the input-error type by (a) **retain the fail-fast disposition they have today**; they do not become retry-eligible as a side effect of being re-typed. The ground is the faults' own nature rather than continuity: an unset runtime memory context and a malformed harness-supplied tool schema are **deterministic and candidate-independent**, so retrying them - and, under a fallback chain, advancing to a different candidate - cannot succeed and would consume the chain against a fault no candidate can fix. Continuity is the corroborating fact, not the argument: this is the disposition those sites already have. Two bounds are stated so this paragraph is not over-read. It is a **constraint on the re-type authorized above**, not a new retry obligation minted at this contract - retry and breaker classification are Control Plane and Runtime surfaces, and this contract neither owns nor amends them. And it fixes the **disposition**, not the mechanism: whether the disposition is realized by admitting a new type to an existing fail-fast set, by a declared attribute, or otherwise is implementation discretion.

**Coherence with the `B-84` validate pre-pass, which landed between the ratification and this leg.** The ratification (§11.1, coverage paragraph) recorded **four** production classifier call sites at its own grounding HEAD and warned that relocating executor-side argument checks onto the unclassified prepare path would **remove** an existing `memory.failure_class` emission - naming preservation mandatory and any infeasibility a Class 1 back-flow against this contract. Re-grounded at this leg's HEAD `3a1300eb`: `B-84`'s ratified reading-A leg landed at PR #1243 and **preserved the emission**, by having the batch pre-pass call a side-effect-free `validate` that emits the same memory tool-call span with the same attributes on its failure path (`harness-runtime/src/harness_runtime/memory_tool_executor.py:319-357`, the classifying emission at `:350-356`). The classify-routed population is therefore **five** production call sites at this HEAD, not four - `memory_redaction.py:273`, `memory_capture.py:778`, `memory_tool_executor.py:315` (the execute catch, cited as `:255` at the ratification's HEAD) and `:355` (the new validate path), and `lifecycle/native_memory_adapter.py:556`. The count moved in the direction that **widens** the class's reach, and the faults the validate path classifies are precisely the argument refusals amendment 2 names, so the seventh member's definition and that landed surface agree without adjustment. Recorded as a drift against the ratification text rather than silently normalized: the ratification's "four" is correct at its HEAD and stale at this one, and no disposition changes.

**A second drift against the ratification text, recorded on the same terms.** §11.1 enumerates the six re-typed internal-fault sites and the two retained model-argument sites by line offset in `lifecycle/llm_dispatch.py`. Every one of those offsets has shifted at this HEAD. Re-resolved **by content, not by position**: the six internal-fault raises are the two unset-runtime-memory-context checks now at `:4359` and `:4363` and the four schema-injection checks now at `:4475`, `:4483`, `:4977`, and `:4988`; the two retained raises - which validate the **model-supplied** `scope_ref` argument, are exactly what the new class exists for, and must **keep** the input-error type - are now at `:4369` and `:4371`. The six/two split the ratification fixed is unchanged; only the offsets moved. Per this spec's own v1.1 cite-hygiene rule, these are cited here by content and symbol, and any later leg re-resolves them at its own HEAD rather than inheriting these numbers.

**Class-3 cite-drift sweep, routed to this leg by the PR #1243 gate.** That PR relocated roughly 180 lines of the standard memory tool executor and left fifteen `memory_tool_executor.py:NNN` cites drifted across this spec (2) and the paired plan (13). All fifteen sit in text this file's own delta convention preserves verbatim - the `## Change-note (v1.1 -> v1.2)` block here, and there the same block plus the bodies of the landed units U-MEM-26 and U-MEM-27, whose code cites the plan's own "Cite hygiene" paragraph already declares historical records of their authoring HEAD. They are therefore corrected **at note level, here, and rewritten nowhere**, which is the convention this file fixed for exactly this drift class at v1.1 (*"Cite this spec by contract ID and section name, not by line, in any text authored after v1.1"*). The two in this file, both in the v1.1 -> v1.2 change-note, re-resolved by direct read at HEAD `3a1300eb`: `MemoryToolExecutor._propose_promotion`, cited `:399-435`, is at **`:576-651`**, and its single source-record read, cited `:408-411`, is at **`:587-591`**; `_promotion_risk_flags`, cited `:882-885`, is at **`:1083-1099`**. The plan's thirteen are tabulated at its own matching change-note paragraph, including three whose **shape** changed rather than merely their offset, so a line-shift repair would have been wrong.

**Surfaced finding, not patched - a cross-entry-path classification divergence this delta makes visible without creating** (out-of-family review round 1 [P1], accepted as an observation and declined as an edit). The model-supplied `scope_ref` **mismatch** check exists at two layers, and they disagree about which class owns it. At the standard memory tool executor, `_require_scope_ref` (`harness-runtime/src/harness_runtime/memory_tool_executor.py:794-800`) raises the **denial** type - it is one of the four sites the `B-88` impl half deliberately re-classified to `policy_denial` on 2026-07-28, named there by message (*"memory tool scope_ref does not match context"*). At the dispatch prepare layer the equivalent check (`lifecycle/llm_dispatch.py:4371`) raises the **input-error** type, which the ratification retained there **by name and called load-bearing**, and which this delta re-classifies to `input_validation_failure`. One logical failure would therefore carry two classes depending on which layer refuses it first. Three facts bound it, and they are why it is recorded rather than repaired here. (i) **It is inert today**: the dispatch prepare path opens no memory telemetry span, so its class is never emitted, and the coverage paragraph above forbids this delta from wiring one. (ii) **Neither side is free to move at this leg**: the executor's `policy_denial` is a landed impl-half decision, and the dispatch site's retention is *ratified operator text* - changing either on this leg's own authority is exactly the silent absorption X-AL-3 forbids. (iii) **The arc that closes the coverage gap is the arc that must resolve it**, because the divergence becomes observable at precisely the moment a dispatch-side emission is wired and not before. It is carried at the `B-88` register row's close-out and at U-MEM-28's out-of-scope list so that arc inherits it rather than rediscovering it.

**No amendment - Memory threat model.** Zero change, deliberately, exactly as at v1.1 and v1.2. This delta names no new trust boundary and moves no material across one: it renames the class under which an already-refused, already-emitted failure is reported.

**Plan absorption (same arc).** `Implementation_Plan_Memory_Substrate_v1.md` v1.2 -> v1.3 extends U-MEM-22's failure-class acceptance criterion to seven values, exactly as the fork prescribes, and adds NEW U-MEM-28 decomposing the impl leg. The U-MEM-25 closeout packet's annotation half rides this same PR per the `B-86` / `B-92` precedent; its re-open-and-extend half is owed at U-MEM-28. The fork's own "no new unit" line predates A-ii's ratified scope and is recorded as departed-from, with reasoning, at the plan change-note. This delta changes no code.

**Out-of-family review record (`just codex-review`, branch-vs-main).** **R1 - four [P1] and one [P2]; four accepted and applied, one accepted-in-part with the declined half pinned.** [P1, applied] The plan permitted the family base as the A-ii receiving type while the next bullet admitted that type to a retry classifier's `isinstance` tuple - subclass-inclusive, so the **denial** and **store** subtypes would have become fail-fast, which that classifier's own docstring explicitly refuses because the denial type wraps genuinely transient store I/O; the base is now forbidden and a dedicated internal-fault subtype (or exact-type handling) required. [P1, applied] The invariant read as an **exclusive** definition of provider adapter failure, which would have made the shipped residual a violation and the A-ii receiving class untruthful; the subsection now states that a residual report is the absence of a claim rather than an assertion of adapter-hood, the invariant is qualified accordingly, and the plan's obligation on the internal six is restated as the **negative** one the contract actually carries (leave the input-validation class) rather than a mandate to land on a named class. [P1, applied as a surfaced finding, declined as an edit] The cross-entry-path `scope_ref`-mismatch divergence - see the paragraph above; the finding is correct and neither side is movable at this leg, so it is carried at three surfaces rather than absorbed. [P1, applied] Root `CLAUDE.md` §2.3 and §2.4 still named the v1.2 heads, so a Phase-7 session reading startup authority would have missed both this version and U-MEM-28; both pointer rows are refreshed in this same PR. **The same finding's second half is DECLINED with its ground stated:** `.harness/claude-artifact-pointers.md` was reported as *"lacking the v1.3 lineage"*, but that file carries **no Memory-family lineage at all** - grep returns zero `Memory_Substrate` occurrences - because the Memory pointers were added to root `CLAUDE.md` after that file's relocation pass. Introducing a Memory section there would be new scope, not a repair, and this leg does not take it. [P2, applied] The `B-88` register heading still said *"spec leg + impl leg owed"* while its own summary recorded the spec leg landed; both heading surfaces now say only the impl leg is owed. **R2 - one [P1], accepted and applied.** The delta advanced both artifacts to v1.3 while `.harness/u-mem-25-memory-closeout-evidence.md` still certified v1.2 / U-MEM-01..27 and asserted in its own header that no row carries a `PENDING` marker - so the provider-free closeout gate would have read `ready: yes` over obligations no unit has built. The **annotation half** is now applied in this same PR, exactly as the `B-86` and `B-92` legs applied theirs: `PENDING - U-MEM-28` markers on C-MEM-19, R-MEM-01, and R-MEM-13, a third version-scoping window paragraph beside the two closed ones, and the header sentence qualified; the **re-open-and-extend half** is owed at U-MEM-28 as an acceptance criterion with a green re-run in Verification. **R3 - two [P2], both accepted and applied.** [P2] The executor's own `_prepare` exhaustiveness fall-through raises the input-error type for an *"unsupported memory tool"* that a `MemoryToolName`-typed request field makes a **harness-internal** fault, so leaving it would have left the new type-boundary rule violated in place at an executor site while the six dispatch sites were repaired; U-MEM-28 now re-types it on the same rule, with the witness arithmetic corrected from eight flipping rows to seven plus one re-keyed **negative** row, and the derivation flagged overturnable by the impl leg's own re-grounding. Recorded honestly rather than presented as ratified scope: the ratification enumerated six sites because the fork surveyed `lifecycle/llm_dispatch.py` and did not survey the executor. [P2] U-MEM-28's dependency set omitted **U-MEM-25**, whose evidence packet its own closeout criterion re-opens - and unlike U-MEM-27 it has no transitive path to it, so the DAG permitted a schedule in which that packet did not yet exist; the edge is added at §4 and at the unit's `Depends on:`, matching the U-MEM-26 precedent. **R4 - one [P1] and two [P2], all three accepted and applied.** [P1] The per-site dispatch witness was **telemetry-only**, and telemetry cannot see the failure mode that matters most here: a site accidentally raising the family **base** instead of the dedicated internal-fault subtype classifies to the *same* residual, so every telemetry assertion passes - while the base is **not** in the fail-fast set, so that one site falls through to transient retry and can re-fire a dispatch that has already committed. Each of the six internal-site assertions is now **paired with a retry-exit assertion on the same instance**, and a single end-to-end witness over one site is explicitly declared insufficient. [P2] The residual paragraph was **internally contradictory**: it permitted a determined adapter fault and an unclassified fault to share `provider_adapter_failure`, then banned a deliberate classification and an unclassified residual from sharing a value, then called the permitted mapping conforming - and the appended invariant required the double use. The prohibition is now **scoped to the population it was always about** (argument refusals at the memory tool boundary must not be reported under the value carrying the residual) rather than stated as a general ban that would forbid the residual this contract permits; the invariant carries the scoped form. [P2] The R3 witness correction had not propagated: the plan clearance marker, the fork's §11.5 owed-list, and both register surfaces still told an implementer to flip **eight** rows, which would have produced exactly the type-boundary violation R3 closed. All four are reconciled to seven flips plus one re-key. **R5 - two [P2], both accepted and applied, both mechanical.** [P2] U-MEM-28's preservation criterion said *"the other five C-MEM-19 classes are unchanged"* while the pre-amendment enum has **six** members (`memory_observability.py:36-41`) - an off-by-one that would have left one legacy class outside the explicit preservation requirement; corrected to six. [P2] The closeout packet gained a trailing blank line at EOF, which fails `git diff --check`; removed, and the gate re-run clean. **SOUNDNESS EXIT DECLARED at R5** per the workspace's deferred-mechanism review-exit discipline. The substance never moved across five rounds: the ratified reading, the seventh member and its name, the boundary, the A-ii type boundary, the coverage narrowing, and the fail-fast retry disposition are exactly what R1 opened on. R1 and R2 were contract-level and changed text; R3 and R4 were consequences of R1's own fixes (the type-boundary rule applied to a site the ratification had not surveyed; then the witness shape that rule needs, and the count propagation that rule's correction owed); R5 is an off-by-one and a whitespace character - the self-referential-drift signature the exit discipline names. **A contract-level finding still reopens this record.** **R6 - one [P2], accepted and applied; it does NOT reopen the exit, being a record-consistency finding rather than a contract one.** The `B-88` register row's `summary` still opened with the 2026-08-05 bracket declaring the status `design_substrate_gated` with *both* a spec leg and an impl leg owed, which contradicted the row's own new `status: open` and could have re-scheduled the landed spec leg; the bracket is **replaced** rather than appended to, per the register's replace-not-append discipline, and the close-out's matching *"what remains is the SPEC LEG"* clause is corrected to name U-MEM-28. **R7 - one [P2], accepted and applied; a governance-record finding, so the exit stands.** The plan change-note and both clearance markers still carried an **unqualified** *"nothing in U-MEM-28 exceeds the ratification's enumerated impl-leg list"* after R3 had added the executor-side `_prepare` re-typing - a claim R3 itself falsified. The seventh site is now named for what it is on every surface: **a SCOPED EXTENSION of the ratified six-site A-ii list, not a decomposition**, with the case for it (spec v1.3's type-boundary rule is general and this site violates it; the fork surveyed `llm_dispatch.py` and never surveyed the executor) stated beside the case against it (**the operator ratified six**, and a spec leg widening a ratified enumeration on its own reading is the shape X-AL-3 polices even when the reading is right), and **two** overturn routes recorded. Surfacing a contradiction to a prior decision rather than silently overriding it is the continuity rule this workspace runs on. **R8 - one [P1] accepted and applied, one [P2] mooted by it; the [P1] is the round that changed this delta most, and it changed it by REMOVING something.** [P1] Round 8 objected that naming the seventh re-typing an extension does not authorize it: **the operator ratified an A-ii list of exactly six**, a spec leg *applies* operator-decided fixes rather than deciding them, and recording a contradiction is not a substitute for deciding it. **The objection is correct on this workspace's own rules and was accepted without argument**: the obligation is removed from U-MEM-28, the witness arithmetic reverts to eight flipping rows, and the finding is carried as an out-of-scope entry with its consequence stated rather than softened - **at U-MEM-28's close, this contract's type-boundary invariant will have one known non-conformant site, recorded and unrepaired**, repairable by an operator ratification of a seventh site or moot if the impl leg's re-grounding finds the branch caller-reachable. U-MEM-28 now contains **nothing** beyond the ratification's enumerated impl-leg list, without qualification. [P2] The paired finding - that the re-keyed table row would not have witnessed the fall-through's *retry* behaviour - was explicitly conditional on the seventh site remaining, and is mooted by its removal. **R9 - one [P1], accepted and applied; the natural completion of R8 rather than a new direction.** R8's withdrawal left the executor-site finding carried only on `B-88`'s close-out - and `B-88` closes when U-MEM-28 lands, so the finding would have **vanished from open work** at exactly the moment it became live, with no row or gate owning the seventh-site decision. That is the informational-carry failure the workspace's back-flow rule exists to prevent. **Register row `B-114` is minted** to own it: it outlives `B-88`, and it carries the grounding pass (re-read the branch at the then-current HEAD; if it has become caller-reachable the site is conformant and the row closes), the discriminator that actually decides the disposition (plain conformance repair against a cleared contract, since the executor site was never before the operator - the stronger reading - versus a genuine extension of the ratified A-ii enumeration needing a Class 2 fork), and the repair recipe with the receiving-type and dual-witness constraints U-MEM-28 fixes for the six. Every surface that named the finding now points at `B-114`. **The R5 soundness exit is NOT claimed over R7, R8, or R9**: all three were governance findings against this leg's own authority and its record-keeping rather than against the contract, and all three were accepted. **The contract text has not moved since R4.** **R10 - CLEAN, zero findings; the review CONVERGED** (*"the design, plan, register, clearance, and closeout updates are internally consistent, and no actionable defect was found in the changed behavior or sequencing"*). Ten rounds, and the shape of the arc is worth one sentence for the next reader: **R1-R2 were contract-level and moved text; R3-R5 were consequences of R1's own fixes and their count propagation; R6-R9 were governance and record-keeping - three of them findings against this leg's own AUTHORITY, which is the class a spec leg is least able to see in itself, and each was accepted rather than argued down.** This line closes the record; it changes no obligation.

**Sections preserved verbatim at v1.3.** The Status section (revision lines appended only); the whole `## Change-note (v1.1 -> v1.2)` block and the whole `## Change-note (v1 -> v1.1)` block; C-MEM-01 through C-MEM-18; **the Memory threat model in full**; C-MEM-20. Within C-MEM-19: the `### Contract` operation list and the `Required attributes` block are byte-unchanged, and the first invariant is byte-unchanged; the second invariant gains one value inside its existing sentence and nothing else. Zero new record type, zero new field, zero new attribute, zero new contract number, zero change to any ledger or packet shape, and zero hash impact - `memory.failure_class` is a span attribute value and is an input to no canonicalization, `content_hash`, `snapshot_hash`, or `packet_hash`, verified rather than assumed at fork §3(iii). Zero cross-axis composition rows are owed: the amendment adds no cross-package consumption, so nothing is due at `Cross_Axis_Composition_Document_v2_23.md` §2.3.
...
**Input validation failure** names a refusal of caller- or model-supplied arguments at the memory tool boundary - a malformed, out-of-domain, or missing argument. It is distinct from **provider adapter failure**, which names a fault of the provider adapter or its transport, and from **serialization failure**, which is reserved for record-codec faults. It is a **vocabulary addition, not a schema widening**: `memory.failure_class` is already a required attribute of this contract, and the seventh value needs no field, no type change, and no shape change here. Where an implementation carries the failure vocabulary as a closed enumeration, admitting this value there is implementation work bound by this contract, not a further contract amendment.

**The separation from provider adapter failure is load-bearing, and the reason is not that the older label was imprecise.** An implementation may reach `provider_adapter_failure` as the **residual** for a failure it could not otherwise classify. Where it does, that one value means either *"this was determined to be an adapter fault"* or *"nothing could be determined"* - and routing a large, well-typed, deliberately classified population into it makes a determination and a failure-to-determine indistinguishable at the telemetry surface. **The rule this yields is scoped, and the scope is what keeps it consistent with the residual's legitimate double duty:** *this* population - argument refusals at the memory tool boundary, large, well-typed, and deliberately classified - must not be reported under the value that also carries the unclassified residual, because then an operator cannot tell a triaged failure from an untriaged one. The prohibition is stated over that population rather than as a general ban on any value serving two roles; a general ban would forbid the residual itself, which this contract permits. This subsection does not otherwise constrain the residual: giving the unclassified case a value of its own is a separate question, neither adopted nor foreclosed here. **One consequence of leaving it open must be stated, or the definitions above read as forbidding what they in fact permit** (out-of-family review round 1 [P1]): where an implementation has no dedicated unclassified value, a fault that is neither an argument refusal, nor a record-codec fault, nor a determined adapter fault - a harness-internal wiring or schema fault among them - still lands on `provider_adapter_failure` **as the residual**, and that is conforming. Such a report is **not** a claim that the fault was an adapter fault; it is the absence of a claim. The definitions above fix what each value **positively names**, and only `input_validation_failure` is stated as exclusive of a population - the harness-internal one. Reading the provider-adapter sentence as an exclusive definition would make the shipped, contract-conformant residual a violation, which it is not.

**Where the class is keyed on an exception type, the class boundary is a TYPE boundary.** An implementation may determine the failure class from the raised exception type. Where it does, the type carrying `input_validation_failure` must be raised **only** for the argument refusals defined above. A harness-internal fault - an unset runtime memory context, or a missing or malformed entry in the harness's own memory tool schema table - is not a caller- or model-supplied argument, and must not be raised as that type. A single type spanning both populations leaves the definition above unfalsifiable at the emission site, which is the condition this rule exists to end. The rule constrains the type-to-class relation only; it fixes neither the number nor the naming of an implementation's exception types, and it changes nothing about what any fault does.

**Coverage, stated in the terms this contract can hold.** The value is emitted wherever memory telemetry already classifies a failure. Argument refusals raised on a path that opens no memory telemetry span - notably a dispatch-side prepare step that composes a memory tool context or injects tool schemas **before** the standard memory tool call's own span exists - emit no `memory.failure_class` today, and this subsection does not wire one; that gap is recorded, not closed, and closing it is a change to the `Contract` list above rather than an implementation's discretion. The converse is **not** licensed: `memory.failure_class` remains required for every operation the `Contract` list covers, so an implementation may not withdraw an emission that exists today by relocating a check onto an unclassified path. A withdrawal is a change to this contract and back-flows as one.

**Retry disposition of faults moved off the input-error type, stated because a re-typing that leaves it unstated ships a control-flow change nobody chose.** Where an implementation composes memory-tool failures with a retry or fail-fast classifier, the harness-internal faults separated out by the type-boundary rule above **retain the fail-fast disposition they already have**; they do not become retry-eligible as a side effect of being re-typed. An unset runtime memory context and a malformed harness-supplied schema are deterministic and candidate-independent, so a retry - and, under a fallback chain, an advance to a different candidate - cannot succeed and consumes the chain against a fault no candidate can fix. This is a constraint on the re-typing this contract authorizes, not a retry obligation minted here: retry and breaker classification are Control Plane and Runtime surfaces, which this contract neither owns nor amends. It fixes the disposition, not the mechanism.
...
- Failure telemetry must distinguish policy denial, path violation, IO failure, serialization failure, provider adapter failure, input validation failure, and retrieval empty-result.
- Input validation failure names a refusal of caller- or model-supplied arguments at the memory tool boundary; serialization failure is reserved for record-codec faults; provider adapter failure names a fault of the provider adapter or its transport, and additionally carries the unclassified residual wherever an implementation declares no dedicated value for one. A harness-internal fault is not an input validation failure, and argument refusals at the memory tool boundary are not reported under the value that carries the unclassified residual.
- Where the failure class is determined from the raised exception type, the type carrying input validation failure is raised only for those argument refusals, and a harness-internal fault is not raised as that type; separating such a fault out does not change its retry disposition.
