...
- Input validation failure names a refusal of caller- or model-supplied arguments at the memory tool boundary; provider adapter failure names a fault of the provider adapter or its transport; serialization failure is reserved for record-codec faults. A harness-internal fault is not an input validation failure.
