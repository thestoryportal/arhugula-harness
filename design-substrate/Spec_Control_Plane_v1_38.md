...
**Topology-scope of the override-ledger entry (v1.38 honest-scope note — PRE-EXISTING, surfaced by out-of-family review; registered, not introduced here).** The override-ledger entry (`emit_override_state_ledger_entry`, gated on `binding.override_applied`) fires at the `SINGLE_THREADED_LINEAR` driver-thread per-step site only (`workflow_driver.py` linear loop). The **non-linear topologies** (`ORCHESTRATOR_WORKERS` / `PARALLELIZATION` / `HIERARCHICAL_DELEGATION` / `DECENTRALIZED_HANDOFF` / evaluator-optimizer) do **not** emit a per-step override-ledger entry — for **any** per-step override dimension (model, since v1.6; prompt, since v1.37; role, this v1.38). This is a **PRE-EXISTING** gap, NOT introduced by the role dimension: the buffered-branch / concurrent-fan-out write boundary (ADR-F2 single-threaded-write; the branch ledgers drain deterministically) precludes a synchronous driver-thread override entry emitted per concurrent worker. **Routing is unaffected** — the per-step role (and model/prompt) override takes effect at dispatch on all topologies (folded onto `step_context.agent_role`); only the dedicated per-step *override-audit-entry* is linear-only. The §6.6 provenance claim above is therefore exact for `SINGLE_THREADED_LINEAR`; on non-linear topologies the per-step override is observable in the branch step-boundary ledger entries but not in a dedicated override-ledger entry. Closing the non-linear override-provenance gap (emitting the override entry through the buffered-branch path / on the driver thread) is a **registered forward item** (`.harness/beyond-mvp-capability-boundary-ledger.md`; a pre-existing all-topologies audit-provenance arc, not a Slice-4 deliverable). This honest-scope note is a v1.38 canonical-reading correction to the v1.37 §6.6 framing (which did not state the topology scope).
