...
>
> **Loud disclosure (the §25.12 Point-2 sacrifice made non-silent).** The synthesis dispatch emits (i) its OWN post-barrier step ledger entry (mirroring `_append_step_ledger_entry`, with a synthesis action_id + a `synthesis=true` / non-deterministic disclosure marker), and (ii) a trace event marking the run's aggregate as synthesis-composed / non-reproducible-across-replay. The §25.12 contract caveat (this §3 + §4) is the standing declaration.
