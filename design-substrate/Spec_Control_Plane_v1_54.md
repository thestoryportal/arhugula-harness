...
>
> **Effect-free enforced at the dispatch boundary, including post-HITL-edit (impl-to-spec trace).** The read-only / effect-free property is enforced at the LLM dispatch boundary (the `RuntimeLLMDispatcher` post-payload-coercion, keyed on `step_kind == POST_JOIN_SYNTHESIS`): a payload binding tools by ANY route (top-level `tools`, or the `params`-merge route `tools` / `tool_choice` / `mcp_servers` / legacy `functions`) is rejected FAILED before the provider call. This is the load-bearing floor — a PRE_ACTION HITL EDIT replaces `step.step_payload` verbatim INSIDE the inner dispatcher (after the synthesis composer's early guard), so the boundary (downstream of the edit) is the only point that holds against an operator EDIT that re-introduces tools. An EDIT that DROPS the appended sibling context is operator intent shown at the gate (not a silent break) — the operator owns the edited synthesis input; only effect-freeness is invariant.
>
