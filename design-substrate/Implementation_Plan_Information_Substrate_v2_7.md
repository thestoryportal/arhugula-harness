...
- **Direct-path preservation control:** `test_direct_append_caller_supplied_semantics_byte_verbatim` — the existing direct-path tests (`test_append_rejects_non_monotonic_timestamp` et al.) pass unchanged, and a direct append's persisted timestamp IS the caller-supplied value (mutation probe: extending writer-owned sampling to the direct path fails this control) — EXCEPT the offloaded sub-agent audit append path, which the §7.6 scope-widening (codex round-28) brings under writer-owned sampling because B-48's own concurrency newly exercises it; witness `test_concurrent_sibling_subagent_audit_appends_never_invert` (two offloaded sync siblings' success-path audit appends land monotonic — mutation probe: restoring pre-lock sampling on that path inverts and fails).
