# Implementation Plan: Control Plane — v2.35 (delta over v2.34)

*v2.35 is the CP-axis leg of **R-FS-1 — E-plan-3** — the atomic-unit decomposition of the E sub-program's LAST engine class (**E-3 RECONCILER_LOOP**, now **impl-against-cleared-spec** post-CP-spec-v1_33 — the E-spec-3 leg landed at PR #568, which added `reconciler-loop` to the C-CP-07 §7.4 impl-discretion clause, operator-ratified, reconciling the spec-named "K8s controller / etcd + CRD" reference substrate with I-6 "hand-rolled, NO vendored K8s" via a harness-hosted hand-rolled etcd-style reconciliation control-loop) into two CP-package units: **U-CP-96** (RECONCILER_LOOP engine materialization — level-triggered read/diff/converge resumption, CP/IS-only), **U-CP-97** (the RECONCILER_LOOP engine-layer recovery-loop firing branch → C-CP-49/50 → R-CXA-2 go-live for the reconciler path). The runtime substrate + R-CXA-2 activation + the NEW non-live reconciler e2e are runtime-homed (runtime plan v2.46 U-RT-123/124); §3.6 is the E-3 aggregate cross-axis DAG home. Authority: CP spec v1_33 §7.4 (the operator-ratified reconciler-loop substrate-deferral reconciliation) + `.harness/class_1_fork_e3_reconciler_loop_substrate_deferral.md` (✅ RATIFIED-AND-APPLIED) + `.harness/architect_recommendation_e_engine_fork_vs_impl.md` §4/§6. ZERO spec amendment, ZERO new contract ID, X-AL-3-clean (closed `EngineClass`/`ResumptionKind`/`F2JoinKind` enums consumed — RECONCILER_LOOP / `reconciler_converge` / `CRD_RECONCILER_LEDGER` already-closed members). v2.34 + earlier PRESERVED VERBATIM per delta-only-plan-chain convention.*
...
`Implementation_Plan_Control_Plane_v2_34.md` (v2.34 — the R-FS-1 E-plan-1/2 CP leg; 3 NEW units U-CP-93/94/95 for EVENT_SOURCED_REPLAY + WAL_SEGMENT engine materialization + the WAL recovery-loop firing branch + the §6 O-CP-4 R-CXA-2-owned-by-E-2 record).

### §0.2 Revision scope (v2.34 → v2.35)

v2.35 decomposes the **CP-package** surfaces of the E sub-program's LAST engine class (**E-3 RECONCILER_LOOP**) into TWO NEW atomic units. **E-3 is now impl-against-cleared-spec** post-CP-spec-v1_33: the ONE genuine operator fork in the E sub-program — the C-CP-07 §7.4 substrate-deferral reconciliation — was the E-spec-3 leg, surfaced + operator-ratified "as written" 2026-06-15, landed as CP spec v1_33 (PR #568). With `reconciler-loop` now in the §7.4 impl-discretion clause (the harness-hosted, hand-rolled etcd-style reconciliation control-loop the spec-blessed candidate per I-6), E-3 materialization is exactly the E-1/E-2 posture (impl against cleared closed-enum contracts; the U-CP-56 → U-CP-94 precedent controls). Authoritative: CP spec v1_33 §7.4 + `.harness/class_1_fork_e3_reconciler_loop_substrate_deferral.md` (✅ RATIFIED-AND-APPLIED 2026-06-15) + `.harness/architect_recommendation_e_engine_fork_vs_impl.md` §4/§6. The E keystone homing fact: the engine driver / `EngineClass` / consumers / C-CP-49/50 composers are `harness-cp`; the engine recovery loop + #475 Journal substrate + the U-RT-121 WAL substrate + the R-CXA-2 factory are `harness-runtime` (runtime plan v2.46 U-RT-123/124). The CP package owns:

| CP surface | Unit | Authority |
|---|---|---|
| RECONCILER_LOOP engine materialization (CP/IS-only) — `_IN_SCOPE_ENGINE_CLASSES` widening (`workflow_driver.py:187-214`) + `:1521`-region driver dispatch fork (level-triggered read/diff/converge → `resume_at` via the F2 `CRD_RECONCILER_LEDGER` read) + `resumption.kind=reconciler_converge` + F2 `idempotency_key` join (reconciler reads ledger) + F3 capability-floor verification (incl. the **CAS lease** — the genuine new capability) + consumer-row VERIFICATION (already enumerated) | **U-CP-96** | C-CP-07 §7.1 row 4 + §7.4 (v1_33 substrate impl-discretion) + C-CP-08 §8.1 `reconciler_converge` + §8.2 row 4 + §8.3; v1_33 §7.4; architect rec §4/§6; U-CP-94 precedent |
| RECONCILER_LOOP engine-layer recovery-loop firing branch — fire `ctx.engine_recovery_loop.capture_pause`/`.attempt_resume` at the RECONCILER_LOOP pause-trigger → C-CP-49/50 emit (duck-typed `ctx`, mirroring the WAL_SEGMENT firing at `workflow_driver.py:1542/:1651` + the workflow-layer `ctx.pause_resume_protocol` fire at `:1200` — no CP→runtime import) | **U-CP-97** | C-CP-22 §22.1 (engine-layer pause/resume free fns) + C-CP-49/50 (= U-CP-49/50 §16.5.2 composers) + R-CXA-2 (CXA §2.3.2) |

No spec amendment; no new contract ID. The `EngineClass` enum is closed at 5 (`engine_class.py:31-53`), the `ResumptionKind` taxonomy is closed at 5 (`resumption_kind.py:34-47`; `RECONCILER_CONVERGE` already mapped to RECONCILER_LOOP at `:65`), and the `F2JoinKind` is closed at 3 (`f2_substrate_join_discipline.py:45-54`; `CRD_RECONCILER_LEDGER` already mapped to RECONCILER_LOOP at `:85`); E-3 **consumes cleared closed enumerations** (X-AL-3-clean — no new primitive, architect rec §3, v1_33 change-note). `_IN_SCOPE_ENGINE_CLASSES` is an impl-internal scoping frozenset; widening it is the U-CP-56 → U-CP-94 move.

| In scope at v2.35 | Out of scope |
|---|---|
| U-CP-96 (RECONCILER_LOOP materialization, CP/IS-only) + U-CP-97 (RECONCILER_LOOP recovery-loop firing branch) | All v2.34 + prior unit bodies — preserved verbatim per §0.3; the durable reconciler substrate + R-CXA-2 activation + the NEW non-live reconciler e2e are U-RT-123/124 (runtime v2.46) |
| The E-3 aggregate cross-axis DAG + topological order (§3.6 — arc home) | The runtime reconciler substrate + R-CXA-2 activation (U-RT-123/124, runtime plan v2.46) — co-published, cited cross-axis |
| Coverage matrix: +2 CP rows + AC-level dispositions (§4.4) | The §7.2 / ADR-D1 §1.2 deployment-admissibility of the hand-rolled reconciler (v1_33 §7.4 DEFERS to E-impl-3 — AC-level deferral, NOT silently resolved; §6 O-CP-5); the live-K8s e2e (deployment-surface gate, NOT a unit); the PathClass IS delta (STATE_LEDGER = existing closed-enum member → AC-level conditional in U-RT-123, NOT an IS amendment) |

### §0.3 Sections preserved verbatim from v2.34

| Section | Status at v2.35 |
|---|---|
| §0 (v2.34 change-note) | Superseded by this §0 (historical record preserved at v2.34) |
| §1 Spec inventory | Refreshed: +2 contract-surface rows (C-CP-07/08 RECONCILER_LOOP resumption; C-CP-22/49/50 reconciler recovery-loop firing); all prior rows (incl. the E-1/E-2 + B3 rows) PRESERVED VERBATIM |
| §2 — U-CP-01..U-CP-95 (all prior units) | **PRESERVED VERBATIM** from v2.34 + lineage (delta-only-plan-chain convention) |
| §3 Dependency graph | Revised: +§3.6 E-3 aggregate cross-axis graph; all prior B1 (§3.1–§3.3) + B3 (§3.4) + E-1/E-2 (§3.5) graphs + acyclicity PRESERVED VERBATIM |
| §4 Coverage matrix | Revised: +§4.4 (2 E-3 unit rows + AC-level dispositions); §4.1/§4.2/§4.3 + all prior PRESERVED VERBATIM |
| §5 Cross-cutting integration units | Unchanged (no new tri-spec unit at v2.35); all prior preserved verbatim |
| §6 Open items | Extended: +O-CP-5 (the §3.6/companion-§5 E-3 finding-record — substrate-selection RT-internal + deployment-admissibility deferred + live-K8s deferred, all recorded-not-gated); O-CP-3 + O-CP-4 + prior PRESERVED VERBATIM |

### §0.4 Authority chain — no operator gate

v2.35 absorbs the **cleared** E-3 reading (CP spec v1_33 §7.4 — operator-ratified substrate-deferral reconciliation; the fork doc §6 ✅ RATIFIED-AND-APPLIED; E-3 materialization is now impl-against-cleared-spec, the E-1/E-2 posture extended to E-3 by the v1_33 deferral). No operator decision owed at this plan-layer arc; ZERO X-AL-3 risk for U-CP-96/97 (closed `EngineClass`/`ResumptionKind`/`F2JoinKind` enums consumed; no new primitive — architect rec §3, v1_33 change-note). The three findings (the substrate-selection engine-class-aware binding = RT-internal impl-discretion; the §7.2 deployment-admissibility = ratified deferral to E-impl-3; the live-K8s e2e = separate deployment-surface gate) are all within-impl-against-cleared-spec dispositions recorded at §6 O-CP-5, NOT forks. The ONE genuine operator decision in the E sub-program — the E-3 §7.4 substrate-deferral reconciliation — was already surfaced + ratified (the E-spec-3 leg, CP spec v1_33), so it is RESOLVED, not re-opened here.

### §0.5 Status posture

`Status: Proposed`. Clearance marker (operator-filed at PR) at `.harness/clearance/Implementation_Plan_Control_Plane-v2_35-cleared-*.md`. Sibling co-publication: runtime plan v2.46 (U-RT-123/124). Companion: `.harness/r-fs-1-e3-plan-decomposition.md`.
...
| Field | Value |
|---|---|
| Plan version | v2.35 (delta over v2.34) |
| Authored at | 2026-06-15 |
| Authoring authority | R-FS-1 E-plan-3; CP spec **v1_33 §7.4** (the operator-ratified reconciler-loop substrate-deferral reconciliation) + `.harness/class_1_fork_e3_reconciler_loop_substrate_deferral.md` (✅ RATIFIED-AND-APPLIED) + `.harness/architect_recommendation_e_engine_fork_vs_impl.md` §4/§6; C-CP-07 §7.1 row 4 / §7.4 + C-CP-08 §8.1 `reconciler_converge` / §8.2 row 4 / §8.3 (cleared); companion `.harness/r-fs-1-e3-plan-decomposition.md` |
| Net delta | +2 NEW units (U-CP-96 RECONCILER_LOOP materialization, CP/IS-only `(none)`; U-CP-97 RECONCILER_LOOP recovery-loop firing branch `[U-CP-96]`) — the durable reconciler substrate + R-CXA-2 activation + the NEW non-live reconciler e2e are U-RT-123/124 (runtime v2.46), NOT CP units; +1 cross-axis edge (U-RT-124 → U-CP-97, RT→CP downstream — recorded at the E-3 aggregate §3.6); +§4.4 (9 coverage rows/dispositions); +1 §6 open-item (O-CP-5, the three E-3 findings, recorded-not-gated); ZERO spec amendment, ZERO new contract ID, ZERO CP-spec fork (closed `EngineClass`/`ResumptionKind`/`F2JoinKind` enums consumed — RECONCILER_LOOP / `reconciler_converge` / `CRD_RECONCILER_LEDGER` already-closed members; architect rec §3 + v1_33 change-note X-AL-3-clean) |
| Sibling co-publication | runtime plan v2.46 (U-RT-123 etcd-style reconciler substrate + U-RT-124 R-CXA-2 activation); clearance markers (operator-filed at PR); workspace `CLAUDE.md` §2.4 + `.harness/claude-artifact-pointers.md` §2.4 plan-head bumps |
| Aggregate E-3 graph | 4 nodes (2 CP + 2 RT), acyclic, topological order at §3.6.2; single cross-axis edge U-RT-124 → U-CP-97 (RT→CP downstream); no CP→RT back-edge (U-CP-97 consumes `ctx.engine_recovery_loop` duck-typed; CP resume_at reads F2 IS). B1 (§3.1–§3.3) + B3 (§3.4) + E-1/E-2 (§3.5) aggregates PRESERVED VERBATIM; the E-3 sub-DAG composes onto the E-1/E-2 sub-DAG with no edge (shared composed-existing substrate Protocol + recovery loop + factory) |
| Forks | ZERO fork in this plan delta. E-3 is impl-against-cleared-spec post-v1_33 (the ONE genuine operator decision in the E sub-program — the §7.4 substrate-deferral reconciliation — was the E-spec-3 leg, already operator-ratified + landed as CP spec v1_33; RESOLVED, not re-opened here). The three O-CP-5 findings (substrate-selection engine-class-aware = RT-internal impl-discretion; deployment-admissibility = ratified deferral to E-impl-3; live-K8s e2e = separate deployment-surface gate) are within-impl dispositions, NOT forks. The conditional F-E3-IS PathClass sub-fork is owed ONLY if the etcd-style store cannot honestly map to an existing closed `PathClass` member (default: `STATE_LEDGER`). |
| Homing decision | engine driver / `EngineClass` / consumers / C-CP-49/50 composers = `harness-cp` (U-CP-96/97); engine recovery loop + #475 Journal substrate + U-RT-121 WAL substrate + R-CXA-2 factory = `harness-runtime` (U-RT-123/124). The reconciler recovery-loop firing branch (U-CP-97) is CP (mirrors the WAL_SEGMENT firing at `workflow_driver.py:1542/:1651` + the workflow-layer `ctx.pause_resume_protocol` fire at `:1200`, duck-typed — no CP→RT import) |
| E-impl sequence (companion §6) | E-impl-3 = U-CP-96 (RECONCILER_LOOP level-triggered reconverge resumption, CP/IS) + U-RT-123 (durable etcd-style reconciler substrate, CAS lease) + U-CP-97 (firing branch) + U-RT-124 (engine-class-aware durable factory bind + NEW non-live reconciler R-CXA-2 go-live e2e). With E-impl-3, **all 5 engine classes are materialized** (`:1398` raise unreachable); the E sub-program closes. Live-K8s e2e + deployment-admissibility = separate E-impl-3 deployment-surface sub-gates (O-CP-5). |
