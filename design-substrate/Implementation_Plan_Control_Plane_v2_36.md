# Implementation Plan: Control Plane — v2.36 (delta over v2.35)

*v2.36 is the CP-axis leg of **R-FS-1 — B2-plan** — the atomic-unit decomposition of the multi-server-MCP sub-program's **gate-axis** CP surface (the 4th §19.1 HITL-gate axis, materialized at CP spec **v1.35 §19.1.2** / B2-spec-2) into ONE CP-package unit: **U-CP-98** (the `gate_level()` 4th-axis composition — commit `MCP_TRUST_GATE_LEVEL_FLOOR` operator-ratified Table A + add `Axis.MCP_TRUST` to `per_axis_floors`, moving the §19.1 composition 3-of-4 → 4-of-4 materialized axes; refresh the 5 stale "spec-silent" docstrings to cite §19.1.2). The multi-server **reshape** (B2-spec-1: runtime v1.51 §14.9.10 + CP v1.34 §27.8 telemetry projection) + the gate-axis **producer** (U-RT-131 — the L3 no-floor default at the host-less gate sites the runtime composer actually gates; the real per-server resolved-host trust feed of `GateLevelInput.mcp_trust_tier` is the registered `B-TOOL-GATE` forward arc, §6 O-CP-6) are runtime-homed (runtime plan v2.47 U-RT-125..131); §3.7 is the B2 aggregate cross-axis DAG home. **The load-bearing finding:** U-CP-98 is HARMFUL-if-landed-alone — composing `Axis.MCP_TRUST` while the composer still pins `mcp_trust_tier=LEVEL_0_REFUSE_REMOTE` (`hitl_gate_composer.py:462`) puts `MCP_TRUST_GATE_LEVEL_FLOOR[L0]=DENY` into the `max()` on EVERY gate → every host-less gate (inference + sub-agent — the only gate sites that exist) becomes `DENY` → a **hard co-land sequencing pin** with runtime U-RT-131 (the inverse of B3's *inert*-if-alone G2c; §3.7 / §6 O-CP-6). Authority: CP spec v1.35 §19.1.2 (operator-ratified Table A) + `.harness/class_1_fork_b2_spec_2_gate_axis_materialization.md` (✅ APPLIED) + `.harness/class_1_fork_b2_multi_server_mcp_client_reshape.md` (✅ APPLIED) + R-FS-1 §5.0 full-spec directive. ZERO spec amendment, ZERO new contract ID, X-AL-3-clean (`Axis.MCP_TRUST` is already a closed `Axis` member at `cp_shared_types.py`; the §19.1.2 floor table is cleared at CP spec v1.35; the composer's `harness_cp` import + the `GateLevelInput.mcp_trust_tier` field pre-exist). v2.35 + earlier PRESERVED VERBATIM per delta-only-plan-chain convention.*
...
`Implementation_Plan_Control_Plane_v2_35.md` (v2.35 — the R-FS-1 E-plan-3 CP leg; 2 NEW units U-CP-96/97 for RECONCILER_LOOP engine materialization + the reconciler engine-layer recovery-loop firing branch).

### §0.2 Revision scope (v2.35 → v2.36)

v2.36 decomposes the **CP-package** surface of the multi-server-MCP sub-program's **gate axis** (B2-spec-2) into ONE NEW atomic unit. **The gate axis is impl-against-cleared-spec** post-CP-spec-v1.35: the §19.1.1.1 row-3 spec-silence (carried since v1.15 — "per-tier → gate-level mapping owed at follow-on spec-extension arc") was **resolved** at CP spec **v1.35 §19.1.2**, committing `MCP_TRUST_GATE_LEVEL_FLOOR` (operator-ratified Table A: L0→DENY / L1→ASK / L2→ASK / L3→AUTO; floor-only/monotone, probe-resolved). With the floor table now determinate, U-CP-98 materializes the 4th §19.1 axis at `gate_level()` — exactly the impl-against-cleared-spec posture (the §19.1.2 formula already named `per_mcp_server_trust_floor`; `Axis.MCP_TRUST` is already a closed `Axis` member). Authoritative: CP spec v1.35 §19.1.2 + `.harness/class_1_fork_b2_spec_2_gate_axis_materialization.md` (✅ APPLIED) + `.harness/class_1_fork_b2_multi_server_mcp_client_reshape.md` §5 (F2-02 carve-out) + `.harness/r-fs-1-b2-plan-decomposition.md` (companion). The B2 keystone homing facts: the `gate_level()` 4-axis composition rule (`gate_level_rule.py`) is `harness-cp` (U-CP-98); the multi-server reshape + the gate-axis *producer* (U-RT-131 — the L3 no-floor default at the host-less gate sites; the real per-server resolved-host feed is the `B-TOOL-GATE` forward arc) are `harness-runtime` (runtime plan v2.47 U-RT-125..131). The CP package owns:

| CP surface | Unit | Authority |
|---|---|---|
| `gate_level()` 4th-axis composition (`gate_level_rule.py`) — commit `MCP_TRUST_GATE_LEVEL_FLOOR` (Table A) + add `Axis.MCP_TRUST: MCP_TRUST_GATE_LEVEL_FLOOR[input.mcp_trust_tier]` to `per_axis_floors` (the `max()` ranges over 4 materialized floors, not 3) + refresh the 5 stale-carry "spec-silent" docstrings (module `:6-13`/`:26-27`; `GateLevelInput.mcp_trust_tier` `:105-111`; `GateLevelComputation` `:138-141`; `gate_level()` `:190-195`) to cite §19.1.2 (F3-01). Reads the **already-present** `GateLevelInput.mcp_trust_tier` field (`:104`). | **U-CP-98** | CP spec v1.35 §19.1.2 (the committed floor table + the "composes `Axis.MCP_TRUST` into `per_axis_floors`" mandate); gate-axis fork §4 items 2+4 |

No spec amendment; no new contract ID. `Axis.MCP_TRUST` is already a closed `Axis` enum member (`cp_shared_types.py:193`), `MCPTrustTier` is already closed at 4 (`cp_shared_types.py:172`), `GateLevel` is closed at 3, and `GateLevelInput.mcp_trust_tier` already exists (`gate_level_rule.py:104`, present-but-unconsumed since v2.20); U-CP-98 **consumes cleared closed enumerations + an existing field + a cleared §19.1.2 floor table** (X-AL-3-clean — no new primitive). The composer's `harness_cp` import (`hitl_gate_composer.py:457`) + the `GateLevelInput` carrier pre-exist, so the gate axis is realized with **no new cross-axis carrier edge**; the CP↔RT coupling is a §3.7 / §6 O-CP-6 **co-land sequencing pin** (U-CP-98 is HARMFUL-if-alone — see §0.4 + §3.7).

| In scope at v2.36 | Out of scope |
|---|---|
| U-CP-98 (`gate_level()` 4th-axis composition + the `MCP_TRUST_GATE_LEVEL_FLOOR` table + the F3-01 docstring refresh) | All v2.35 + prior unit bodies — preserved verbatim per §0.3; the multi-server reshape + the composer resolved-host producer are U-RT-125..131 (runtime v2.47) |
| The B2 aggregate cross-axis DAG + topological order + the **U-CP-98 ⊕ U-RT-131 co-land pin** (§3.7 — arc home) | The runtime reshape (D1–D4) + the composer wiring (U-RT-125..131, runtime plan v2.47) — co-published, cited cross-axis |
| Coverage matrix: +1 CP row + AC-level dispositions (§4.5) | The `per_tool_gate_level` producer (O-CP-3, the remaining inert §19.1 axis — registered forward, NOT this arc); the reshape forward items B2-restart / server-qualified addressing / B6 (reshape fork §6, runtime-registered) |

### §0.3 Sections preserved verbatim from v2.35

| Section | Status at v2.36 |
|---|---|
| §0 (v2.35 change-note) | Superseded by this §0 (historical record preserved at v2.35) |
| §1 Spec inventory | Refreshed: +1 contract-surface row (C-CP-19 §19.1.2 gate-axis); all prior rows (incl. the E-3 + E-1/E-2 + B3 rows) PRESERVED VERBATIM |
| §2 — U-CP-01..U-CP-97 (all prior units) | **PRESERVED VERBATIM** from v2.35 + lineage (delta-only-plan-chain convention) |
| §3 Dependency graph | Revised: +§3.7 B2 aggregate cross-axis graph (+ the co-land pin); all prior B1 (§3.1–§3.3) + B3 (§3.4) + E-1/E-2 (§3.5) + E-3 (§3.6) graphs + acyclicity PRESERVED VERBATIM |
| §4 Coverage matrix | Revised: +§4.5 (1 B2 unit row + AC-level dispositions); §4.1/§4.2/§4.3/§4.4 + all prior PRESERVED VERBATIM |
| §5 Cross-cutting integration units | Unchanged (no new tri-spec unit at v2.36); all prior preserved verbatim |
| §6 Open items | Extended: +O-CP-6 (the B2 co-land pin + the reshape forward items, recorded); O-CP-3 + O-CP-4 + O-CP-5 + prior PRESERVED VERBATIM |

### §0.4 Authority chain — no operator gate

v2.36 absorbs the **cleared** gate-axis reading (CP spec v1.35 §19.1.2 — operator-ratified Table A, floor-only/monotone probe-resolved; the B2-spec-2 fork ✅ APPLIED; gate-axis materialization is now impl-against-cleared-spec). No operator decision owed at this plan-layer arc; ZERO X-AL-3 risk for U-CP-98 (closed `Axis`/`MCPTrustTier`/`GateLevel` enums consumed + an existing field + a cleared floor table; no new primitive). **The one load-bearing planner finding (NOT a fork — a build-sequencing constraint):** U-CP-98 is **HARMFUL-if-landed-alone**. The composer pins `mcp_trust_tier=MCPTrustTier.LEVEL_0_REFUSE_REMOTE` (`hitl_gate_composer.py:462`), admissible only while `gate_level()` ignores the field; Table A maps `L0→DENY`; the gate is `max()` over `AUTO<ASK<DENY`. So if U-CP-98 composes `Axis.MCP_TRUST` while the composer still pins L0, `MCP_TRUST_GATE_LEVEL_FLOOR[L0]=DENY` enters the `max()` on **every** gate → every host-less gate (inference + sub-agent — the only gate sites that exist) becomes `DENY`. This is the **inverse** of B3's G2c (which was *inert*-if-alone). It is encoded as a **hard co-land sequencing pin** with runtime U-RT-131 (the composer **no-floor-default** change — replaces the L0 constant with the L3 AUTO-mapping default at the host-less gate sites; the real resolved-host feed is the `B-TOOL-GATE` forward arc): U-CP-98 + U-RT-131 land in the **same final impl arc (B2-impl-3)**, U-CP-98 MUST NOT merge in an earlier arc (§3.7 / §6 O-CP-6). It cannot be a DAG dependency edge — the constraint is "the harmful consumer must NOT land before the safe producer," the reverse of a dependency, and the CP→RT edge direction is forbidden anyway. Surfaced to the operator in the B2-plan deliverable; both spec legs are cleared, so no back-flow is owed.

### §0.5 Status posture

`Status: Proposed`. Clearance marker (operator-filed at PR) at `.harness/clearance/Implementation_Plan_Control_Plane-v2_36-cleared-*.md`. Sibling co-publication: runtime plan v2.47 (U-RT-125..131). Companion: `.harness/r-fs-1-b2-plan-decomposition.md`.
...
| Field | Value |
|---|---|
| Plan version | v2.36 (delta over v2.35) |
| Authored at | 2026-06-16 |
| Authoring authority | R-FS-1 B2-plan; CP spec **v1.35 §19.1.2** (the operator-ratified `MCP_TRUST_GATE_LEVEL_FLOOR` Table A gate-axis materialization) + `.harness/class_1_fork_b2_spec_2_gate_axis_materialization.md` (✅ APPLIED) + `.harness/class_1_fork_b2_multi_server_mcp_client_reshape.md` §5 (F2-02 carve-out) + R-FS-1 §5.0 full-spec directive; C-CP-19 §19.1 / §19.1.1.1 row 3 (cleared); companion `.harness/r-fs-1-b2-plan-decomposition.md` |
| Net delta | +1 NEW unit (U-CP-98 `gate_level()` 4th-axis composition + the `MCP_TRUST_GATE_LEVEL_FLOOR` table + the F3-01 docstring refresh; `(none)` DAG-deps — reads the existing `GateLevelInput.mcp_trust_tier` field) — the multi-server reshape (D1–D4) + the gate-axis *producer* (U-RT-131 — the L3 no-floor default at the host-less gate sites; the real per-server resolved-host feed is the `B-TOOL-GATE` forward arc) are U-RT-125..131 (runtime v2.47), NOT CP units; **+0 cross-axis DAG edges** (the gate axis needs no new carrier; `GateLevelInput.mcp_trust_tier` + the composer's `harness_cp` import pre-exist); **+1 cross-axis CO-LAND SEQUENCING PIN** (U-CP-98 ⊕ U-RT-131, B2-impl-3 — §3.7.3, a build-sequencing constraint NOT a DAG edge); +§4.5 (6 coverage rows/dispositions); +1 §6 open-item (O-CP-6, the co-land pin + reshape forward items, recorded-not-gated); ZERO spec amendment, ZERO new contract ID, ZERO CP-spec fork (`Axis.MCP_TRUST` already a closed member; the §19.1.2 floor table cleared at CP spec v1.35; `GateLevelInput.mcp_trust_tier` field + the composer's import pre-exist — X-AL-3-clean) |
| Sibling co-publication | runtime plan v2.47 (U-RT-125..131: D1 host-dict reshape + D2 routing/collision + D3 telemetry projection + D4 per-host sandbox + U-RT-131 composer **no-floor default** for the host-less gate sites); clearance markers (operator-filed at PR); workspace `CLAUDE.md` §2.4 + `.harness/claude-artifact-pointers.md` §2.4 plan-head bumps (the `harness-cp/CLAUDE.md` §1.2 plan-row is left at its pre-existing pin per the B1/B3/E precedent — its lag is a Q1 doc-hygiene drift item logged in the companion §7, not bumped here) |
| Aggregate B2 graph | 8 nodes (1 CP + 7 RT), acyclic, topological order at §3.7.2; **0 cross-axis DAG edges** (the gate axis introduces no new cross-axis carrier — unlike B1/B3); the CP↔RT coupling is the **U-CP-98 ⊕ U-RT-131 co-land pin** (§3.7.3, B2-impl-3). B1 (§3.1–§3.3) + B3 (§3.4) + E-1/E-2 (§3.5) + E-3 (§3.6) aggregates PRESERVED VERBATIM; the B2 nodes attach without contesting them (disjoint MCP-host/dispatch/gate surfaces) |
| Forks | ZERO fork in this plan delta. The gate axis is impl-against-cleared-spec post-v1.35 (the ONE genuine operator decision — the §19.1.2 mapping direction + Table A — was the B2-spec-2 leg, probe-resolved floor-only + operator-ratified Table A, landed as CP spec v1.35; RESOLVED, not re-opened here). The one load-bearing planner finding — **U-CP-98 is HARMFUL-if-landed-alone** (composing `Axis.MCP_TRUST` while the composer pins `mcp_trust_tier=L0→DENY` forces every host-less gate — inference + sub-agent, the only gate sites that exist — to DENY) — is a **build-sequencing constraint** (the U-CP-98 ⊕ U-RT-131 co-land pin, B2-impl-3; §3.7.3 / §6 O-CP-6), NOT a fork (both B2 spec legs cleared). The composer-architecture sub-finding (no tool-step gate site → the resolved-host feed is the forward `B-TOOL-GATE` arc, U-RT-131 a no-floor default) is a bounded re-scope of the runtime unit, NOT a spec defect — §3.7.3 / O-CP-6 item 2. |
| Homing decision | the `gate_level()` 4-axis composition rule (`gate_level_rule.py`) = `harness-cp` (U-CP-98); the multi-server reshape (`mcp_client_host_factory.py` / `runtime_tool_dispatcher*.py` / `mutable_context.py` / `types.py`) + the gate-axis producer (`hitl_gate_composer.py`) = `harness-runtime` (U-RT-125..131). The D3 trust projection realizes the CP §27.8 contract in runtime code (code-location homing; U-RT-129 cites CP §27.8) |
| B2-impl sequence (companion §6) | B2-impl-1 = U-RT-125 (host-dict + `ServerName`) + U-RT-126 (stage-3a all-hosts factory) + U-RT-129 (D3 projection + docstring); B2-impl-2 = U-RT-127 (routing index + collision fail-class) + U-RT-128 (dispatch resolution + consumers) + U-RT-130 (per-host sandbox) [e2e vs ≥2 mock MCP servers + the broader-suite impl-AC]; **B2-impl-3 = U-CP-98 + U-RT-131 (the co-land arc) [direct `gate_level()` unit test: per-tier L0→DENY…L3→AUTO table; + production non-regression: host-less gate composes MCP_TRUST=AUTO → identical to the 3-axis path = co-land safety]**. With B2-impl-3, the §19.1 gate composes 4-of-4 materialized axes; BOTH degenerate-default producers are registered forward (`per_tool_gate_level` = O-CP-3; `mcp_trust` real per-server feed = `B-TOOL-GATE`, O-CP-6 item 2). Reshape forward items (B2-restart / server-qualified addressing / B6) = registered forward (O-CP-6 / runtime v2.47 §6). |
