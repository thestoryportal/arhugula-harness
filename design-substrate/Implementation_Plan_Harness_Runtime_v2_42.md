...
2. Compute `routing_manifest_sha: str` via `sha256(json.dumps(harness_context.routing_manifest.model_dump(mode="json"), sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()` (RoutingManifest is a frozen Pydantic v2 BaseModel per `harness-cp/.../routing_manifest_residence.py:118`; no `.sha` method at HEAD; canonicalize-at-resolver per IS spec v1.3 §5.2 implementer-discretion footer). **NOTE — sort_keys=True is load-bearing for AC #6 cross-instance determinism**: Pydantic v2's `model_dump_json` preserves dict insertion order, which would produce different hashes for logically-identical RoutingManifests constructed with different mapping insertion orders (empirically verified at PR #89 adversarial review 2026-05-30). The `json.dumps(sort_keys=True)` path produces canonical sorted-key JSON independent of insertion order.
...
| 12 | RoutingManifest sha derivation | `routing_manifest_sha` derived via `sha256(json.dumps(model_dump(mode="json"), sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()`; deterministic across Pydantic v2 instances with identical field values **independent of dict insertion order** per PR #89 adversarial-review Finding A fix 2026-05-30 | IS §5.2 (implementer-discretion footer) |
| 6.A | Cross-instance determinism under mapping insertion order | Resolver output MUST be byte-identical for two `RoutingManifest` instances constructed with the same field values but different mapping insertion orders (regression test `test_resolve_routing_manifest_sha_invariant_under_dict_insertion_order`) | IS §5.2 + AC #6 cross-instance determinism + PR #89 adversarial-review Finding A 2026-05-30 |
...
- `test_resolve_routing_manifest_sha_invariant_under_dict_insertion_order` *(PR #89 Finding A regression guard 2026-05-30)*
