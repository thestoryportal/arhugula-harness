...
| harness-od/CLAUDE.md | §4.1 OD-3 + OD-4 rows | Updated this arc — both rows stay PARTIAL (refined) per X-AL-2 partial-retirement-is-non-retirement at tier-up direction. OD-3 gate (b) §10.3 persona-tier-aware base_rate envelope CLOSED; OD-3 gate (a) §9.1 tail-keep still deferred. OD-4 gate (a) §13.1 PARTIALLY CLOSED (deployment-level persona_tier + multi-tenant non-toggleability); per-session toggle + gate (b) §13.2 tokenization still deferred. Cumulative-counts line refreshed per workflow v1.12 §7.4.7.3.C audit-template; pipeline-advanced 6/8 = 75.0% UNCHANGED (within-PARTIAL refinement, not tier transit). Tier transit to RETIRE-READY requires all remaining gates closed at follow-on arcs (OD-3 §9.1 tail-keep; OD-4 per-session toggle + §13.2 tokenization). |
...
| Successor | OD plan v2.24 → v2.25 absorption (co-published this arc); H_T-OD-3 + H_T-OD-4 remain PARTIAL (refined); tier transit to RETIRE-READY requires follow-on arcs closing OD-3 §9.1 tail-keep gate + OD-4 §13.1 per-session toggle + OD-4 §13.2 opaque-token tokenization mode |
