...
**Revision scope, and the one place this delta departs from the fork's own prescription - stated first, because it is the delta's only judgement call.** The fork (§4 reading A, §8 option table, §9 and §11.5) prescribes **an acceptance-criterion amendment and no new unit**, on the ground that U-MEM-22 already owns the failure vocabulary and has landed. That prescription is **followed for the half it was written about**: U-MEM-22's `Acceptance:` failure-class line is extended from six values to seven, in place, and nothing else in that unit is touched. It is **departed from for the half it predates.** The fork's "no new unit" assessment was authored when reading A meant one enum member plus one declaration flip; **sub-decision A-ii did not exist yet** - it was raised at the fork's own out-of-family review round 4, and its scope was fixed only at the 2026-08-05 ratification, which added a six-site source re-typing in `lifecycle/llm_dispatch.py`, a receiving-type choice, a classification-reachability witness scoped to the classify-routed population, a preserved fail-fast retry disposition, and an end-to-end dispatch/retry witness. **That work has no owning unit.** U-MEM-22 is *Implement memory observability* (contracts C-MEM-19; axis action surface plus operational discipline plus runtime; depends on U-MEM-03, U-MEM-12, U-MEM-16, U-MEM-17, U-MEM-21), and a dispatch-side re-typing whose load-bearing witness is a **retry** exit classification is neither observability nor within that unit's dependency set. Folding it into a landed unit's acceptance list would put the work somewhere no impl arc would look for it - the precise failure the ratification's own "leg ownership, stated explicitly so the work has an owner" paragraph was written to prevent. This plan's own convention is the tie-breaker: each of the two preceding spec legs decomposed its impl leg as one new unit (`B-86` -> U-MEM-26 at G6, `B-92` -> U-MEM-27 at G7), and this delta follows it with **U-MEM-28 at G8**. **Nothing in U-MEM-28 exceeds the ratification's own enumerated impl-leg list** - the one candidate extension is surfaced and withdrawn in the next paragraph - so the unit is a decomposition of it, not an extension. The departure is recorded rather than absorbed so it can be overturned by inspection: an operator or reviewer who prefers the literal prescription can fold U-MEM-28's `Implement:` and `Acceptance:` items into U-MEM-22 without changing a single obligation.

**The seventh re-typing, SURFACED AND WITHDRAWN - the one place this leg was told it had authority it does not have** (raised at out-of-family review round 3, mislabelled at round 7, **withdrawn as an obligation at round 8**). Round 3 found that `_prepare`'s executor-side exhaustiveness fall-through (`memory_tool_executor.py:373`) raises the input-error type for a **harness-internal** fault, which spec v1.3's own type-boundary rule forbids - and rounds 3 through 7 had U-MEM-28 requiring it re-typed, first described as a decomposition and then, at round 7, honestly relabelled a **scoped extension** of the ratified A-ii list from six sites to seven. **Round 8 pointed out that relabelling it does not authorize it, and that is correct on this workspace's own rules:** the operator ratified an A-ii list of **exactly six**, all in `lifecycle/llm_dispatch.py`; a spec leg applies operator-decided fixes and does not widen a ratified enumeration on its own reading of the rule, however sound the reading. Recording a contradiction is not a substitute for deciding it. **The obligation is therefore removed from U-MEM-28 and the finding is carried instead**, at the unit's Out-of-scope list, at the spec change-note, at the fork's §11.6, and on the register row - with its consequence stated rather than softened: **at U-MEM-28's close, C-MEM-19's type-boundary invariant will have ONE known non-conformant site, recorded and unrepaired.** **That finding is owned by its own register row, `B-114`**, which outlives `B-88` and carries the grounding pass, the conformance-repair-vs-ratified-extension discriminator, and the repair recipe - so it does not disappear when `B-88` closes at U-MEM-28 (out-of-family review round 9 [P1]). **The witness arithmetic reverts with it**: eight population-table rows flip, as the ratification's own witness shape says, and no executor-side negative row is owed. **U-MEM-28 therefore contains nothing beyond the ratification's enumerated impl-leg list**, and that claim is now true without qualification.
...
- **The population table flips, and only where it should.** `harness-runtime/tests/test_memory_failure_classification.py` carries one `@pytest.mark.parametrize` table of **39** rows (decorator at `:57`, count re-verified by AST at this leg's HEAD). Exactly **8** rows - the `MemoryToolExecutionInputError` rows at `:90`, `:92`, `:96`, `:100`, `:103`, `:105`, `:110`, `:112` - change their expected value to `input_validation_failure`. The other **31 are untouched**, and that is asserted by leaving them untouched rather than by re-deriving them. (The `:90` row carries the *"unsupported memory tool"* message, whose raise site is the recorded-but-**not**-re-typed executor fall-through - see Out of scope. Because that site keeps the input-error type at this unit, its row flips with the rest.)
...
- **Re-typing `_prepare`'s executor-side exhaustiveness fall-through, recorded here because it is a REAL non-conformance this unit deliberately does not repair** (surfaced at out-of-family review round 3, withdrawn as an obligation at round 8). `_prepare`'s final branch (`memory_tool_executor.py:359-373`, the raise at `:373`) raises the input-error type for an *"unsupported memory tool"* - but `MemoryToolExecutionRequest.tool_name` is typed `MemoryToolName` (`:205-212`, the field at `:210`) on an `extra="forbid"` model, so that branch fires only when the executor lacks a `_prepare_*` for a valid enum member: a **harness-internal exhaustiveness fault**, which spec v1.3's type-boundary rule says must not be raised as that type. **It is nonetheless OUT OF SCOPE here, and the reason is authority rather than merit.** The operator ratified an A-ii site list of **exactly six**, all in `lifecycle/llm_dispatch.py`; this leg applies ratified decisions and does not widen a ratified enumeration on its own reading, however sound that reading is. So the consequence is stated plainly instead: **at U-MEM-28's close, C-MEM-19's type-boundary invariant has ONE known non-conformant site, recorded and unrepaired.** **Repairing it is owned by register row `B-114`**, which outlives `B-88`: it carries the grounding pass, the discriminator between plain conformance repair against the cleared contract and a genuine extension of the ratified A-ii enumeration, and the repair recipe including the receiving-type and dual-witness constraints.
