...
**Axis placement and dependencies for U-MEM-28.** Action surface plus operational discipline plus runtime - the same three U-MEM-22 carries, because the unit's contract is C-MEM-19 and its edits land in `harness-is` (the vocabulary enum) and `harness-runtime` (the declaration, the dispatch re-typing, the retry tuple). **Information substrate is deliberately not claimed**, unlike at U-MEM-26 and U-MEM-27: the `harness-is` edit is one enum member on an already-declared telemetry vocabulary, adding no record type, no field, and no store or index obligation, so no IS isolation posture is engaged. **Control plane is deliberately not claimed** either, and the reason is worth stating because the retry witness invites the opposite conclusion: the unit **preserves** an existing retry disposition rather than choosing a new one, adds no exit class, and consults no policy - the witness exists to prove nothing moved. `Depends on: U-MEM-16, U-MEM-22, U-MEM-25.` U-MEM-16 owns the standard memory tool executor, which is where the exception family and the declaration live; U-MEM-22 owns the telemetry surface and the vocabulary the seventh member joins, and its acceptance criterion is amended in this same delta. **U-MEM-25 is declared explicitly, and that is a correction rather than boilerplate** (out-of-family review round 3 [P2]): U-MEM-28's closeout criterion re-opens and rewrites the evidence packet U-MEM-25 produces, and unlike U-MEM-27 - which reaches U-MEM-25 transitively through U-MEM-26 - **U-MEM-28 has no such path**, so without the edge the canonical DAG would permit a schedule in which the packet this unit must amend does not yet exist. The edge matches the U-MEM-26 precedent, which declares U-MEM-25 for the same reason. U-MEM-17 and U-MEM-21 are reached transitively through U-MEM-22 and are not re-declared.

**Back-reference reconciliation inside this file.** §3 axis placement gains U-MEM-28 under action surface, operational discipline, and runtime; §4 gains three dependency edges (`U-MEM-16` -> `U-MEM-28`, `U-MEM-22` -> `U-MEM-28`, `U-MEM-25` -> `U-MEM-28`); §4.1 extends the R-MEM-01 range to U-MEM-28 and adds U-MEM-28 to R-MEM-13, the observability requirement U-MEM-22 already carries alone; §7 gains a G8 review-boundary row; §9 extends the completion range to U-MEM-28. U-MEM-22's `Acceptance:` failure-class bullet is the one pre-existing unit-body line this delta edits. No other row is touched. R-MEM-09 is deliberately **not** extended: the dispatch-side re-typing changes no provider routing behaviour and adds no access mode - it changes which class a fault is reported under and preserves its retry disposition.

**Closeout staleness at v1.3 - annotation half APPLIED HERE, at this spec leg, per the `B-86` and `B-92` precedents** (out-of-family review round 2 [P1], which caught its omission). `.harness/u-mem-25-memory-closeout-evidence.md` scopes itself to spec **v1.2** and plan **v1.2** (U-MEM-01..27), and three of its rows grow obligations at v1.3: **C-MEM-19** in the C-MEM matrix (the contract this delta amends) and **R-MEM-01** and **R-MEM-13** in the R-MEM matrix (the two requirements §4.1 above maps U-MEM-28 into). Without the annotation the packet's own header sentence - *"No matrix row carries a `PENDING` marker of any generation"* - and its `ready: yes` would assert certification of obligations no unit has yet built. `just memory-closeout-check` stays green regardless, because it tests `## C-MEM-NN` and `### R-MEM-NN` heading-derived id coverage and v1.3 adds neither - the identical interim-window shape both prior legs recorded, and the reason the red-gate alternative was DECLINED there (a red `just check` on `main` for the whole interim window blocks every unrelated arc) applies unchanged. **The honest-annotation half is applied in this same PR**: `PENDING - U-MEM-28` markers on all three rows, a third "Version scoping" window paragraph stated beside the two closed ones, and the header sentence qualified. **The re-open-and-extend half is owed at U-MEM-28**, as an acceptance criterion with a green re-run in Verification. R-MEM-05 and R-MEM-09 are deliberately **not** annotated, for the same reason §4.1 does not extend them: the unit adds no promotion obligation and changes no provider routing behaviour.
...
| tool path branches on `auto_promote_allowed` `:448-460` | change-note `:45` | **`:632-645`** (`if candidate.auto_promote_allowed:` at `:632`, the `else` at `:639`) |
...
Depends on: U-MEM-16, U-MEM-22, U-MEM-25.
...
- **The receiving type for the six, with three binding constraints and otherwise discretion.** The type must (a) **not** be a subclass of `MemoryToolExecutionInputError`, since the declaration is read through the MRO and an inheriting subtype would carry `input_validation_failure` straight back into the internal population, defeating the re-typing entirely; (b) **not** be the family base `MemoryToolExecutionError` itself, and this constraint is the one an earlier draft of this unit got wrong (out-of-family review round 1 [P1]) - the next bullet admits the receiving type to a retry classifier's `isinstance` tuple, and `isinstance` is subclass-inclusive, so admitting the base would silently make **`MemoryToolExecutionDeniedError`** (`:106-120`) and **`MemoryToolExecutionStoreError`** (`:123-148`) fail-fast too, which that classifier's own docstring explicitly refuses on the ground that the denied type wraps genuinely transient store I/O (`retry_breaker_fallback.py:317-323`). A **dedicated internal-fault subtype** of the base, admitted by name, is therefore required - or exact-type rather than `isinstance` handling, if an implementation prefers that; and (c) declare a failure class **other than** `input_validation_failure`. On (c) the conforming choice today is `provider_adapter_failure`, which the family base already declares as the **residual** (`memory_tool_executor.py:87-103`, the declaration at `:101-103`, the docstring calling it *"the RESIDUAL class"*) and which a subtype inherits for free; spec v1.3's own subsection states that a residual report is the absence of a claim rather than an assertion of adapter-hood, so this does not label an internal fault an adapter fault. What the contract **requires** here is only that the internal population stop carrying the input-validation class; if a later arc gives the vocabulary a dedicated unclassified value, this receiving type takes it without a further contract amendment.
...
- **One EXECUTOR-side site falls under the same type-boundary rule, and it is named here rather than left to violate the boundary in place** (out-of-family review round 3 [P2], accepted after direct read; **flagged as a derivation the impl leg may overturn by re-grounding**). `_prepare`'s final fall-through (`memory_tool_executor.py:359-373`, the raise at `:373`) raises the input-error type for *"unsupported memory tool {tool_name}"* - but `MemoryToolExecutionRequest.tool_name` is typed `MemoryToolName` (`:205-212`, the field at `:210`) on an `extra="forbid"` model, so a request reaching `_prepare` **always** carries a valid enum member and that branch fires only when the executor lacks a `_prepare_*` for one: a harness-internal **exhaustiveness** fault, not a caller- or model-supplied argument. Under spec v1.3's type-boundary rule it therefore must not carry `input_validation_failure` either; re-type it to the same internal-fault receiving type the six dispatch sites take. **Scope, stated honestly:** the ratification enumerated **six** sites because the fork surveyed `lifecycle/llm_dispatch.py` and did not survey the executor, so this is the ratified **rule** applied where it also bites rather than a widening of the ratified reading - and if the impl leg's own re-grounding finds the branch reachable from caller input at its HEAD, it keeps the input class and the witness arithmetic below reverts to eight flipping rows.
...
- The other six C-MEM-19 classes are **unchanged**, and the seventh member is additive: no existing member is removed, renamed, or re-valued, and no population other than the input-error family changes the class it emits. In particular the family base `MemoryToolExecutionError` still declares `provider_adapter_failure`, the store subtype still declares `io_failure`, the denial subtype still declares `policy_denial`, and the classifier residual is untouched.
- Every `lifecycle/llm_dispatch.py` raise site that reports a **harness-internal** fault - unset runtime memory context, missing or malformed harness-supplied tool schema - raises a type that does **not** carry `input_validation_failure`, and every site that refuses a **model-supplied** argument still raises the input-error type. The split is asserted per site, not in aggregate. Every **executor** raise site reporting a harness-internal fault leaves the input-validation class on the same rule - at this HEAD that is `_prepare`'s exhaustiveness fall-through alone. **The obligation on the internal six is negative** - they must leave the input-validation class - **not** that they land on any one named class; asserting the residual they land on today is permitted, but a witness that pins it as a requirement would over-constrain the contract (out-of-family review round 1 [P1]).
- The six re-typed sites retain **fail-fast** exit classification end-to-end, **each of the six and not a representative one**: a dispatch that trips any of them abandons the candidate without a same-candidate retry staircase, exactly as before the re-typing.
...
- The U-MEM-25 closeout evidence rows for **C-MEM-19** in the C-MEM matrix and for **R-MEM-01** and **R-MEM-13** in the R-MEM matrix - all three carrying a `PENDING - U-MEM-28` marker filed at the spec-leg PR - are re-opened and extended to cover the v1.3 obligations (the seventh value; the boundary and type-boundary rules; the coverage statement and what it does and does not claim; the preserved retry disposition), the packet's window-3 version-scoping paragraph is closed the way windows 1 and 2 were, and the packet's header sentence is lifted back off v1.2. **The unit cannot close while the packet still says the v1.3 obligations are uncertified.**

Verification:

- **The population table flips, and only where it should.** `harness-runtime/tests/test_memory_failure_classification.py` carries one `@pytest.mark.parametrize` table of **39** rows (decorator at `:57`, count re-verified by AST at this leg's HEAD). Eight rows construct `MemoryToolExecutionInputError` (`:90`, `:92`, `:96`, `:100`, `:103`, `:105`, `:110`, `:112`). **Seven flip** their expected value to `input_validation_failure`. The eighth, `:90`, carries the *"unsupported memory tool"* message - the executor exhaustiveness fall-through re-typed above - and is **re-keyed to the internal-fault type with the residual as its expected value**, making it the executor-side **negative** row for the type boundary rather than a flipped row. The other **31 are untouched**, and that is asserted by leaving them untouched rather than by re-deriving them. (If the impl leg's re-grounding overturns the `:373` derivation, this row flips with the other seven and the count is eight.)
...
- **The dispatch six/two split, witnessed per site.** For each of the six internal-fault sites, the raised exception classifies to something **other than** `input_validation_failure` (the residual `provider_adapter_failure` under the receiving type above); for each of the two retained model-argument sites, it classifies to `input_validation_failure`. Eight assertions, not two aggregates: an aggregate over "the dispatch sites" is satisfiable by an implementation that re-typed all eight, which is the exact error the ratification corrected. **Each of the six internal-site assertions is PAIRED with a retry-exit assertion on the same exception instance** (out-of-family review round 4 [P1]): the telemetry half alone cannot see a site that accidentally raises the family **base** rather than the dedicated internal-fault subtype, because the base classifies to the same residual - yet the base is **not** in the fail-fast set, so that site would silently fall through to transient retry and re-fire a dispatch that may already have committed. Assert the fail-fast exit **per site** - `_classify_provider_exception(exc) is None` for each of the six - or drive all six through the retry path; a single end-to-end witness over one site does not discharge it.
...
- Re-run of `just memory-closeout-check` with all three refreshed evidence rows and every `PENDING - U-MEM-28` marker lifted, green.
- **Mutation probes, each restored after.** (a) Revert the declaration at `memory_tool_executor.py:171-173` to `PROVIDER_ADAPTER_FAILURE`: exactly the 7 flipped table rows fail with `assert 'provider_adapter_failure' == 'input_validation_failure'`, and the two end-to-end span witnesses fail with them - while the re-keyed `:90` negative row still passes, which is what distinguishes the declaration flip from the executor re-type. (b) Re-type one of the two **retained** model-argument sites along with the six: its own per-site assertion fails and no other does - proving the split is asserted per site rather than in aggregate. (c) Remove the receiving type from `_classify_provider_exception`'s fail-fast tuple: the end-to-end dispatch/retry witness fails while every telemetry witness still passes - which is the correct discriminating shape, since the two obligations are independent and a single suite that could not tell them apart would let the retry regression ship green.
...
- **Resolving the cross-entry-path `scope_ref`-mismatch divergence.** The same model-supplied `scope_ref` mismatch is refused as a **denial** at the executor (`memory_tool_executor.py:794-800`, one of the four sites the `B-88` impl half re-classified to `policy_denial`) and as an **input error** at the dispatch prepare layer (`lifecycle/llm_dispatch.py:4371`, retained by ratified name), so this unit leaves one logical failure carrying two classes. It is **inert** - the dispatch site emits nothing - and **neither side is movable here**: the executor's class is a landed decision and the dispatch site's retention is ratified operator text. It is recorded at the spec v1.3 change-note and at the `B-88` register close-out, and it belongs to whichever arc closes the dispatch-side coverage gap, since that is when it becomes observable.
