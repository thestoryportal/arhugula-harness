...
- **Re-typing `_prepare`'s executor-side exhaustiveness fall-through, recorded here because it is a REAL non-conformance this unit deliberately does not repair** (surfaced at out-of-family review round 3, withdrawn as an obligation at round 8). `_prepare`'s final branch (`memory_tool_executor.py:359-373`, the raise at `:373`) raises the input-error type for an *"unsupported memory tool"* - but `MemoryToolExecutionRequest.tool_name` is typed `MemoryToolName` (`:205-212`, the field at `:210`) on an `extra="forbid"` model, so that branch fires only when the executor lacks a `_prepare_*` for a valid enum member: a **harness-internal exhaustiveness fault**, which spec v1.3's type-boundary rule says must not be raised as that type. **It is nonetheless OUT OF SCOPE here, and the reason is authority rather than merit.** The operator ratified an A-ii site list of **exactly six**, all in `lifecycle/llm_dispatch.py`; this leg applies ratified decisions and does not widen a ratified enumeration on its own reading, however sound that reading is. So the consequence is stated plainly instead: **at U-MEM-28's close, C-MEM-19's type-boundary invariant has ONE known non-conformant site, recorded and unrepaired.** Repairing it needs an operator ratification of a seventh site - which a later arc may seek - or a finding at the impl leg's own re-grounding that the branch is reachable from caller input after all, in which case it is conformant as it stands and nothing is owed.
