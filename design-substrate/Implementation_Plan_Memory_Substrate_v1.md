...
- Write-boundary coverage: the C-MEM-03 "never a provider key" value domain is enforced at **every** durable write of a `MemoryScope`, not only at the automatic capture path. The grounded surfaces are (a) automatic capture, which constructs its own scope today (`memory_capture.py:585-591`) and is the `B-89` half above; (b) the promotion record write, which persists `candidate.suggested_scope` **verbatim** into `MemoryRecordEnvelope.scope` (`memory_promotion.py:741-753`), whose scope arrives either from a caller/model-supplied promotion hint (`_candidate_from_hint`, `memory_promotion.py:568-582` - today only risk-flagged `CROSS_SCOPE` at `:609`, never normalized) or from the tool-execution context (`memory_tool_executor.py:405-412`); and (c) a statically-supplied `RuntimeMemoryContext` on the non-recomposing dispatcher path (`llm_dispatch.py:669`, `:1475`), whose `record_scope` reaches (b) through that same context. Each surface validates or canonicalizes `provider_family` before the write: canonicalize through the existing provider-to-family authority (`lifecycle/cross_family_cost_tag.py:60-69`, whose scope-check variant already fails closed on unregistered keys) where the input is a registered provider key; where it is not, the promotion path denies the write - C-MEM-09 already requires promotion to deny on failed policy resolution - and the capture path records the field as `null`, the spec-defined unpartitioned value, rather than persisting a non-value identifier. No surface silently persists an out-of-domain value.
...
- Write-boundary test per grounded surface: a promotion hint carrying a raw provider key is denied rather than persisted, a tool-executor promotion under a statically-supplied context lands a canonicalized family value, and a capture write with an unregistered provider key records `null` rather than the key.
