...
| EVENT_SOURCED_REPLAY materialization (U-CP-93) + WAL_SEGMENT resumption + `test_u_rt_95` un-skip (U-CP-94) + recovery-loop firing branch (U-CP-95) | **CP plan v2.34** | engine driver / `EngineClass` / consumers / C-CP-49/50 composers are `harness-cp`; the firing branch consumes `ctx.engine_recovery_loop` duck-typed (no CP→RT import) |
...
| C-CP-49/50 (§16.5.2 composers) + R-CXA-2 CP→IS engine-layer seam go-live + CP §16.5.9 invariant 5 | **U-RT-122** (NEW; factory bind + go-live e2e; consumes U-CP-95 cross-axis) |
