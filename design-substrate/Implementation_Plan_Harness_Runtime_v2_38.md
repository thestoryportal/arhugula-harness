...
- `harness-runtime/tests/integration/test_cp_is_caller_site_integration.py` (NEW — 1 scoped e2e covering the pause-resume workflow-layer site in a single workflow lifecycle + 1 negative-path test (no-binding short-circuit) + 2 per-caller-site unit tests for AC #3 (RESUME_ATTEMPTED + PAUSE_CAPTURED drain-flag) = **4 tests; reframed from v2.37's 4 to v2.38's 4**. **HITL-signal site `:894` test DEFERRED to follow-on integration arc** — `HITLPauseRequestedSignal(brief, delivery_result)` constructor requires non-trivial Pydantic models duplicating the validator_framework + webhook_delivery composer surface already exercised at `test_validator_framework_types.py`; the emission code at site `:894` mirrors site `:769` structurally (same guard, same composer kwargs, `event_kind_index=2` vs `=1`); logical coverage implied by drain-flag site test plus symmetric code shape. **Path-location refresh:** file relocated from plan-canonical `harness-runtime/tests/` to `harness-runtime/tests/integration/` because `patched_runtime` fixture is auto-discovered only under integration/ via the local conftest — mirrors `test_u_rt_89_pause_resume_full_execution_path.py` placement convention.)
...
**Per-composer kwarg derivation rule at firing site (NEW at v2.38; explicit-derivation-documented under spec composer-kwarg silence per advisor 42nd application + operator AskUserQuestion ratification 2026-05-29):**

CP spec v1.26 §16.5 declares the composer signature but is silent on derivation rules for `event_sequence_id: int` + `protocol_state_snapshot: Mapping[str, Any]` at runtime call sites; §16.5 implementer-discretion clauses cover only `_canonicalize_outcome_bytes` helper home + outcome-canonical-bytes scheme. v2.38 documents the derivation rule explicitly in plan body to avoid silent synthesis at impl arc:

- **`workflow_id: str`** — sourced from `manifest_entry.workflow_id` at all 3 sites.
- **`step_id: str`** — at site `:559` (RESUME_ATTEMPTED), `str(pause_snapshot_input.step_index)`; at sites `:769` + `:894` (PAUSE_CAPTURED), `str(step_index)` (the per-step index local variable in the driver loop).
- **`protocol_event_kind: PauseResumeProtocolEventKind`** — literal at each site (`RESUME_ATTEMPTED` at `:559`; `PAUSE_CAPTURED` at `:769` + `:894`).
- **`event_sequence_id: int`** — **derivation: `(step_index << 2) | event_kind_index`** where `event_kind_index ∈ {0: RESUME_ATTEMPTED, 1: PAUSE_CAPTURED_DRAIN, 2: PAUSE_CAPTURED_HITL}` (3-value local enum at workflow_driver scope, distinct from `PauseResumeProtocolEventKind` 2-value composer enum). The `<< 2` reserves the low 2 bits for the event_kind_index; `step_index` occupies bits 2+. Deterministic-on-replay (the formula consumes only persisted inputs); guarantees unique `event_sequence_id` per `(step_index, firing_site)` 2-tuple within a workflow run, so the §16.5.4 row 3 idempotency_key formula `workflow_id || step_id || pause_resume_protocol_event_kind || event_sequence_id || sha256(outcome_canonical_bytes).hex()` honors uniqueness at the per-event-fire granularity. CP spec §16.5 silent on derivation; v2.38 documents the rule explicitly per operator-ratified implementer-discretion 2026-05-29.
- **`protocol_state_snapshot: Mapping[str, Any]`** — **derivation: `<snapshot-returned-from-protocol-method>.model_dump(mode="json")`** at each site. Spec §16.5.5 row U-CP-30 characterizes the outcome semantic as "protocol-state-transition outcome canonical JSON bytes — the protocol state snapshot after the class-level event"; `model_dump(mode="json")` of the Pydantic BaseModel returned by the protocol method IS the faithful realization of "state snapshot after". At `:559`: `resume_result.model_dump(mode="json")` (the `ResumeResult` BaseModel from `attempt_resume(...)`). At `:769` + `:894`: `pause_snapshot.model_dump(mode="json")` (the `PauseSnapshot` BaseModel from `capture_pause_snapshot(...)`). NOT silent synthesis — `model_dump()` is the canonical Pydantic-v2 serialization surface anchored at spec §16.5.5 semantic; framed under v2.38 AC #3 as explicit-derivation-documented per operator ratification.
- **`actor: ActorIdentity`** — sourced from `ctx.ledger_writer.actor` per AC #9 (workflow_driver scope clean; `ledger_writer` attribute on `DriverContext` Protocol per workflow_driver.py:303).

**Discretion-vs-synthesis framing.** The v2.38 derivation rule is documented explicitly in plan body BEFORE impl; the derivation is not invented at impl time. Per advisor 42nd application + operator ratification, this falls under implementer-discretion under spec composer-kwarg silence — distinct from the prior 4 STRIKES at U-RT-111 (which were "substrate doesn't exist" shape: missing fields on downstream types, NotImplementedError stubs, wrong scope, wrong lifecycle stage). The `event_sequence_id` derivation rule could become structurally stale if CP spec v1.27+ amends §16.5.4 row 3 with a canonical derivation rule that conflicts; in that case AC #3 would route to a spec-conformance amendment arc per the same pattern as v1.20 `default_gate_level` lift. Future-CP-spec-amendment risk is acknowledged but does not block v2.38 impl per operator-opt-in `cp_is_wiring=None` default.
