...
**All three invariants are IS-chain-checkable in isolation.** None of (a)/(b)/(c) requires the OD ledger, the B-36 signing backend, or any cross-axis composition — a consumer can verify all three by reading `StateLedgerEntry.rotation_correlation_id` across the claimed window's entries alone. This is the deliberate scope boundary against the fork's §2 points 2–4 (`harness-is` has zero outbound cross-axis edges): the FURTHER requirement a genuine rotation proof needs beyond (a)/(b)/(c) — that the id also matches a real dual-signature OD audit-pair (the OD-join) — is necessarily CP/runtime-owned, since only a composition root with access to both ledgers can perform that join (per the fork's own §2 point 4 reasoning: "`harness-cp` cannot import the OD ledger type without reversing the axis-import direction"). §5.6/§7.7 supply the IS-side half of the evidence that join consumes; they do not perform the join.

**Failure discipline.** None of (a)/(b)/(c) is a "warning" — a non-emptiness, presence, or uniqueness failure means the claimed rotation window is NOT verified, exactly as the CP `verify_rotation_6_steps` fork registration already establishes for the pre-v1.12 generic-continuity gap ("zero entries is an absence of evidence, not proof of chain integrity" — the same fail-closed posture (c) extends to the rotation-correlation carrier, and the same posture extends further still to a malformed or torn non-empty window post-v1.12).

**Deferred to implementation discretion.** The precise consumer-side error taxonomy for a non-emptiness, presence, or uniqueness failure (a CP-owned typed exception, per `B-33-A`'s impl leg); the CONCRETE boundary-derivation mechanism a consumer uses to identify a window's entries in the first place (positional bounds / a dedicated boundary marker / OD-pair-named evidence — a CP-side write-cadence + read-cadence decision, mirroring the §5.4/§5.6 write-cadence deferrals) — only the mechanism is deferred; the REQUIREMENT that it be independent of `rotation_correlation_id` is pinned above, not deferred. §7.7 states what the IS chain guarantees once an independently-derived window is identified, not how a window is identified.
