...
**Producer-supplied, not resolver-derived.** Mirroring §5.4's own framing: unlike §5.1's `procedural_tier_snapshot_ref` (derived from `HarnessContext` by the §5.2 resolver), `rotation_correlation_id` is composed by the **CP rotation orchestration** (`execute_key_rotation`'s writes, `harness_cp.five_axis_composition`) at rotation-window entries. There is therefore **no resolver contract** for this field (no §5.2 analogue) — population is an impl-leg concern, deferred to the separate `B-33-A` impl arc.
