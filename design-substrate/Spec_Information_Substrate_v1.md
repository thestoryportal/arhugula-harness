...
**Closes `B-33`** (registered at `.harness/post-phase-8-forward-register.md`; Codex out-of-family rounds 1 + 5 on the `B-22` seam, `Spec_Control_Plane_v1_98.md` §1) via the RATIFIED Class 1 fork `.harness/class_1_fork_b33_rotation_correlation_carrier.md` §4 Option A. `verify_rotation_6_steps` (`harness_cp.five_axis_composition`) verifies a key-rotation window with a generic hash-chain continuity check that PASSES on a ledger where no rotation boundary occurred at all — the IS state-ledger rows the walk traverses carry no identity for the rotation event itself, so the walk proves chain continuity, not rotation. OD already closed the symmetric gap on its own ledger at OD spec v1.31 §24.7 (`audit.rotation_correlation_id`, joining the two-row dual-signature rotation pair); this delta authors the IS-side counterpart the walk needs to JOIN against.
...
**Additive carrier, precisely reconciled against the ratified "open-dict" framing.** The RATIFIED fork (`.harness/class_1_fork_b33_rotation_correlation_carrier.md` §2 point 1 + §4 Option A) describes the shape as "an open-dict sidecar field … mirroring the OD §24.7-style additive convention." `StateLedgerEntry` / `EntryPayload` carry no open-dict/namespace-attrs mechanism (unlike OD's `AuditPayload.audit_namespace_attrs`); the IS axis's own established idiom for exactly this kind of extension is the **typed D-derivative sidecar** (§5.1, §5.4), and §5.6 follows that idiom. This is a faithful, not a loose, reading of the ratified decision: OD §24.7 — the cited precedent — is ITSELF hash-covered (a tampered value on either sibling changes that sibling's `entry_hash`); "additive" at both precedents has always meant "absent-by-default, non-schema-breaking," never "unhashed." A correlation id that were not hash-covered would be trivially strippable or swappable post-write, which would defeat the verifier's entire tamper-evidence purpose — so hash-coverage is a requirement of the ratified intent, not an optional strengthening this delta adds on top of it, and the typed-sidecar mechanism delivers it by construction via the existing §6.1 canonicalization path with zero new mechanism.
...
**Both invariants are IS-chain-checkable in isolation.** Neither requires the OD ledger, the B-36 signing backend, or any cross-axis composition — a consumer can verify (a) and (b) by reading `StateLedgerEntry.rotation_correlation_id` across the claimed window's entries alone. This is the deliberate scope boundary against the fork's §2 points 2–4 (`harness-is` has zero outbound cross-axis edges): the third invariant a genuine rotation proof requires — that the id also matches a real dual-signature OD audit-pair (the OD-join) — is necessarily CP/runtime-owned, since only a composition root with access to both ledgers can perform that join (per the fork's own §2 point 4 reasoning: "`harness-cp` cannot import the OD ledger type without reversing the axis-import direction"). §5.6/§7.7 supply the IS-side half of the evidence that join consumes; they do not perform the join.

**Failure discipline.** Neither invariant is a "warning" — a presence or uniqueness failure means the claimed rotation window is NOT verified, exactly as the CP `verify_rotation_6_steps` fork registration already establishes for the pre-v1.12 generic-continuity gap ("zero entries is an absence of evidence, not proof of chain integrity" — the same fail-closed posture extends to a malformed or torn window post-v1.12).

**Deferred to implementation discretion.** The precise consumer-side error taxonomy for a presence or uniqueness failure (a CP-owned typed exception, per `B-33-A`'s impl leg); how a consumer determines a window's boundary entries in the first place (a CP-side write-cadence + read-cadence decision, mirroring the §5.4/§5.6 write-cadence deferrals) — §7.7 states what the IS chain guarantees once a window is identified, not how a window is identified.
