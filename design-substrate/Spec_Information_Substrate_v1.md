...
| Revision | v1.11 → v1.12 (`B-33-A` spec leg — **IS rotation-correlation carrier**, ratified Option A — Class 1 fork `.harness/class_1_fork_b33_rotation_correlation_carrier.md`, operator-ratified 2026-07-21 "OPTION A AS RECOMMENDED"; unblocked by `B-36`/ADR-D8 landing a real signing backend, 2026-07-16). NEW §5.6 D-derivative sidecar field `rotation_correlation_id: str \| None` on `StateLedgerEntry` / `EntryPayload` (C-IS-05) + NEW §7.7 read-side presence/uniqueness invariants (C-IS-07). **The defect closed:** CP's `verify_rotation_6_steps` (`harness_cp.five_axis_composition`) walks generic IS `StateLedgerEntry` chain continuity via `harness_is.chain_verification.verify_chain`, which PASSES on a window where NO rotation boundary occurred at all — nothing in the IS chain carried the rotation event's identity, so the walk proved continuity, not rotation (registered `B-33`, Codex out-of-family rounds 1 + 5 on the `B-22` seam, `Spec_Control_Plane_v1_98.md` §1). **The carrier, precisely.** The fork's own framing names this "an open-dict additive IS carrier … mirrors OD §24.7." IS has no open-dict extensibility mechanism on `StateLedgerEntry` — the established IS carrier idiom for exactly this shape of additive, non-schema-breaking extension is the **typed D-derivative sidecar** (§5.1 `procedural_tier_snapshot_ref`; §5.4 `branch_metadata`), and §5.6 follows that idiom: an `Optional[str]` field, `None`-default, omitted from the §6.1 canonical payload when `None` (zero migration cost — every pre-v1.12 entry canonicalizes byte-identically). This is a **precise, not loose, reading of "mirrors OD §24.7"**: OD's `audit.rotation_correlation_id` (OD spec v1.31 §24.7) is *itself* hash-covered ("a value tampered on either sibling changes that sibling's `entry_hash`") — "additive" at both precedents means "absent-by-default, non-schema-breaking," never "unhashed." A carrier a caller could strip or swap without detection would defeat the verifier's entire purpose, so hash-coverage is a requirement here, not an optional strengthening — the typed sidecar delivers it by construction via the existing §5.1/§5.4 omit-when-`None` canonicalization path, with no new mechanism. **Producer-supplied, no §5.2 resolver analogue** (§5.6, mirroring §5.4's own framing) — the producer is the CP rotation orchestration (`execute_key_rotation`'s writes, `harness_cp.five_axis_composition`), not a `HarnessContext` resolver; population is impl-leg scope. **Shared value-space, not IS-local identity.** The IS-side `rotation_correlation_id` and the OD-side `audit.rotation_correlation_id` (OD spec v1.31 §24.7) are the SAME UUID value for one rotation event — IS state-ledger entries and OD audit-ledger entries are distinct ledgers, so a genuine rotation-boundary proof requires a caller-side JOIN across both, which is exactly why the fork's §2 point 4 routes the OD-join through a composition-root-injected evidence DTO rather than a caller-supplied string the verifier would have to trust unverified. **Scope discipline — CP-owned verifier logic stays OUT of the IS spec.** §5.6/§7.7 author ONLY what is IS-observable: the carrier declaration + population contract + hash-coverage (§5.6) and the read-side **presence** (every entry in a claimed rotation window carries the id) and **uniqueness** (one id per window) invariants a consumer walk can check against the IS chain alone (§7.7) — mirroring how §5.4 declared the `branch_metadata` carrier + its own invariants while leaving the CP `WorkflowDriver`'s write-cadence and `api.resume`'s consumption to the CP spec. The fork's §2 points 2-4 (the `verify_rotation_6_steps` REQUIRED-argument 3-step extension, the OD-join mechanism, the B-36 `AwsKmsSigningBackend` key-identity boundary attestation) are CP-owned and explicitly OUT of this delta — IS is the axis with zero outbound cross-axis edges (IS-AL invariant; `harness-is` never imports `harness-cp`), so the join/verification composition is authored at the CP spec + the impl leg, cross-referenced here, never restated. **ZERO change to the §5 six-field shape / §5.1 / §5.2 recipe / §5.3 / §5.4 / §5.5 / §6 hash-chain construction / §7.1–§7.6 / §10 seam exports.** IS plan absorption (NEW foundational unit, mirroring the U-IS-19 precedent) + clearance marker owed in the same PR per workspace `CLAUDE.md` §4.5. **SPEC-ONLY leg** — the carrier code (`StateLedgerEntry`/`EntryPayload` field + canonicalization contribution) and the CP-side `verify_rotation_6_steps` extension are the separate impl leg (mirrors the `B-59-A` spec-leg → impl-leg precedent, PRs #1080 → #1081), not landed at this delta. v1.11 + v1.10 + v1.9 + v1.8 + v1.7 + v1.6 + v1.5 + v1.4 + v1.3 + v1.2 + v1.1 + v1 PRESERVED VERBATIM per delta-only convention. 2026-07-22) |
...
**Closes the spec half of `B-57`** (`.harness/forward-register.yaml`, the *remedy owner* row; cause-and-evidence record at `B-112`, PR #1241) via the **RATIFIED Class 2 fork** `.harness/class_2_fork_b57_direct_append_writer_owned_opt_in.md` — **operator-ratified 2026-08-07, Reading A** (per-call-site opt-in), recorded at that filing's §11. **`B-57` itself does NOT close at this delta** — the call-site conversions, the two injection-caveat resolutions, the `as_is_wiring` event-time control and the two-process contention witness are the separate impl leg; the register row transits `registered_finding` → `open` in this same PR, reflecting that only the design-substrate precondition this delta supplies has landed (the `B-96`/`B-88` enum precedent — a ratified finding whose spec AND plan deltas are applied but whose implementation is still owed is `open`, not `closed` and no longer merely `registered_finding`).

**The defect the fork priced.** A DIRECT state-ledger append — the default `append_ledger_entry` contract — samples its `timestamp` in the CALLER, outside the write serialization point, and the writer then validates it against PHYSICAL APPEND ORDER at ZERO clock-skew tolerance (`_CLOCK_SKEW_TOLERANCE = timedelta(0)`). Two uncoordinated concurrent direct writers can therefore sample in one order and physically append in the opposite order, and the later arrival is **refused**, not reordered. Nothing is silently corrupted — the refusal is loud and mutual exclusion is unaffected — but the **availability of an honest write** is lost: a direct producer whose entry means *"when this was appended"* can be refused for a reason that has nothing to do with its own correctness. It merely lost a lock race after sampling. `B-112` records the mechanism that raised the probability (a polling wait does not preserve the kernel's `flock` handoff fairness; reproduced 12/20 by a two-process probe), and one production site — `audit_writer`'s bounded 5-attempt resample-retry loop — already works around the refusal by hand.

**The amendment — NEW §7.6.1 under C-IS-07 §7.6, ONE authorizing site.** A DIRECT-append call site MAY **elect** writer-owned sampling by supplying the EXISTING `WRITER_OWNED_TIMESTAMP` sentinel (§7.6, v1.11) as its write payload's `timestamp`; on an electing call the persisted `timestamp` is sampled by the writer at the entry's append, INSIDE the write serialization point — the same authority, the same critical section, and the same by-construction monotonicity §7.6 already commits for the buffered/branch-drain surface. The election is **PER CALL SITE**: never a default, never a mode on the writer. A direct producer that supplies any other timestamp value retains caller-supplied semantics **BYTE-VERBATIM**, so §7.6's direct-path contract is unchanged for every non-electing producer, and no producer can be affected by another's election. §7.6.1 further pins the **eligibility rule** (a site MAY elect only where its entry's `timestamp` means *when the entry was appended*; where it means *when the event happened*, caller-supplied semantics are REQUIRED and an out-of-order refusal is the honest outcome), the **checkable default-preservation property** (a conforming implementation must be able to demonstrate NEGATIVELY that a non-electing direct append persists the caller's value and still refuses an inversion — if it cannot, the election has silently become a default), and an explicit **does-NOT-authorize** list.

**Why the authorization, and not a mechanism, is the whole delta.** The sentinel check at the writer is **UNCONDITIONAL at HEAD** — re-verified by direct read at `acfc1afa`, `harness-is/src/harness_is/state_ledger_write.py:316`–`:320`: the substitution fires for ANY caller that stamps the sentinel, inside the same `cross_process_write_lock(...), _WRITE_LOCK` composition (`:306`) that reads the prior entry and computes `prior_event_hash`. There is no surface discriminator, no mode flag and no path gate. A direct producer that stamps the sentinel today therefore *already* gets writer-owned sampling; the direct-path restriction is held by **PROSE ALONE** (the sentinel docstring `:78`–`:80` and §7.6's own `"Surfaces that do NOT change"` paragraph), while a test at `harness-is/tests/test_shadow_git_rollback.py` already opts in on a direct surface (adjudicated PERMITTED at the `B-93` leg). §7.6.1 therefore does not create a bypass — it **names and bounds** one that structurally exists, and turns a prose-only restriction into a contract with a stated eligibility rule an auditor can check a call site against.

**Preservation discipline — the §7.6 residual is DISCHARGED, not retracted.** §7.6's `"Surfaces that do NOT change"` paragraph and its `Registered residual (surfaced, NOT absorbed)` paragraph — whose own words are *"Extending writer-owned authority to direct surfaces would change caller-supplied semantics for every existing direct producer and requires its own back-flow"* — are **PRESERVED VERBATIM**. They were accurate when authored and remain accurate for every producer that does not elect; the ratified fork **is** the back-flow the residual demanded. ONE new status paragraph is added immediately after the residual recording that the demanded back-flow has been performed and discharged in this narrow per-call-site form, so a later reader cannot mistake the preserved text for a live prohibition — the same shape §7.5 used when it lifted the §7.4 keying-tuple deferral while preserving the §7.4 text as historical record (`[[stale-carry-text-disposition]]`). **The discriminator the fork named, and how it resolves:** the residual's own sentence is scoped to *"every existing direct producer"* — it guards the **DEFAULT**, not a whole-surface guarantee a reader may rely on blind — and a per-call-site election that leaves every non-electing producer byte-identical therefore does not touch what it protects. The `B-93` leg already adjudicated this exact question in the same direction for the test opt-in (*"a single explicit opt-in changes no producer's semantics and sets no default"*); v1.13 generalizes an adjudication the workspace has already made once under review, on this same contract.

**One cross-reference-only second touch, authored deliberately.** The §7.1 row-7 `"Timestamp authority (v1.11)"` cell gains a clause naming §7.6.1. It carries **ZERO new semantics** — every word of authorization lives at §7.6.1 — and exists solely so the §7.1 summary row does not become stale-as-described (a summary row that enumerates the writer-owned surfaces while omitting the one v1.13 adds is precisely the workspace's most-recurrent defect class). Reading A's *"ONE site, one added subsection"* is preserved in substance: there is exactly one authorizing site.

**Costs, determined rather than assumed.** **ZERO contract numbers** — the EXISTING C-IS-07 §7.6 is extended (and per `[[spec-leg-cannot-mint-contract-number]]` a spec leg cannot mint one). **ZERO hash / canonicalization / migration impact** — no field added, no shape changed, no recipe touched; the `timestamp` field was already hash-covered and its type, meaning and hash participation are untouched, so there is no `snapshot_hash` delta and no historical-entry migration. **ZERO CXA rows** — the sentinel is already exported from `harness-is` (`state_ledger_write.WRITER_OWNED_TIMESTAMP`) and every candidate call site already imports the C-IS-07 §7.1 write contract it uses, so no new package edge and no new typed seam arises; nothing is owed at `Cross_Axis_Composition_Document_v2_23.md` §2.3 (the same disposition `B-97`(a) and `B-88` reached, for the same reason). **ZERO new plan units** — U-IS-11 already owns the §7.1/§7.6 write surface and is already landed; the plan delta is an acceptance-criterion widening (the `B-100`/`B-88` shape).

**Council — NOT owed; probe-resolved, and re-determined at this leg rather than inherited.** `B-112`'s council field makes convening conditional on reaching *step (3)* — widening `_CLOCK_SKEW_TOLERANCE`, which would trade information-substrate integrity (C3) against operator-loop robustness (C11) across every ledger writer at once — and step (3) is reached only if step (2) leaves an **exposed** caller that cannot adopt writer-owned sampling. The classification re-grounded at this leg (carried at the plan) leaves exactly ONE site retaining caller-supplied semantics — `as_is_wiring.py:129`, `timestamp=composed.timestamp`, a pass-through of an upstream composed record and therefore genuine **event time** — and that site is a caller for which an out-of-order refusal is the honest outcome by design, not a caller stranded by an inability to adopt. No exposed caller is left stranded, so step (3) is never reached and the C3 ⊥ C11 tension does not arise (root `CLAUDE.md` §10.9 posture amendment 5 — probe-first; amendment 1's nameable-tension discriminator fails). **The re-determination carries a live trigger, not a closed one:** the two injection-caveat sites are resolved at the impl leg, and **if EITHER is judged RETAIN, this determination must be re-run before any tolerance change is authored** — a RETAIN there would be an exposed caller of exactly the shape step (3) tests for. That trigger is recorded at the plan's U-IS-11 amendment, not left to recall.

**Grounding pass — every cite in this delta re-resolved by direct read at HEAD `acfc1afa`, and the filing's own counts CORRECTED rather than carried.** All fourteen classification rows in the fork's §3(ii) table were re-read at their cited lines and are byte-identical to the filing; `state_ledger_write.py:69`/`:83`/`:306`/`:316`–`:320`/`:321`–`:324` and the sentinel docstring `:71`–`:82` verified exact; `audit_writer.py:727`–`:752`'s resample-retry loop read in full; and `emit_sibling_ledger_entry`'s zero-non-test-caller sweep re-run independently (19 hits, **exactly 3 non-test**: the definition at `harness-runtime/src/harness_runtime/lifecycle/cp_is_wiring.py:124` plus two docstring mentions), confirming `sibling_ledger_entry_composition.py:163` is production-UNREACHABLE. **TWO count claims in the filing do NOT reconcile with its own table and are corrected at the plan** (`[[count-claims-drift-every-review-round]]`): the filing's *"fourteen rows, twelve SITES"* is not derivable from a table that enumerates fourteen numbered rows in which the two folded companions (`audit_writer.py:751`, `per_step_override_evaluator.py:523`) are ALREADY folded into rows 12 and 9 — the twelve appears to be a residue of an earlier draft; and the ELECT disposition is *"≈8 conversions"* in the filing's prose while its own ELECT cell enumerates **TEN** payload-construction rows. Neither correction changes any disposition, any reading, or any cost — it changes the SIZE of the surface the impl leg must convert, which is exactly what a re-grounding pass owes. The corrected roster (14 rows = 10 ELECT + 2 ELECT-with-injection-caveat + 1 RETAIN + 1 DEFER) is authored at the plan.

**Scope discipline.** v1.13 amends ONLY C-IS-07: one NEW §7.6.1, one added residual-status paragraph inside §7.6, and one cross-reference-only clause at the §7.1 row-7 cell. §1 through §4, §5 and §5.1–§5.6, §6, §7.2–§7.5, §7.7, §8, §9, §10, and the `[carry-forwards]` / `[traceability]` / `[coherence pass]` sections are **PRESERVED VERBATIM** — as are §7.6's own `"Amendment authority"`, `"The surface that changes"`, `"The contract"`, `"Surfaces that do NOT change"`, `"Registered residual"`, `"Witness obligation"` and `"Deferred to implementation discretion"` paragraphs. **SPEC-ONLY leg** — no code lands at this delta. v1.12 + earlier lineage PRESERVED VERBATIM per the delta-only-spec-file convention except the additive amendments above.

---

## Change-note (v1.11 → v1.12)

**Closes the IS-side half of `B-33`** (registered at `.harness/post-phase-8-forward-register.md`; Codex out-of-family rounds 1 + 5 on the `B-22` seam, `Spec_Control_Plane_v1_98.md` §1) via the RATIFIED Class 1 fork `.harness/class_1_fork_b33_rotation_correlation_carrier.md` §4 Option A. **`B-33` itself remains OPEN** — the CP-owned residual (the `verify_rotation_6_steps` 3-step extension, the composition-root-injected OD-join, the B-36 boundary attestation) is the separate, not-yet-opened impl leg; `.harness/forward-register.yaml`'s `B-33` row is updated in this same PR to `status: open` (not `closed`), reflecting that only the design-substrate precondition this delta supplies has landed. `verify_rotation_6_steps` (`harness_cp.five_axis_composition`) verifies a key-rotation window with a generic hash-chain continuity check that PASSES on a ledger where no rotation boundary occurred at all — the IS state-ledger rows the walk traverses carry no identity for the rotation event itself, so the walk proves chain continuity, not rotation. OD already closed the symmetric gap on its own ledger at OD spec v1.31 §24.7 (`audit.rotation_correlation_id`, joining the two-row dual-signature rotation pair); this delta authors the IS-side counterpart the walk needs to JOIN against.

**Why a typed sidecar, not a literal open dict, is the faithful reading of ratified Option A.** The fork's §2/§4 text describes the ratified shape as "an open-dict additive IS carrier." Grounding against the actual `StateLedgerEntry` / `EntryPayload` schema (`harness_is.state_ledger_entry_schema`, `harness_is.state_ledger_write`) shows IS has no open-dict/namespace-attrs field to extend — the established IS convention for an additive, non-schema-breaking, D-derivative sidecar is a directly-typed `Optional` field (§5.1, §5.4), always hash-covered via the §6.1 canonical payload's omit-when-`None` inclusion rule. This is not a deviation from Option A's intent: OD §24.7 (the precedent Option A itself cites) is likewise hash-covered, so "additive" at both precedents already meant "absent-by-default, non-schema-breaking" — never "unhashed." A rotation-correlation id that were NOT hash-covered would be trivially strippable or swappable post-write, defeating the verifier's tamper-evidence purpose outright. §5.6 therefore follows the IS axis's own established additive-carrier idiom, which delivers the required hash-coverage by construction with zero new mechanism.

**Carrier scope — presence + uniqueness only, at the IS spec.** §5.6 declares the field + its population contract (producer-supplied, no resolver) + hash-coverage. §7.7 adds the two read-side invariants that are checkable against the IS chain alone: (a) presence — every entry in a claimed rotation window carries a non-`None` `rotation_correlation_id`; (b) uniqueness — a window MAY carry at most one distinct id (a torn/mixed window carrying more than one is malformed). The fork's remaining points — the `verify_rotation_6_steps` REQUIRED-argument extension, the OD-side join (a composition-root-injected typed evidence DTO, never a caller-supplied string trusted on its own), and the boundary attestation against the B-36 `AwsKmsSigningBackend` key-identity mapping — are CP-owned per the fork's own §2 point 4 reasoning ("`harness-cp` cannot import the OD ledger type without reversing the axis-import direction") and are deliberately OUT of this IS delta; IS remains the consumer-most-upstream axis with zero outbound cross-axis edges (the same invariant §5.1/§5.4 already establish and preserve here).

**No operator gate on this delta.** The fork's ONE operator decision (Option A vs B vs hold) is ALREADY RATIFIED at the fork's §4; this delta is the spec-leg mechanical apply of that decision, following the `B-59-A`/`B-65-A` spec-leg-then-impl-leg precedent (start fresh, land the spec + plan + clearance, then a separate impl-leg PR lands the code + tests).

**Scope discipline.** v1.12 amends ONLY C-IS-05 (adds §5.6) and C-IS-07 (adds §7.7). §1 through §4, §5.1–§5.5, §6, §7.1–§7.6, §8, §9, §10, and the `[carry-forwards]` / `[traceability]` / `[coherence pass]` sections are PRESERVED VERBATIM. v1.11 + earlier lineage PRESERVED VERBATIM per the delta-only-spec-file convention except the §5.6 / §7.7 additive amendments above.
...
### §5.6 D-derivative sidecar field — `rotation_correlation_id` (v1.12 — `B-33-A` IS rotation-correlation carrier)

**Contract surface.** Optional additional field extending the §5 F-layer six-field shape (alongside the §5.1 sidecar `procedural_tier_snapshot_ref` and the §5.4 sidecar `branch_metadata`), carrying the rotation-event correlation identity a CP-side audit-walk verifier REQUIRES and JOINS to prove a genuine key-rotation boundary occurred at C-CP-20's two-row rotation-pair protocol.

**The defect this carrier's IS-side half closes.** `verify_rotation_6_steps` (CP spec `Spec_Control_Plane_v1_2.md` §20.3.1, implemented at `harness_cp.five_axis_composition`) verifies a claimed rotation window against the IS state-ledger via `harness_is.chain_verification.verify_chain` — a **generic** hash-chain `prior_event_hash` continuity check. That check PASSES on any intact chain, including one where NO rotation boundary occurred at all: nothing in the IS entries traversed carried the rotation event's own identity, so continuity was being mistaken for proof of rotation. OD's audit ledger already carries the symmetric join key at OD spec v1.31 §24.7 (`audit.rotation_correlation_id`, joining the two-row dual-signature rotation-pair siblings) — but the IS-side entries the walk actually traverses had no counterpart field to join against. §5.6 authors that counterpart; `verify_rotation_6_steps` itself does not consume it until the separate CP-owned impl leg lands (`B-33` stays OPEN until then — see the v1.12 change-note).

**Field specification.**

| Field | Type / format | Semantic | Constraint |
|---|---|---|---|
| `rotation_correlation_id` | `str` (canonical-form UUID) OR `None` | Identifies the key-rotation event whose window this entry belongs to | `None` at every entry outside a rotation window — the overwhelming majority of entries; **every pre-v1.12 entry**. Non-`None` only on entries the CP rotation orchestration (`execute_key_rotation`) writes within a claimed rotation window. When non-`None`, the value MUST be a canonical-form UUID (36-char hyphenated, RFC 4122 string form) — a present-but-malformed value (not parseable as a canonical UUID) is REJECTED at construction, mirroring OD spec v1.31 §24.7's own "UUID string, or key absent" admission rule (a non-UUID non-empty value is neither, and OD's external-auditor verification treats it as a verification failure at its own partition step). When non-`None`, the value is the **same UUID** as the OD-side rotation pair's `audit.rotation_correlation_id` (OD spec v1.31 §24.7) for the same rotation event — the shared value is what makes the cross-ledger join possible (the IS state ledger and the OD audit ledger are distinct ledgers; see §7.7). |

**Producer-supplied, not resolver-derived — and the producer's write path does not exist yet.** Mirroring §5.4's own framing: unlike §5.1's `procedural_tier_snapshot_ref` (derived from `HarnessContext` by the §5.2 resolver), `rotation_correlation_id` is intended to be composed by the **CP rotation orchestration** (`execute_key_rotation`, `harness_cp.five_axis_composition`) at rotation-window entries — there is therefore **no resolver contract** for this field (no §5.2 analogue). Unlike §5.4's producer (the CP `WorkflowDriver`, which already has real `append_ledger_entry` call sites at branch-spawn/termination that a write-cadence decision merely schedules), `execute_key_rotation` at HEAD has **no ledger-write call site to extend**: per its own docstring, five of its six narrated steps (`STAGE_NEW_KEY`, `WRITE_DUAL_VERIFY_ENTRY`, `PROBE_VERIFY_AT_READ`, `ROTATE_SIGNING_TO_NEW`, `RETIRE_OLD_KEY`) are explicitly simulated narration with no real signing backend or ledger write wired — the function reads/verifies caller-supplied entries; it does not append to the ledger today. Populating this carrier therefore requires the CP-owned impl leg to **build genuine rotation-event ledger-write capability**, not merely extend an existing write call — a materially larger scope than a simple write-cadence decision, explicitly out of scope for this IS-only delta and deferred whole to the separate `B-33-A` impl arc (which must design that write path, including how it receives the shared OD-side correlation id, before this carrier can be populated for real).

**Authorization basis.** ADR-F2 §Consequences (c) "per-workload-class extensions to the entry shape are D-derivative downstream" + §5 "Field-shape extensibility commitment" declaring the six-field shape as F-layer minimum + D-derivative authorization — the same authorization §5.1 and §5.4 invoke. `rotation_correlation_id` is the documented D-derivative extension shape for rotation-boundary correlation; no §5 main-contract amendment owed, ZERO change to the six-field shape.

**Additive carrier, precisely reconciled against the ratified "open-dict" framing — including the fork's own Option-B hash-coverage parenthetical.** The RATIFIED fork (`.harness/class_1_fork_b33_rotation_correlation_carrier.md` §2 point 1 + §4) describes Option A as "an open-dict sidecar field … mirroring the OD §24.7-style additive convention" and describes Option B as "first-class C-IS-05 field variant (heavier; only if the operator wants the correlation **hash-covered by the chain itself** rather than carried additively)." Read at face value, that Option-B parenthetical implies Option A's "additive" carrier would be WEAKER — not hash-covered. This delta does not adopt that implication, and states explicitly why: it is factually incorrect at the IS axis, independent of which of A/B is chosen.

`StateLedgerEntry` / `EntryPayload` carry no open-dict/namespace-attrs mechanism (unlike OD's `AuditPayload.audit_namespace_attrs`, the literal precedent Option A names); the IS axis's own established idiom for an additive, non-schema-breaking extension is the **typed D-derivative sidecar** (§5.1 `procedural_tier_snapshot_ref`, §5.4 `branch_metadata`) — and BOTH of those existing sidecars are, and always have been, hash-covered via the §6.1 canonical payload (§5.1's own text: "tamper-evident per §6.5"; §5.4's own text: "tamper-evident per §6.5, exactly as `procedural_tier_snapshot_ref`"). There is no IS-axis carrier mechanism, additive or first-class, that is NOT hash-covered — the fork's implied A-vs-B hash-coverage axis simply does not correspond to any real distinction in this codebase's actual sidecar convention. (It DOES correspond to a real distinction for OD's literal open-dict mechanism only in the sense that a hypothetical unhashed OTHER dict would be weaker — but OD's actual `audit_namespace_attrs`, the mechanism Option A names as its mirror, is ALSO hash-covered, per OD spec v1.31 §24.7's own "Hash coverage (tamper-evidence)" section.) Given that, the parenthetical's hash-coverage caveat is satisfied by EITHER option at the IS axis, and cannot be the basis for choosing between them — the dimension Option A vs Option B actually discriminates on (per fork §2 point 1's own framing: "an open-dict sidecar field … vs a first-class C-IS-05 field … heavier; needs canonicalize-contribution rules") is carrier WEIGHT: an additive, optional, `None`-default field requiring no canonicalize-contribution-rule redesign (Option A) versus a heavier, more central schema commitment (Option B, "needs canonicalize-contribution rules" — i.e. NOT simply reusing the existing omit-when-`None` convention). §5.6's typed sidecar is unambiguously the LIGHT reading on that actual dimension — reusing the established §5.1/§5.4 omit-when-`None` canonicalization verbatim, adding zero new canonicalization rule — which is the Option-A-as-ratified shape. Hash-coverage, being present either way, is not evidence for either reading; it is a correctly-satisfied prerequisite this section states so a future reader does not independently re-raise it as a gap.

**Hash-chain composition (canonicalization-contribution discipline; §6 construction unchanged).** `rotation_correlation_id` is entry **payload content**: when non-`None` it participates in the §6.1 canonicalization and therefore the §6.2 `response_hash` — tamper-evident per §6.5, exactly as `procedural_tier_snapshot_ref` (§5.1) and `branch_metadata` (§5.4) are, and subject to the SAME chain-head limitation every C-IS-05 sidecar shares (§6.4's successor-link mechanism has no downstream entry to detect tampering of the current chain head against — a pre-existing property of the unchanged §6 design, not introduced here; see §7.7's join-key note for the rotation-specific consequence and its OD-signature-based remedy, CP-owned). The §6 construction discipline is **unchanged**: the canonical payload includes the field when non-`None` and **omits it when `None`** (the established §5.1/§5.4 pattern). Consequence: every pre-v1.12 entry (all of which predate this carrier, so carry `rotation_correlation_id = None`) canonicalizes **byte-identically** to a v1.12 entry with `rotation_correlation_id = None` — ZERO breaking change at the hash level for the existing chain; forward-only, no historical-entry migration.

**Composition with cross-axis seams.** `rotation_correlation_id` composes orthogonally with the existing `idempotency_key` join surface (§C-IS-10 §10.2) and travels on the already-exported state-ledger entry shape (§C-IS-10 §10.1) — it is not itself a new export seam. The CP rotation orchestration is the sole producer; the CP audit-walk verifier (`verify_rotation_6_steps`'s `B-33`-scoped extension) is the sole consumer, per the fork's own §2 point 2 ("gains a REQUIRED correlation argument").

**Scope discipline — the CP-owned verifier contract stays out of this spec, and out of this delta's own impl leg.** The fork's §2 points 2–4 (the `verify_rotation_6_steps` 3-step extension requiring and joining the id; the boundary-attestation check against the B-36 `AwsKmsSigningBackend` key-identity mapping; the OD-join mechanism via a composition-root-injected typed evidence DTO) are **CP-owned** — `harness-is` has zero outbound cross-axis edges (the same invariant §5.1's resolver-residence deferral and §5.4's "hard constraint: NOT `harness-cp`" already preserve), so the verifier logic that CONSUMES this carrier is authored at the CP spec + a SEPARATE, later CP-axis arc, cross-referenced at §7.7, never restated here. **This means "the `B-33-A` impl leg" is two distinct arcs, not one**: (i) the U-IS-20 code arc (`harness-is/src` only — the carrier + validation + canonicalization + the §7.7 helper functions; IS's 0-outbound invariant means this arc never touches `harness-cp`), and (ii) a later, separately-spec'd CP-axis arc (its own CP spec delta + plan unit(s), not yet decomposed here) that extends `verify_rotation_6_steps` to consume (i)'s already-landed carrier. See the IS plan's own U-IS-20 AC #8 for the same split stated as a scope fence.

**Canonical-form means EXACT 36-char hyphenated string equality, not mere UUID-parseability.** A naive `uuid.UUID(value)` parse-then-accept is INSUFFICIENT to enforce the field table's constraint: Python's `uuid.UUID` also parses 32-char unhyphenated hex, brace-wrapped (`{...}`), and `urn:uuid:`-prefixed forms, none of which is the canonical 36-char hyphenated form the field table requires — and each of those non-canonical spellings is a DISTINCT raw string that would collide-by-parse-equivalence but NOT by the join-key string-equality both the presence/uniqueness invariants (§7.7) and the OD-join (CP-owned) actually perform. The REQUIRED check is `str(uuid.UUID(value)) == value` (or an equivalent canonical-round-trip check) — parse it, re-render it, and reject unless the re-rendered form matches the input byte-for-byte — not bare parseability.

**Deferred to implementation discretion.** The carrier-home of the field is not a separate question at IS (it lives directly on `StateLedgerEntry`/`EntryPayload`, exactly like §5.1/§5.4 — there is no auxiliary record type to home elsewhere, unlike `BranchMetadata`); the CONCRETE validation mechanism enforcing the canonical-round-trip constraint above (a `uuid.UUID`-then-re-render check inline vs a validated `Identifier`-style newtype vs a Pydantic field validator — the REQUIREMENT is pinned above, only the implementation mechanism is deferred); whether the CP producer writes the id on every entry within a window or only at the window's boundary entries (a CP-side write-cadence decision, mirroring the §5.4 "which entry carries `terminal_status`" deferral — §7.7's presence invariant is scoped to what the CP verifier's OWN definition of "window" requires, not pinned here).
...
**Residual status at v1.13 (`B-57`) — DISCHARGED, in the narrow per-call-site form only.** The back-flow the registered residual above demands (*"requires its own back-flow; it is registered here as a surfaced finding, not silently absorbed"*) **HAS NOW BEEN PERFORMED**: Class 2 fork `.harness/class_2_fork_b57_direct_append_writer_owned_opt_in.md`, **operator-ratified 2026-08-07 as Reading A**, and applied at **§7.6.1** below. Both the `"Surfaces that do NOT change"` paragraph and the residual paragraph above are **PRESERVED VERBATIM as historical record** — they were accurate when authored and remain accurate for **every direct producer that does not elect**, which after v1.13 is still the default and still the overwhelming majority. They are NOT, at v1.13, a live prohibition on a call site that elects per §7.6.1: what the residual's own sentence protects is the caller-supplied **DEFAULT** for *"every existing direct producer"*, and a per-call-site election leaves every non-electing producer byte-identical. The scope-widening carve-out named in the residual (the OFFLOADED sub-agent audit appends, B-48 rounds 28/29) is unaffected and remains B-48-owned. This preservation-plus-status-paragraph shape mirrors §7.5's treatment of the §7.4 keying-tuple deferral it lifted.
...
#### §7.6.1 Per-call-site writer-owned election on DIRECT append surfaces (v1.13 — `B-57`, ratified Reading A)

**Amendment authority.** Class 2 fork `.harness/class_2_fork_b57_direct_append_writer_owned_opt_in.md` (FILED 2026-08-07, PR #1253; **operator-ratified 2026-08-07 — Reading A**, recorded at that filing's §11) — the back-flow §7.6's registered residual demanded, now performed. `B-57` is the remedy-owner register row; `B-112` is its cause-and-evidence record. This subsection is the ONE authorizing site of that ratification; §7.6's preceding paragraphs are preserved verbatim per the residual-status paragraph above.

**The contract.** A **DIRECT**-append call site MAY **ELECT** writer-owned sampling by supplying `WRITER_OWNED_TIMESTAMP` — the EXISTING §7.6 sentinel, unchanged — as its write payload's `timestamp`. On an electing call the persisted `timestamp` is **writer-owned**: sampled at that entry's append, INSIDE the write serialization point (the same critical section that reads the prior entry, computes `prior_event_hash`, and appends), so on that call sampling order equals physical-append order and the C-IS-05 §5 monotonic-non-decreasing constraint holds **by construction** — the identical authority, critical section and consequence §7.6 already commits for the buffered/branch-drain surface. Any other value the electing call site might have sampled is not the persisted value. Nothing else about the direct-append contract changes.

**A limit stated honestly — the DIRECT-surface population is MIXED by design, so election is not a refusal immunity.** On the buffered/branch-drain surface every append is writer-owned, so §7.6's *"concurrent sibling drains cannot invert"* is TOTAL for that population. The direct surface is different **by construction**: electing and non-electing producers coexist on one ledger, which is exactly what preserves the default for everyone who does not elect. An electing entry's own sample is therefore taken in append order, but it is still validated against the prior entry — so an electing append CAN still be refused if a preceding NON-electing producer persisted a future-skewed caller-supplied timestamp. What election removes is the electing site's exposure to **its own** sample-then-lose-the-race inversion, which is the whole of `B-57`'s named failure; it does not remove exposure to another producer's clock. This is a consequence of the mixed population the per-call-site form deliberately keeps, not a defect in it, and it is recorded here so no consumer reads §7.6.1 as a guarantee of never being refused.

**The election is PER CALL SITE — never a default, never a mode on the writer.** A direct producer that supplies **any other** timestamp value retains **caller-supplied semantics BYTE-VERBATIM**, so §7.6's direct-path contract is unchanged for it. The writer acquires **no surface discriminator, no mode flag and no path gate**: election is expressed *only* by the value a call site supplies on its own payload, so one producer's election cannot alter another producer's semantics, and there is no configuration or global state by which an election could become ambient. A per-writer, per-producer or per-path "writer-owned mode" is NOT authorized by this subsection and would be a distinct contract change requiring its own back-flow.

**Eligibility rule — the contract obligation that bounds the election.** A call site MAY elect **only** where its entry's `timestamp` means **when the entry was appended**. Where it means **when the event happened** — a value carried from an upstream composed record, or any instant a later reader would interpret as event time rather than append time — caller-supplied semantics are **REQUIRED**, and an out-of-order refusal on that path is the **honest outcome**, not a defect to be engineered away. This rule, not the mechanism, is what distinguishes an authorized election from an unauthorized one: the writer's sentinel substitution has always been unconditional, so the code cannot make the distinction and the contract must. Electing for convenience, to silence a refusal on an event-time path, is a contract violation regardless of whether the code accepts it.

**Default preservation, stated as a property an implementation MUST be able to demonstrate.** For every non-electing direct producer the `"Surfaces that do NOT change"` paragraph above holds byte-verbatim: caller-supplied `timestamp`, validated monotonic-non-decreasing per C-IS-05 §5, detect-then-refuse via `NonMonotonicTimestampError` beyond clock-skew tolerance. A conforming implementation MUST be able to show this **negatively** — that a non-electing direct append persists the caller's own value, and that an inversion on a non-electing direct path is still refused. If that property cannot be demonstrated, the election has silently become a default and this amendment is violated. The demonstration obligation is carried as an acceptance criterion at the plan (U-IS-11), not left implicit here.

**Reconciliation with §7.6's deferred-to-implementation-discretion paragraph.** That paragraph constrains the drain path's API carrier *"to preserve the direct-path caller-supplied contract byte-verbatim."* The constraint is **UNCHANGED and still binding**, and v1.13 satisfies it: the direct-path caller-supplied contract is preserved byte-verbatim for every producer that does not elect, and the election introduces **no new carrier at all** — it reuses the same sentinel §7.6 already deferred to, so no new mechanism appears on either surface.

**What this subsection does NOT authorize.**

- It does **not** make writer-owned sampling a default on any surface, nor extend it to any producer that does not elect on its own payload.
- It does **not** change the C-IS-05 §5 six-field shape, the §5.1–§5.6 sidecars, the §6 hash-chain construction or canonicalization, or any read contract. The persisted `timestamp` field's type, meaning and hash participation are untouched; only **which instant a consenting call site records** changes. There is no migration surface and no `snapshot_hash` delta.
- It does **not** widen `_CLOCK_SKEW_TOLERANCE`, add a configuration surface for it, or otherwise alter the C-IS-05 §5 clock-skew-tolerance framing. That remains out of scope and, if ever reached, is its own back-flow.
- It does **not** authorize an election on an event-time path, and it does **not** convert the eligibility rule into guidance: the rule is a contract obligation.

**Per-site classification is carried at the PLAN, not here.** Which existing direct-append call sites ELECT, which RETAIN caller-supplied semantics, and which are DEFERRED is a code-inventory fact that drifts with the tree; pinning that roster in a cleared contract would guarantee stale-carry. The auditable roster lives at `Implementation_Plan_Information_Substrate_v2_9.md` (the U-IS-11 amendment) and is re-grounded at the impl arc — this contract states the **RULE**, the plan states the **ROSTER**, and a conforming impl arc converts exactly the sites its table classifies ELECT. A site whose classification is DEFERRED because it has no production caller acquires no election by default; it is re-classified when a real producer appears.

### §7.7 Rotation-correlation carrier — read-side invariants (v1.12 — `B-33-A`)

**Contract surface.** Two IS-observable read-side invariants over the §5.6 `rotation_correlation_id` sidecar, checkable against the IS state-ledger chain alone — the presence and uniqueness half of what a rotation-boundary verifier must confirm before it can even attempt the (CP-owned) OD-join. This section does NOT define `verify_rotation_6_steps` itself; it defines the contract the IS chain guarantees a compliant consumer of §5.6 can rely on.

**A "claimed rotation window" is a DERIVED sub-sequence — not the same input `verify_chain` requires, and its boundaries MUST be derived INDEPENDENTLY of `rotation_correlation_id` itself.** `verify_rotation_6_steps`'s existing `VERIFY_HASH_CHAIN_LINK` step (§20.3.1 step 3) calls `harness_is.chain_verification.verify_chain` on the FULL genesis-anchored chain per that function's own pre-existing contract (position 1's `prior_event_hash` MUST equal the all-zeros sentinel; an arbitrary filtered slice fails with `INCEPTION_SENTINEL_MISMATCH` — unchanged by this delta). Any real ledger containing rotation history also contains non-rotation entries (`rotation_correlation_id = None` at every one of them, per §5.6's field table) interleaved before, after, and possibly around a rotation event. The (a)/(b)/(c) invariants below therefore do NOT apply to that full genesis-anchored sequence — applying (a) presence to the whole chain would fail on every ordinary non-rotation entry. "A claimed rotation window" is instead a caller-DERIVED sub-sequence: the subset of entries a consumer has identified as belonging to one rotation event.

**The boundary-derivation method is load-bearing, and MUST NOT use `rotation_correlation_id` as its selection criterion.** Deriving the window by filtering the full chain FOR non-`None` `rotation_correlation_id` (or for a specific claimed id) would make (a)/(b)/(c) SELF-VALIDATING, not fail-closed: filtering for non-`None` first, then checking "every entry in the filtered set is non-`None`" (a), is tautological — it can never fail, because non-`None`-ness was the selection criterion, not an independently-observed property. Symmetrically, filtering for entries matching one CLAIMED id and then checking uniqueness (b) over that pre-filtered set would silently DISCARD a conflicting second id rather than detect a torn window — exactly the tamper this invariant exists to catch. The window boundaries MUST therefore be derived from evidence INDEPENDENT of the field these invariants validate — per the join-key note below, that evidence MUST also be OD-anchored (a caller-asserted, unauthenticated positional `[start, end)` range is independent of `rotation_correlation_id` but is NOT by itself sufficient — see below). The concrete form: key-period evidence from the OD-side dual-signature pair (which entries' `action_id`/`timestamp` the OD pair's own metadata names — a SPARSE SET of specific entry references, not necessarily a contiguous positional range, since a concurrent writer's unrelated entry may interleave between the rotation-pair siblings; `append_ledger_entry`'s write lock serializes one append at a time, never a multi-entry atomic rotation-window write, so positional contiguity is not a property a genuine window can rely on) is the load-bearing mechanism; a dedicated boundary marker distinct from the correlation id itself (if the CP-owned impl leg adds one) is admissible only if it is ITSELF hash-covered / OD-anchored, for the same reason a bare positional range is not. ONLY AFTER the window is identified by such evidence are (a)/(b)/(c) evaluated over every entry the window includes. This precise derivation mechanism is CP-owned (left to implementation discretion below, exactly as the write-cadence already is) — §7.7 pins the REQUIREMENT that it be independent of `rotation_correlation_id`, not the mechanism itself. The full-chain continuity check (unchanged, §20.3.1 step 3) and the §7.7 window invariants remain **two DISTINCT checks over two DISTINCT inputs**, composed together at the CP-owned `verify_rotation_6_steps` extension — never substituted for each other.

**(a) Presence.** Within a claimed rotation window — a SET of entries a caller asserts belongs to one rotation event, **not necessarily a contiguous positional range** (see the boundary-derivation note below: an OD-anchored window is the sparse set of entries the OD dual-signature pair's own references name, which need not be positionally adjacent if an unrelated concurrent writer's entry interleaves between the rotation-pair siblings — `append_ledger_entry`'s write lock serializes one append at a time, not a multi-entry atomic rotation-window write, so positional contiguity is NOT a property a genuine rotation window can rely on) — every entry the CP rotation orchestration writes as part of that window carries a non-`None` `rotation_correlation_id`. An entry inside the claimed window carrying `None` is a **malformed window** — the caller's claim that a rotation occurred at that boundary is unsubstantiated by the IS chain, and any consumer walk built on §5.6 (the CP-owned `verify_rotation_6_steps` extension per the fork's §2 point 2) MUST treat this as a failed presence check, never as a silent pass. (A genuinely unrelated, non-rotation entry interleaved BETWEEN the rotation-pair siblings by a concurrent writer is not itself a presence-check failure — it is simply not a MEMBER of the OD-anchored sparse-set window in the first place, precisely because that window is defined by OD-pair reference membership, not by position.)

**(b) Uniqueness.** A single rotation window carries **at most one** distinct `rotation_correlation_id` value across its non-`None` entries. Two or more distinct non-`None` values within one claimed window is a **torn/mixed window** — evidence of either a caller-side windowing error or tampering — and MUST fail a compliant consumer's uniqueness check, never resolve silently to one of the values.

**(c) Non-emptiness (the vacuous-pass guard).** A claimed rotation window MUST be a non-empty sequence of entries. Presence (a) and uniqueness (b) are both universally-quantified checks over the window's entries ("every entry…", "at most one distinct value across…") and therefore PASS VACUOUSLY on an empty sequence — zero entries trivially satisfies both "every entry is non-`None`" and "at most one distinct value present." An empty claimed window MUST fail a compliant consumer's check at this invariant BEFORE (a)/(b) are even evaluated, mirroring the existing `verify_rotation_6_steps` docstring's own established posture on the pre-v1.12 generic-continuity check: "zero entries is an absence of evidence, not proof of chain integrity" (`harness_cp.five_axis_composition`) — the same reasoning applies identically to an empty window's rotation-correlation evidence.

**`rotation_correlation_id` is a JOIN KEY, not a trust anchor — (a)/(b)/(c) are COMPUTABLE against the IS chain alone, but not MEANINGFUL alone.** None of (a)/(b)/(c) requires the OD ledger, the B-36 signing backend, or any cross-axis composition to EVALUATE — a consumer can compute all three by reading `StateLedgerEntry.rotation_correlation_id` across a given window's entries. But an IS-only pass proves nothing about whether that window itself is the real one: an untrusted, caller-asserted window (e.g., an unauthenticated positional range) can be chosen so that (a)/(b)/(c) all trivially pass while excluding a conflicting or absent entry that would have failed them — passing (a)/(b)/(c) is necessary evidence, not sufficient proof. The actual trust anchor for a genuine rotation-boundary proof is the **OD-side dual-signature pair** (OD spec v1.31 §24.7): a window's boundaries are trustworthy only when they are tied to that pair's own entry references (or an equivalently cryptographically-anchored marker) — a caller-asserted-but-unauthenticated boundary is NOT sufficient on its own, independent of whether its derivation avoided referencing `rotation_correlation_id` (the §7.7-(pre-this-note) independence requirement above is necessary but not sufficient). This reframes the composition at the fork's §2 points 2–4 (`harness-is` has zero outbound cross-axis edges, so this reframe changes no IS-side code, only the accuracy of the claim): §5.6/§7.7 supply the IS-side JOIN-KEY evidence + its own internal consistency checks (a)/(b)/(c); the CP-owned impl leg supplies the OD-anchored TRUST — window-boundary authentication against the OD pair, and (for a rotation entry that is the current chain head, where `verify_chain`'s successor-link mechanism has no downstream entry to detect tampering against — a PRE-EXISTING property of §6.4's design, unchanged by this delta, shared by every C-IS-05 sidecar including §5.1/§5.4) independent OD-signature-based tamper coverage, not IS chain succession. Both requirements — OD-anchored boundaries, and OD-signature coverage for a chain-head rotation entry — are CP-owned impl-leg constraints, registered at the IS plan's U-IS-20 Notes and at `.harness/post-phase-8-forward-register.md`'s `B-33` entry (both cross-referenced, not restated as spec prose here — registering the constraint is not the same as solving it at this leg).

**Failure discipline.** None of (a)/(b)/(c) is a "warning" — a non-emptiness, presence, or uniqueness failure means the claimed rotation window is NOT verified, exactly as the CP `verify_rotation_6_steps` fork registration already establishes for the pre-v1.12 generic-continuity gap ("zero entries is an absence of evidence, not proof of chain integrity" — the same fail-closed posture (c) extends to the rotation-correlation carrier, and the same posture extends further still to a malformed or torn non-empty window post-v1.12). A window that passes (a)/(b)/(c) but whose boundaries are NOT OD-anchored is likewise NOT a verified rotation — see the join-key note above.
