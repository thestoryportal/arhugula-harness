...
**Scope discipline — the CP-owned verifier contract stays out of this spec.** The fork's §2 points 2–4 (the `verify_rotation_6_steps` 3-step extension requiring and joining the id; the boundary-attestation check against the B-36 `AwsKmsSigningBackend` key-identity mapping; the OD-join mechanism via a composition-root-injected typed evidence DTO) are **CP-owned** — `harness-is` has zero outbound cross-axis edges (the same invariant §5.1's resolver-residence deferral and §5.4's "hard constraint: NOT `harness-cp`" already preserve), so the verifier logic that CONSUMES this carrier is authored at the CP spec + the separate impl leg, cross-referenced at §7.7, never restated here.
...
**A "claimed rotation window" is a DERIVED sub-sequence — not the same input `verify_chain` requires.** `verify_rotation_6_steps`'s existing `VERIFY_HASH_CHAIN_LINK` step (§20.3.1 step 3) calls `harness_is.chain_verification.verify_chain` on the FULL genesis-anchored chain per that function's own pre-existing contract (position 1's `prior_event_hash` MUST equal the all-zeros sentinel; an arbitrary filtered slice fails with `INCEPTION_SENTINEL_MISMATCH` — unchanged by this delta). Any real ledger containing rotation history also contains non-rotation entries (`rotation_correlation_id = None` at every one of them, per §5.6's field table) interleaved before, after, and possibly around a rotation event. The (a)/(b)/(c) invariants below therefore do NOT apply to that full genesis-anchored sequence — applying (a) presence to the whole chain would fail on every ordinary non-rotation entry. "A claimed rotation window" is instead a caller-DERIVED sub-sequence: the subset of entries a consumer has identified as belonging to one rotation event (e.g., by filtering the full chain for non-`None` `rotation_correlation_id`, or by whatever boundary-derivation mechanism the CP-owned impl leg defines — left to implementation discretion below, exactly as the write-cadence already is). The full-chain continuity check (unchanged, §20.3.1 step 3) and the §7.7 window invariants are therefore **two DISTINCT checks over two DISTINCT inputs**, composed together at the CP-owned `verify_rotation_6_steps` extension — never substituted for each other, and neither is circular: deriving the window (filter-by-non-`None`) and then checking (a)/(b)/(c) OVER that already-derived subset is a straightforward two-step pipeline, not a self-referential one.
...
**Deferred to implementation discretion.** The precise consumer-side error taxonomy for a non-emptiness, presence, or uniqueness failure (a CP-owned typed exception, per `B-33-A`'s impl leg); how a consumer determines a window's boundary entries in the first place (a CP-side write-cadence + read-cadence decision, mirroring the §5.4/§5.6 write-cadence deferrals) — §7.7 states what the IS chain guarantees once a window is identified, not how a window is identified.
