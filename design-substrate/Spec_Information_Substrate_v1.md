...
| **Timestamp authority (v1.11)** | Caller-supplied `timestamp` on DIRECT appends, validated monotonic-non-decreasing per C-IS-05 §5 (detect-then-refuse); **writer-owned** on the buffered/branch-drain append surface — the writer samples the persisted `timestamp` at each entry's append inside the write serialization point, per §7.6 |
