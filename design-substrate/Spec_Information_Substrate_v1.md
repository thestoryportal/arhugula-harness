...
**The boundary-derivation method is load-bearing, and MUST NOT use `rotation_correlation_id` as its selection criterion.** Deriving the window by filtering the full chain FOR non-`None` `rotation_correlation_id` (or for a specific claimed id) would make (a)/(b)/(c) SELF-VALIDATING, not fail-closed: filtering for non-`None` first, then checking "every entry in the filtered set is non-`None`" (a), is tautological — it can never fail, because non-`None`-ness was the selection criterion, not an independently-observed property. Symmetrically, filtering for entries matching one CLAIMED id and then checking uniqueness (b) over that pre-filtered set would silently DISCARD a conflicting second id rather than detect a torn window — exactly the tamper this invariant exists to catch. The window boundaries MUST therefore be derived from evidence INDEPENDENT of the field these invariants validate — e.g., positional/index bounds in the chain (a caller-asserted `[start, end)` range), a boundary marker distinct from the correlation id itself (a dedicated rotation-event start/end marker, if the CP-owned impl leg adds one), or key-period evidence from the OD-side dual-signature pair (which entries' `action_id`/`timestamp` the OD pair's own metadata names, independent of what those IS entries' `rotation_correlation_id` values happen to be) — and ONLY THEN are (a)/(b)/(c) evaluated over every entry the independently-derived boundary includes. This precise derivation mechanism is CP-owned (left to implementation discretion below, exactly as the write-cadence already is) — §7.7 pins the REQUIREMENT that it be independent of `rotation_correlation_id`, not the mechanism itself. The full-chain continuity check (unchanged, §20.3.1 step 3) and the §7.7 window invariants remain **two DISTINCT checks over two DISTINCT inputs**, composed together at the CP-owned `verify_rotation_6_steps` extension — never substituted for each other.

**(a) Presence.** Within a claimed rotation window (a contiguous run of entries a caller asserts belongs to one rotation event), every entry the CP rotation orchestration writes as part of that window carries a non-`None` `rotation_correlation_id`. An entry inside the claimed window carrying `None` is a **malformed window** — the caller's claim that a rotation occurred at that boundary is unsubstantiated by the IS chain, and any consumer walk built on §5.6 (the CP-owned `verify_rotation_6_steps` extension per the fork's §2 point 2) MUST treat this as a failed presence check, never as a silent pass.
