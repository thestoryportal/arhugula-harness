...
- **(CP v1.103 §1 row 4 — the folds.)** Under `proceed`/`cascade-cancel`, the PARTIAL/FAILED report fold CARRIES the carrier's `result_ref` — the completed effect's reference is never dropped from the fold. The ref is carried as an OPAQUE string (CP neither reads nor writes the protected store — Runtime-owned at v1.103 §14.8.11 / U-RT-145).
