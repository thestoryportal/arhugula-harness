...
| `pause` | Halt the fan-out at a HITL/pause boundary (composes with C-CP-26 PauseResumeProtocol + C-RT-30 `api.resume`). | Allowed to finish in-flight, then pause. | **`PAUSED`**. |
...
7. **Resume-idempotency-terminality.** Branch-scoped idempotency keys (§25.16) so `api.resume` (C-RT-30) reads each branch's persisted `terminal_status` and MUST NOT re-dispatch a branch that is `cancelled` / `completed` / `timed_out`. (Corpus: make every interrupt-resume path idempotent.)
