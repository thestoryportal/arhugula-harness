...
On validator failure event from C5 gate evaluation:
  ...
  Discriminate by the retry-exit classification (ValidatorRetryExitClass; §1.10.1 v1.6 —
  NOT the wire attribute validator.fail.class, which carries the ValidatorFailClass domain):
...
When the retry-exit classification ∈ {transient-retry, Reflexion-recoverable}:
...
- **`validator.fail.class`** — fail-class enum carried per validator-failure event. Type (v1.6, per CP spec v1.116 §1.1, operator-ratified `B-138` disposition (a)): enum string ∈ `{schema_violation / semantic_inconsistency / safety_policy / resource_constraint / external_rejection}` — the C-CP-25→C-CP-28 §25.2 `ValidatorFailClass` domain. Emitted on every validator-failure event carrying a populated `fail_class` (the C-CP-28 §25.2 `ValidatorResult.fail_class` is `ValidatorFailClass | None`); absent when `fail_class=None`; the runtime escalation composer materializes the sentinel `unspecified` on a `None` brief. Ownership: emitted by C5 at fail-classification time. Cardinality is bounded (five values). A known NONCONFORMING emission exists and is not counted in the cardinality: the runtime escalation composer writes the sentinel `unspecified` on a `None` brief — out-of-domain; its disposition (absent-on-`None` vs widening) is register row `B-140`'s. *The v1.2–v1.5 type line's five retry-exit values (`transient-retry / Reflexion-recoverable / HITL-recoverable / permanent-fail-exit / terminal-fail-exit`) remain the §1.10 escalation-order discriminator as the retry-exit classification (`ValidatorRetryExitClass`, C-CP-21 §21.1) — demoted from the wire name, never deleted; every §1.10 occurrence of `validator.fail.class` naming those values denotes that classification, not the wire attribute.*

- **`validator.fail.cause_attribution`** — cause-attribution annotation per fail-class signal. Type: enum string from the open set per `c5-validation-contract` SKILL.md (`network_timeout`, `provider_outage`, `model_misfire`, `contract_violation`, `schema_violation`, `semantic_disagreement`, `policy_denial`, `human_rejection`, `time_budget_exhaust`, `capability_shortfall`) plus F5-introduced refinements (`secret_unknown`, `secret_unavailable`, `secret_expired`, `secret_locked`, `secret_revoked` per ADR-F5 §Decision). Always-emitted on every validator-failure event per s12 §7.5(a) standing pre-check (a fail-class without cause_attribution is FM-J per `c5-validation-contract` SKILL.md). Ownership: emitted by C5 at fail-classification time. Cardinality is medium (open set; phase-2 may add). *(v1.6 note: zero producers emit this attribute at HEAD; the declared-required-vs-unemitted gap is tracked at register row `B-139`.)*

- **`validator.fail.permanence`** — derived discriminator boolean / enum. Type: enum string ∈ `{transient / permanent}`. Derivation (v1.6): the v1.2–v1.5 clause derived it from the retry-exit classes (`permanent` if class ∈ {`permanent-fail-exit`, `terminal-fail-exit`}; `transient` otherwise); that base is demoted with the taxonomy — the wire producer holds only the validator outcome and the `ValidatorFailClass`, and the outcome projection is lossy (`ESCALATE` conflates `permanent-fail-exit` with `HITL-recoverable`), so the wire derivation base is deliberately NOT re-declared here; it routes to register row `B-124`'s arc per CP spec v1.116 §1.3. Not emitted at HEAD; emission build-owed at `B-124`. Cardinality is bounded (two values). Rationale: provides a C7-instrumentable boolean discriminator for cross-attribute filtering and dashboard binding without requiring downstream consumers to enumerate the full five-class set. Redundant-by-construction with the retry-exit CLASSIFICATION (should a wire carrier for it ever be minted) — NOT with the wire attribute `validator.fail.class`, whose `ValidatorFailClass` domain cannot derive permanence, and not with the outcome, whose projection is lossy (see the demoted clause above); operationally useful at the dashboard / sampling-discipline layer.
