...
10. **(`signatures_verified` always `False` in this delta, OD spec v1.35 §24.8 row 8a — out-of-family review round-2 [P1] correction.)** `find_rotation_pair_evidence` returns `signatures_verified=False` on EVERY call in this delta — including the criterion #7 success path — because no rotation-period-aware cryptographic verifier exists yet. This is a machine-checkable, necessary-but-not-sufficient signal distinct from `pair_present`, consumed by CP spec v1.105 §2 row 5a. Witness: `test_find_rotation_pair_evidence_always_returns_signatures_verified_false_in_this_delta` (asserts `signatures_verified is False` across the zero-match, tampered, AND valid-pair fixtures alike; mutation probe: hardcoding `signatures_verified=True` on the valid-pair success path passes criterion #7's own witness but fails this one — the two witnesses are independently load-bearing).
