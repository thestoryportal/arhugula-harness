...
| Gate | Finding |
|---|---|
| Instance-cardinality >=2 of independent arcs | PASS, with an honesty caveat (mirrors the cadence-2 PD-5 precedent). 2 genuinely independent instances, verified via `gh pr view --json mergedAt` + each commit's `Claude-Session` trailer, not the memory file's self-reported count: (1) PR #927 + PR #933 (both 2026-07-11, CP `B-18-3C-PREWARM-CASCADE` / `B-18-PREWARM-OW`) share the identical `Claude-Session` trailer — same session, same lesson (the `task.result()` resurface check applied to two sibling PRs), collapsing to ONE instance per the same-session-same-lesson discriminator; (2) PR #972 / `B-26` (2026-07-14, IS hash-chain rotation-verification — a wholly separate session and axis, 4 independent mutation-probes within one PR: the hash-chain-succeeded gate, the downstream-step-blocking gate, and the empty-ledger-fails gate, each confirmed by reverting and observing the expected test failure before restoring). 2 clears the literal gate but at the thinnest margin of any PD promoted so far — recorded honestly rather than as "3 distinct arcs." |
...
(e) **Two factual corrections made pre-merge, caught by out-of-family Codex review of this PD-8 promotion's own PR — PD-8 applied to itself.** The initial draft of this amendment (i) misdated the candidate memory file as a file-delta discovery *since* cadence-7 close, when its mtime actually predates the close commit by 17 minutes (it was already flagged by cadence-7's own write-up, which explicitly deferred re-evaluation — corrected in the trigger paragraph above); and (ii) overstated instance-cardinality as "3 distinct arcs" without verifying independence against the primary source, when PR #927 and PR #933 in fact share one `Claude-Session` trailer and collapse to a single instance under the cited same-session discriminator (corrected to the honest 2-instance count in the provenance note and gate table above). Neither correction changes the promotion outcome (2 still clears the literal gate); both are recorded because a promotion that misstates its own evidentiary basis undermines the discipline it promotes.
