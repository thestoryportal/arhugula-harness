...
(e) **Four factual corrections made pre-merge, across two rounds of out-of-family Codex review of this PD-8 promotion's own PR — PD-8 applied to itself.** Round 1 caught: (i) the candidate memory file was misdated as a file-delta discovery *since* cadence-7 close, when its mtime actually predates the close commit by 17 minutes (it was already flagged by cadence-7's own write-up, which explicitly deferred re-evaluation — corrected in the trigger paragraph above); (ii) instance-cardinality was overstated as "3 distinct arcs" without verifying independence against the primary source, when PR #927 and PR #933 in fact share one `Claude-Session` trailer and collapse to a single instance under the cited same-session discriminator (corrected to the honest 2-instance count). Round 2 caught two further errors introduced by round 1's own correction: (iii) PR #972/`B-26` was mischaracterized as a separate "IS-axis" arc — its commit is `fix(cp)`, wiring real IS hash-chain verification into a CP-owned module, a CP→IS seam rather than a distinct axis (corrected throughout); (iv) the memory file's claimed "4 independent probes" for that PR was carried forward unverified — direct read of the committed test diff shows exactly 2 distinct discriminating-witness mutation-probes, not 4 (corrected in the gate table and table row above). None of the four corrections changes the promotion outcome (2 instances still clears the literal gate); all are recorded because a promotion that misstates its own evidentiary basis undermines the discipline it promotes, and because round 2 catching round 1's own new errors is itself a small demonstration of PD-8's point — a first pass of scrutiny does not guarantee the corrected claims are themselves correct without a second, independent check.
