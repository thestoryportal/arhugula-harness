...
| **§14.23.5 invariants (NEW)** | Opt-out byte-identical; store-write BEFORE the ledger-append (RESERVE-before-COMMIT); rehydrate driven by `resume_at` + FAIL-CLOSED on missing-output / identity-mismatch; keyed by `run_idempotency_key`; no §5.2-hash / IS-contract change (dedicated store, not the F2 ledger); genuine non-vacuity (real store on disk → rehydrate → real LLM dispatcher → real provider receives the recovered output + a negative control, no proxy). | Fork §2/§4 |
