...
**At-most-once, NOT exactly-once (the honest residual — mirrors the reconciler).** `try_reserve(idempotency_key)` is the atomic claim: the FIRST dispatch wins (the effect fires); any re-dispatch of the same effect LOSES — both the cross-process RESUME re-dispatch of an effected-but-uncommitted step AND an in-process `RetryBreakerToolDispatcher` retry (the fence error is not a transient class, so the breaker fail-fasts it; for a non-idempotent effect, fail-closing a transient-failure retry is the *correct* conservative behavior — you cannot safely retry an effect that may have fired). A lost claim raises `EffectFenceReservedUncommittedError` → fail-closed to §22.1 HITL rather than risk a double-execution. The genuine fire-then-crash-before-commit window is ambiguous to the harness (did the effect fire?), so the conservative fail-close (a reserved-but-unfired effect also fail-closes its re-dispatch) is the honest at-most-once answer, exactly mirroring the reconciler's §22.1 posture. **COMMIT = the EXISTING per-step ledger entry** (one source of truth); the fence adds only the RESERVE (pre-fire) marker — no second commit record.
...
3. **At-most-once across BOTH resume and retry.** A re-dispatch of an already-reserved effect loses — the cross-process resume re-dispatch AND the in-process `RetryBreakerToolDispatcher` retry (the fence error fail-fasts, not retried). For a non-idempotent effect, fail-closing a transient-failure retry is the correct conservative behavior. The residual fire-then-crash-before-commit window is ambiguous to the harness, so the conservative fail-close (a reserved-but-unfired effect also fail-closes) is the honest at-most-once answer (mirrors the reconciler §22.1 posture) — at-most-once, NOT exactly-once.
