...
`ManagedAgentsConfig` (RuntimeConfig sub-model §3, empty-marker — presence signals operator opt-in; the concrete SDK client is supplied out-of-band at the operator bootstrap site, parallel to `SkillActivationHookConfig` / `ValidatorFrameworkConfig` / `PauseResumeProtocolConfig`):

```python
@dataclass(frozen=True)
class ManagedAgentsConfig:
    """Empty-marker sub-model — presence + DeploymentSurface.MANAGED_CLOUD
    signals operator opt-in to managed-agents step dispatch. The concrete
    ManagedAgentsClientProtocol (e.g. AnthropicManagedAgentsClient over a
    live SDK client) is supplied out-of-band at the operator-controlled
    bootstrap site (FM-2; mechanism is implementation discretion at the
    landing arc). No credentials embedded in the contract surface.
    """
```
...
4. **Poll to terminal** — `await client.retrieve_session(session_id=...)` on a bounded poll loop until a terminal status (`IDLE` / `COMPLETED` / `FAILED` / `CANCELED` / `TERMINATED`) or the poll budget is exhausted (budget mechanism §14.20.7). The R-820 proof drove to `IDLE`.
...
**Stage 5 (LOOP_INIT):** NEW factory `materialize_managed_agents_dispatcher_stage(config: RuntimeConfig, ctx: _MutableHarnessContext) -> None`. Factory body:

1. **If `config.managed_agents_config is None` OR `config.deployment_surface != DeploymentSurface.MANAGED_CLOUD`:** bind `ctx.managed_agents_client = None`; do **not** register a `StepKind.MANAGED_AGENTS` dispatcher in the `StepKindDispatcherRegistry`. Return. (Opt-out / non-managed-cloud path — a `MANAGED_AGENTS` step then fails closed at `registry.lookup` with `StepKindDispatcherNotBoundError` → `RT-FAIL-STEP-KIND-DISPATCHER-NOT-BOUND`, no silent under-execution.)
2. **Else:** resolve the operator-supplied `ManagedAgentsClientProtocol` (out-of-band per §14.20.7); bind it to `ctx.managed_agents_client`; construct `ManagedAgentsStepDispatcher(client=..., tracer_provider=ctx.tracer_provider)`; register it (wrapped via `SyncDispatcherFacade`) in the `StepKindDispatcherRegistry` for `StepKind.MANAGED_AGENTS`.
3. **Return.**
...
- **Poll-budget mechanism + interval.** The bounded poll loop to a terminal status (a `RuntimeConfig` field, a per-step `step_payload` value, or a fixed default) + the poll interval/backoff are implementation discretion. The value MUST bound the loop (no unbounded poll).
