...
| **§14.21.5 invariants (NEW)** | Opt-out byte-identical; dispatch signature unchanged; §25.3.3.4 preserved; ADR-F2 single-threaded-write for the wired scope; no §5.2-hash change; genuine-consumer non-vacuity; **invariant 7 run-scoped-no-cross-run-leak sequential AND concurrent (per-run reset + per-loop single-flight lock when the channel is bound — out-of-family Codex review r4).** | Fork §2/§4/§5 |
...
**Per-run boundary (`run_workflow` tool handler).** Because the channel is allocated at bootstrap but is run-scoped state, the `run_workflow` tool handler `reset()`s `ctx.inter_step_output_channel` at the start of each (non-resume) invocation — the run boundary a reused (daemon-client mode) `HarnessContext` re-enters — before dispatching `execute_workflow` (§14.21.5 invariant 7). No-op when the channel is unbound. When the channel IS bound, `[reset + execute]` runs under a per-running-loop single-flight lock so concurrent invocations sharing the one channel are serialized rather than interleaved (§14.21.5 invariant 7); the unbound default path never touches the lock.
...
7. **Run-scoped, not bootstrap-scoped — no cross-run leak (sequential AND concurrent).** Although the channel is *allocated* at bootstrap (stage 5), its contents are run-scoped. The `run_workflow` tool handler `reset()`s the channel at the per-run boundary (on a non-resume invocation; a resume continues the same run) so a REUSED `HarnessContext` — the daemon-client mode where one bootstrapped ctx serves many `run_workflow` invocations (U-RT-108) — does NOT leak a prior run's step outputs into a later run's first dispatch (out-of-family Codex review). A resume is NOT reset (it continues the same run's in-flight outputs). **Concurrent inter-step runs are single-flight.** Because the bootstrap-scoped ctx exposes ONE channel, two *concurrent* `run_workflow` invocations on the same ctx would share it and corrupt each other (run B's `reset()`/records overwriting run A's in-flight upstream reads — out-of-family Codex review r4). So **when the channel is bound, the handler runs `[reset + execute]` under a per-running-loop single-flight lock** — inter-step runs queue rather than interleave. The opt-out default (channel unbound) NEVER acquires the lock → full concurrency is unchanged (byte-identical to pre-v1.59). The proper per-run channel *isolation* (a ContextVar-scoped channel that removes the serialization AND covers the identical bootstrap-scoped `cost_record_accumulator` exposure) is a registered follow-on (§14.21.7).
...
- **Per-run channel isolation (registered follow-on `B-INTERSTEP-PERRUN-ISOLATION`).** v1.59 forecloses concurrent-inter-step corruption with a per-loop single-flight lock (§14.21.5 invariant 7) — proportionate, but it *serializes* concurrent inter-step runs rather than isolating them. The proper fix scopes the channel per run (e.g. a `ContextVar`-bound channel resolved at dispatch), which both removes the serialization (restoring full concurrency for the bound path) AND closes the IDENTICAL bootstrap-scoped shared-holder exposure that `cost_record_accumulator` carries today (CA #625 — a concurrent-daemon cost-rollup cross-run mix is the symmetric latent issue; it is observability-additive so lower-severity, but the same root). This is a runtime-wide concurrency arc, registered here because B-INTERSTEP is the first capability whose *correctness* (not just observability) depends on it.
