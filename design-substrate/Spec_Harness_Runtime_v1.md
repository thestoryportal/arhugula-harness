# Specification — Harness Runtime v1.68
...
**Scope boundary.** The inner's `gen_ai` span carries the DECLARATIVE-echo `routing.layer` (not the real EMBEDDING/L3 layer the wrapper used) — a minor observability fidelity gap registered as the forward arc `B-L2-ROUTING-SPAN-LAYER-ATTRIBUTION`; the routed model itself is faithfully dispatched and the capability is production-dormant until a second provider makes routing-among-candidates meaningful.
