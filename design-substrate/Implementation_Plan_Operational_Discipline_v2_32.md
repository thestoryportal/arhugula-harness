...
- **The `B-116` waiver guard and its t1/t2 waived-charge attributes.** Runtime-owned at U-RT-152 (Runtime plan v2.60), merged separately as PR #1271. This unit closes the *other* half of the `B-116` gate — the observability floor — and touches none of that surface.
- **Whether the CP `:409` tail-keep mitigation is live.** Registered as `B-123`; head=1.0 is deliberately the guarantee that does not depend on its answer (OD spec v1.37 change-note ground 3).
