# Implementation Plan — Operational Discipline (OD) Axis v2

**Project.** Multi-LLM Agent Harness — Phase 6 Implementation Planning
**Session.** Phase 6 Session 4 of 5 (revision pass per `P6-CK_Iter1_Path_alpha_IS_CP_OD_Revision_Kickoff.md`)
**Substrate.** `Spec_Operational_Discipline_v1.md` v1.2 (P5-CK-cleared); `Spec_Information_Substrate_v1.md` v1.2 (IS plan v2 substrate); `Spec_Action_Surface_v1.md` v1.1; `Spec_Control_Plane_v1.md` v1.2 (CP plan v2 substrate); `Project_Workflow_v1_5.md` §4.1.2 + §7 + §8; `Adversarial_Review_6.md` §3.5 + §3.6 + §3.7 + §3.8
**Skill.** `implementation-planner` (revision-pass sub-mode per §8)
**Deliverable.** This file — 34 atomic units across 8 clusters; v2 absorbs 4 P6-CK Iteration 1 findings (F2-OD-01 + F2-OD-02 + F2-OD-03 + F1-OD-01)
**Status.** v2 — `Status: Proposed` pending P6-CK Iteration 2 clearance per Workflow v1.5 §4.1.2
**Closure status.** v1 — `closure_pending_at_v1 == true` per F2-12 carry-forward; closure target = OD plan v2 (this artifact, revision-pass mode per SKILL.md §8); F2-12 closure-path execution (steps 1–6 of canonical chain) OUT-OF-SCOPE at this revision pass per kickoff §1; routed to parallel `council-orchestrator` C7+C9 session.
...
Single-pass revision absorbing four `Adversarial_Review_6.md` P6-CK Iteration 1 findings, all local to OD plan v1:
...
|---|---|
| §0a Front matter (renamed from §0 to free §0 slot for Change note) | OD selections (OD-S4-1.A / OD-S4-2.A / OD-S4-3.A), plan-level invariants, F2-12 ACTIVE engagement summary all preserved verbatim from v1 |
...
| §3.1 Cluster 1 (U-OD-01 through U-OD-03) | No finding |
| §3.2 Cluster 2 — U-OD-04, U-OD-05, U-OD-06, U-OD-08 | No finding |
| §3.2 Cluster 2 — U-OD-07 non-acceptance-#2 sections | No finding at scope, depends-on, inputs, files, signatures, acceptance #1 / #3 / #4 / #5, tests, rollback boundary |
| §3.3 Cluster 3 (U-OD-09 through U-OD-15) | No finding |
| §3.4 Cluster 4 (U-OD-16 through U-OD-19) | No finding |
| §3.5 Cluster 5 — U-OD-18, U-OD-19, U-OD-21, U-OD-22 | No finding |
| §3.5 Cluster 5 — U-OD-20 non-amendment sections | No finding at scope, depends-on, inputs, files, F2-12 ACTIVE engagement narrative, signatures (except `F2_12_CLOSURE_PATH` const steps 3–4), acceptance #1–#7 + #9–#11, tests, rollback boundary |
| §3.6 Cluster 6 — U-OD-24, U-OD-25, U-OD-26 | No finding |
| §3.6 Cluster 6 — U-OD-23 non-acceptance-#3 sections | No finding at scope, depends-on, inputs, files, signatures, acceptance #1 / #2 / #4–#10, tests, rollback boundary |
| §3.7 Cluster 7 — U-OD-27, U-OD-28, U-OD-29, U-OD-31 | No finding |
| §3.7 Cluster 7 — U-OD-30 non-amendment sections | No finding at scope, depends-on, inputs, files, signatures (except 4th `AuditSignatureAttributes` field), acceptance #1–#6 + #8–#14, tests (test names unchanged; semantics preserved post-amendment), rollback boundary |
| §3.8 Cluster 8 — U-OD-32, U-OD-33 | No finding |
| §3.8 Cluster 8 — U-OD-34 non-amendment sections | No finding at scope, depends-on, inputs, files, signatures, manifest content table, acceptance #1–#6 + #8–#12; acceptance #5 confirmed substantively unchanged per Stage S.2 §2.1.5; acceptance #7 amended |
| §4 dependency graph | No graph delta; node count + edge count + topological-sort order unchanged |
...
|---|---|---|
| U-OD-07 acceptance #2 | 3 per-prefix anchor corrections: topology §14.1 → §14.2; subagent C-CP-19 → C-CP-14 §14.2; engine C-CP-09 → C-CP-09 §9.1 | F1-OD-01 |
| U-OD-20 `F2_12_CLOSURE_PATH` step 3 | Append `per revised ADRs` to scope string | F2-OD-01 |
| U-OD-20 `F2_12_CLOSURE_PATH` step 4 | Drop `+ R-OD-05` from scope string | F2-OD-01 |
| U-OD-20 acceptance #8 | Grammar shift from "verbatim" to structural-fidelity grammar acknowledging shared-prefix + axis-substituted-tail pattern per kickoff §6.3 | F2-OD-01 |
| U-OD-23 acceptance #3 | Formula token realignment: `cache_read.input_tokens` → `anthropic.cache_read_input_tokens`; `cache_creation.input_tokens` → `anthropic.cache_creation_input_tokens`; AS spec §14.2 + U-AS-31 source citation added | F2-OD-02 |
| U-OD-30 `AuditSignatureAttributes` field 4 | Field rename `audit_signature_sha256` → `audit_signature_key_period`; comment realigned from hash-digest semantic to rotation-cycle-anchor semantic | F2-OD-03 |
| U-OD-30 acceptance #7 | No textual change; "byte-exact attribute names" claim now substantively true post-realignment | F2-OD-03 (consequence) |
| U-OD-34 acceptance #7 | Grammar shift from "verbatim" to structural-fidelity grammar parallel to U-OD-20 acceptance #8 | F2-OD-01 |

### §0.4 Coverage matrix delta

No delta. Affected units' `Implements:` declarations unchanged:

| Unit | `Implements:` v1 | `Implements:` v2 |
|---|---|---|
| U-OD-07 | C-OD-05 §5.1 (CP-source rows) | (unchanged) |
| U-OD-20 | C-OD-14 §14.4, §14.5 | (unchanged) |
| U-OD-23 | C-OD-17 §17.1, §17.2 | (unchanged) |
| U-OD-30 | C-OD-21 §21.1, §21.2, §21.3 | (unchanged) |
| U-OD-34 | C-OD-22 §22.1–§22.4 + C-OD-23 §23.1 | (unchanged) |

Per-cluster coverage at §5 unchanged.

### §0.5 Dependency graph delta

No delta. Affected units' `Depends on:` declarations unchanged. Aggregate DAG: 34 nodes, edge set unchanged from v1, topological sort preserved (Level 0 foundational units → Level 9 terminal units U-OD-31 + U-OD-34). Cross-axis edge count (28) + breakdown ({IS: 6, AS: 10, CP: 12}) unchanged.
...
|---|---|
| OD spec | v1.2 (P5-CK-cleared) |
| ADR-D5 | v1.3 |
| ADD | v1.2 |
| PRD | v1.0.1 |
| Cross-axis CP plan | v2 (filed at this session Plan 2; U-CP-54 + U-CP-55 cross-axis edge targets) |
| Cross-axis IS spec | v1.2 (§10.3 canonical for audit.signature.* attribute set) |
| Cross-axis AS spec | v1.1 (§14.2 canonical for anthropic.* cache attribute names) |
...
|---|---|---|
| Line 986 hash-digest classification example list cites `audit.signature.sha256` as a `HASH_DIGEST_OF_CONTENT` attribute; post-F2-OD-03 resolution, `audit.signature.sha256` does not exist in IS spec §10.3 canonical (canonical 4th attribute is `audit.signature.key_period`, semantically a rotation-cycle anchor, NOT a hash digest) | Not amended at v2 per Path (α) strict-narrow scope discipline + `P6-CK_Iter1_Path_alpha_IS_CP_OD_Revision_Kickoff.md` §6.2 scope-expansion anti-pattern | Candidate finding for P6-CK Iteration 2; resolution likely removes `audit.signature.sha256` from hash-digest example list |
| F2-CXA-04 (CXA doc §6.4 propagates F2-OD-01 verbatim-inheritance claim) | Path α scope is plan revisions only | Path β kickoff per `P6-CK_Iter1_Path_beta_CXA_Revision_Kickoff.md` |
| F1-CXA-01 (CXA doc §3.2 row 11 propagates F1-OD-01 subagent.* anchor drift) | Path α scope is plan revisions only | Path β kickoff per `P6-CK_Iter1_Path_beta_CXA_Revision_Kickoff.md` |

### §0.9 Path α completion declaration

This is the third and final plan v2 in Path α per `P6-CK_Iter1_Path_alpha_IS_CP_OD_Revision_Kickoff.md` §8.2. On filing of this artifact:

- IS plan v2 filed (Plan 1 of 3; F2-IS-01 + F1-IS-01 absorbed)
- CP plan v2 filed (Plan 2 of 3; F2-CP-01 + F2-CP-02 absorbed)
- OD plan v2 filed (Plan 3 of 3; F2-OD-01 + F2-OD-02 + F2-OD-03 + F1-OD-01 absorbed)
- AS plan v1 unchanged (no finding at AS plan per Adv Review 6 §2.1)

Path α aggregate exit gate met. Path β (CXA doc v1 → v2) kickoff per `P6-CK_Iter1_Path_beta_CXA_Revision_Kickoff.md` becomes the next routing target. P6-CK Iteration 2 entry conditional on Path α + Path β completion.
...
4. `classify_attribute` returns the correct classification per attribute; hash-digest attributes (e.g., `hitl.response.summary_hash`, `audit.signature.sha256`, `mcp.primitive.signature.sha256`) classify as `HASH_DIGEST_OF_CONTENT` and are default-on.
...
| F2-12 closure_pending_at_v1 = true; closure target = OD plan v2 | ✅ |
| Pattern P1 within-axis compliance | ✅ |
| Cross-axis coverage matrix scope deferred to Session 5 per OD-S4-2.A | ✅ |
| Coherence pass 5/5 dimensions PASS | ✅ |

**OD plan v1 disposition: ✅ FILED.**

Forward routing: Phase 6 Session 5 cross-axis composition session (4-plan ensemble: IS + AS + CP + OD) per session prompt `Phase_6_Session_5_Session_Prompt.md`.

**End of `Implementation_Plan_Operational_Discipline_v1.md`.**
