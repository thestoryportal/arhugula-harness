...
**Acceptance criterion (functional — read-side invariants, §7.7).** (5) **ONE public composed validator, not three independently-callable helpers.** IS exposes a SINGLE public function over a `Sequence[StateLedgerEntry]` that internally sequences non-emptiness → presence → uniqueness and returns one typed result (pass, or a typed failure naming WHICH invariant failed: empty-window / presence / uniqueness). Non-emptiness, presence, and uniqueness are NOT separately public, independently-callable functions — a CP-side caller invoking only a presence-or-uniqueness sub-check directly against `[]`, bypassing the non-emptiness guard, would recreate the exact vacuous-pass defect this delta exists to prevent (round-5 out-of-family Codex finding). Internal helper functions MAY exist for testability, but the unit's PUBLIC surface is the single composed validator. (6) **Non-emptiness.** Given an EMPTY sequence, the composed validator fails at the empty-window check BEFORE any presence/uniqueness predicate runs — per §7.7 invariant (c), an empty claimed window is an absence of evidence, not a vacuous pass. (7) **Presence.** Given a NON-EMPTY sequence, the validator's presence stage passes when every entry carries a non-`None` `rotation_correlation_id`, and fails when any entry carries `None`. (8) **Uniqueness.** Given a non-empty sequence that passed presence, the validator's uniqueness stage passes when the set of non-`None` `rotation_correlation_id` values has cardinality ≤ 1, and fails when cardinality ≥ 2 (a torn/mixed window). The composed validator operates purely against the IS chain — no OD ledger, no signing backend — per §7.7's explicit scope boundary; it is the IS-side evidence a CP-owned consumer (the `B-33`-scoped `verify_rotation_6_steps` extension) composes with OD-anchored window-boundary authentication at the impl leg, NOT the authentication itself (per §7.7's join-key note).

**Acceptance criterion (non-goal — explicit scope fence).** (9) This unit MUST NOT implement `verify_rotation_6_steps`'s 3-step extension, the composition-root-injected OD-join evidence DTO, or any check against the B-36 `AwsKmsSigningBackend` key-identity mapping — all three are CP-owned per the fork's §2 point 4 axis-import-direction reasoning. **The "`B-33-A` impl leg" named throughout this delta is therefore TWO SEPARATE ARCS, not one PR:** (i) the U-IS-20 CODE ARC — implements ACs #1–#8 above at `harness-is/src` ONLY; closes U-IS-20; touches ZERO `harness-cp/**` files (satisfies this AC literally — a PR closing U-IS-20 that also touches `harness-cp/**` has exceeded THIS arc's scope); and (ii) a LATER, separate CP-AXIS ARC — opened only after (i) lands, with its own new CP spec delta + CP plan unit(s) (not yet decomposed; no CP unit ID is reserved by this delta) implementing the `verify_rotation_6_steps` extension + OD-join + B-36 boundary check, consuming U-IS-20's already-landed carrier from the CP side (the CP→IS inbound edge declared at that arc's own plan delta, mirroring U-CP-84 → U-IS-19). IS's own 0-outbound invariant means (i) can NEVER need to touch `harness-cp` — the consumer wiring is always (ii)'s concern, in its own arc.
