# Specification — Control Plane v1.4

## Status block

| Field | Value |
|---|---|
| Artifact | `Spec_Control_Plane_v1_4.md` |
| Status | **Proposed** — Phase-7 design-substrate revision applying operator-ratified C-CP-25 `WorkflowDriver` contract (see Change-note v1.3 → v1.4). |
| Revision | v1 → v1.1 (P5-CK iter-1 close mechanical revision) → v1.2 (P5-CK iter-2 mechanical-alignment) → v1.3 (F2-12 cascade Step 5a revision pass authored 2026-05-14) → **v1.4 (Phase-7 architectural-tension revision pass authored 2026-05-20 per `.harness/class_1_tension_u_rt_44_workflow_loop_drain.md` + `.harness/c_cp_25_workflow_driver_recommendation.md`; absorbs C-CP-25 `WorkflowDriver` contract scoped to `SINGLE_THREADED_LINEAR` topology pattern + engine classes `pure-pattern-no-engine` + `save-point-checkpoint`)** |
| Revision date | 2026-05-20 (v1.4 revision pass) |
| Phase | 7 — Phase-7 design-substrate revision absorbing operator-ratified `systems-architect` tension-resolution recommendation; in-CLI per workspace convention (design-phase back-flow deprecated 2026-05-15); `spec-writer` skill (spec-revision-pass sub-mode) applies the decided fix. |
| Skill | `spec-writer` (spec-revision-pass sub-mode) at v1.4 |
| Promotion path | Accepted at C-CP-25 plan + implementation landing close (CP plan v2.11 + landed driver units + runtime un-strike of U-RT-44 AC #2 + U-RT-49 workflow-execution ACs) |
| Source-set | All v1.3 inputs (preserved) + `.harness/c_cp_25_workflow_driver_recommendation.md` (operator-ratified 2026-05-20, 4/4 sign-off points) + ADR-F3 v1.1 §Decision (iv) (workflow lifecycle event surface F3 capability-floor) + `Spec_Harness_Runtime_v1.md` §11 C-RT-11 (drain semantics composition seam) + Phase-7 sub-phase 7b execution context |
| Entry authorization | `.harness/class_1_tension_u_rt_44_workflow_loop_drain.md` (Class 1 fork, status OPEN-RESOLVING 2026-05-20) + operator ratification 2026-05-20 of `.harness/c_cp_25_workflow_driver_recommendation.md` §8 sign-off points 1–4 + `CLAUDE.md` §4.3 back-flow routing (Class 1 spec revision authorized in-CLI per workspace governance) |
| Exit gate | CP plan v2.11 revision-pass (`implementation-planner` skill) consuming this CP spec v1.4 §25 contract as substrate; downstream runtime un-strike at U-RT-44 AC #2 + U-RT-49 workflow-execution ACs at C-CP-25 unit land |
...
3. **Iterate steps in declaration order.** For each step `s` in the `steps` parameter (declaration-order per in-session amendment §E; SINGLE_THREADED_LINEAR has no parallel / fan-out branching):
   ...
   4. **Dispatch.** Invoke the step body through the cap-aware router (`route_invocation(binding, s.payload, ctx)` per §1 / §1.3 manifest-as-auditable-default routing surface). Step body is opaque to the driver; the router owns provider / model / engine dispatch.
...
|---|---|
| Artifact | `Spec_Control_Plane_v1_4.md` |
| Filing destination | `design-substrate/Spec_Control_Plane_v1_4.md` |
| Status | Proposed (pending C-CP-25 atomic units land at CP plan v2.11 + runtime un-strike at U-RT-44 AC #2 + U-RT-49 workflow-execution ACs) |
| Predecessor | `Spec_Control_Plane_v1_3.md` (v1.0 → v1.1 → v1.2 → v1.3 → v1.4 baseline) |
| Substrate consumed | All v1.3 substrate (preserved) + `.harness/c_cp_25_workflow_driver_recommendation.md` (operator-ratified 2026-05-20) + ADR-F3 v1.1 §Decision (iv) + `Spec_Harness_Runtime_v1.md` §11 C-RT-11 |
| Successor | `Implementation_Plan_Control_Plane_v2_11.md` (Phase-7 CP plan revision-pass absorbing §25 C-CP-25 contract) |
| Phase-7 fork-resolution status | `.harness/class_1_tension_u_rt_44_workflow_loop_drain.md` — spec-side ABSORBED at v1.4; plan-side PENDING at CP plan v2.11; implementation PENDING at C-CP-25 atomic units land; runtime un-strike PENDING at U-RT-44 AC #2 + U-RT-49 workflow-execution ACs |
| Workflow discipline | `Project_Workflow_v1_8.md` §7 fidelity-grammar + `CLAUDE.md` invariant I-1 (citations resolve byte-exact) |
| Date | 2026-05-20 |

*Filed at Phase-7 architectural-tension-resolution revision pass close per `systems-architect` skill §4A protocol + `spec-writer` skill spec-revision-pass sub-mode. C-CP-25 `WorkflowDriver` contract added as new §25 scoped to `SINGLE_THREADED_LINEAR` topology + `pure-pattern-no-engine` / `save-point-checkpoint` engine classes per operator ratification 2026-05-20 of `.harness/c_cp_25_workflow_driver_recommendation.md` 4/4 sign-off points. §[traceability] matrix extended with C-CP-25 row. §[coherence pass] extended with v1.4 amendment-site verification line. Filing footer updated to v1.4. Adjacent-defect findings surfaced in Change-note "Adjacent-defect findings surfaced (not patched at v1.4)" section per `spec-writer` no-extension discipline. Cascade segment boundary per Phase-7 sub-phase 7b execution discipline. Recommended next step: CP plan v2.10 → v2.11 revision-pass (`implementation-planner` skill) consuming §25 C-CP-25 contract as substrate for new atomic-unit decomposition.*
