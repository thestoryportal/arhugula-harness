# Implementation Plan — Control Plane (CP axis) — v2.12
...
|---|---|
| Plan version | v2.12 |
| Predecessor | `Implementation_Plan_Control_Plane_v2_11.md` (preserved verbatim except U-CP-13 + U-CP-56 §6 amendments per §0) |
| Spec authority | `Spec_Control_Plane_v1_4.md` (unchanged; no spec bump at v2.12) |
| Unit count | 60 (unchanged) |
| Fork resolution | `[[fork-u-cp-56-resumption-underspec]]` Class 1 → CLOSED |
| Filing date | 2026-05-20 |
| Authoring authority | In-CLI revision pass per `Project_Workflow_v1_8.md` §2.7.6 back-flow routing — Class 1 routing path applied to a CP plan v2.11 atomic-unit-signature defect (U-CP-13 missing `entry_version` field; U-CP-56 AC #6 unimplementable as v2.11-shipped) |
