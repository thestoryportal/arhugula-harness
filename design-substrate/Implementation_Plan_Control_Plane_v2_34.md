...
| WAL_SEGMENT engine materialization — `_IN_SCOPE` widening + `:1445` dispatch fork (segment replay → `resume_at` via the F2 per-segment metadata) + `resumption.kind=segment_replay` + per-segment F2 join + F3 floors + consumer updates + **Path-(i) `test_u_rt_95` un-skip** (WAL_SEGMENT = canonical DURABLE_ASYNC class) | **U-CP-94** | C-CP-07 §7.1 row 5 + §7.4 ("specific WAL implementation" deferred) + C-CP-08 §8.1 `segment_replay` + §8.2 row 5 + §8.3 |
...
|---|---|
| U-CP-93 (EVENT_SOURCED_REPLAY) + U-CP-94 (WAL_SEGMENT resumption + `test_u_rt_95` un-skip) + U-CP-95 (WAL_SEGMENT recovery-loop firing branch) | All v2.33 + prior unit bodies — preserved verbatim per §0.3 |
...
#### U-CP-94 — WAL_SEGMENT engine materialization (segment replay + per-segment dedup) + Path-(i) `test_u_rt_95` un-skip

**Scope.** Materialize the `WAL_SEGMENT` engine class as impl against cleared C-CP-07/08: widen `_IN_SCOPE_ENGINE_CLASSES` to include `WAL_SEGMENT`; add a driver dispatch fork at `:1445` computing `resume_at` by **replay from the last durably-written WAL segment** (reading the F2 per-segment metadata per §8.2 row 5) + emit `WorkflowEventClass.RESUMPTION` carrying `resumption.kind = segment_replay` (§8.1); per-segment idempotent dedup against the F2 ledger; verify the 4 F3 floors (§6.1); update the engine-class consumers; **materialize `test_u_rt_95_hitl_pause_trigger_durable_async_full_execution_path` into a real durable-async e2e** (its `:347` `@pytest.mark.skip` body is currently VACUOUS — `:368-375` — so un-skipping ALONE is a false green; see the functional AC) — WAL_SEGMENT is the canonical DURABLE_ASYNC class (`test_u_rt_95:129-131` maps RECONCILER_LOOP / WAL_SEGMENT to §18.1 DURABLE_ASYNC cells) — then remove the skip + correct the skip-reason + flip the Path-(i) fork (`class_1_fork_path_i…`) status CLOSED-DEFERRED → CLOSED-BUILT. One coherent CP-package engine-materialization unit. The durable segment-log substrate is U-RT-121 (runtime, cross-axis); the recovery-loop firing is U-CP-95 (separate — §6 O-CP-4). **Does NOT fire the engine recovery loop** (that is U-CP-95).
...
**Surfaces affected.** `harness_cp.workflow_driver` — `_IN_SCOPE_ENGINE_CLASSES`; the `:1445` dispatch fork + a segment-replay resume_at helper (reads the F2 per-segment metadata, §8.2 row 5 — a CP→IS read, NOT a read of the runtime substrate, so no CP→RT edge); `resumption.kind=segment_replay`. The engine-class consumers (overlay/matrix/selection). `harness-runtime/tests/integration/test_u_rt_95_…py` (un-skip + skip-reason correction). `.harness/class_1_fork_path_i_durable_async_engine_class_materialization.md` (status flip). The relevant CP substitution rows.
...
**Acceptance criterion (functional).** A workflow with `engine_class=WAL_SEGMENT` no longer raises `EngineClassNotYetMaterializedError`; on re-entry it replays from the last durable segment (`resume_at` advances over the materialized segment prefix) and emits `WorkflowEventClass.RESUMPTION` with `resumption.kind=segment_replay`; post-resumption `step.boundary` spans carry `resumption.is_replay: bool` (§8.3 item 2 — span re-emission cadence deferred under F2-12 [CF-1] per §8.3 item 3, so the AC asserts the flag value); per-segment dedup against the F2 `idempotency_key` does not double-apply a segment. **Path-(i) `test_u_rt_95` is MATERIALIZED into a real durable-async e2e and PASSES — un-skipping alone is a FALSE GREEN.** The currently-skipped `test_path_i_durable_async_pause_trigger_returns_paused` body is **vacuous** (`harness-runtime/tests/integration/test_u_rt_95_hitl_pause_trigger_durable_async_full_execution_path.py:368-375` — only `_ = patched_runtime, tmp_path`; NO `execute_workflow` call, NO paused/resume assertions; its docstring marks it "documentation of the intended e2e shape for the follow-on arc"). So merely removing `@pytest.mark.skip` proves NOTHING (`[[test-bypass-as-runtime-truth-pattern]]`; Codex pre-merge catch). This AC therefore **requires authoring the real test body**: drive a WAL_SEGMENT workflow through `execute_workflow` to a `RunStatus.PAUSED`, then resume, asserting the full DURABLE_ASYNC pause-trigger cycle (paused → resume) **by execution**; and a contrasting check that the test FAILS if WAL_SEGMENT is not materialized (proving it exercises the real path, not a tautology). ONLY after this non-vacuous test passes are the `@pytest.mark.skip` removed AND the Path-(i) fork status flipped (CLOSED-DEFERRED → CLOSED-BUILT).

**Acceptance criterion (F3 capability-floors — §6.1).** Four sub-ACs as in U-CP-93, against WAL_SEGMENT: (i) durable-replay-across-restart by e2e (a fresh driver over the same segment log — the durable proof composes with U-RT-121's segment-log substrate); (ii)-(iv) idempotency / lease / 8-lifecycle-events by integration test.
...
| U-CP-94 (WAL_SEGMENT materialization + `test_u_rt_95` un-skip) | CP | (none) — foundational CP-package driver materialization |
...
| EVENT_SOURCED_REPLAY → §18.1 DURABLE_ASYNC cell? (design OQ-2) | RESOLVED — NO (`test_u_rt_95:129-131`); only U-CP-94 (WAL_SEGMENT) un-skips `test_u_rt_95` |
...
| Net delta | +3 NEW units (U-CP-93 EVENT_SOURCED_REPLAY `(none)`; U-CP-94 WAL_SEGMENT + `test_u_rt_95` un-skip `(none)`; U-CP-95 WAL_SEGMENT recovery-loop firing branch `[U-CP-94]`); +1 cross-axis edge (U-RT-122 → U-CP-95, RT→CP downstream — recorded at the E aggregate §3.5); +§4.3 (8 coverage rows); +1 §6 open-item (O-CP-4, R-CXA-2-owned-by-E-2, recorded-not-gated); ZERO spec amendment, ZERO new contract ID, ZERO CP-spec fork (closed `EngineClass`/`ResumptionKind` enums consumed; architect rec §3 X-AL-3-clean) |
...
| E-impl sequence (companion §6) | E-impl-1 = U-CP-93 (EVENT_SOURCED_REPLAY deterministic event-replay; no recovery-loop firing; does NOT un-skip `test_u_rt_95`) → E-impl-2 = U-CP-94 + U-RT-121 + U-CP-95 + U-RT-122 (WAL_SEGMENT segment-replay + `test_u_rt_95` un-skip + durable segment-log substrate + firing branch + R-CXA-2 durable go-live e2e) → (separate) E-spec-3 → E-impl-3 = RECONCILER_LOOP |
