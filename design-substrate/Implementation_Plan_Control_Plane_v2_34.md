...
**Scope.** Materialize the `WAL_SEGMENT` engine class as impl against cleared C-CP-07/08: widen `_IN_SCOPE_ENGINE_CLASSES` to include `WAL_SEGMENT`; add a driver dispatch fork at `:1445` computing `resume_at` by **replay from the last durably-written WAL segment** (reading the F2 per-segment metadata per §8.2 row 5) + emit `WorkflowEventClass.RESUMPTION` carrying `resumption.kind = segment_replay` (§8.1); per-segment idempotent dedup against the F2 ledger; verify the 4 F3 floors (§6.1); update the engine-class consumers; **un-skip `test_u_rt_95_hitl_pause_trigger_durable_async_full_execution_path`** (`:347` `@pytest.mark.skip`) — WAL_SEGMENT is the canonical DURABLE_ASYNC class (`test_u_rt_95:129-131` maps RECONCILER_LOOP / WAL_SEGMENT to §18.1 DURABLE_ASYNC cells) — and correct the skip-reason text + flip the Path-(i) fork (`class_1_fork_path_i…`) status CLOSED-DEFERRED → CLOSED-BUILT. One coherent CP-package engine-materialization unit. The durable segment-log substrate is U-RT-121 (runtime, cross-axis); the recovery-loop firing is U-CP-95 (separate — §6 O-CP-4). **Does NOT fire the engine recovery loop** (that is U-CP-95).
...
**Acceptance criterion (functional).** A workflow with `engine_class=WAL_SEGMENT` no longer raises `EngineClassNotYetMaterializedError`; on re-entry it replays from the last durable segment (`resume_at` advances over the materialized segment prefix) and emits `WorkflowEventClass.RESUMPTION` with `resumption.kind=segment_replay`; per-segment dedup against the F2 `idempotency_key` does not double-apply a segment. **Path-(i) `test_u_rt_95` is un-skipped and PASSES** — the full DURABLE_ASYNC pause-trigger cycle through `execute_workflow` against the materialized WAL_SEGMENT class returns paused / resumes as the test asserts (the by-execution proof, not a grep); the skip-reason is removed and the Path-(i) fork status is flipped to CLOSED-BUILT.
