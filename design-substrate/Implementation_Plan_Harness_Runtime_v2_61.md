...
### §0.3-ter Round-3 realization — the serialization boundary

`[HIGH]` Out-of-family review round 3 found that retaining a LIVE exception on
`MemoryCaptureResult` regressed `model_dump_json()` (`PydanticSerializationError`)
and `model_json_schema()` (`PydanticInvalidForJsonSchema`) — both of which worked
before this leg. **Round 2's grounding was too narrow and is retracted here:** it
verified only that no CURRENT in-repo consumer serializes the model, which is
true and beside the point. The model is **publicly exported**, so those two APIs
are part of its contract regardless of who calls them today, and the schema break
was at the CLASS level — every instance, not only conflicting ones. This is the
serialization-boundary hazard class the workspace polices in the `B-101` /
`B-107` lineage, and it is the same defect shape: a claim scoped to observed
callers rather than to the reachable surface.

**Shape chosen: exclusion, not a serializer.** `SkipJsonSchema[...]` plus
`Field(exclude=True)`. A `field_serializer` emitting a string was the real
alternative and was rejected because `failure_reason` ALREADY carries
`f"{type(exc).__name__}: {exc}"` — a serialized copy would be a second, lossier
authority for one fact, free to drift. The deciding test is that exclusion loses
NOTHING: the JSON projection already states the whole failure, and the exception
object exists for chaining, which has no JSON meaning at all. Attribute access is
unaffected, and the witness asserts that explicitly so exclusion is not read as
...
2. **(The capture boundary — discriminate as a VALUE, not by propagating.)** `[HIGH]` `EpisodicMemoryCapture._capture` folds the ledger refusal into a `MemoryCaptureResult(status=FAILED)` whose only carrier is a `failure_reason` STRING; the exception is deliberately not retained, so **no honest discriminator survived that boundary**. Add a closed `MemoryCaptureFailureKind` enum (`LEDGER_CONFLICT` / `STORE_IO`) and an optional `failure_kind` field on `MemoryCaptureResult`, set on every `FAILED` result and `None` on the CAPTURED path by construction. The span's `failure_class` follows the same split — the **RESIDUAL** for a ledger refusal (not an I/O fault; per C-MEM-19 v1.3 a residual report is the ABSENCE of a claim rather than a wrong one), `io_failure` for everything else — which keeps the inner capture span and the outer standard-tool span AGREEING about the same event, the property `B-88` round 2 introduced the store subtype to protect.
   ...
   **The result ALSO retains the ledger exception itself**, as `failure_cause`, typed as the CONCRETE `MemoryOperationIdempotencyConflictError` and set exactly when `failure_kind is LEDGER_CONFLICT` — enforced BOTH WAYS by a model validator, because the missing-cause direction is the dangerous one: the executor's `raise ... from result.failure_cause` would degrade to `raise ... from None`, which **suppresses** the context rather than merely omitting it, actively hiding the origin. This is what lets AC #2's chaining requirement hold on the capture surface, and it is required for SYMMETRY: the two direct-append surfaces chain naturally (they catch), so without it the same substrate event would lose its traceback origin depending on which surface caught it. `model_config` gains `arbitrary_types_allowed=True` — a FIELD-TYPE permission that weakens neither `extra="forbid"` nor `frozen=True` — and the field is **EXCLUDED from serialization and SKIPPED in schema generation** (`SkipJsonSchema[...]` + `Field(exclude=True)`). Every consumer of a `FAILED` result raises immediately, so the retained traceback does not outlive the dispatch that produced it. **The exclusion is required, not cosmetic:** `MemoryCaptureResult` is PUBLICLY EXPORTED, so `model_dump_json()` and `model_json_schema()` are part of its API contract, and a bare live-exception field regresses BOTH from working to raising — the schema break at the CLASS level, affecting every instance. **A `field_serializer` emitting a string was the alternative and is REJECTED:** it would publish a second, lossier copy of a fact `failure_reason` already carries verbatim, giving two drifting authorities for one concept. Exclusion loses nothing — the JSON projection already states the whole failure via `status` + `failure_kind` + `failure_reason`, while the exception OBJECT serves a purpose with no JSON meaning at all (you cannot `raise ... from` a deserialized string).
