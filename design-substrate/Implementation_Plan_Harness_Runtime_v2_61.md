...
**#2 — All three executor surfaces, each EXECUTED.** `[HIGH]` One witness per surface (capture / standard-tool-call / redaction), each driving a REAL executor over a real filesystem store and asserting the new type. The capture witness additionally asserts the re-type **chains** the ledger's exception (`__cause__`) rather than discarding it.
...
**#8 — PD-8 mutation probes, all confirmed RED then restored.** `[HIGH]` (a) narrow the tuple by the new member; (b) broaden the tuple to the memory family base; (c) de-list from the classifier while keeping the waiver (the consistency-rule probe); (d) collapse the capture-boundary discriminator so every `FAILED` reports `STORE_IO`; (e) re-parent the new type under `MemoryToolExecutionStoreError`. Each must fail a NAMED witness, and (c) and (e) must fail the consistency and residual witnesses specifically.
