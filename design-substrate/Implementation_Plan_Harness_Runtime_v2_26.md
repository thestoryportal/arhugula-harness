# Implementation Plan — Harness Runtime v2.26

## Change-note (v2.25 → v2.26)

**Scope of revision.** Narrow-scope canonical-reading amendment at U-RT-94 AC #13 in-place text absorbing CP spec v1.18 → v1.19 NEW §25.2.Y `HITLEscalationBrief.fail_detail_hash` Optional widening. v2.25 §7.2 NEW AC #13 declared "`fail_detail_hash` parallel posture preserved per v1.18 change-note (no parallel widening at v1.18)" — text becomes stale at v1.19 publication. v2.26 amends AC #13 text in-place to reflect the v1.19 widening. NO new units; NO new cluster; NO DAG topology change. Co-published with CP spec v1.19 + CP plan v2.23 + harness-cp impl + harness-runtime impl + workspace `CLAUDE.md` row bumps. 2026-05-25.

**v2.25 substantive content preserved verbatim.** All v2.25 NEW §6 L9-quaterdecies cluster (U-RT-96/97/98) preserved unchanged. v2.25 §7.1 + §7.3 preserved verbatim. §7.2 AC #13 text amended in-place at v2.26 §1 below; all other §7.2 ACs preserved verbatim.

**Source of fix.** CP spec v1.18 → v1.19 NEW §25.2.Y publication this arc + advisor pre-substantive-work flag re AC #13 staleness.

**Single amendment site (1 in-place AC text amendment).**

| Site | Amendment shape |
|---|---|
| **§1 (NEW at v2.26)** | U-RT-94 AC #13 text amended in-place: "`fail_detail_hash` parallel posture preserved per v1.18 change-note (no parallel widening at v1.18)" → "`fail_detail_hash=None` directly per CP spec v1.19 §25.2.Y widening (sentinel `\"0\" * 64` dropped post-v1.19 absorption)". |

**Status posture.** Proposed (v2.25) → **Proposed (v2.26)**. Fidelity-pure in-place AC text amendment. NO net AC change; NO net unit change.

---

## §1 — U-RT-94 AC #13 in-place text amendment (NEW at v2.26)

**v2.26 AC #13 canonical text (replaces v2.25 §7.2 AC #13 text):**

> NEW AC #13: HITLEscalationBrief construction at §14.8.8.1 step 1 uses `fail_class=None` AND `fail_detail_hash=None` directly per CP spec v1.18 §25.2.X widening + CP spec v1.19 §25.2.Y widening (no sentinel value pattern at either field; the `"0" * 64` sentinel at the pre-v1.19 impl `hitl_gate_composer.py:985` is dropped post-v1.19 absorption — composer constructs both Optional fields as None directly). Test fixtures at `harness-runtime/tests/test_lifecycle_hitl_pause_trigger_helpers.py::_build_brief` are aligned with the post-v1.19 production shape (both fields None).

The v2.25 file body at §7.2 U-RT-94 AC #13 line is NOT edited — delta-only plan-chain preservation discipline preserved per v2.18 §1 + v2.22 §3 precedent. Consumers reading the delta chain interpret v2.25 §7.2 AC #13 AS canonically supplemented at v2.26 §1 above.

---

## §2 — v2.25 change-note preserved verbatim
