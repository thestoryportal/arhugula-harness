...
3. The disable is MTC-SCOPED per ratified gate item 8 (codex rounds 2/9/21 — twice-contested scope resolved to the ratified letter): at MULTI_TENANT_COMPLIANCE with the flag ON, both surfaces are disabled; an EXPLICIT lower-tier `fail_closed=true` is REJECTED at config validation per U-RT-134 criterion 3 (interim — the NAMED RESIDUAL stands for operator disposition: extend the disable, or define propagation; until then the half-honored state is unrepresentable and the ten-site policy still governs all real dispatch paths at MTC). Flag-OFF and non-MTC behavior BYTE-PRESERVED — the v1.99 contract stands verbatim (opt-in-default-off; 5m-TTL-only keep-alive; swallow-all best-effort posture).
