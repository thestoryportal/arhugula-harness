...
**Depends on:** [U-RT-102 (the dispatch surface it extends), U-RT-134 (the record config fields + bootstrap validation), U-OD-55 (cross-axis: OD — the record carrier/canonical bytes/signing API), U-RT-138 partial (the record-authentication component shared with inspect)].
...
- LEDGER-BINDING ESTABLISHMENT IS U-RT-139's OBLIGATION FOR EXISTING SIDECARS (codex round-44): before record authoring on a sidecar lacking a genesis identity, the migration ESTABLISHES it (writes the genesis line under the same B-46 exclusion + all-or-nothing discipline) and every subsequent record binds to it; verification and retag COMPARE the record's binding against the target sidecar's genesis identity (a mismatch is the cross-sidecar rejection — witness `test_record_from_other_sidecar_rejected_by_binding_compare`, pairing with U-OD-55's carrier-level mismatch test which alone could pass on synthetic ids).
...
New (L2-within-delta): U-RT-139 (← U-RT-102, U-RT-134, U-RT-138 partial, U-OD-55 cross-axis — the record-mode home; breaks the U-RT-102→U-RT-134→U-RT-103→U-RT-102 cycle codex round-40 exposed)
```
