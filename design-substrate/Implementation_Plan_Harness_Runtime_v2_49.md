...
3. At `persona_tier == MULTI_TENANT_COMPLIANCE`, config validation at bootstrap REJECTS all three of: (i) explicit `audit_signing_fail_closed=false` (OD v1.34 §21.2.3 row 3); (ii) fail-closed ON — explicit or per-persona-defaulted — WITHOUT a configured `SigningBackend` (§21.2.3 row 4 zeroth-site invariant; the `unsigned:*` placeholder is a policy violation at MTC); (iii) `tenant_id=None` (§21.2.1 row 6 bootstrap invariant). All three surface via the existing `RT-FAIL-CONFIG` (permanent) taxonomy row.
4. The rejections are bootstrap/config-validation events, not runtime catch-site events; non-MTC tiers accept all three configurations (byte-preserved behavior).

**Tests (mutation-probed per PD-8):** **Witness (a) — flag config-validation:** `test_mtc_explicit_false_rejected_at_config_validation`, `test_mtc_fail_closed_on_without_backend_rejected_at_bootstrap`, `test_env_only_override_honored_through_both_loaders` (mutation probe: removing EITHER loader registration fails the test). **Witness (c) — tenant bootstrap invariant:** `test_mtc_tenant_id_none_rejected_at_config_validation`.
...
**Tests (mutation-probed per PD-8):** `test_converter_call_sites_thread_step_context_tenant_id_producing_five_segment_message` (pairs with CP witness (a) / OD witness (a)), `test_tenant_absent_call_sites_byte_identical_end_to_end` (pairs with OD witness (b)).
...
**Files affected (logical):** the `harness_runtime.admin.inspect` surface + its CLI/config input parsing.
...
2. At MULTI_TENANT_COMPLIANCE, backend inputs ((iii)+(iv)) are REQUIRED, OR the inspection result is an EXPLICIT `UNVERIFIED` disposition with a NONZERO exit — silent hash-only success PROHIBITED. At lower tiers with no verification inputs supplied, the pre-v1.101 hash-only inspection behavior is PRESERVED VERBATIM.
