...
§0 (this change note); §1 (NEW U-RT-134..U-RT-139 + the U-RT-102 amendment); §2 (DAG delta); §3 (coverage delta). All v2.48-and-earlier unit bodies (U-RT-01..U-RT-133) PRESERVED VERBATIM except U-RT-102 as amended below (dispatch-only; record modes at U-RT-139).

### §0.4 Scope + witness discipline

The Runtime-owned PD-8 witness obligations of the v1.101 change-note ((a)–(e), each mutation-probed per Workflow v1.18 PD-8) home as `Tests:` criteria: (a) + (c) → U-RT-134; (b) → U-RT-135; (d) → U-RT-138; (e) → U-RT-102 (amended); (f) + the record-mode witnesses → U-RT-139 (NEW). Cross-axis co-land pins (recorded, not DAG edges): U-RT-136 ⊕ CP plan v2.38 U-CP-73; U-RT-137 ⊕ CP plan v2.38 U-CP-72 ⊕ OD plan v2.29 U-OD-30 — all land in the one B-51/B-52/B-54 impl arc per fork gate item 10.
...
3. Config validation at bootstrap REJECTS, at EVERY persona tier, the RESOLVED `audit_signing_fail_closed` flag ON without a configured `SigningBackend`; at EVERY tier that supplies `audit_cutover_record_path` it additionally REJECTS a pinned `audit_cutover_record_key_id` sharing its RESOLVED backing material (ARN) with any row-signing key (codex rounds 38/45 — the separation checks apply WHENEVER a record is configured, not only at MTC: a lower-tier record consumed by the live writer is equally forgeable under a shared key; the spec's physical-distinctness rule enforced at BOOTSTRAP and inspection, not only at the migration test; witnesses `test_bootstrap_rejects_record_key_sharing_row_material` and `test_record_algorithm_authority_is_the_mapping` (a record whose stored algorithm differs from the pinned mapping entry's algorithm is REJECTED even when its key_id matches — codex round-39; mutation probe: trusting the record's own algorithm passes the alternate-algorithm forgery and fails)). A lower-tier explicit `true` WITH a backend is ACCEPTED (the ratified non-MTC opt-in — the ten-site policy, typed boundary, and redaction path honor it; witness `test_lower_tier_explicit_true_with_backend_accepted_and_sites_fail_closed`); ONLY the prewarm/keepalive swallow boundary keeps its v1.99 posture there until `B-55` dispositions (the fork is HELD at that register row — codex rounds 2/9/21/34/36; not re-decided per review round) (§21.2.3 row 4 is TIER-AGNOSTIC — a solo/team `fail_closed=true` opt-in without a backend would be silently ineffective, `unsigned:*` placeholders emitted without raising); ADDITIONALLY at `persona_tier == MULTI_TENANT_COMPLIANCE` it REJECTS (i) explicit `audit_signing_fail_closed=false` (§21.2.3 row 3) and (ii) a `tenant_id` that is `None` OR refused by the OD-exported normalizer (empty string; reserved `"_single"` — validation DELEGATES to the same U-OD-30 normalizer signing uses, codex round-4 P1). Rejections follow the PARTITION (codex rounds 46/48/49): missing v1-required-at-MTC inputs (absent backend / tenant / record inputs) → typed `IncompatibleConfigVersion` via `RT-FAIL-CONFIG-VERSION`; invalid v2 VALUES (explicit `false`, refused tenant token, key-material sharing, mapping-algorithm mismatch) → `RT-FAIL-CONFIG`.
4. The rejections are bootstrap/config-validation events, not runtime catch-site events; every OTHER configuration at non-MTC tiers is byte-preserved (flag unset/OFF behavior unchanged; flag ON WITH a backend accepted at every tier per criterion 3; at lower tiers only the prewarm/keepalive boundary keeps v1.99 posture pending B-55) — EXCEPT the redaction-token signing path (codex round-26: OD v1.34 §21.2.3 row 6 makes an absent backend on THAT path a typed failure at every tier by ratified design — raw values must never persist unsigned — so a backend-less deployment that constructs the redaction map raises where it previously placeholder-signed; the carve-out is explicit, not a silent compat break). MTC bootstrap additionally REQUIRES the record inputs (`audit_cutover_record_path` + `audit_cutover_record_key_id`) — greenfield initialization signs the empty record with them (witness extension: `test_mtc_bootstrap_without_record_inputs_rejected` (asserting the `RT-FAIL-CONFIG-VERSION` typed `IncompatibleConfigVersion` path for EVERY missing-v1-input case — absent backend, tenant, or record input (path / key / binding id) on a v1-shaped MTC config, parametrized; invalid v2 VALUES — explicit `false`, refused tenant token, key-material sharing — assert generic `RT-FAIL-CONFIG`; codex rounds 46/48: the partition IS the public discriminator)). The LEDGER BINDING is the CONFIG-anchored `audit_ledger_binding_id` (codex rounds 44/47 — trusted substrate, never a sidecar-resident genesis line, which could be copied with the record): REQUIRED with the other record inputs at MTC; greenfield initialization signs it into the empty record; witness `test_greenfield_empty_record_signs_configured_binding`.
...
3. U-RT-137 OWNS the runtime redaction call-site rewire (codex rounds 41/44 — the COMPOSER widening is OD-owned at U-OD-30 acc #19, but the two production invocations live in `harness-runtime`): `AuditLedgerRedactionTokenMap`'s compose calls (`redaction_token_audit_map.py:115/:131`) pass `self._tenant_id` through `compose_redaction_token_audit_entry` to signing. End-to-end witness `test_redaction_map_append_signs_five_tuple_with_tenant` (an MTC-configured map append produces a five-segment-signed entry whose tenant tag matches — mutation probe: dropping the call-site tenant argument leaves the four-tuple and fails).
...
**Depends on:** [U-RT-47 (the landed `harness-inspect` admin surface), U-RT-134 (the record-key/binding RuntimeConfig carriers + bootstrap validation the inspect path consumes — codex round-49), U-OD-55 (cross-axis: OD — the verification API this surface invokes), U-CP-44/U-CP-45 (cross-axis: CP — the injected-verifier Protocol + CP-owned result boundary this unit's walk adapter implements; co-land)].
...
| §13.4 `harness migrate-audit-sidecar` row + count invariant 5 → 6 (surface F / B-53) | U-RT-102 (amended — dispatch) + U-RT-139 (NEW — record modes) |
