...
3. Config validation at bootstrap REJECTS, at EVERY persona tier, the RESOLVED `audit_signing_fail_closed` flag ON without a configured `SigningBackend` (§21.2.3 row 4 is TIER-AGNOSTIC — a solo/team `fail_closed=true` opt-in without a backend would be silently ineffective, `unsigned:*` placeholders emitted without raising); ADDITIONALLY at `persona_tier == MULTI_TENANT_COMPLIANCE` it REJECTS (i) explicit `audit_signing_fail_closed=false` (§21.2.3 row 3) and (ii) `tenant_id=None` (§21.2.1 row 6 bootstrap invariant). All rejections surface via the existing `RT-FAIL-CONFIG` (permanent) taxonomy row.
...
**Tests (mutation-probed per PD-8):** **Witness (d) — inspect UNVERIFIED-nonzero:** `test_mtc_inspection_without_backend_inputs_exits_nonzero_with_explicit_unverified_disposition` (never silent hash-only success), `test_lower_tier_no_inputs_preserves_hash_only_behavior_verbatim`, `test_inspect_verification_writes_nothing_readonly_fixture`. **Forged-record rejection (codex round-3 P1, §13.5 row 4):** `test_forged_cutover_record_rejected_typed_never_treated_as_absent` — a cutover record failing authentication against the operator-designated record-signing key (or out-of-band anchor) is REJECTED with a typed error by BOTH the inspect verification path AND the `harness migrate-audit-sidecar` retag mode (mutation probe: downgrading the rejection to absent-record fallback fails the test — a forged record must never drive exemption, era selection, or retagging).
