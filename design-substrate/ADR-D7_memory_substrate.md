...
> **Update — 2026-07-09 (PR #914, `R-300-external-cli-oauth-routing`).** The external-CLI routing surface described in the two paragraphs of this section has since been ported into `arhugula-harness` `main` (this repo) as well, so the routing now lives in **both** repos — the deployed `thestoryportal/arhugula` and here — on the operator-ratified prefer-OAuth default posture. The present-tense framing above ("a separate existing substrate … the local `main` branch does not yet include that deployed external CLI routing surface") and the §Decision line "Existing external CLI routing remains the provider-construction authority" are accurate as the filing-head (`cc612ec8`) historical record but no longer describe HEAD: the routing is now in-repo, and the memory layer composes with the in-repo route (F2: memory retrieval reaches the CLI dispatch path). The commit-pinned filing-head note in the next paragraph remains a correct historical statement of the pre-port state.
