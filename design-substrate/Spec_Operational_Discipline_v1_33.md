...
| **Passthrough surfaces** | Every production writer that routes through `sign_audit_entry` gains the same optional passthrough so a composition root can reach it: the CXA-homed `cp_audit_to_od_audit` converter (CXA §2.3.7) and the U-OD-4/R-008 `compose_redaction_token_audit_entry` composer (+ its runtime `AuditLedgerRedactionTokenMap` carrier) |
| **Out of scope here** | Backend selection/construction/config (composition root, `B-47` remainder); rotation-aware key-period selection (`B-33`) — including `sign_rotation_pair`, which post-signs relabels `audit_signature_key_period` to real period values and therefore CANNOT take this seam without B-33's rotation-aware message binding (threading it naively would sign the `"DEPLOYMENT_BOUND"` token while stamping a different period attr — a metadata-inconsistent signature); signature *verification* surface (C-OD-21 exposes hash-chain integrity only — unchanged); tenant-scope binding into the signed message under a DEPLOYMENT-scoped key (`B-47` close-out item (f) — resolve jointly with the persistence design at the composition-root arc; a TENANT-scoped `key_id` already binds tenant identity via the canonical message's `key_id` segment) |
