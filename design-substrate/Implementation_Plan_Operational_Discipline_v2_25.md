...
**OD-3 (Composite Sampler) PARTIAL → RETIRE-READY at apply-arc close.** Substrate-criterion-B MET at v2.25 apply (sampler base_rate envelope now materializes §10.3 8-row table via `PER_CELL_BASE_RATE_ENVELOPE` consumer at `materialize_tracer_provider_stage`). Tail-keep-on-classification at OTLP collector boundary per OD spec §9.1 remains deferred per §9.3 implementer-discretion clause (out of scope at this arc per fork doc §1.4 (α) recommendation). Full RETIRED gates on deployment-time-opt-in exercise per X-AL-2 + sub-species 7 deployment-time-opt-in-gate precedent.

**OD-4 (Pre-Collector redaction SpanProcessor) PARTIAL → RETIRE-READY at apply-arc close.** Substrate-criterion-B MET at v2.25 apply (per-persona override toggle wired at `RedactionSpanProcessor` ctor + `materialize_span_processor_stage` threads `config.persona_tier` through; multi-tenant-compliance non-toggleability enforced via `MultiTenantOverrideRefusedError`). Opaque-token tokenization mode per OD spec §13.2 multi-tenant-compliance eval-grade pipeline remains deferred (strip-not-tokenize MVP scope-lock preserved at apply per fork doc §2.4 (α) recommendation). Full RETIRED gates on deployment-time-opt-in exercise per X-AL-2.

**Joint retirement event batch-36 filing owed.** OD-3 + OD-4 joint PARTIAL → RETIRE-READY transit; mirror-shape to OD-3 batch-34 + OD-4 batch-35 substrate-retirement single-PR-arc precedents but at within-OD-axis cross-row joint pattern (sibling within-OD-axis joint pattern to PR #18 OD-6 batch-33 + OD-3 batch-34 single-day arc). Owed at this arc close OR follow-on PR per operator-discretion timing.
...
| Successor | H_T-OD-3 + H_T-OD-4 PARTIAL → RETIRE-READY transit at batch-36 retirement event filing (co-published this arc OR follow-on arc per operator-discretion timing) |
