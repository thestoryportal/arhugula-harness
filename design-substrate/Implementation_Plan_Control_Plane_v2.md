# Implementation Plan — Control Plane (v2)

## Status block

| Field | Value |
|---|---|
| Artifact | `Implementation_Plan_Control_Plane_v2.md` |
| Status | **Proposed** (v2 pending P6-CK Iteration 2 clearance per `Project_Workflow_v1_5.md` §4.1.2; absorbs F2-CP-01 + F2-CP-02 per `Adversarial_Review_6.md` §3.3 + §3.4) |
| Date | 2026-05-14 |
| Phase | 6 — atomic implementation plan (Session 3 of 5 per `Project_Workflow_v1_5.md` §2.6); revision pass per `P6-CK_Iter1_Path_alpha_IS_CP_OD_Revision_Kickoff.md` |
...
| Source-set | `Spec_Control_Plane_v1.md` v1.2 §1–§24 (24 contracts: C-CP-01 through C-CP-24); `Implementation_Plan_Information_Substrate_v1.md` v1 (cross-axis IS substrate; U-IS-17 manifest); `Implementation_Plan_Action_Surface_v1.md` v1 (cross-axis AS substrate; U-AS-33 manifest); `Project_Workflow_v1_5.md` v1.5 §2.6 + §6.4 + §7; `implementation-planner` SKILL.md §1–§11; `Phase_6_Entry_Handoff.md` §3 inputs + §5 sequencing; `Phase_6_Session_3_Session_Prompt.md`; background substrate (consulted but not cited at units per SKILL.md §2): `Architectural_Design_Document_v1.md` v1.2 §3.2 + §5.2–§5.3, `PRD_v1_0.md` v1.0.1 R-CP-01 through R-CP-12, `Persona_Document_v1.md` v1 §3 + §6 + §10–§11; ADRs at F1 v1.2, F2 v1.2, F3 v1.1, F5 v1.1, D1 v1.1, D2 v1.1, D3 v1.2, D4 v1.1, D5 v1.3, D6 v1.1 |
| ODs applied | OD-S3-1.A (per-unit-cluster operator confirmation cadence); OD-S3-2.A (per-axis coverage matrix only at Session 3; aggregate cross-axis composition deferred to Session 5); OD-S3-3.A (per-unit cross-axis IS + AS dependency declarations annotated; Session 5 retroactive verification) |
| Entry authorization | `P6-CK_Iter1_Path_alpha_IS_CP_OD_Revision_Kickoff.md` §2 entry-gate verified |
| Exit gate | This v2 plan filed at `/mnt/user-data/outputs/`; per-plan coherence pass cleared at v2; Path α continues to OD plan v1 → v2 |
| Sub-mode | Revision pass (P6-CK Iteration 1 findings absorbed; v1 → v2; `## §0 Change note` section authored per SKILL.md §8) |
...
Single-pass revision absorbing two `Adversarial_Review_6.md` P6-CK Iteration 1 findings, both local to CP plan v1:

| Finding | Class | Resolution path absorbed | Affected sites |
|---|---|---|---|
| F2-CP-01 | 2 | U-CP-54 §24.1.A/B/C realignment per `Adversarial_Review_6.md` §3.3 per-namespace placement table + acceptance #5 Path (i) preserve | U-CP-54 acceptance #2 / #3 / #4 / #6 / #8 |
| F2-CP-02 | 2 | U-CP-21 `Implements:` retarget to C-CP-09 §9.1 + §4.1 coverage matrix update per Path (α) strict-narrow | U-CP-21 `Implements:` field; §4.1.5; §4.1.9 |
...
| §2.1–§2.3 Clusters 1–3 (U-CP-01 through U-CP-20) | No finding at units; no finding at C-CP-01 through C-CP-08 coverage |
| §2.4 Cluster 4 — U-CP-21 non-`Implements:` sections (scope, depends-on, inputs, files, signatures, acceptance criteria, tests, rollback boundary) | No finding at these sub-sections |
| §2.4 Cluster 4 — U-CP-22 onward through end-of-cluster | No finding |
| §2.5–§2.11 Clusters 5–11 | No finding |
| §2.12 Cluster 12 — U-CP-54 non-acceptance-criteria-table sections (scope, depends-on, inputs, files, signatures, tests, rollback boundary) | No finding at these sub-sections |
| §2.12 Cluster 12 — U-CP-55 | No finding |
| §3 dependency graph | No graph delta; node count + edge count + topological-sort order unchanged |
| §4.1.1–§4.1.4; §4.1.6–§4.1.8; §4.1.10–§4.1.24 | No finding at these contract-coverage sub-sections |
| §5 / §6 coherence pass (if present in v1 file body) | v1 coherence-pass results preserved as historical record per PRD v1.0 → v1.0.1 precedent; v2 audit results captured in this revision-pass session's Stage S.3 |

### §0.3 Sections revised

| Section | Revision shape | Resolves |
|---|---|---|
| U-CP-21 `Implements:` field | Retarget from `C-CP-05 §5.3 (engine.* namespace declaration; consumed at §5.2 per-class attribute composition and §9.2 resumption observable behavior)` to `C-CP-09 §9.1 (engine.* namespace declaration per three-attribute schema; consumed at C-CP-05 §5.2 per-class attribute composition and C-CP-09 §9.4 D6 ingestion contract)` | F2-CP-02 |
| U-CP-54 acceptance #2 (§24.1.A sub-table) | 6-namespace list realigned to spec §24.1.A canonical order (engine, topology, subagent, hitl, audit, validator.fail); sub-table label parenthetical extended to "specialization-layer namespaces; D6 §1.2 direct ingest" | F2-CP-01 |
| U-CP-54 acceptance #3 (§24.1.B sub-table) | 4-namespace list realigned to spec §24.1.B canonical order (fallback, retry, lease, harness.breaker); sub-table label rewritten to "F3-capability-floor lifecycle-event-attribute namespaces; D6 §1.2 lines 124–133 event sub-tree"; harness.breaker substrate-anchored annotation preserved | F2-CP-01 |
| U-CP-54 acceptance #4 (§24.1.C sub-table) | Single namespace replaced (harness.breaker → routing); sub-table label rewritten to "inheritance-composition note; NOT ingested at D6 §1.2"; inheritance-from-parent-llm.inference clause added | F2-CP-01 |
| U-CP-54 acceptance #5 | No textual change; "byte-exact verbatim transcription" claim now substantively true post-realignment | F2-CP-01 (consequence) |
| U-CP-54 acceptance #6 | Per-sub-table attribute breakdown re-expressed under realignment; total 63 invariant | F2-CP-01 |
| U-CP-54 acceptance #7 | No textual change; routing now correctly at §24.1.C consistent with inheritance claim | F2-CP-01 (consequence) |
| U-CP-54 acceptance #8 | Sub-table reference for harness.breaker bumped to §24.1.B post-realignment | F2-CP-01 |
| §4.1.5 row at "§5.3 lease.* + engine.* namespaces" | engine.* + U-CP-21 removed; row becomes "§5.3 lease.* namespace \| U-CP-11" | F2-CP-02 |
| §4.1.9 header + rows | Header parenthetical bumped from "(2 sub-sections)" to "(2 sub-sections; §9.1 covered by 2 units)"; new row "§9.1 engine.* attribute declarations \| U-CP-21" added; existing rows preserved | F2-CP-02 |

### §0.4 Coverage matrix delta

| Site | v1 | v2 |
|---|---|---|
| §4.1.5 row | `§5.3 lease.* + engine.* namespaces \| U-CP-11 + U-CP-21` | `§5.3 lease.* namespace \| U-CP-11` |
| §4.1.9 header | `Contract C-CP-09 (2 sub-sections)` | `Contract C-CP-09 (2 sub-sections; §9.1 covered by 2 units)` |
| §4.1.9 rows | 2 rows | 3 rows; new row `§9.1 engine.* attribute declarations \| U-CP-21` inserted between existing §9.1 ResumptionKind row and §9.2 row |

Aggregate spec-coverage invariant: every spec sub-section covered by ≥1 unit at v2 (preserved from v1; engine.* declaration now correctly traced to C-CP-09 §9.1 via U-CP-21).

### §0.5 Dependency graph delta

No delta. Affected units' `Depends on:` declarations unchanged:

| Unit | `Depends on:` v1 | `Depends on:` v2 |
|---|---|---|
| U-CP-21 | [U-CP-15] | [U-CP-15] (unchanged) |
| U-CP-54 | [U-CP-01, U-CP-07, U-CP-11, U-CP-21, U-CP-31, U-CP-37, U-CP-46, U-CP-47] | (unchanged) |

Aggregate DAG acyclic invariant preserved by inspection (no node, no edge, no topology change). Cross-axis edge count + targets unchanged.

### §0.6 Substrate-version-citation table

Substrate versions cited at v2 are unchanged from v1:

| Substrate | Version cited |
|---|---|
| CP spec | v1.2 (P5-CK-cleared post-F-iter2-01 Path A restructure) |
| ADR-F2 | v1.2 |
| ADD | v1.2 |
| PRD | v1.0.1 |
| ADR body-citations | F1 v1.2, F2 v1.2, F3 v1.1, F5 v1.1, D1 v1.1, D2 v1.1, D3 v1.2, D4 v1.1, D5 v1.3, D6 v1.1 |

Per Workflow v1.5 §7 use-latest-version body-citation-alignment.

### §0.7 Status

`Status: Proposed` preserved at v2 per `implementation-planner` SKILL.md §8 (analog to spec and PRD post-CK clearance patterns). Bump to `Status: P6-CK-cleared` on P6-CK Iteration 2 CLEARED disposition.

### §0.8 Forward-flagged concerns (not absorbed)

| Concern | Disposition | Routing |
|---|---|---|
| §4.1.9 existing row "§9.1 ResumptionKind 5-class taxonomy \| U-CP-19" mislabels CP spec §9.1 (canonical content per spec v1.2 line 788: "Three engine.* attribute declarations") | Not amended at v2 per Path (α) strict-narrow scope discipline | Candidate finding for P6-CK Iteration 2 |
| U-CP-19 declares `ResumptionKind` 5-class enum citing `Implements: [C-CP-09 §9.1]` with acceptance #1 "per C-CP-09 §9.1 verbatim" — spec §9.1 does not declare ResumptionKind; clean spec-trace anchor for ResumptionKind not located in C-CP-09 (potential spec-extension at U-CP-19 or under-decomposition at C-CP-09) | Not amended at v2 per `P6-CK_Iter1_Path_alpha_IS_CP_OD_Revision_Kickoff.md` §6.2 scope-expansion anti-pattern | Candidate finding for P6-CK Iteration 2; may require Phase 5 spec back-flow per SKILL.md §2 consequence (1) if confirmed as spec-shaped gap |
| F2-CXA-04 (CXA doc §6.4 propagates F2-CP-01 verbatim-inheritance claim across axes) | Path α scope is plan revisions only | Path β kickoff per `P6-CK_Iter1_Path_beta_CXA_Revision_Kickoff.md` |
...
**Acceptance criteria:**
1. `F2JoinKind` declares exactly three values per C-CP-08 §8.1 verbatim discrimination.
...
5. This unit is the **R-CP-07-satisfying contract** per spec §8.1.
...
**Acceptance criteria:**
1. `ResumptionKind` declares exactly five values per C-CP-09 §9.1 verbatim.
2. `RESUMPTION_KIND_BINDINGS` declares 1:1 mapping `EngineClass → ResumptionKind` per §9.1 verbatim.
...
**Acceptance criteria:**
1. `PER_RESUMPTION_OBSERVABLE_BEHAVIOR` declares exactly five entries per C-CP-09 §9.2 verbatim.
...
#### §4.1.8 Contract C-CP-08 (2 sub-sections)

| Sub-section | Covering units |
|---|---|
| §8.1 R-CP-07 F2-substrate-join contract | U-CP-18 |
| §8.2 F2 six-field shape preservation | U-CP-18 |

#### §4.1.9 Contract C-CP-09 (2 sub-sections; §9.1 covered by 2 units)

| Sub-section | Covering units |
|---|---|
| §9.1 ResumptionKind 5-class taxonomy | U-CP-19 |
| §9.1 engine.* attribute declarations | U-CP-21 |
| §9.2 per-resumption observable behavior | U-CP-20 |
...
| Field | Value |
|---|---|
| Status | Filed at `/mnt/user-data/outputs/Implementation_Plan_Control_Plane_v1.md` (Phase 6 Session 3 close) |
| Aggregate adversarial review | Deferred to Phase 6 Session 5 close per `Phase_6_Entry_Handoff.md` §1.1 OD-6-4.A; standalone P6-CK runs against cross-axis composition output |
| Operator action | Push from `/mnt/user-data/outputs/` to `/mnt/project/Implementation_Plan_Control_Plane_v1.md` at session close |
| Next session | Phase 6 Session 4 (Operational Discipline axis) per `/mnt/user-data/outputs/Phase_6_Session_4_Session_Prompt.md` |
| Exit-gate verification | 9 / 9 criteria met per Session 3 prompt §7; §5 coherence pass returns ✅ PASS at all 5 audit dimensions; cross-cutting concerns 4 / 4 covered |
| Unit count | 55 atomic units across 9 clusters |
| Contract coverage | 24 / 24 CP-axis contracts (107 / 107 sub-sections) |
| Within-axis edges | 124; acyclic DAG verified at §3.4 |
| Cross-axis edges | 60 (36 IS + 24 AS); unidirectional per OD-S3-3.A |

**Plan filed.** v1 implementation plan for CP axis closes Phase 6 Session 3.
