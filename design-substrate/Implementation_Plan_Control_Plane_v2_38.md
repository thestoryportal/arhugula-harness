...
- **(§3 row 1.)** The walk's step-2 per-entry verification mechanics are performed via the OD v1.34 §21.2.2 backend-aware API (resolver keyed on stored `(algorithm, key_id)`; era decided by the authenticated cutover record; tenant scope as verifier input with the §21.2.1 row-2 normalization; typed taxonomy) — bound, not restated (definition site: OD plan v2.29 U-OD-55).
