...
| **CXA** — Cross-Axis Composition | 94 canonical cross-axis relationships across 7 composition buckets per `Cross_Axis_Composition_Document_v2_5.md` §2.3 (24 genuine typed seams; 46 convention-level; 24 phase-2-runtime); §2.3.7 CP→OD bucket extended 1→2 typed seams at v2.5 (U-CP-28→U-OD-00 sub-agent dispatch + U-CP-46→U-OD-00 HITL gate response sharing `cp_audit_to_od_audit` converter) per U-RT-60 HITL gate composer arc; first cross-axis back-edge per U-RT-59 Fork 2 Path D landing |
...
| OD | `Spec_Operational_Discipline_v1_7.md` (v1.7 — Form A NOTE state-transition over v1.6 — §24.5 helper-materialization NOTE deferred → RESOLVED; `compute_entry_hash` materialized at `harness-od/src/harness_od/audit_ledger_types.py`; converter at `harness-cxa/` refactored to import per F2-04 follow-on arc closure, 2026-05-20; v1.6 §24.4 + v1.5 C-OD-24 contract preserved; OD contract count unchanged at 24) |
...
| OD | `Implementation_Plan_Operational_Discipline_v2_13.md` | 35 units (U-OD-00, U-OD-01 – U-OD-34); v2.8–v2.10 prior Class 1 + FF-2 + FF-3 landings; v2.11 — 7c-prereq Form A OD-outbound placeholder carrier resolution (2026-05-16); v2.12 — U-RT-59 Fork 2 implementation arc absorption (C-OD-24 citation + X-AL-3 retirement at v1.5/v2.12 co-publication); v2.13 — F2-04 follow-on arc closure: `compute_entry_hash` materialized at OD axis package per OD spec v1.7 §24.5 RESOLVED (2026-05-20) |
| CXA | `Cross_Axis_Composition_Document_v2_5.md` | 94 canonical cross-axis relationships (7 composition buckets with the §2.3.7 CP→OD bucket at v2.5 carrying 2 typed seams); v2.3 — 7c reclassification: every edge re-tagged genuine-typed-seam (24) / convention-level (46) / phase-2-runtime (24) / spurious (10 struck); v2.4 — new §2.3.7 CP→OD bucket with U-CP-28 → U-OD-00 typed seam (class G, Pattern P1) (2026-05-20); v2.5 — §2.3.7 extended 1→2 typed seams: U-CP-46 → U-OD-00 HITL gate response audit-write (class G, Pattern P1) sharing the `cp_audit_to_od_audit` converter at `harness-cxa/` per U-RT-60 HITL gate composer arc (2026-05-20) |
