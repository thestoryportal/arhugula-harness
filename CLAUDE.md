...
| Discipline | When |
|---|---|
| **Transcript-brief review at decision-forks + pre-done** (transcript-aware half) | Before committing to an interpretation/approach, and before declaring done. Spawn ONE fresh-context Agent reviewer handed a written brief of the session — the goal, the fork or claimed-done state, the evidence, your candidate (a `fork` subagent, which inherits the full conversation, is the stronger form where the venue supports it); the one reviewer that sees the session's context; value is front-loaded. Skip on mechanical steps + repeated reviews on a settled path. Pairs with Codex below: the two are **decorrelated** (brief-reviewer = Claude-on-the-session-brief; Codex = out-of-family on the artifact) — the strongest signal is when they **disagree** (surface it). (Renamed at U-SR-06, charter WR-13, from prose that named an unprovisioned advisor tool no venue ever exposed.) |
