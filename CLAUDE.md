...
| Discipline | When |
|---|---|
| **Transcript-brief review at decision-forks + pre-done** (transcript-aware half) | Before committing to an interpretation/approach, and before declaring done. Spawn ONE fresh-context Agent reviewer handed a written brief of the session — the goal, the fork or claimed-done state, the evidence, your candidate (a `fork` subagent, which inherits the full conversation, is the stronger form where the venue supports it); the one reviewer that sees the session's context; value is front-loaded. Skip on mechanical steps + repeated reviews on a settled path. Pairs with Codex below: the two are **decorrelated** (brief-reviewer = Claude-on-the-session-brief; Codex = out-of-family on the artifact) — the strongest signal is when they **disagree** (surface it). (Renamed at U-SR-06, charter WR-13, from prose that named an unprovisioned reviewer tool no venue ever exposed.) |
| **`just codex-review` as the default out-of-family code/diff reviewer** (artifact half) | The default reviewer for a concrete diff/artifact pre-merge — `just codex-review` (gpt-5.6-sol at medium reasoning effort, pinned per invocation rather than inherited from `~/.codex/config.toml` — operator direction 2026-09-15, token cost; GPT-5.6 flagship tier, on the operator's ChatGPT subscription, $0 metered, X-AL-1 H_E dev tooling, NOT H_T's OpenAI provider; upgraded from gpt-5.5 2026-07-13 once GPT-5.6 shipped — needs codex-cli ≥0.144.3, `codex update` if the banner errors "requires a newer version of Codex"). It does NOT replace the transcript-brief review — it complements it (R-600 division of labor): Codex catches what Claude-reviewing-Claude misses (decorrelated blind spots) but has **no transcript**, so it cannot judge the session-context decisions the brief-reviewer owns. Pre-merge it is the codex half of each PR's ONE bounded review cycle (spec v1.9 X9a: pass 1 → one fix round → pass 2 → escalation at most once → pass 3; `merge-gate`'s `## The review cycle` is the definition), launched on push and concurrent with CI — never review to convergence, and no pass past pass 3 without a recorded operator decision; it earned its keep on Wave 1/2 (`[[hooks-codex-pilots-decorrelation-validated]]`). `_require-codex-subscription` fails loud on a stale login — no silent metered fallback. |
...
Solo (mechanical/linear) · transcript-brief review (decision-forks, transcript-aware) · `just codex-review` (default out-of-family reviewer for a concrete diff) · council (design decision with a nameable multi-domain tension; dyadic by default) · adversarial reviewer (pre-merge red-team) · Workflow fan-out (**opt-in**, flag before deploying). Full matrix at `docs/governance/orchestration.md` §13.2.
...
Council activation, out-of-family review, the overlay cite-grounding instrument, transcript-brief-review/memory/checkpoint composition, posture. Links at `docs/governance/orchestration.md` §13.5.
