...
| **CXA** — Cross-Axis Composition | **107 canonical cross-axis relationships** across 7 composition buckets per `Cross_Axis_Composition_Document_v2_19.md` §2.3 (**37 genuine typed seams**; 48 convention-level; 22 phase-2-runtime = 107 — **CXA v2.19 (2026-06-01, PR pending) corrects v2.18's erroneous aggregate**: v2.18 re-absorbed the C3-15 OD→IS cleanup already completed at v2.3 (a wrong-version-read of the v2.1 baseline), corrupting the §2.1 matrix (AS→IS 13→11, CP→OD 0→8, OD→CP 8→12) and publishing aggregate **105** plus the downstream-fabricated **37/48/20** sub-split; canonical is **107 / 37+48+22** = v2.17's value, which v2.18 should have left unchanged. v2.18's OD→IS=4 / OD-outbound=26 end-state was already correct since v2.3 and is preserved — only the §2.1 matrix cells + aggregate + downstream sub-split carried harm. The "CXA-OD-IS-EDGE-DRIFT" halt-doc Item 11 that v2.18 claimed to close was a **phantom drift** — CXA's canonical reading was already conformed to the OD plan at v2.3 (2026-05-17); v2.18 = PR #110 `2f14604`, corrected at v2.19); **§2.3.2 CP→IS bucket grew 37→43 canonical / 9→15 genuine at v2.17 2026-05-31** (6 NEW Pattern-P1 typed seams absorbing U-CP-74..U-CP-79 §16.5 CP→IS composer atomic-unit LANDED events at PRs #39-#44 2026-05-28..29; rows 38-43 each consumer `harness_cp.<composer_module>` imports `EntryPayload` from `harness_is.state_ledger_write` per CP spec v1.25 §16.5.3 contract; CP outbound 63→69; CP genuine 22→28; aggregate 101→107; genuine 31→37; closes the workspace pre-commitment authored at CXA v2.16 §0.4 forward-tracking marker — 6-of-6 PENDING transit to ABSORBED; PATTERN_P1_SEAMS test extension 25→31 seams co-published; mirror precedent v2.6 5-row absorption + v2.9 1-row + v2.15 1-row); §2.3.7 CP→OD bucket extended 2→7 typed seams at v2.6 (Phase A.2 composer-arc absorption: ValidatorFramework + PauseResumeProtocol + PerServerTrustEvaluator + HITL webhook delivery + HITL operator-burden — all 5 sharing the `cp_audit_to_od_audit` converter with distinct F2 action_id prefixes); **§2.3.7 row 8 cost-attribution audit-write seam LANDED at v2.9** (bucket grows 7→8; aggregate 99→100; genuine 29→30; U-OD-41 SpanCostRecord composer → `cp_audit_to_od_audit` converter via `cost:` action_id prefix → U-OD-00 audit-ledger ingestion; per-axis attribution = OD outbound 26→27 per established v2.6 namespace-ownership convention); **§2.3.3 row 6 U-CP-68 → U-AS-03 ToolContract CP→AS Pattern-P1 seam LANDED at v2.15** (filed 2026-05-21 at cluster 10-CP-C close per `.harness/class_3_drift_cxa_v2_5_cp_as_bucket_extension_u_cp_68.md`; runtime Pattern-P1 enforcement landed at U-CP-70 commit `2e417e0`; CXA canonical enumeration absorbed at v2.15 after carrying across CXA v2.7..v2.14 narrow-scope publications; bucket grows 5→6 genuine + 18→19 aggregate; aggregate 100→101; genuine 30→31; CP-side consumer `harness_cp.per_server_trust_evaluator` imports AS-side `ToolContract` from `harness_as.tool_contract` per CP spec v1.10 §27.1 PerServerTrustEvaluator.evaluate signature mandate; per-axis attribution = CP outbound 62→63 per vanilla CP→AS Pattern-P1 symbol-equality convention; harness-as/CLAUDE.md `[[class_3_tension_per_axis_claude_md_v2_1_to_v2_4_count_drift]]` carry **PARTIAL-CLOSED at PR #91 2026-05-31** — active axis-posture claim refreshed to CXA v2.16 (AS outbound 11; CP→AS 19/6; OD→AS 10; 29 inbound at the substrate seam exports manifest); §2.3.6 → §2.3.5 OD→AS cite typo fix-up bundled; §2.4 plan-internal 13-edge table + 13-vs-11 plan-vs-CXA divergence preserved as separate-arc residual per FM-2 (the divergence pre-dates the named carry — CXA v2.0 was already at 11; full closure requires plan-vs-CXA reconciliation at a future arc)) |
...
Canonical spec heads are IS `Spec_Information_Substrate_v1.md`, AS `Spec_Action_Surface_v1.md`, CP `Spec_Control_Plane_v1_30.md`, OD `Spec_Operational_Discipline_v1_27.md`, and Runtime `Spec_Harness_Runtime_v1.md`. Full per-spec lineage is indexed at `claude-artifact-pointers.md` §2.3 in `.harness`.

### 2.4 Per-Axis Plans + CXA (Phase 6 Execution Authority)

Canonical plan heads are core `Implementation_Plan_Harness_Core_v1_2.md`, IS `Implementation_Plan_Information_Substrate_v2_5.md`, AS `Implementation_Plan_Action_Surface_v1_4.md`, CP `Implementation_Plan_Control_Plane_v2_31.md`, OD `Implementation_Plan_Operational_Discipline_v2_26.md`, Runtime `Implementation_Plan_Harness_Runtime_v2_42.md`, and CXA `Cross_Axis_Composition_Document_v2_19.md`. Full per-plan and CXA lineage is indexed at `claude-artifact-pointers.md` §2.4 in `.harness`.
...
- The ONLY file changed is `.harness/roadmap_status.md` (this content rule — not title-verb-purity — is what guarantees the PR is dashboard-only).
