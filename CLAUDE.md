...
Canonical spec heads are IS `Spec_Information_Substrate_v1.md` (v1.12, cleared 2026-07-22), AS `Spec_Action_Surface_v1.md` (v1.13), CP `Spec_Control_Plane_v1_112.md` (cleared 2026-07-30 — the RATIFIED `B-69` spec leg: the REQUIRED non-downgradable `ResumeContext` response-provenance carrier + the NEW public projection-returning surface over the two private resume-tree walks + two scope-limit lifts), OD `Spec_Operational_Discipline_v1_36.md` (cleared 2026-07-30 — NEW §C-OD-30.5, the `B-69` read + staleness-refusal emission home within the EXISTING C-OD-30 namespace family), Runtime `Spec_Harness_Runtime_v1.md` (**v1.107**, cleared 2026-07-30 — the `B-69` Runtime leg: NEW §14.14.9 durable-pause-state read accessor + NEW §30 refusal-only staleness precondition + NEW §14.14.8 append-only/never-truncated substrate invariant), and **Memory `Spec_Memory_Substrate_v1.md` (the `C-MEM-*` contract family; head **v1.2**, cleared 2026-07-29)**. Full per-spec lineage is indexed at `claude-artifact-pointers.md` §2.3 in `.harness`.

### 2.4 Per-Axis Plans + CXA (Phase 6 Execution Authority)

Canonical plan heads are core `Implementation_Plan_Harness_Core_v1_3.md`, IS `Implementation_Plan_Information_Substrate_v2_8.md`, AS `Implementation_Plan_Action_Surface_v1_4.md`, CP `Implementation_Plan_Control_Plane_v2_47.md` (2026-07-30 — the `B-69` plan leg: U-CP-64 amended for both CP-owned surfaces; zero new units), OD `Implementation_Plan_Operational_Discipline_v2_31.md` (2026-07-31 — the `B-69` **impl** leg's OD plan delta, the one piece OD spec v1.36 §30.5.2 deliberately held for this leg: NEW U-OD-57 carrying the §C-OD-30.5 pre-bootstrap pause-state audit carrier; the impl leg selected the SIBLING-payload option (b)), Runtime `Implementation_Plan_Harness_Runtime_v2_55.md` (2026-07-30 — the `B-69` plan leg: NEW U-RT-148 carrying BOTH ratified surfaces under the arc's conjunctive closure criterion), CXA `Cross_Axis_Composition_Document_v2_23.md` (2026-07-30 — CLASSIFICATION-ONLY for `B-69`: NO new row, aggregate FROZEN at 111, payload widening recorded), and **Memory `Implementation_Plan_Memory_Substrate_v1.md` (U-MEM-01..27; head **v1.2** cleared 2026-07-29)**. Full per-plan and CXA lineage is indexed at `claude-artifact-pointers.md` §2.4 in `.harness`.
