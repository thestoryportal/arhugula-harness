...
| Hooks | 13 event types, 21 matcher groups, and 27 command handlers in `.claude/settings.json` (U-CTX-09: the dead `Bash(git commit*)` matcher was removed 2026-08-10; counts are computed at test time by `tools/test_codex_workflow_parity.py`, never hand-maintained; the 2026-08-16 rise from 15/20 is graft's five Claude-runtime hooks, exempt from the Codex mirror by explicit decision — see the Graft row below; the 2026-09-02 rise to 27 is U-SR-09's `tools/hooks/rtk-shape-guard.sh`, CLAUDE-ONLY by the adapter's `CLAUDE_ONLY_HOOKS` declaration: it compensates for the rtk rewrite hook that only the Claude user settings register, so a Codex mirror would deny grep shapes Codex runs unmangled) | Every Codex-supported behavior is wired in `.codex/hooks.json`; see `.codex/hooks/README.md` |
| Graft retrieval graph | 5 Claude-runtime hooks (`SessionStart`, `UserPromptSubmit`, `Stop`, 2x `PostToolUse`) — four driving `.claude/helpers/graft-hooks.cjs` and, since U-SR-09 (2026-09-02), the `Write\|Edit\|MultiEdit` PostToolUse one driving `.claude/helpers/graft-mark-dirty.cjs` (the dirty-flag-only replacement for graft's `post-edit`, whose `graft check` ran 46 s inside an 8 s budget on every edit) — plus `.claude/skills/graft/SKILL.md` | **Exempt from the hook mirror by explicit decision (operator-approved 2026-08-16).** These hooks inject retrieval context into Claude's own prompt/session lifecycle; Codex exposes no equivalent injection point, so a mirrored command would have nothing to bind to. Codex receives the same guidance declaratively via the marker-fenced Graft block in `AGENTS.md`. The exemption is not a skip: `test_codex_hook_map_tracks_every_canonical_claude_hook_command` still asserts each exempt command references a helper that exists AND is git-tracked. Upstream limits (timeout unit conflict, `PATH` prerequisite, silent hook-load failure) are registered at `.harness/graft-integration-notes.md`. |
| Tracked project skills | 33 canonical skills, including council and shipping workflows (measured 2026-08-16: `git ls-files '.claude/skills/**/SKILL.md'`. The prior figure of 35 was already stale by 3 before graft was tracked — it is prose, not a test-asserted count, unlike the hook counts above) | 33/33 discovery entrypoints under `.agents/skills/` (41 entrypoints total; the test asserts a subset, not equality); native workflow skills where runner mechanics differ, full-body bridges elsewhere |
