...
|---|---|---|
| `Spec_Information_Substrate_v1.md` | **v1.13 — canonical HEAD** (in-place delta chain; v1.13 = 2026-08-07 `B-57` spec leg, ratified **Reading A** — NEW **§7.6.1** under C-IS-07 §7.6 authorizing **PER-CALL-SITE** election of writer-owned timestamp sampling on DIRECT append surfaces via the EXISTING `WRITER_OWNED_TIMESTAMP` sentinel (never a default, never a writer mode; every non-electing direct producer keeps caller-supplied semantics BYTE-VERBATIM; elect only where the timestamp means *when the entry was appended*), plus ONE residual-status paragraph discharging §7.6's registered residual (the residual + "Surfaces that do NOT change" paragraphs PRESERVED VERBATIM) and ONE cross-reference-only clause at the §7.1 row-7 cell; ZERO contract numbers / hash / CXA / new units; clearance `spec-information-substrate-v1-13-cleared-2026-08-07.md`; prior v1.12 = 2026-07-22 `B-33-A` spec leg — NEW §5.6 D-derivative sidecar `rotation_correlation_id` on `StateLedgerEntry`/`EntryPayload` (C-IS-05) + NEW §7.7 presence/uniqueness read-side invariants (C-IS-07), closing the IS-side half of `B-33`; clearance `spec-information-substrate-v1-12-cleared-2026-07-22.md`; prior v1.11 = 2026-07-19 B-48 apply arc — C-IS-07 §7.1 timestamp-authority row + NEW §7.6 writer-owned drain-path sampling inside `_WRITE_LOCK`, clearance `spec-information-substrate-v1-11-cleared-2026-07-19.md`; per-version detail at the spec's own change-notes) | Contract authority — the `C-IS-*` contract family (see the canonical spec for the full enumeration) |
| `Implementation_Plan_Information_Substrate_v2_9.md` | **v2.9 — canonical HEAD** (2026-08-07 `B-57` plan leg — U-IS-11 AMENDED with ACs #14–#20 plus #14-bis (the per-call-site election with no writer-side mode permitted; the negatively-demonstrable default-preservation property; the eligibility rule; the sentinel-docstring refresh; per-site table conformance; the two injection-caveat resolutions; the `audit_writer` retry-loop disposition; the `shadow_git_rollback` `restored_at` decoupling pin) + the per-site classification table IS spec v1.13 §7.6.1 routes to the plan (14 rows = 10 ELECT + 2 injection-caveat + 1 RETAIN + 1 DEFER); ACs #1–#13 PRESERVED VERBATIM; ZERO new units / nodes / edges / auxiliary types; clearance `implementation-plan-information-substrate-v2-9-cleared-2026-08-07.md`; prior v2.8 = 2026-07-22 `B-33-A` spec leg — NEW foundational unit U-IS-20 (`rotation_correlation_id` sidecar + presence/uniqueness invariants), `Depends on: (none)`, IS 0-outbound; clearance `implementation-plan-information-substrate-v2-8-cleared-2026-07-22.md`; prior v2.7 amended U-IS-11 for IS v1.11 §7.6, zero new units) | Execution authority — 19 units (U-IS-01 – U-IS-17 + U-IS-19 + U-IS-20; U-IS-18 retired) |
