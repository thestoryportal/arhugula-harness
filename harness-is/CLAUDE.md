...
| H_T-IS-2 (artifact-tier registry) | **PARTIAL** (post-batch-49, 2026-05-30) | Phase 7 substantive amendment substrate landed: IS spec v1.3 §C-IS-05 §5.1 sidecar field + §5.2 resolver contract authored; production binding at `harness-runtime/.../lifecycle/procedural_tier_snapshot.py` `resolve_procedural_tier_snapshot(ctx)` + `make_procedural_tier_snapshot_resolver(ctx)` factory (residence pinned at runtime axis per Q-γ=(γ-2) ratification per `.harness/architect_recommendation_h_t_is_2_artifact_tier_registry_wiring.md` §11.4); EntryPayload sidecar field carrier + StateLedgerEntry D-derivative field + canonicalize sidecar contribution discipline LANDED at harness-is. PARTIAL → RETIRED gated on **7 producer-site lifts remaining post-PR #107** (the 6 §16.5 composers populate `procedural_tier_snapshot_ref=procedural_tier_snapshot_resolver()` at EntryPayload construction since PR #107 Reading C apply 2026-05-31 `89915af`). Remaining 7 sites per X-AL-2 second conjunct: `harness-cp/.../sibling_ledger_entry_composition.py:144` (U-CP-34) + `harness-cp/.../workflow_driver.py:1360` (workflow_driver direct emission); `harness-runtime/.../lifecycle/sub_agent_dispatch.py:474` + `lifecycle/hitl_gate_composer.py:770` + `lifecycle/audit_writer.py:120` + `lifecycle/as_is_wiring.py:110`; `harness-is/src/harness_is/shadow_git_rollback.py:114` (intra-IS rollback). Each remaining site is a distinct producer surface; lifts deferred per Q2=narrow + workflow v1.12 §7.4.7.3.C retirement-tier-transit audit-template applied 2026-05-31 (refreshed from pre-PR-#107 "~13 producer-site lifts" estimate; 6-of-13 LANDED at PR #107; 7 remaining). See `.harness/phase-7d-retirement-events-batch-49.md` |
...
IS-axis post-batch-49 (2026-05-30): **8 / 9 RETIRED (88.9%) + 0 RETIRE-READY + 1 PARTIAL (IS-2 only; post-batch-49 STILL-BOUNDED → PARTIAL via Phase 7 substantive amendment substrate-lift) + 0 STILL-BOUNDED + 0 STILL-BOUNDED-INDEFINITELY = 9 ✓**. Pipeline-advanced 9/9 = 100% (FIRST IS-axis-clean state at pipeline-advanced view). H_T-IS-2 PARTIAL → RETIRED gated on **7 producer-site lifts remaining post-PR #107** (6-of-13 LANDED at PR #107 `89915af` Reading C apply 2026-05-31; enumerated 7 remaining sites at the row-table gate-text above) per X-AL-2 second conjunct per `.harness/phase-7d-retirement-events-batch-49.md`. Cumulative-counts line maintenance per workflow v1.12 §7.4.7.3.C retirement-tier-transit audit-template — refreshed at batch-49 publication + re-refreshed at the §7.4.7.3.C audit 2026-05-31 post-PR #107 (sibling cumulative-counts line to the row-table gate-text; both refreshed in lockstep).
